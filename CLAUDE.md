...
Modesto is a minimal web GUI for using coding agents. It is multi-provider: `ProviderKind` currently spans Codex, Claude (`claudeAgent`), Cursor, Gemini, Grok, Meta (Muse Code / Muse Spark), Factory Droid (`droid`), Kilo, OpenCode, and Pi, plus additional opt-in drivers. Each provider has its own model options and capabilities (reasoning effort, thinking budget/level, context window, fast mode), defined in `packages/contracts` and resolved in `packages/shared/src/model.ts`.

Codex was the first integration and remains the most fleshed-out reference (see the Codex App Server section), but Modesto is not Codex-only.
