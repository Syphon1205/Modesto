...
You are GPT-5.5 (xhigh) in the Modesto monorepo at `/Users/dev/modesto`.
...
- `config.homeDir` = `OS.homedir()` ALWAYS (`apps/server/src/main.ts:172,217`) — it is NOT affected by
  `--home-dir`.
- `config.baseDir` = `--home-dir` / `MODESTO_HOME` (dev: `./.modesto/electron-dev`; prod: `~/.modesto` or
  `~/Documents/Modesto`). `config.dbPath` = `baseDir/{dev|userdata}/state.sqlite`.
...
- The dev runner sets `MODESTO_HOME/MODESTO_HOME/MODESTO_HOME=baseDir` but does NOT set `CODEX_HOME` or
...
5. **Run the checks** to back every claim:
   - `sqlite3` on `./.modesto/electron-dev/dev/state.sqlite` (dev) and `~/.modesto/userdata/state.sqlite`
     (prod) where relevant.
