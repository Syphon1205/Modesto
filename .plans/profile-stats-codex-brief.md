...
You are GPT-5.5 (xhigh) working in the Modesto monorepo at `/Users/dev/modesto`.
...
- BRANCH dev DB (what the dev instance uses): `./.modesto/electron-dev/dev/state.sqlite`  (~40 turns)
- REAL app DB: `~/.modesto/userdata/state.sqlite`  (~437 turns, 192 MB) — the important one
...
2. `npx turbo run typecheck --filter=@modesto/contracts --filter=@modesto/cli` must pass. DO NOT run the web
   (`@modesto/web`) typecheck — its UI is being rewritten in parallel and will be red until Opus finishes.
