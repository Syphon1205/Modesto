...
- Dev commands default `MODESTO_HOME` to `~/.modesto`, which keeps dev state under `~/.modesto/dev`.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --home-dir ~/.modesto-2`
...
- Desktop production windows load the bundled UI from `modesto://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the `modesto` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `MODESTO_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `MODESTO_DEV_INSTANCE`)
- Example: `MODESTO_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `MODESTO_PORT_OFFSET` to a numeric offset.
