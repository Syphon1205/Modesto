...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `modesto` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@modesto/shared/git`, `@modesto/shared/DrainableWorker`) — no barrel index.
