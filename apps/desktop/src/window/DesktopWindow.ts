...
import { DEFAULT_CLIENT_SETTINGS } from "@modesto/contracts";
...
import { isAuxiliaryWindow } from "./auxiliaryWindows.ts";
import * as PreviewManager from "../preview/Manager.ts";
import * as DesktopAppSettings from "../settings/DesktopAppSettings.ts";
import * as DesktopClientSettings from "../settings/DesktopClientSettings.ts";
import * as ElectronApp from "../electron/ElectronApp.ts";
import { makeQuitHoldHandler } from "./QuitHold.ts";
import * as MainWindowBackgroundThrottling from "./mainWindowBackgroundThrottling.ts";
...
>()("@modesto/desktop/window/DesktopWindow") {}
  ...
  const withoutSplash = (window: Option.Option<Electron.BrowserWindow>) =>
    Ref.get(splashWindowRef).pipe(
      Effect.map((splash) => {
        ...
        if (isAuxiliaryWindow(window.value)) {
  ...
  > {
    ...
    const window = yield* electronWindow.create({
      ...
      webPreferences: {
        ...
        // which stalls first paint. Boot unthrottled; first reveal restores
        // throttling unless an agent-live / frame-capture hold is active.
    ...
    MainWindowBackgroundThrottling.bindMainWindowBackgroundThrottling(window);
    ...
    bindFirstRevealTrigger(revealSubscribers, () => {
      // Boot is done; restore normal hidden-window throttling unless an
      // agent-live / frame-capture hold still needs the renderer awake.
      MainWindowBackgroundThrottling.markMainWindowRevealed();
    ...
    window.on("closed", () => {
      MainWindowBackgroundThrottling.clearMainWindowBackgroundThrottling(window);
      clearDevelopmentLoadRetry();
      clearBoundsPersist();
