...
describe("resolveLaunchVersionRecordPath", () => {
  it("places the record file inside the userData directory", () => {
    expect(resolveLaunchVersionRecordPath("/home/me/AppData/Modesto")).toBe(
      "/home/me/AppData/Modesto/last-launch-version.json",
    );
  });
});
...
describe("resolveMacAppBundlePath", () => {
  it("resolves the .app bundle from the Electron executable on macOS", () => {
    expect(
      resolveMacAppBundlePath("/Applications/Modesto.app/Contents/MacOS/Modesto", "darwin"),
    ).toBe("/Applications/Modesto.app");
  });

  it("returns null off macOS", () => {
    expect(
      resolveMacAppBundlePath("/Applications/Modesto.app/Contents/MacOS/Modesto", "linux"),
    ).toBeNull();
  });

  it("returns null when the executable is not inside a .app bundle", () => {
    expect(resolveMacAppBundlePath("/usr/local/bin/modesto", "darwin")).toBeNull();
  });
});
