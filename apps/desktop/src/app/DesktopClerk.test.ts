...
const makeDesktopClerkLayer = (isDevelopment = true, events: string[] = []) => {
  const environment = DesktopEnvironment.DesktopEnvironment.of({
    ...
    userDataDirName: isDevelopment ? "modesto-dev" : "modesto",
    legacyUserDataDirName: isDevelopment ? "Modesto (Dev)" : "Modesto (Alpha)",
...
describe("DesktopClerk", () => {
  ...
  it.effect("acquires and releases the SDK bridge with the layer", () => {
    ...
    return Effect.gen(function* () {
        ...
        [
          {
            ...
            passkeys: true,
            renderer: { scheme: "modesto-dev", host: "app" },
          },
        ],
      ]);
      ...
      assert.deepEqual(events, ["setPath:userData:/tmp/app-data/modesto-dev", "createClerkBridge"]);
  ...
  it.each([
    { isDevelopment: true, scheme: "modesto-dev" },
    { isDevelopment: false, scheme: "modesto" },
