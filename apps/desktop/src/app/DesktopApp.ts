...
import { configureMediaPermissions } from "./DesktopMediaPermissions.ts";
...
export class DesktopDevelopmentBackendPortRequiredError extends Schema.TaggedErrorClass<DesktopDevelopmentBackendPortRequiredError>()(
  ...
  override get message(): string {
    return "MODESTO_PORT is required in desktop development.";
  }
}
...
const bootstrap = Effect.gen(function* () {
  ...
  const appshots = yield* AppshotsManager.AppshotsManager;
  yield* appshots.start;
  yield* logBootstrapInfo("bootstrap appshots helper started");
...
const startup = Effect.gen(function* () {
  ...
  yield* appIdentity.configure;
  // Must run after `whenReady`: the default session does not exist before it.
  yield* configureMediaPermissions;
