    ...
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          ...
          MODESTO_DEV_REMOTE_T3_SERVER_ENTRY_PATH: " /remote/server.mjs ",
          MODESTO_OTLP_TRACES_URL: " http://127.0.0.1:4318/v1/traces ",
          MODESTO_OTLP_EXPORT_INTERVAL_MS: "2500",
        },
      );
      ...
      assert.equal(environment.appUserModelId, "com.fabweavr.modesto.dev");
      assert.equal(environment.linuxWmClass, "modesto-dev");
      assert.deepEqual(
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          MODESTO_DESKTOP_APP_USER_MODEL_ID: " com.fabweavr.modesto.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.fabweavr.modesto.dev.local");
    }),
  );
