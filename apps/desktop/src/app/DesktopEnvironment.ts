...
>()("@modesto/desktop/app/DesktopEnvironment") {}

const APP_BASE_NAME = "Modesto";
...
function resolveDesktopAppBranding(input: {
  ...
  // Modesto's stable channel shows the bare app name (matching the desktop
  // package's own productName convention) - unlike T3's three-suffixed-tier
  // scheme this was ported from, "Alpha" here means "not Dev, not Nightly"
  // and carries no visible suffix, while Dev/Nightly keep theirs.
  return {
...
const make = Effect.fn("desktop.environment.make")(function* (
  ...
  const userDataDirName = isDevelopment ? "modesto-dev" : "modesto";
  const legacyUserDataDirName = isDevelopment ? "Modesto (Dev)" : "Modesto";
  ...
  return DesktopEnvironment.of({
    ...
    appUserModelId: Option.getOrElse(config.appUserModelIdOverride, () =>
      isDevelopment ? "com.fabweavr.modesto.dev" : "com.fabweavr.modesto",
    ),
    linuxDesktopEntryName: isDevelopment ? "modesto-dev.desktop" : "modesto.desktop",
    linuxWmClass: isDevelopment ? "modesto-dev" : "modesto",
