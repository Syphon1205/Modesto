...
function makeTempDir(): string {
  const directory = FS.mkdtempSync(Path.join(OS.tmpdir(), "modesto-profile-test-"));
...
describe("desktopUserDataProfile", () => {
  it("resolves Modesto and previous profile names", () => {
    const appDataBase = "/Users/tester/Library/Application Support";
    const previousProfileName = String.fromCharCode(115, 121, 110, 97, 114, 97);
    expect(resolveDesktopUserDataPath({ appDataBase, isDevelopment: true })).toBe(
      "/Users/tester/Library/Application Support/modesto-dev",
    );
    expect(resolveDesktopUserDataPath({ appDataBase, isDevelopment: false })).toBe(
      "/Users/tester/Library/Application Support/modesto",
    );
    expect(resolveLegacyDesktopUserDataPath({ appDataBase, isDevelopment: false })).toBe(
      Path.join(appDataBase, previousProfileName),
    );
  });

  it("copies a previous profile without deleting it", () => {
    const appDataBase = makeTempDir();
    const sourcePath = resolveLegacyDesktopUserDataPath({ appDataBase, isDevelopment: false });
    const targetPath = Path.join(appDataBase, "modesto");
    FS.mkdirSync(sourcePath);
    FS.writeFileSync(Path.join(sourcePath, "Preferences"), "legacy-state");

    expect(migrateLegacyDesktopProfile({ sourcePath, targetPath }).status).toBe("migrated");
    expect(FS.readFileSync(Path.join(targetPath, "Preferences"), "utf8")).toBe("legacy-state");
    expect(FS.readFileSync(Path.join(sourcePath, "Preferences"), "utf8")).toBe("legacy-state");
  });
  ...
  it("repairs missing browser data from the profile recorded by the bridge", () => {
    ...
    const targetPartitionPath = Path.join(targetPath, "Partitions", "modesto-browser");
    ...
    FS.writeFileSync(
      Path.join(targetPath, "modesto-profile-seed.json"),
  ...
  it("rejects bridge manifests that point outside the Modesto profile parent", () => {
    const appDataBase = makeTempDir();
  ...
  it("never adds a foreign SQLite sidecar beside an existing Modesto database", () => {
    const appDataBase = makeTempDir();
    const targetPath = Path.join(appDataBase, "modesto");
    const sourcePath = Path.join(appDataBase, "previous-profile");
    const sourcePartitionPath = Path.join(sourcePath, "Partitions", "previous-browser");
  ...
  it("replaces an orphaned target sidecar with one from the repaired database generation", () => {
    ...
    expect(
      FS.readdirSync(targetPartitionPath).some((entryName) =>
        entryName.startsWith(".modesto-bridge-"),
      ),
    ).toBe(false);
  });
  ...
  it("ignores a malformed bridge manifest without attempting a repair", () => {
    ...
    const targetPath = Path.join(appDataBase, "modesto");
    FS.mkdirSync(targetPath, { recursive: true });
    FS.writeFileSync(Path.join(targetPath, "modesto-profile-seed.json"), "{");
