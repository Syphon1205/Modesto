...
import type { BrowserExecuteCdpInput, ThreadBrowserState, ThreadId } from "@modesto/contracts";
...
const BROWSER_USE_PIPE_NAME_PREFIX = "modesto-iab";
export const MODESTO_BROWSER_USE_PIPE_ENV = "MODESTO_BROWSER_USE_PIPE_PATH";
...
export function resolveConfiguredBrowserUsePipePath(
  ...
  const configured = env[MODESTO_BROWSER_USE_PIPE_ENV]?.trim();
  return configured || resolveDefaultBrowserUsePipePath(platform);
}

export const MODESTO_BROWSER_USE_PIPE_PATH = resolveConfiguredBrowserUsePipePath();
...
export class BrowserUsePipeServer {
  ...
  constructor(
    ...
    options: BrowserUsePipeServerOptions | string = MODESTO_BROWSER_USE_PIPE_PATH,
  ) {
    this.pipePath =
      typeof options === "string" ? options : (options.pipePath ?? MODESTO_BROWSER_USE_PIPE_PATH);
  ...
  private async createTabForSession(sessionId: string): Promise<{
    ...
    if (!snapshot) {
      throw new Error("No active Modesto browser pane available");
    }
