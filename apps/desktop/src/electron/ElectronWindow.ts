...
import { isAuxiliaryWindow } from "../window/auxiliaryWindows.ts";
...
>()("@modesto/desktop/electron/ElectronWindow") {}
...
export const make = Effect.gen(function* () {
  ...
  const currentMainOrFirst = Effect.gen(function* () {
    ...
    const first = Option.fromNullishOr(
      (yield* listWindows).find((window) => !isAuxiliaryWindow(window)) ?? null,
    );
  ...
  const focusedMainOrFirst = Effect.gen(function* () {
    ...
    if (
      ...
      !isAuxiliaryWindow(focused.value) &&
