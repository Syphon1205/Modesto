import {
  ...
  DesktopPreviewSignInProbeInputSchema,
...
/**
 * Answer "is the preview browser signed in to this site?" from its own cookie
 * jar - the same jar the user's sign-in wrote to, since both go through
 * `BrowserSession.getSession(environmentId)`.
 *
 * The handler reads cookies but returns only names and expiry. A session
 * cookie's value is the credential itself; returning one would let any
 * renderer (or anything that later reads a log) impersonate the user on that
 * site, and presence is the entire question being asked.
 */
export const probeSignIn = DesktopIpc.makeIpcMethod({
  channel: IpcChannels.PREVIEW_PROBE_SIGN_IN_CHANNEL,
  payload: DesktopPreviewSignInProbeInputSchema,
  result: DesktopPreviewSignInProbeResultSchema,
  handler: Effect.fn("desktop.ipc.preview.probeSignIn")(function* ({
    environmentId,
    domain,
    cookieNames,
  }) {
    ...
    // Electron matches a bare domain against its subdomains too, so the
    // leading dot in a catalog entry (".google.com") is stripped rather than
    // passed through - with it, nothing matches.
    const normalizedDomain = domain.startsWith(".") ? domain.slice(1) : domain;
    const cookies = yield* Effect.tryPromise({
      try: () => browserSession.cookies.get({ domain: normalizedDomain }),
      // A jar that cannot be read is not evidence of being signed out, but
      // this probe has only one bit to report; "not signed in" is the safe
      // reading, and the caller re-probes rather than caching it forever.
      catch: () => new Error(`Failed to read cookies for ${normalizedDomain}.`),
    }).pipe(Effect.orElseSucceed(() => []));

    const wanted = new Set(cookieNames);
    const matched = cookies.filter((cookie) => wanted.has(cookie.name));
    const expiries = matched
      .map((cookie) => cookie.expirationDate)
      .filter((value): value is number => typeof value === "number");

    return {
      signedIn: matched.length > 0,
      matchedCookieNames: matched.map((cookie) => cookie.name),
