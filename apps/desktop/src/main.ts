...
import type { ContextMenuItem } from "@modesto/contracts";
import { getMacTrafficLightPosition } from "@modesto/shared/desktopChrome";
import {
  MODESTO_DESKTOP_ENTRY_URL,
  MODESTO_DESKTOP_SCHEME,
  modestoBundleId,
...
import { NetService } from "@modesto/shared/Net";
import { RotatingFileSink } from "@modesto/shared/logging";
import { ensureStaticSnapshot, findAsarArchivePath } from "@modesto/shared/staticSnapshot";
...
import type { NpmPlatformCliProvider } from "@modesto/shared/npmPlatformCliRuntime";
import {
  ensureBundledProviderClisInstalled,
  type BundledProviderCli,
} from "./providerCliBootstrap";
import {
...
import {
  ...
  resolveLegacyDesktopUserDataPath,
...
import {
  ...
  resolveModestoStorageSnapshotPath,
...
const UPDATE_SET_CHANNEL = "desktop:update-set-channel";
const NOTIFICATIONS_IS_SUPPORTED_CHANNEL = "desktop:notifications-is-supported";
const NOTIFICATIONS_SHOW_CHANNEL = "desktop:notifications-show";
const BASE_DIR = process.env.MODESTO_HOME?.trim() || Path.join(OS.homedir(), ".modesto");
...
const BACKEND_MAX_OLD_SPACE_ENV_KEYS = ["MODESTO_BACKEND_MAX_OLD_SPACE_MB"] as const;
const UPDATE_CHANNEL_PREFERENCE_FILE = Path.join(STATE_DIR, "update-channel.json");
...
const MODESTO_BROWSER_LABEL = "Modesto browser";
const browserPerfLoggingEnabled = process.env.MODESTO_BROWSER_PERF === "1";
...
function startBrowserPerformanceLogging(): void {
  ...
  browserPerfInterval = setInterval(() => {
    ...
    console.info(`[${MODESTO_BROWSER_LABEL} perf]`, {
      ...snapshot.counters,
...
function readSelectedUpdateChannel(): DesktopUpdateChannel {
  try {
    const parsed = JSON.parse(FS.readFileSync(UPDATE_CHANNEL_PREFERENCE_FILE, "utf8")) as {
      channel?: unknown;
    };
    return normalizeDesktopUpdateChannel(parsed.channel, buildUpdateChannel);
  } catch {
    return buildUpdateChannel;
  }
}

function persistSelectedUpdateChannel(channel: DesktopUpdateChannel): void {
  FS.mkdirSync(Path.dirname(UPDATE_CHANNEL_PREFERENCE_FILE), { recursive: true });
  FS.writeFileSync(
    UPDATE_CHANNEL_PREFERENCE_FILE,
...
function isMissingUpdateMetadataError(message: string): boolean {
  ...
  return (
    (normalized.includes("latest-mac.yml") ||
      normalized.includes("modesto-mac.yml") ||
      normalized.includes("latest.yml") ||
      normalized.includes("modesto.yml") ||
      normalized.includes("latest-linux.yml") ||
      normalized.includes("modesto-linux.yml")) &&
...
async function reserveBackendEndpoint(reason: string): Promise<void> {
  ...
  process.env.MODESTO_DESKTOP_WS_URL = backendWsUrl;
...
let lastNotifiedDownloadedUpdateVersion: string | null = null;
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { modestoCommitHash?: unknown };
    return normalizeCommitHash(parsed.modestoCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.MODESTO_COMMIT_HASH);
  if (envCommitHash) {
...
// modesto:// protocol here and, via MODESTO_STATIC_DIR, for the backend's HTTP static
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Modesto failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function hasConfiguredUpdateFeed(): boolean {
  return readAppUpdateYml() !== null || Boolean(process.env.MODESTO_DESKTOP_MOCK_UPDATES);
}
...
function resolveAutoUpdateDisabledReason(): string | null {
  return getAutoUpdateDisabledReason({
    ...
    disabledByEnv: process.env.MODESTO_DISABLE_AUTO_UPDATE === "1",
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Modesto ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
    ...
    void dialog.showMessageBox({
      type: "info",
      title: "Update found",
      message: "Modesto is preparing the update in the background.",
      buttons: ["OK"],
    });
...
function resolveNotificationIconPath(): string | null {
  ...
  if (process.platform === "win32") {
    return resolveResourcePath("modesto.png") ?? resolveIconPath("ico");
  }
  return resolveResourcePath("modesto.png") ?? resolveIconPath("png");
}
 ...
 * package.json. We override it to a clean lowercase Modesto name.
 */
...
function migrateLegacyProfileBeforeElectronReady(userDataPath: string): void {
  const appDataBase = resolveDesktopAppDataBase();
  const sourcePath = resolveLegacyDesktopUserDataPath({ appDataBase, isDevelopment });
  const result = migrateLegacyDesktopProfile({ sourcePath, targetPath: userDataPath });
  if (result.status === "migrated") {
    console.info("[desktop] Migrated legacy application profile to Modesto", {
      sourcePath: result.sourcePath,
      targetPath: result.targetPath,
    });
  } else if (result.status === "failed") {
    console.warn("[desktop] Could not migrate the legacy application profile", result.error);
  }
}
...
function repairBrowserProfileBeforeElectronReady(userDataPath: string): void {
  ...
  if (browserProfileRepair.status === "repaired") {
    console.info("[desktop] Completed Modesto browser profile bridge repair", {
    ...
    console.warn("[desktop] Failed to complete Modesto browser profile bridge repair", {
      sourcePath: browserProfileRepair.sourcePath,
      targetPath: browserProfileRepair.targetPath,
...
function restartAfterStartupBundleSwap(error: BundleChangedDuringStartupError): void {
  ...
  void dialog
    .showMessageBox({
      type: "warning",
      title: "Modesto needs to restart",
      message: "Modesto changed while it was opening.",
      detail:
        "The current process cannot safely read the replaced application bundle. Restart Modesto to finish opening with one consistent version.",
      buttons: ["Restart Modesto"],
      defaultId: 0,
    })
    .catch(() => undefined)
    .then(() => {
...
// every archive read in this process — the modesto:// protocol, the backend's static
...
function startBundleSwapWatcher(): void {
  ...
  bundleSwapPollTimer = setInterval(() => {
    ...
    void dialog
      .showMessageBox({
        type: "warning",
        title: "Modesto was replaced on disk",
        message: "The installed Modesto app changed while it was running.",
        detail:
...
function processInstallMarkerOnStartup(): void {
  ...
  const message = `Modesto restarted, but update ${marker.toVersion} was not installed. Try again.`;
  setUpdateState(
    reduceDesktopUpdateStateOnInstallRestartFailure(
      updateState,
      marker.toVersion,
...
async function checkForUpdates(reason: string): Promise<void> {
  ...
  try {
    ...
    if (isMissingUpdateMetadataError(message)) {
      setUpdateState(reduceDesktopUpdateStateOnNoUpdate(updateState, new Date().toISOString()));
      console.warn("[desktop-updater] This release channel has no automatic-update manifest yet.");
      return;
    }
    setUpdateState(
...
function configureAutoUpdater(): void {
  ...
  // default feed while Modesto versions advance independently.
  autoUpdater.channel = resolveUpdaterChannelName(selectedUpdateChannel);
  autoUpdater.allowPrerelease = selectedUpdateChannel === "development";
  ...
  autoUpdater.on("error", (error) => {
    ...
    if (errorContext === "check" && isMissingUpdateMetadataError(message)) {
      setUpdateState(reduceDesktopUpdateStateOnNoUpdate(updateState, new Date().toISOString()));
      console.warn(
        "[desktop-updater] Ignored missing update manifest; no automatic update is published for this channel yet.",
      );
      return;
    }
    if (
  ...
  autoUpdater.on("update-downloaded", (info) => {
    ...
    if (lastNotifiedDownloadedUpdateVersion !== info.version) {
      lastNotifiedDownloadedUpdateVersion = info.version;
      showDesktopNotification({
        title: "Modesto update ready",
        body: `Version ${info.version} is ready. Open Modesto and click the blue download button to restart and install.`,
      });
    }
  });
...
// Builds process-local Node args so provider/tool children do not inherit Modesto's heap guard.
...
function backendEnv(): NodeJS.ProcessEnv {
  ...
  const providerRuntimePaths = app.isPackaged
    ? [
        Path.join(BASE_DIR, "provider-tools", "bin"),
        Path.join(process.resourcesPath, "provider-runtimes", "bin"),
        Path.join(`${resolveAppRoot()}.unpacked`, "node_modules", ".bin"),
      ]
    : [
        Path.join(ROOT_DIR, "apps", "server", "node_modules", ".bin"),
        Path.join(ROOT_DIR, "node_modules", ".bin"),
      ];
  const executablePath = providerRuntimePaths
  ...
  return {
    ...
    // modesto:// protocol serves, so both surfaces survive app.asar being replaced.
    ...(servedStaticRoot?.snapshotted ? { MODESTO_STATIC_DIR: servedStaticRoot.dir } : {}),
    MODESTO_MODE: "desktop",
    MODESTO_NO_BROWSER: "1",
    MODESTO_PORT: String(backendPort),
    MODESTO_HOME: BASE_DIR,
    MODESTO_AUTH_TOKEN: backendAuthToken,
    [MODESTO_BROWSER_USE_PIPE_ENV]: MODESTO_BROWSER_USE_PIPE_PATH,
  };
}

function packagedProviderCliSources(): BundledProviderCli[] {
  if (!app.isPackaged) return [];
  const nodeModulesRoot = Path.join(`${resolveAppRoot()}.unpacked`, "node_modules");
  const nodeLauncher = (name: string, ...segments: string[]): BundledProviderCli => ({
    name,
    sourcePath: Path.join(nodeModulesRoot, ...segments),
    kind: "node-launcher",
  });
  const native = (name: string, sourcePath: string): BundledProviderCli => ({
    name,
    sourcePath,
    kind: "native",
  });
  const clis: BundledProviderCli[] = [
    {
      name: "modesto",
      sourcePath: Path.join(resolveAppRoot(), "apps", "server", "dist", "index.mjs"),
      kind: "node-launcher",
    },
    nodeLauncher("gemini", "@google", "gemini-cli", "bundle", "gemini.js"),
    nodeLauncher("droid", "@factory", "cli", "bin", "droid"),
    nodeLauncher("kilo", "@kilocode", "cli", "bin", "kilo"),
    nodeLauncher("kilocode", "@kilocode", "cli", "bin", "kilo"),
    nodeLauncher("pi", "@earendil-works", "pi-coding-agent", "dist", "cli.js"),
    nodeLauncher("grok", "@xai-official", "grok", "bin", "grok"),
  ];

  // Codex, Claude, and OpenCode aren't bundled (see providerCliRuntimeBootstrap.ts) — they're
  // downloaded on first run, so only show up here once that download has landed.
  const npmPlatformCliInstallRoot = Path.join(BASE_DIR, "provider-tools");
  for (const provider of NPM_PLATFORM_CLI_PROVIDERS) {
    const executable = providerCliRuntimeExecutablePath({
      provider,
      installRoot: npmPlatformCliInstallRoot,
    });
    if (executable && FS.existsSync(executable)) {
      clis.push(native(provider, executable));
    }
  }

  const cursorExecutable = cursorAgentRuntimeExecutablePath({
    installRoot: Path.join(BASE_DIR, "provider-tools"),
  });
  if (cursorExecutable && FS.existsSync(cursorExecutable)) {
    clis.push(native("cursor-agent", cursorExecutable));
  }
  return clis;
}

function packagedProviderCliBundleKey(): string {
  const hash = Crypto.createHash("sha256");
  hash.update(app.getVersion());
  hash.update(process.platform);
  hash.update(process.arch);
  hash.update(resolveAppRoot());
  try {
    const appStats = OriginalFS.statSync(resolveAppRoot());
    hash.update(`${appStats.size}:${appStats.mtimeMs}`);
  } catch {
    // Version/platform/path still provide a stable fallback key.
  }
  const runtimeManifest = Path.join(process.resourcesPath, "provider-runtimes", "manifest.json");
  try {
    hash.update(FS.readFileSync(runtimeManifest));
  } catch {
    // npm-distributed provider CLIs remain installable without a native manifest.
  }
  // Hash cursor-agent's resolved (version-pinned) path and on-disk presence: without this, a
  // launch where the first-run download failed would produce a manifest byte-identical to one
  // from a later successful download, so the launcher-refresh check below would never notice
  // cursor-agent showed up.
  const cursorExecutable = cursorAgentRuntimeExecutablePath({
    installRoot: Path.join(BASE_DIR, "provider-tools"),
  });
  hash.update(
    cursorExecutable && FS.existsSync(cursorExecutable)
      ? cursorExecutable
      : "cursor-agent:unavailable",
  );
  // Same reasoning as cursor-agent above, for each first-run-downloaded provider CLI.
  const npmPlatformCliInstallRoot = Path.join(BASE_DIR, "provider-tools");
  for (const provider of NPM_PLATFORM_CLI_PROVIDERS) {
    const executable = providerCliRuntimeExecutablePath({
      provider,
    ...
    hash.update(executable && FS.existsSync(executable) ? executable : `${provider}:unavailable`);
  }
  return hash.digest("hex");
}

function installBundledProviderClisOnce(): void {
  if (!app.isPackaged) return;
  const clis = packagedProviderCliSources();
  const result = ensureBundledProviderClisInstalled({
    bundleKey: packagedProviderCliBundleKey(),
    clis,
    electronExecutable: process.execPath,
    installRoot: Path.join(BASE_DIR, "provider-tools"),
    sourceRoot: resolveAppRoot(),
  });
  writeDesktopLogHeader(
    result.installed
      ? `provider CLI bootstrap installed commands=${result.commands.join(",")}`
      : `provider CLI bootstrap reused commands=${result.commands.join(",")}`,
  );
}
...
function registerIpcHandlers(): void {
  const storageSnapshotPath = resolveModestoStorageSnapshotPath(app.getPath("userData"));
  ...
  ipcMain.on(STORAGE_MIGRATION_IPC_CHANNELS.read, (event: IpcMainEvent) => {
    event.returnValue = readModestoStorageSnapshot(storageSnapshotPath);
  });
  ...
  ipcMain.handle(STORAGE_MIGRATION_IPC_CHANNELS.acknowledge, async () => {
    await acknowledgeModestoStorageSnapshot(storageSnapshotPath);
  });
  ...
  ipcMain.removeHandler(UPDATE_SET_CHANNEL);
  ipcMain.handle(UPDATE_SET_CHANNEL, async (_event, requestedChannel: unknown) => {
    const nextChannel = normalizeDesktopUpdateChannel(requestedChannel, selectedUpdateChannel);
    if (nextChannel === selectedUpdateChannel) return updateState;
    selectedUpdateChannel = nextChannel;
    persistSelectedUpdateChannel(nextChannel);
    autoUpdater.channel = resolveUpdaterChannelName(nextChannel);
    autoUpdater.allowPrerelease = nextChannel === "development";
    await clearPendingUpdateCache("update channel changed");
    setUpdateState({
      ...initialUpdateState(),
      enabled: updateState.enabled,
      status: updateState.enabled ? "idle" : "disabled",
  ...
  void ensureBrowserUsePipeServer().catch((error) => {
    console.warn("[Modesto browser] Failed to start browser-use native pipe", error);
  });
...
function getTitleBarOptions(): BrowserWindowConstructorOptions {
  ...
  return {
    ...
    // Derived from the shared chat-surface header geometry (@modesto/shared/desktopChrome)
...
function createWindow(): BrowserWindow {
  ...
  if (isDevelopment) {
    ...
    void window.loadURL(MODESTO_DESKTOP_ENTRY_URL);
  }
...
if (hasSingleInstanceLock) {
  migrateLegacyProfileBeforeElectronReady(userDataPath);
...
async function bootstrap(): Promise<void> {
  ...
  try {
    installBundledProviderClisOnce();
  } catch (error) {
    // Direct bundled paths remain on PATH as a fallback; keep startup usable
    // while recording enough detail to diagnose a damaged installation.
    writeDesktopLogHeader(`provider CLI bootstrap failed message=${formatErrorMessage(error)}`);
    console.error("[desktop] Failed to install bundled provider CLIs", error);
  }
  ...
  if (isDevelopment) {
    ...
    startOptionalCursorAgentRuntimeBootstrap();
    startOptionalNpmPlatformCliRuntimeBootstraps();
    return;
  }
...
function startOptionalCursorAgentRuntimeBootstrap(): void {
  if (!app.isPackaged) return;
  writeDesktopLogHeader("cursor-agent runtime bootstrap scheduled in background");
  startCursorAgentRuntimeBootstrapInBackground({
    installRoot: Path.join(BASE_DIR, "provider-tools"),
    onResult: (cursorResult) => {
      writeDesktopLogHeader(
        `cursor-agent runtime bootstrap status=${cursorResult.status}` +
          (cursorResult.error ? ` error=${cursorResult.error}` : ""),
      );
      if (cursorResult.status !== "installed" || isQuitting) return;
      try {
        // Refresh the user-facing launcher only after a first-time background install.
        // Already-installed runtimes were included by the synchronous bootstrap above.
        installBundledProviderClisOnce();
      } catch (error) {
        writeDesktopLogHeader(
          `cursor-agent launcher refresh failed message=${formatErrorMessage(error)}`,
        );
      }
    },
    onError: (error) => {
      writeDesktopLogHeader(
        `cursor-agent runtime bootstrap crashed message=${formatErrorMessage(error)}`,
      );
    },
  });
}

const NPM_PLATFORM_CLI_PROVIDERS: readonly NpmPlatformCliProvider[] = [
  "codex",
  "claude",
  "opencode",
];

// Codex, Claude, and OpenCode are too large to bundle in every install (see
// providerCliRuntimeBootstrap.ts) — same "download on first run, degrade to unavailable on
// failure" contract as cursor-agent, just for three providers instead of one.
function startOptionalNpmPlatformCliRuntimeBootstraps(): void {
  if (!app.isPackaged) return;
  for (const provider of NPM_PLATFORM_CLI_PROVIDERS) {
    writeDesktopLogHeader(`${provider} runtime bootstrap scheduled in background`);
    startProviderCliRuntimeBootstrapInBackground({
      provider,
      installRoot: Path.join(BASE_DIR, "provider-tools"),
      onResult: (result) => {
        writeDesktopLogHeader(
          `${provider} runtime bootstrap status=${result.status}` +
            (result.error ? ` error=${result.error}` : ""),
        );
        if (result.status !== "installed" || isQuitting) return;
        try {
          // Refresh the user-facing launcher only after a first-time background install.
          installBundledProviderClisOnce();
        } catch (error) {
          writeDesktopLogHeader(
            `${provider} launcher refresh failed message=${formatErrorMessage(error)}`,
          );
        }
      },
      onError: (error) => {
        writeDesktopLogHeader(
          `${provider} runtime bootstrap crashed message=${formatErrorMessage(error)}`,
        );
      },
    });
  }
...
if (process.platform !== "win32") {
  process.on("uncaughtException", (error: unknown) => {
    ...
    // Throwing again from an uncaughtException handler makes Electron abort with
    // SIGABRT and produces the macOS "quit unexpectedly" dialog. Record the
    // callback failure and keep the desktop shell alive so isolated updater or
    // child-process failures can recover through their existing state machines.
    console.error("[desktop] Recovered from an uncaught main-process exception:", error);
    writeDesktopLogHeader(`uncaught exception recovered: ${formatErrorMessage(error)}`);
  });

  process.on("unhandledRejection", (reason: unknown) => {
    console.error("[desktop] Recovered from an unhandled promise rejection:", reason);
    writeDesktopLogHeader(`unhandled rejection recovered: ${formatErrorMessage(reason)}`);
  });
