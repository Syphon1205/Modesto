...
import * as SshAuth from "@modesto/ssh/auth";
import { discoverSshHosts } from "@modesto/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@modesto/ssh/errors";
import * as SshTunnel from "@modesto/ssh/tunnel";
...
>()("@modesto/desktop/ssh/DesktopSshEnvironment") {}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Modesto window is not available for SSH authentication.";
      break;
