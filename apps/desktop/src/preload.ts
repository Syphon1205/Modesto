...
contextBridge.exposeInMainWorld("desktopBridge", {
  ...
  listListeningLibraries: () =>
    ipcRenderer.invoke(IpcChannels.LIST_LISTENING_LIBRARIES_CHANNEL, undefined),
  requestListeningLibraryAccess: (id) =>
    ipcRenderer.invoke(IpcChannels.REQUEST_LISTENING_LIBRARY_CHANNEL, { id }),
  listListeningLibraryPlaylists: (id) =>
    ipcRenderer.invoke(IpcChannels.LIST_LISTENING_LIBRARY_PLAYLISTS_CHANNEL, { id }),
  listListeningLibraryTracks: (id, playlistId) =>
    ipcRenderer.invoke(IpcChannels.LIST_LISTENING_LIBRARY_TRACKS_CHANNEL, { id, playlistId }),
  controlListeningLibrary: (input) =>
    ipcRenderer.invoke(IpcChannels.CONTROL_LISTENING_LIBRARY_CHANNEL, input),
  getListeningLibraryPlayback: (id) =>
    ipcRenderer.invoke(IpcChannels.LISTENING_LIBRARY_PLAYBACK_CHANNEL, { id }),
  ambientPresence: {
    open: () => ipcRenderer.invoke(IpcChannels.AMBIENT_PRESENCE_OPEN_CHANNEL),
    close: () => ipcRenderer.invoke(IpcChannels.AMBIENT_PRESENCE_CLOSE_CHANNEL),
    setBubbles: (bubbles) =>
      ipcRenderer.invoke(IpcChannels.AMBIENT_PRESENCE_SET_BUBBLES_CHANNEL, { bubbles }),
  },
  openApplication: (id) => ipcRenderer.invoke(IpcChannels.OPEN_APPLICATION_CHANNEL, { id }),
  floatingChat: {
    open: (routePath) => ipcRenderer.invoke(IpcChannels.FLOATING_CHAT_OPEN_CHANNEL, { routePath }),
    close: () => ipcRenderer.invoke(IpcChannels.FLOATING_CHAT_CLOSE_CHANNEL),
  },
  rendererActivity: {
    setLive: (live) => ipcRenderer.invoke(IpcChannels.SET_RENDERER_ACTIVITY_CHANNEL, { live }),
  },
  appshots: {
    captureNow: () => ipcRenderer.invoke(IpcChannels.APPSHOT_CAPTURE_NOW_CHANNEL),
    getPermissionStatus: () => ipcRenderer.invoke(IpcChannels.APPSHOT_PERMISSION_STATUS_CHANNEL),
    openPrivacySettings: () =>
      ipcRenderer.invoke(IpcChannels.APPSHOT_OPEN_PRIVACY_SETTINGS_CHANNEL),
  },
  onAppshotReceived: (listener) => {
    const wrappedListener = (_event: Electron.IpcRendererEvent, payload: unknown) => {
      if (typeof payload !== "object" || payload === null) return;
      listener(payload as Parameters<typeof listener>[0]);
    };
    ipcRenderer.on(IpcChannels.APPSHOT_RECEIVED_CHANNEL, wrappedListener);
    return () => {
      ipcRenderer.removeListener(IpcChannels.APPSHOT_RECEIVED_CHANNEL, wrappedListener);
    };
  },
  ...
  preview: {
    ...
    probeSignIn: (input) => ipcRenderer.invoke(IpcChannels.PREVIEW_PROBE_SIGN_IN_CHANNEL, input),
