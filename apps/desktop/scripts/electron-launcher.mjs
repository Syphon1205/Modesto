// This file mostly exists because we want dev mode to say "Modesto (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Modesto (Dev)" : "Modesto (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.fabweavr.modesto.dev.${devBundleIdSuffix || "local"}`
  : "com.fabweavr.modesto";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["modesto-dev"] : ["modesto"];
...
// oxlint-disable-next-line modesto/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["MODESTO_PORT", environment.MODESTO_PORT],
    ["MODESTO_HOME", environment.MODESTO_HOME],
    ["MODESTO_COMMIT_HASH", environment.MODESTO_COMMIT_HASH],
    ["MODESTO_OTLP_TRACES_URL", environment.MODESTO_OTLP_TRACES_URL],
    ["MODESTO_OTLP_EXPORT_INTERVAL_MS", environment.MODESTO_OTLP_EXPORT_INTERVAL_MS],
    ["MODESTO_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --modesto-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Modesto name.
