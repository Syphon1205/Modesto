...
const remoteDebuggingPort = process.env.MODESTO_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line modesto/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.MODESTO_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.MODESTO_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--modesto-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--modesto-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
