...
function cleanupStaleDevApps() {
  ...
  spawnSync("pkill", ["-f", "--", `--modesto-dev-root=${desktopDir}`], { stdio: "ignore" });
}
...
function listStaleComputerUsePids() {
  ...
  const result = spawnSync("pgrep", ["-fal", "Modesto \\(Dev\\).*(computerUseMcp\\.mjs mcp)"], {
    encoding: "utf8",
  });
...
function cleanupStaleComputerUseApps() {
  ...
  console.error(
    `[desktop-dev] Cleaning up ${stalePids.length} stale Modesto (Dev) Computer Use helper process${stalePids.length === 1 ? "" : "es"} from other worktrees.`,
  );
...
function warnIfAlphaAppRunning() {
  ...
  const result = spawnSync(
    "pgrep",
    ["-fal", "/Applications/Modesto\\.app/Contents/MacOS/Modesto"],
    {
      encoding: "utf8",
    },
  );
  ...
  console.error(
    "[desktop-dev] Modesto is still running. Close it before testing voice in Modesto (Dev), or you may be looking at the wrong app/runtime.",
  );
...
function startApp() {
  ...
  const app = spawn(
    resolveElectronPath(),
    [`--modesto-dev-root=${desktopDir}`, "dist-electron/main.js"],
    {
      cwd: desktopDir,
      env: {
        ...childEnv,
