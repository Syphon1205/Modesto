...
const shouldLaunchElectronAfterPack = process.env.MODESTO_DESKTOP_DEV === "1";
...
export default defineConfig({
  run: {
    tasks: {
      ...
      dev: {
        command:
          "node scripts/build-preview-annotation-css.mjs && cross-env MODESTO_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["t3#build"],
        cache: false,
      },
      "dev:bundle": {
  ...
  pack: [
    {
      ...
      clean: true,
      deps: {
        alwaysBundle: (id) => id.startsWith("@modesto/"),
      },
    ...
    },
    {
      ...
      sourcemap: true,
      outExtensions: () => ({ js: ".cjs" }),
      entry: ["src/ambient-presence-preload.ts"],
    },
  ],
});
