#!/bin/bash
# Compiles modesto-device-helper with the user's own Xcode toolchain.
#
# This is Modesto's helper, not the user's app: sources live next to this
# script and are compiled with `xcrun clang` / `xcrun swiftc`. Never
# `xcodebuild -project` / `-workspace`, never a dummy `.xcodeproj`, never
# `open -a Xcode`.
...
#   MODESTO_DEVICE_HELPER_OUT=/path build.sh
#
...
OUT_DIR="${1:-${MODESTO_DEVICE_HELPER_OUT:-$SOURCE_DIR/build}}"
BINARY_NAME="modesto-device-helper"
