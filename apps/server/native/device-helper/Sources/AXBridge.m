...
id ModestoEmptyTranslatorResponse(void);
...
static id ModestoMsgSend0(id target, NSString *selectorName) {
...
static NSString *ModestoStringAttribute(id element, NSString *selectorName) {
  id value = ModestoMsgSend0(element, selectorName);
  return [value isKindOfClass:NSString.class] ? value : nil;
}

static BOOL ModestoBoolAttribute(id element, NSString *selectorName) {
  SEL selector = NSSelectorFromString(selectorName);
...
static id ModestoJSONValue(id value) {
  if (value == nil) {
    return NSNull.null;
  }
...
@implementation ModestoAXBridge {
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _callbackQueue = dispatch_queue_create("dev.modesto.device-helper.ax", DISPATCH_QUEUE_CONCURRENT);
    _requestTimeout = 5.0;
  }
  return self;
}
...
- (id)accessibilityTranslationDelegateBridgeCallbackWithToken:(NSString *)token {
  ...
  id (^callback)(id) = ^id(id request) {
    if (device == nil) {
      return ModestoEmptyTranslatorResponse();
    }
    ...
    return response ?: ModestoEmptyTranslatorResponse();
  };
...
- (nullable NSDictionary *)frontmostTreeWithMaxDepth:(NSInteger)maxDepth error:(NSError **)error {
  if (_translator == nil) {
    if (error) {
      *error = [NSError errorWithDomain:@"dev.modesto.device-helper.ax"
                                   code:1
...
- (NSDictionary *)serializeElement:(id)element depth:(NSInteger)depth maxDepth:(NSInteger)maxDepth {
  ...
  NSString *role = ModestoStringAttribute(element, @"accessibilityRole");
  ...
  NSString *label = ModestoStringAttribute(element, @"accessibilityLabel");
  if (label.length > 0) {
    node[@"label"] = label;
  }
  id value = ModestoJSONValue(ModestoMsgSend0(element, @"accessibilityValue"));
  if (value != NSNull.null) {
    node[@"value"] = value;
  }
  NSString *identifier = ModestoStringAttribute(element, @"accessibilityIdentifier");
  if (identifier.length > 0) {
    node[@"identifier"] = identifier;
  }
  NSString *title = ModestoStringAttribute(element, @"accessibilityTitle");
  if (title.length > 0) {
    node[@"title"] = title;
  }
  ...
  NSString *subrole = ModestoStringAttribute(element, @"accessibilitySubrole");
  ...
  node[@"enabled"] = @(ModestoBoolAttribute(element, @"isAccessibilityEnabled"));
  ...
  id rawChildren = ModestoMsgSend0(element, @"accessibilityChildren");
