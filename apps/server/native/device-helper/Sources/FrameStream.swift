...
// `@modesto/contracts` (see HEADER.md).
//
...
final class FrameSocketWriter {
  ...
  private let queue = DispatchQueue(label: "dev.modesto.device-helper.socket")
...
final class FrameStream {
  ...
  private let encodeQueue = DispatchQueue(label: "dev.modesto.device-helper.encode")
