...
typedef union {
  ModestoIndigoTouch touch;
  ModestoIndigoButton button;
...
typedef struct {
  ...
  unsigned int field3;
  ModestoIndigoEvent event;
} ModestoIndigoPayload;

typedef struct {
  ModestoIndigoMachHeader header;
  unsigned int innerSize;
  unsigned char eventType;
  ModestoIndigoPayload payload;
} ModestoIndigoMessage;
...
typedef ModestoIndigoMessage *(*ModestoIndigoButtonFn)(int keyCode, int op, int target);
typedef ModestoIndigoMessage *(*ModestoIndigoKeyboardFn)(uint32_t usage, int op);
typedef ModestoIndigoMessage *(*ModestoIndigoMouseFn)(CGPoint *point0, CGPoint *point1, int target,
                                                    int eventType, BOOL extra);
typedef ModestoIndigoMessage *(*ModestoIndigoArbitraryFn)(int target, uint32_t page, uint32_t usage,
                                                        int op);

BOOL ModestoHardwareButtonFromName(NSString *name, ModestoHardwareButton *outButton) {
  NSDictionary<NSString *, NSNumber *> *map = @{
    @"home": @(ModestoHardwareButtonHome),
    @"lock": @(ModestoHardwareButtonLock),
    @"side": @(ModestoHardwareButtonSide),
    @"siri": @(ModestoHardwareButtonSiri),
    @"volume-up": @(ModestoHardwareButtonVolumeUp),
    @"volume-down": @(ModestoHardwareButtonVolumeDown),
  };
  ...
  if (outButton != NULL) {
    *outButton = (ModestoHardwareButton)found.integerValue;
  }
  return YES;
}

static int ModestoButtonSource(ModestoHardwareButton button) {
  switch (button) {
    case ModestoHardwareButtonHome: return kButtonSourceHome;
    case ModestoHardwareButtonLock: return kButtonSourceLock;
    case ModestoHardwareButtonSide: return kButtonSourceSide;
    case ModestoHardwareButtonSiri: return kButtonSourceSiri;
    case ModestoHardwareButtonVolumeUp:
    case ModestoHardwareButtonVolumeDown: return -1;
  }
...
static uint32_t ModestoConsumerUsage(ModestoHardwareButton button) {
  switch (button) {
    case ModestoHardwareButtonVolumeUp: return kHIDUsageVolumeIncrement;
    case ModestoHardwareButtonVolumeDown: return kHIDUsageVolumeDecrement;
    default: return 0;
  }
}
...
static uint32_t ModestoUsageForCharacter(unichar c, BOOL *outShift) {
  BOOL shift = NO;
  uint32_t usage = 0;
...
@implementation ModestoHIDBridge {
  id _client;
  dispatch_queue_t _sendQueue;
  ModestoIndigoButtonFn _buttonFn;
  ModestoIndigoKeyboardFn _keyboardFn;
  ModestoIndigoMouseFn _mouseFn;
  ModestoIndigoArbitraryFn _arbitraryFn;
...
- (BOOL)attachToDevice:(id)device error:(NSError **)error {
  NSString *developerDir = NSProcessInfo.processInfo.environment[@"MODESTO_DEVELOPER_DIR"];
  ...
  if (kit == NULL) {
    if (error) {
      *error = [NSError errorWithDomain:@"dev.modesto.device-helper.hid"
                                   code:1
  ...
  _arbitraryFn = (ModestoIndigoArbitraryFn)dlsym(kit, "IndigoHIDMessageForHIDArbitrary");
  _buttonFn = (ModestoIndigoButtonFn)dlsym(kit, "IndigoHIDMessageForButton");
  _keyboardFn = (ModestoIndigoKeyboardFn)dlsym(kit, "IndigoHIDMessageForKeyboardArbitrary");
  _mouseFn = (ModestoIndigoMouseFn)dlsym(kit, "IndigoHIDMessageForMouseNSEvent");
  ...
  if (client == nil) {
    if (error) {
      *error = initError ?: [NSError errorWithDomain:@"dev.modesto.device-helper.hid"
                                                code:4
  ...
  _client = client;
  _sendQueue = dispatch_queue_create("dev.modesto.device-helper.hid", DISPATCH_QUEUE_SERIAL);
  return YES;
}

- (void)sendMessage:(ModestoIndigoMessage *)message {
  ...
  ((void (*)(id, SEL, ModestoIndigoMessage *, BOOL, dispatch_queue_t, void (^)(NSError *)))objc_msgSend)(
...
- (void)sendTouchAtX:(double)x y:(double)y down:(BOOL)down {
  ...
  ModestoIndigoMessage *seed = _mouseFn(&point, NULL, kTouchTarget, down ? kButtonOpDown : kButtonOpUp, NO);
  if (seed == NULL) {
    [self noteUndelivered];
    return;
  }
  ...
  size_t stride = sizeof(ModestoIndigoPayload);
  ModestoIndigoMessage *message = calloc(1, sizeof(ModestoIndigoMessage) + stride);
  if (message == NULL) {
    free(seed);
    [self noteUndelivered];
    return;
  ...
  memcpy(&message->payload.event.touch, &seed->payload.event.touch, sizeof(ModestoIndigoTouch));
  ...
  ModestoIndigoPayload *secondPayload = (ModestoIndigoPayload *)second;
...
- (void)sendKeyUsage:(uint32_t)usage down:(BOOL)down {
  ...
  ModestoIndigoMessage *message = _keyboardFn(usage, down ? kButtonOpDown : kButtonOpUp);
  [self sendMessage:message];
}
...
- (NSInteger)typeText:(NSString *)text {
  ...
  for (NSUInteger index = 0; index < length; index++) {
    ...
    BOOL needsShift = NO;
    uint32_t usage = ModestoUsageForCharacter(character, &needsShift);
    if (usage == 0) {
      skipped++;
      continue;
    }
...
- (void)sendButton:(ModestoHardwareButton)button down:(BOOL)down {
  int op = down ? kButtonOpDown : kButtonOpUp;
  uint32_t consumerUsage = ModestoConsumerUsage(button);
  ...
  [self sendMessage:_buttonFn(ModestoButtonSource(button), op, kButtonTargetHardware)];
}

- (void)tapButton:(ModestoHardwareButton)button {
