// modesto-device-helper — the native side of Modesto's Device Pane.
//
...
final class HelperSession {
  ...
  private(set) var hid: ModestoHIDBridge?
  private(set) var accessibility: ModestoAXBridge?
  ...
  func attach(udid: String) throws -> [String: Any] {
    ...
    let hidBridge = ModestoHIDBridge()
  ...
  func requireHID() throws -> ModestoHIDBridge {
  ...
  func requireAccessibility() throws -> ModestoAXBridge {
...
///
/// Every injection path in ModestoHIDBridge returns silently when it has no
...
private func withHIDDelivery(_ hid: ModestoHIDBridge, _ body: () -> Void) throws {
...
func handle(method: String, params: Params, session: HelperSession) throws -> Any {
  ...
  case "button":
    ...
    var button = ModestoHardwareButton.home
    guard ModestoHardwareButtonFromName(name, &button) else {
      throw RPCError(
        .invalidParams,
...
let requestQueue = DispatchQueue(label: "dev.modesto.device-helper.rpc")
