...
import * as AutomationScheduler from "../src/automation/AutomationScheduler.ts";
...
const startupDependencies = Layer.mergeAll(
  ...
  // Startup asks the scheduler to begin ticking; this test is about session
  // recovery, so it gets a scheduler that does nothing rather than a live one
  // reaching for SQL and the orchestration engine.
  Layer.succeed(AutomationScheduler.AutomationScheduler, { start: () => Effect.void }),
