  ...
  Effect.gen(function* () {
    ...
    const providerLayer = useRealCodex
      ...
      : makeProviderServiceLive({
          // Replay adapters do not connect to MCP. Keep their sessions independent
          // of HTTP test servers that register a process-wide credential issuer.
          issueMcpCredential: () => Effect.succeed(undefined),
          touchMcpCredential: () => Effect.succeed(undefined),
    ...
    const layer = Layer.empty.pipe(
      ...
      Layer.provideMerge(ServerConfig.layerTest(workspaceDir, rootDir)),
      Layer.provideMerge(
        ServerSecretStore.layer.pipe(Layer.provide(ServerConfig.layerTest(workspaceDir, rootDir))),
      ),
      Layer.provideMerge(NodeServices.layer),
    );
