...
import { fromJsonStringPretty } from "@modesto/shared/schemaJson";
import { fromYaml } from "@modesto/shared/schemaYaml";
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      if (yield* fs.exists(webIndex)) {
        // Replace rather than merge so a prior incomplete client cannot leave
        // stale entry bundles that reference deleted lazy chunks.
