...
import {
  ...
  type ProviderSetPluginInstalledResult,
...
import { getModelSelectionBooleanOptionValue, normalizeModelSlug } from "@modesto/shared/model";
import { decodeSubagentReceiverThreadIds } from "@modesto/shared/subagents";
import { prepareWindowsSafeProcess } from "@modesto/shared/windowsProcess";
...
import { collectSkillsFromRoots } from "./provider/skillsCatalog.ts";
...
interface CodexSetPluginInstalledInput {
...
// Maps Modesto's simple runtime toggle to Codex thread-level permission overrides.
...
// Modesto re-sends turn-level Codex permission overrides, so keep "always allow"
...
function killChildTree(child: { readonly pid?: number | undefined; kill(): boolean }): void {
...
function spawnCodexAppServer(input: {
  ...
  // Routes this process at a Settings → Model Routers custom endpoint via Codex's
  // own `-c key=value` global config override, applied before the subcommand.
  readonly modelProvider?: string;
}): ChildProcessWithoutNullStreams {
  const args = input.modelProvider
    ? ["-c", `model_provider=${input.modelProvider}`, "app-server"]
    : ["app-server"];
...
const CODEX_PLUGIN_MUTATION_TIMEOUT_MS = 120_000;
const CODEX_PLUGIN_MUTATION_MAX_OUTPUT_BYTES = 1024 * 1024;

export function codexPluginMutationArgs(input: {
  readonly marketplaceName: string;
  readonly pluginName: string;
  readonly installed: boolean;
}): string[] {
...
function runCodexPluginMutation(input: {
  readonly binaryPath: string;
  readonly cwd: string;
  readonly env: NodeJS.ProcessEnv;
  readonly marketplaceName: string;
  ...
  return new Promise((resolve, reject) => {
    const args = codexPluginMutationArgs(input);
    const prepared = prepareWindowsSafeProcess(input.binaryPath, args, {
      cwd: input.cwd,
      env: input.env,
    });
    ...
    const appendOutput = (current: string, chunk: Buffer): string =>
      (current + chunk.toString("utf8")).slice(-CODEX_PLUGIN_MUTATION_MAX_OUTPUT_BYTES);
    const timeout = setTimeout(() => {
      if (settled) return;
      settled = true;
      killChildTree(child);
      reject(new Error("Codex plugin operation timed out."));
    }, CODEX_PLUGIN_MUTATION_TIMEOUT_MS);
    timeout.unref();
    ...
    child.once("close", (code) => {
      ...
      const detail =
        stderr.trim() || stdout.trim() || `Codex exited with status ${code ?? "unknown"}.`;
      reject(new Error(detail));
    });
  });
}

interface CodexCliPluginRecord {
  readonly pluginId?: unknown;
  readonly name?: unknown;
  readonly marketplaceName?: unknown;
  readonly installed?: unknown;
  readonly enabled?: unknown;
  readonly installPolicy?: unknown;
  readonly authPolicy?: unknown;
  readonly source?: unknown;
  readonly marketplaceSource?: unknown;
}

interface CodexCliPluginList {
  readonly installed?: unknown;
...
function runCodexPluginList(input: {
  readonly binaryPath: string;
  readonly cwd: string;
  readonly env: NodeJS.ProcessEnv;
}): Promise<CodexCliPluginList> {
  return new Promise((resolve, reject) => {
    const args = ["plugin", "list", "--available", "--json"];
    const prepared = prepareWindowsSafeProcess(input.binaryPath, args, {
      cwd: input.cwd,
      env: input.env,
    });
    ...
    const timeout = setTimeout(() => {
      ...
      settled = true;
      killChildTree(child);
      reject(new Error("Codex plugin listing timed out."));
    }, CODEX_PLUGIN_MUTATION_TIMEOUT_MS);
    timeout.unref();
    child.stdout.on("data", (chunk: Buffer) => {
      stdout = (stdout + chunk.toString("utf8")).slice(-CODEX_PLUGIN_MUTATION_MAX_OUTPUT_BYTES);
    });
    child.stderr.on("data", (chunk: Buffer) => {
      stderr = (stderr + chunk.toString("utf8")).slice(-CODEX_PLUGIN_MUTATION_MAX_OUTPUT_BYTES);
    });
    child.once("error", (error) => {
      if (settled) return;
      settled = true;
    ...
    child.once("close", (code) => {
      ...
      clearTimeout(timeout);
      if (code !== 0) {
        reject(new Error(stderr.trim() || `Codex exited with status ${code ?? "unknown"}.`));
        return;
      }
      try {
        resolve(JSON.parse(stdout) as CodexCliPluginList);
      } catch (error) {
        reject(new Error("Codex returned invalid plugin list JSON.", { cause: error }));
      }
    });
  });
}

async function readJsonFileRecord(filePath: string): Promise<Record<string, unknown> | null> {
  try {
    return stringRecord(JSON.parse(await readFile(filePath, "utf8")));
  } catch {
    return null;
  }
}

function resolveCliMarketplacePath(record: CodexCliPluginRecord, sourcePath: string): string {
  const marketplaceSource = stringRecord(record.marketplaceSource);
  const registeredPath = nonEmptyString(marketplaceSource?.source);
  return registeredPath ?? path.dirname(path.dirname(sourcePath));
}

export function resolveCodexPluginArtworkPath(
  pluginRoot: string,
  artworkPointer: string | undefined,
): string | undefined {
  const pointer = artworkPointer?.trim();
  if (!pointer) return undefined;
  if (/^(?:data:|https?:)/i.test(pointer)) return pointer;
...
function parseLocalPluginInterface(
  manifest: Record<string, unknown> | null,
  pluginRoot: string,
): ProviderPluginDescriptor["interface"] | undefined {
  if (!manifest) return undefined;
  const rawInterface = stringRecord(manifest.interface);
  const author = stringRecord(manifest.author);
  const values = rawInterface ?? {};
  const stringArray = (value: unknown): string[] =>
    Array.isArray(value)
      ? value.map(nonEmptyString).filter((item): item is string => item !== undefined)
      : [];
  const displayName = nonEmptyString(values.displayName) ?? nonEmptyString(manifest.name);
  const shortDescription =
    nonEmptyString(values.shortDescription) ?? nonEmptyString(manifest.description);
  const developerName = nonEmptyString(values.developerName) ?? nonEmptyString(author?.name);
  const longDescription = nonEmptyString(values.longDescription);
  const category = nonEmptyString(values.category);
  const capabilities = stringArray(values.capabilities);
  const websiteUrl = nonEmptyString(values.websiteURL ?? values.websiteUrl ?? manifest.homepage);
  const privacyPolicyUrl = nonEmptyString(values.privacyPolicyURL ?? values.privacyPolicyUrl);
  const termsOfServiceUrl = nonEmptyString(values.termsOfServiceURL ?? values.termsOfServiceUrl);
  const defaultPrompt = stringArray(values.defaultPrompt);
  const brandColor = nonEmptyString(values.brandColor);
  const composerIcon = nonEmptyString(values.composerIcon);
  const logo = resolveCodexPluginArtworkPath(
    pluginRoot,
    nonEmptyString(values.logo) ?? composerIcon,
  );
  return {
    ...
    ...(defaultPrompt.length ? { defaultPrompt } : {}),
...
export async function readCodexPluginBundledSkillSummaries(input: {
  readonly pluginRoot: string;
  readonly pluginId: string;
  readonly manifest: Record<string, unknown> | null;
}): Promise<ProviderPluginSkillSummary[]> {
  const skillsPointer = nonEmptyString(input.manifest?.skills) ?? "./skills/";
  const skills = await collectSkillsFromRoots([
    {
      path: path.resolve(input.pluginRoot, skillsPointer),
      scope: `codex-plugin:${input.pluginId}`,
    },
  ]).catch(() => []);
...
async function parseCodexCliPluginMarketplaces(
  result: CodexCliPluginList,
): Promise<ProviderListPluginsResult["marketplaces"]> {
  ...
  const records = [...installed, ...available]
    .map((value) => stringRecord(value) as CodexCliPluginRecord | null)
    .filter((value): value is CodexCliPluginRecord => value !== null);
  const grouped = new Map<string, { path: string; plugins: ProviderPluginDescriptor[] }>();
  for (const record of records) {
    const id = nonEmptyString(record.pluginId);
    const name = nonEmptyString(record.name);
    const marketplaceName = nonEmptyString(record.marketplaceName);
    const source = stringRecord(record.source);
    const sourcePath = nonEmptyString(source?.path);
    const installPolicy = nonEmptyString(record.installPolicy);
    const authPolicy = nonEmptyString(record.authPolicy);
    if (
      !id ||
      !name ||
      !marketplaceName ||
    ...
    const manifest = await readJsonFileRecord(
      path.join(sourcePath, ".codex-plugin", "plugin.json"),
    );
    const pluginInterface = parseLocalPluginInterface(manifest, sourcePath);
    const bundledSkills = await readCodexPluginBundledSkillSummaries({
    ...
    const descriptor: ProviderPluginDescriptor = {
      ...
      ...(bundledSkills.length > 0 ? { bundledSkills } : {}),
    };
    const current = grouped.get(marketplaceName);
    if (current) {
      current.plugins.push(descriptor);
    } else {
      grouped.set(marketplaceName, {
        path: resolveCliMarketplacePath(record, sourcePath),
        plugins: [descriptor],
      });
    }
  }
  return [...grouped.entries()].map(([name, marketplace]) => ({
    name,
...
async function readLocalCodexPluginDetail(input: {
  readonly marketplaceName: string;
  readonly marketplacePath: string;
  readonly summary: ProviderPluginDescriptor;
}): Promise<ProviderPluginDetail> {
  const pluginRoot = input.summary.source.path;
  const manifest = await readJsonFileRecord(path.join(pluginRoot, ".codex-plugin", "plugin.json"));
  const skillsPointer = nonEmptyString(manifest?.skills) ?? "./skills/";
  const skills = await collectSkillsFromRoots([
    {
      path: path.resolve(pluginRoot, skillsPointer),
      scope: `codex-plugin:${input.summary.id}`,
    },
  ]).catch(() => []);

  const appsPointer = nonEmptyString(manifest?.apps);
  const appsManifest = appsPointer
    ? await readJsonFileRecord(path.resolve(pluginRoot, appsPointer))
    : null;
  const appsRecord = stringRecord(appsManifest?.apps);
  const apps: ProviderPluginAppSummary[] = appsRecord
    ? Object.entries(appsRecord).flatMap(([name, value]) => {
        const app = stringRecord(value);
        const id = nonEmptyString(app?.id);
        return id ? [{ id, name, needsAuth: true }] : [];
      })
    : [];

  const mcpPointer = nonEmptyString(manifest?.mcpServers);
  const mcpManifest = mcpPointer
    ? await readJsonFileRecord(path.resolve(pluginRoot, mcpPointer))
    : null;
  const mcpServers = Object.keys(stringRecord(mcpManifest?.mcpServers) ?? {});
  const description =
    nonEmptyString(stringRecord(manifest?.interface)?.longDescription) ??
    nonEmptyString(manifest?.description) ??
    input.summary.interface?.longDescription ??
    input.summary.interface?.shortDescription;
...
export function buildCodexInitializeParams() {
  return {
    clientInfo: {
      name: "modesto_desktop",
      title: "Modesto Desktop",
      version: "0.1.0",
    },
    capabilities: {
...
export class CodexAppServerManager extends EventEmitter<CodexAppServerManagerEvents> {
  ...
  private readonly modestoSkillsDir: string | undefined;
  // Settings → Model Routers: Modesto's own settings.json/secrets dir, forwarded to
  // buildCodexProcessEnv so custom endpoint definitions reach every spawned process.
  private readonly settingsPath: string | undefined;
  private readonly secretsDir: string | undefined;
  constructor(
    services?: ServiceMap.ServiceMap<never>,
    options?: {
      readonly modestoSkillsDir?: string;
    ...
    this.modestoSkillsDir = options?.modestoSkillsDir;
    this.settingsPath = options?.settingsPath;
    this.secretsDir = options?.secretsDir;
  }

  private buildCodexEnv(
    input: Omit<Parameters<typeof buildCodexProcessEnv>[0], "settingsPath" | "secretsDir"> = {},
  ): NodeJS.ProcessEnv {
    return buildCodexProcessEnv({
      ...input,
      ...(this.settingsPath ? { settingsPath: this.settingsPath } : {}),
      ...(this.secretsDir ? { secretsDir: this.secretsDir } : {}),
    });
  }

  // Registers `~/.modesto/skills` as a codex skill root so portable skills are
  ...
  private async registerModestoSkillsRoot(context: CodexSessionContext): Promise<void> {
    if (!this.modestoSkillsDir) {
      return;
    }
    try {
      await this.sendRequest(context, "skills/extraRoots/set", {
        extraRoots: [this.modestoSkillsDir],
      });
    } catch (error) {
      // Older codex builds (< extra-roots support) keep working; Modesto-only
  ...
  async startSession(input: CodexAppServerStartSessionInput): Promise<ProviderSession> {
    ...
    try {
      ...
      const child = spawnCodexAppServer({
        ...
        cwd: resolvedCwd,
        env: codexEnv,
        ...(codexOptions.customEndpointId ? { modelProvider: codexOptions.customEndpointId } : {}),
      });
      ...
      await this.registerModestoSkillsRoot(context);
      try {
  ...
  async listPlugins(input: CodexPluginListInput): Promise<ProviderListPluginsResult> {
    ...
    const appServerResult = this.parsePluginListResponse(response);
    let cliMarketplaces: ProviderListPluginsResult["marketplaces"] = [];
    let cliListError: string | null = null;
    try {
      cliMarketplaces = await parseCodexCliPluginMarketplaces(
        await runCodexPluginList({
      ...
      cliListError = error instanceof Error ? error.message : String(error);
    }

    const appMarketplaceByName = new Map(
      appServerResult.marketplaces.map((marketplace) => [marketplace.name, marketplace] as const),
    );
    const mergedMarketplaces = cliMarketplaces.map((marketplace) => {
      const appMarketplace = appMarketplaceByName.get(marketplace.name);
      if (!appMarketplace) return marketplace;
      appMarketplaceByName.delete(marketplace.name);
      const appPluginById = new Map(
        appMarketplace.plugins.map((plugin) => [plugin.id, plugin] as const),
      );
      return {
        ...marketplace,
        interface: appMarketplace.interface ?? marketplace.interface,
        plugins: marketplace.plugins.map((plugin) => {
          const appPlugin = appPluginById.get(plugin.id);
          if (!appPlugin) return plugin;
          const mergedInterface =
            plugin.interface || appPlugin.interface
              ? {
                  ...plugin.interface,
                  ...appPlugin.interface,
                  // App-server metadata can contain a relative path. The CLI
                  // projection resolves and validates artwork against the real
                  // plugin root, so keep that path authoritative.
                  ...(plugin.interface?.logo ? { logo: plugin.interface.logo } : {}),
                }
              : undefined;
          return {
            ...plugin,
            ...(mergedInterface ? { interface: mergedInterface } : {}),
          };
        }),
      };
    });
    mergedMarketplaces.push(...appMarketplaceByName.values());

    const result: ProviderListPluginsResult = {
      ...appServerResult,
      marketplaces: mergedMarketplaces,
      remoteSyncError: appServerResult.remoteSyncError ?? cliListError,
      source: cliMarketplaces.length > 0 ? "codex-app-server+codex-cli" : "codex-app-server",
      cached: false,
    };
  ...
  async readPlugin(input: CodexPluginReadInput): Promise<ProviderReadPluginResult> {
    ...
    let plugin: ProviderPluginDetail;
    ...
    try {
      ...
      const catalog = await this.listPlugins({ forceReload: true });
      const marketplace = catalog.marketplaces.find(
        (candidate) =>
          candidate.path === marketplacePath &&
          candidate.plugins.some((candidatePlugin) => candidatePlugin.name === pluginName),
      );
      const summary = marketplace?.plugins.find(
        (candidatePlugin) => candidatePlugin.name === pluginName,
      );
      if (!marketplace || !summary) {
        throw appServerError;
      }
      plugin = await readLocalCodexPluginDetail({
        marketplaceName: marketplace.name,
        marketplacePath: marketplace.path,
        summary,
      });
      source = "codex-cli-local-cache";
    }
  ...
  async setPluginInstalled(
    input: CodexSetPluginInstalledInput,
  ): Promise<ProviderSetPluginInstalledResult> {
    ...
    const mutate = async (): Promise<ProviderSetPluginInstalledResult> => {
      ...
      // The CLI owns installation state. Drop Modesto's projections and recycle
    ...
    this.pluginMutationTail = operation.then(
      () => undefined,
      () => undefined,
    );
    return operation;
  ...
  private parseModelListResponse(response: unknown): ProviderListModelsResult["models"] {
    ...
    return rawModels.flatMap((value) => {
      ...
      // Accept both Modesto's legacy string array and Remodex-style reasoning objects.
...
function readCodexProviderOptions(input: CodexAppServerStartSessionInput): {
  readonly binaryPath?: string;
  readonly homePath?: string;
  readonly customEndpointId?: string;
} {
  ...
  return {
    ...
    ...(options.customEndpointId ? { customEndpointId: options.customEndpointId } : {}),
  };
}
