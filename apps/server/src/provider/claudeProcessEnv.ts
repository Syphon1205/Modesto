...
/**
 * Path to the `claude` launcher Modesto's desktop shell bootstraps at
 * `{MODESTO_HOME}/provider-tools/bin/claude(.cmd)` (see
 * `apps/desktop/src/providerCliBootstrap.ts` / `packagedProviderCliSources`).
 * That launcher execs the real per-platform binary unpacked outside
 * `app.asar` (native binaries can't be spawned from inside an asar archive).
 *
 * Left unset, the Claude Agent SDK falls back to its own bundled/downloaded
 * runtime resolution, which has no notion of Electron's asar layout and ends
 * up trying to spawn a path inside `app.asar` in a packaged app — surfacing
 * as `spawn ENOTDIR`. Passing this default explicitly sidesteps that without
 * requiring the user to have a system-wide `claude` CLI installed (an
 * explicit user-configured Settings path still always wins over this).
 */
export function resolveBundledClaudeBinaryPath(input?: {
  readonly env?: NodeJS.ProcessEnv;
  readonly platform?: NodeJS.Platform;
}): string | undefined {
  const env = input?.env ?? process.env;
  const platform = input?.platform ?? process.platform;
  const homeDir = trimToUndefined(env.MODESTO_HOME);
  if (!homeDir) {
    return undefined;
  }
  const executableName = platform === "win32" ? "claude.cmd" : "claude";
  const candidate = nodePath.join(homeDir, "provider-tools", "bin", executableName);
