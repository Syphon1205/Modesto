...
const ACP_CHILD_FORCE_KILL_AFTER = "2 seconds" as const;
...
export interface AcpSessionRuntimeOptions {
  ...
  /**
   * `undefined` (or empty) skips the `authenticate` RPC call entirely rather
   * than sending a bogus method id - needed by the generic Custom ACP Agent
   * driver, which spawns an arbitrary agent whose real auth method id (if
   * any) isn't known ahead of time. Every built-in provider still passes a
   * real, verified id and keeps calling `authenticate` unconditionally.
   */
    ...
    const child = yield* spawner
      .spawn(
        ChildProcess.make(spawnCommand.command, spawnCommand.args, {
          ...
          ...(options.spawn.env ? { env: options.spawn.env, extendEnv: true } : {}),
          forceKillAfter: ACP_CHILD_FORCE_KILL_AFTER,
        ...
        promptSerializationSemaphore.withPermit(
          Effect.gen(function* () {
            ...
            return yield* Fiber.join(promptRpcFiber).pipe(
              ...
              // ACP delivers token usage once, on the prompt response - there
              // is no streaming usage notification - so this is the only place
              // an ACP-backed provider can report what a turn cost.
              Effect.tap((response) => {
                // A cancelled turn is synthesized locally and carries no usage,
                // which is also the honest answer - an interrupted turn has no
                // final accounting to report.
