...
describe("resolveDroidCliBinaryPath", () => {
  it("prefers PATH and then ~/.local/bin/droid when either exists", () => {
    const localBin = join(homedir(), ".local", "bin", "droid");
    const pathCandidate = (process.env.PATH ?? "")
      .split(delimiter)
      .filter(Boolean)
      .map((directory) => join(directory, "droid"))
      .find((candidate) => existsSync(candidate));
    const resolved = resolveDroidCliBinaryPath("");
    expect(resolved).toBe(pathCandidate ?? (existsSync(localBin) ? localBin : "droid"));
  });
});
