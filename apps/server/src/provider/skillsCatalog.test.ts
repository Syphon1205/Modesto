...
import type { ProviderSkillDescriptor } from "@modesto/contracts";
...
beforeEach(() => {
  clearSkillsCatalogCacheForTests();
  root = mkdtempSync(path.join(os.tmpdir(), "modesto-skills-catalog-"));
  homeDir = path.join(root, "home");
  modestoBaseDir = path.join(homeDir, ".modesto");
});
...
describe("discoverSkillsCatalog", () => {
  it("creates the Modesto skills folder on first discovery", async () => {
    await discoverSkillsCatalog({ homeDir, modestoBaseDir });
    await expect(access(path.join(modestoBaseDir, "skills"))).resolves.toBeUndefined();
  });

  it("aggregates skills from modesto and provider home folders with origin scopes", async () => {
    await writeSkill(path.join(modestoBaseDir, "skills", "portable"), "portable", "Modesto skill");
    ...
    const skills = await discoverSkillsCatalog({ homeDir, modestoBaseDir });
    const byName = new Map(skills.map((skill) => [skill.name, skill]));

    expect(byName.get("portable")?.scope).toBe("modesto");
  ...
  it("can include duplicate skill names from different origins for settings", async () => {
    ...
    const defaultCatalog = await discoverSkillsCatalog({ homeDir, modestoBaseDir });
  ...
  it("prefers the provider-native copy and falls back to Modesto for that provider", async () => {
    await writeSkill(path.join(modestoBaseDir, "skills", "shared"), "shared", "Modesto copy");
    ...
    await writeSkill(
      path.join(modestoBaseDir, "skills", "only-modesto"),
      "only-modesto",
      "Fallback",
    );

    const codexView = await discoverSkillsCatalog({ homeDir, modestoBaseDir, provider: "codex" });
    ...
    expect(codexView.some((skill) => skill.name === "only-modesto")).toBe(true);

    // A provider without its own copy resolves the Modesto fallback.
    ...
    expect(claudeShared?.scope).toBe("modesto");
  ...
  it("uses documented provider alias roots before Modesto fallbacks", async () => {
    await writeSkill(path.join(modestoBaseDir, "skills", "shared"), "shared", "Modesto copy");
...
`,
  ...
  it("serves cached results within the TTL and rescans on forceReload", async () => {
    await writeSkill(path.join(modestoBaseDir, "skills", "first"), "first", "First skill");

    const initial = await discoverSkillsCatalog({ homeDir, modestoBaseDir });
    expect(initial.map((skill) => skill.name)).toEqual(["first"]);

    // A skill added after the first scan is invisible to the cached entry...
    await writeSkill(path.join(modestoBaseDir, "skills", "second"), "second", "Second skill");
    const cached = await discoverSkillsCatalog({ homeDir, modestoBaseDir });
    expect(cached.map((skill) => skill.name)).toEqual(["first"]);

    // ...but forceReload bypasses the cache and refreshes it.
    const reloaded = await discoverSkillsCatalog({ homeDir, modestoBaseDir, forceReload: true });
    expect(reloaded.map((skill) => skill.name).sort()).toEqual(["first", "second"]);
  });

  it("includes project-level .modesto skills when a cwd is provided", async () => {
    const cwd = path.join(root, "repo", "packages", "web");
    await mkdir(cwd, { recursive: true });
    await writeSkill(
      path.join(root, "repo", ".modesto", "skills", "repo-skill"),
      "repo-skill",
      "Project skill",
    );

    const skills = await discoverSkillsCatalog({ cwd, homeDir, modestoBaseDir });
  ...
  it("keeps home origins when the cwd lives under the home dir", async () => {
    ...
    await writeSkill(path.join(modestoBaseDir, "skills", "portable"), "portable", "Modesto");
    ...
    expect(skills.find((skill) => skill.name === "portable")?.scope).toBe("modesto");
  });

  it("dedupes same-named skills within a root deterministically", async () => {
    await writeSkill(path.join(modestoBaseDir, "skills", "zeta"), "twin", "Copy in zeta");
    await writeSkill(path.join(modestoBaseDir, "skills", "alpha"), "twin", "Copy in alpha");
...
describe("mergeSkillsIntoCatalog", () => {
  ...
  it("keeps provider-native entries and appends catalog-only entries", () => {
    const merged = mergeSkillsIntoCatalog({
      ...
      catalog: [descriptor("Shared", "modesto"), descriptor("extra", "modesto")],
    });
