...
import type { ChatAttachment, ProviderApprovalDecision, RuntimeMode } from "@modesto/contracts";
import {
...
import * as NetService from "@modesto/shared/Net";
...
export function resolveKiloConfigContent(
  inputEnvironment: Readonly<Record<string, string | undefined>> | undefined,
  inheritedEnvironment: Readonly<Record<string, string | undefined>> = process.env,
): string {
  return (
    inputEnvironment?.KILO_CONFIG_CONTENT ??
    inheritedEnvironment.KILO_CONFIG_CONTENT ??
    OPENCODE_EMPTY_CONFIG_CONTENT
  );
}

const OPENCODE_SERVER_READY_PREFIX = "opencode server listening";
// Kilo's CLI is OpenCode-compatible but ships as its own binary with its own
// ready-line prefix ("kilo server listening" instead of "opencode server
// listening..."). Real values, taken from the Kilo CLI itself - not guessed.
const KILO_SERVER_READY_PREFIX = "kilo server listening";
const OPEN_CODE_COMPATIBLE_SERVER_READY_PREFIXES = [
  OPENCODE_SERVER_READY_PREFIX,
  KILO_SERVER_READY_PREFIX,
];
...
function parseServerUrlFromOutput(output: string): string | null {
  for (const line of output.split("\n")) {
    if (!OPEN_CODE_COMPATIBLE_SERVER_READY_PREFIXES.some((prefix) => line.startsWith(prefix))) {
      continue;
    }
...
/** @internal */
export function parseListenPortFromServerUrl(url: string): number | null {
  try {
...
/** @internal */
export function parseLsofPidList(stdout: string): ReadonlyArray<number> {
...
function signalProcessGroup(pid: number, signal: NodeJS.Signals): void {
  try {
    ...
    // The pid may not be a group leader, or the group is already gone.
  }
}

function resolveTcpListenerPids(port: number): Effect.Effect<ReadonlyArray<number>> {
  return Effect.tryPromise({
    try: () =>
      new Promise<string>((resolve) => {
        execFile(
          ...
          { timeout: 1_000, killSignal: "SIGKILL", maxBuffer: 64 * 1024 },
          (_error, stdout) => {
    ...
    Effect.map((stdout) => parseLsofPidList(stdout)),
    Effect.orElseSucceed((): ReadonlyArray<number> => []),
  );
}

function waitUntilPidsExit(pids: ReadonlyArray<number>): Effect.Effect<number> {
  if (pids.length === 0) {
...
const makeOpenCodeRuntime = Effect.gen(function* () {
  ...
  const startOpenCodeServerProcess: OpenCodeRuntimeShape["startOpenCodeServerProcess"] = (input) =>
    Effect.gen(function* () {
        ...
        .spawn(
          ChildProcess.make(spawnCommand.command, spawnCommand.args, {
            ...
            env: {
              ...
              // Kilo's CLI reads its own env var name for the same purpose;
              // this function is binary-agnostic (it doesn't know which of
              // the OpenCode-compatible binaries is being spawned), so set
              // both - only the one the spawned binary actually reads does
              // anything.
              KILO_CONFIG_CONTENT: resolveKiloConfigContent(input.environment),
            },
      ...
      const wrapperPid = Number(child.pid);
      const trackedPidsRef = yield* Ref.make<ReadonlyArray<number>>(
        uniquePositivePids([wrapperPid]),
      );
      const listenPortRef = yield* Ref.make<number | null>(port);

      const signalTrackedPids = (signal: NodeJS.Signals) =>
        Effect.gen(function* () {
          const knownPids = yield* Ref.get(trackedPidsRef);
          const listenPort = yield* Ref.get(listenPortRef);
          const listenerPids =
            hostPlatform === "win32" || listenPort === null
              ? []
              : yield* resolveTcpListenerPids(listenPort);
          const pids = uniquePositivePids([...knownPids, ...listenerPids, wrapperPid]);
          yield* Ref.set(trackedPidsRef, pids);
      ...
      const terminateChild = signalTrackedPids("SIGTERM").pipe(
        Effect.andThen(Effect.sleep("1 second")),
        Effect.andThen(signalTrackedPids("SIGKILL")),
        Effect.ignore,
      );
      ...
      const listenPort = parseListenPortFromServerUrl(url) ?? port;
      yield* Ref.set(listenPortRef, listenPort);
      const listenerPids =
        hostPlatform === "win32" ? [] : yield* resolveTcpListenerPids(listenPort);
      const trackedPids = uniquePositivePids([...listenerPids, wrapperPid]);
      yield* Ref.set(trackedPidsRef, trackedPids);
      const wrapperIsServeProcess =
        listenerPids.length === 0 || (wrapperPid > 0 && listenerPids.includes(wrapperPid));
      // Kilo/OpenCode wrappers can fork the ~200MB bun `serve` process and
      // exit. Watching the wrapper's exit would then look like a crash and
      // skip the listener on teardown. Track the process actually bound to
      // the port and kill that tree when the session scope closes.
      yield* Fiber.interrupt(exitFiber).pipe(Effect.ignore);

      return {
        url,
        exitCode: wrapperIsServeProcess
          ? child.exitCode.pipe(
              Effect.map(Number),
              Effect.orElseSucceed(() => 0),
            )
          : waitUntilPidsExit(listenerPids),
