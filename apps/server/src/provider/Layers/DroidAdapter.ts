...
const DROID_ACP_DEBUG_ENV = "MODESTO_DROID_ACP_DEBUG";
...
// override with MODESTO_DROID_TURN_IDLE_TIMEOUT_MS when a workload needs longer.
const DROID_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "MODESTO_DROID_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
...
const DROID_PLAN_MODE_PROMPT_PREFIX = [
  "Modesto Droid plan mode is active.",
        ...
        Effect.gen(function* () {
          ...
          if (resumeSessionId !== undefined && started.sessionSetupMethod === "new") {
            return yield* new ProviderAdapterRequestError({
              ...
              detail:
                "Droid could not resume the requested native session. Modesto refused the fresh fallback to avoid silently losing conversation context.",
            });
          }
          ...
          Effect.gen(function* () {
            if (!(yield* runtime.supportsSessionFork)) {
              return yield* new ProviderAdapterValidationError({
                ...
                operation: "forkThread",
                issue:
                  "This Droid ACP version does not advertise session/fork; Modesto will rebuild the fork from its retained transcript.",
              });
            }
        ...
        const forked = activeSource
          ...
          : yield* Effect.gen(function* () {
              ...
              const runtime = yield* makeDroidAcpRuntime({
                ...
                clientInfo: { name: "Modesto Fork", version: "0.0.0" },
              });
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "Modesto Model Discovery",
          });
          yield* runtime.start();
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "Modesto Command Discovery",
          });
          yield* runtime.start();
