// FILE: DroidAdapter.ts
// Purpose: ProviderAdapter implementation for Factory Droid CLI's ACP server
//          (`droid exec --output-format acp`). Adapted from
...
//          generic, not Copilot-specific, so it carries over unchanged.
//
//          What's genuinely different, all documented in more detail in
//          DroidAcpSupport.ts's header (including what this v1 deliberately
//          did NOT port from the primary Modesto tree's much more hardened
//          DroidAdapter.ts):
//          - Model and reasoning-effort are applied once via
//            `session/set_config_option` right after the session starts
//            (applyDroidAcpModelSelection), not passed as spawn flags -
//            `droid exec` ignores `-m`/`-r` in ACP mode per the source
//            file's own comment, so the config-option call is the real
//            mechanism. Still spawn-time-only / `sessionModelSwitch:
//            "unsupported"`, same simplification GitHub Copilot uses.
//          - `interruptTurn` is the same bare fire-and-forget
//            `ctx.acp.cancel` every other ACP provider in this tree uses
//            (Copilot, Grok, Custom ACP Agent), not the source's cancel-
//            then-wait-then-escalate DroidTurnCancellation.ts. That utility
//            needs a real `Fiber` for the in-flight prompt call to wait on
//            and, if needed, interrupt - the source gets one because it
//            forks the prompt call onto its own fiber; this tree's `sendTurn`
//            runs the prompt call inline in its own fiber instead (the
//            proven pattern shared by every ACP adapter here). Forking it
//            just to enable this one enhancement would be a real,
//            untested change to the concurrency-sensitive core this port
//            was told to leave alone - a reasonable follow-up, not done here.
// @module provider/Layers/DroidAdapter
...
import { applyDroidAcpModelSelection, makeDroidAcpRuntime } from "../acp/DroidAcpSupport.ts";
...
interface DroidSessionContext {
  ...
  // Never populated - Droid ACP has no ask-user-question style extension in
  // this v1 - but kept so respondToUserInput can report "unknown request"
  // honestly rather than throwing on a map that doesn't exist.
  ...
  /** Fixed for the session's lifetime - see the module header. */
...
export function droidPromptSettlementBelongsToContext(input: {
  ...
  return Effect.gen(function* () {
    ...
    const settlePromptInFlight = (
      ...
      Effect.gen(function* () {
        ...
        const settlementBelongsToLiveContext = droidPromptSettlementBelongsToContext({
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const started = yield* Effect.gen(function* () {
                ...
                Effect.mapError(
                  (cause) =>
                    new EffectAcpErrors.AcpTransportError({
                      detail: "Failed to process Droid ACP elicitation callback.",
                      cause,
                    }),
                ),
              ),
            ...
            yield* acp.handleRequestPermission((params) =>
              Effect.gen(function* () {
                ...
                Effect.mapError(
                  (cause) =>
                    new EffectAcpErrors.AcpTransportError({
                      detail: "Failed to process Droid ACP permission callback.",
                      cause,
                    }),
                ),
              ),
            ...
            // Applied once, right after start - see the module header for
            // why this doesn't live-switch mid-session in this v1.
            yield* applyDroidAcpModelSelection({
              runtime: acp,
              model: droidModelSelection?.model,
              reasoningEffort: droidSettings.reasoningEffort,
          ...
          const ctx: DroidSessionContext = {
            ...
            promptsInFlight: 0,
            modelId: droidModelSelection?.model,
            stopped: false,
          };
    ...
    // No ask-user-question style extension wired in this v1 - pendingUserInputs
    ...
    return {
      provider: PROVIDER,
      // No live model switch in this v1 - see the module header. A model
      // change on a started thread restarts the session instead (Modesto's
      // orchestration layer already does this whenever this capability isn't
      // "in-session").
