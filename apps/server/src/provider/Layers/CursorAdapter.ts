...
/**
 * Default cursor-family member. The adapter is parameterized by a
 * `CursorFamilyDescriptor` (see `provider/cursorFamily.ts`) so Kimi, Qwen and
 * Poolside — whose CLIs speak the same ACP dialect — reuse this exact session
 * machinery while stamping their own driver kind on sessions, events and
 * errors. Omitting `options.family` yields Cursor, unchanged.
 */
const DEFAULT_FAMILY_DESCRIPTOR = CURSOR_FAMILY_DESCRIPTOR;
...
export interface CursorAdapterLiveOptions {
   ...
   */
  readonly resolveSettings?: Effect.Effect<CursorFamilySettings>;
  /**
   * Which cursor-family agent this adapter drives. Defaults to Cursor.
   * The descriptor decides the spawned binary, the ACP auth method, the
   * driver kind stamped on every session/event/error, and the name used in
   * diagnostics.
   */
  readonly family?: CursorFamilyDescriptor;
}
...
export function makeCursorAdapter(
  cursorSettings: CursorFamilySettings,
  options?: CursorAdapterLiveOptions,
) {
  return Effect.gen(function* () {
    const family = options?.family ?? DEFAULT_FAMILY_DESCRIPTOR;
    const PROVIDER = family.driverKind;
    const DISPLAY_NAME = family.displayName;
      ...
      Effect.mapError(
        (cause) =>
          new ProviderAdapterRequestError({
            ...
            detail: `Failed to generate ${DISPLAY_NAME} runtime identifier.`,
            cause,
          }),
      ),
    );
        ...
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: `Failed to process ${DISPLAY_NAME} ACP extension event.`,
              cause,
            }),
        ),
      );
    ...
    const startSession: CursorAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          // Resolve the settings used to spawn the ACP child. Production
          ...
          const nf = yield* Stream.runDrain(
            ...
            Effect.catch((cause) =>
              Effect.logError(`Failed to process ${DISPLAY_NAME} runtime notification.`, { cause }),
            ),
          ...
          yield* offerRuntimeEvent({
            ...
            provider: PROVIDER,
            threadId: input.threadId,
            payload: { state: "ready", reason: `${DISPLAY_NAME} ACP session ready` },
          });
          yield* offerRuntimeEvent({
            type: "thread.started",
            ...(yield* makeEventStamp()),
    ...
    yield* Effect.addFinalizer(() =>
      Effect.forEach(sessions.values(), stopSessionInternal, { discard: true }).pipe(
        Effect.catch((cause) =>
          Effect.logError(`Failed to emit ${DISPLAY_NAME} session shutdown event.`, { cause }),
        ),
