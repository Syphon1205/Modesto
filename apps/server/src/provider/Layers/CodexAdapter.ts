...
// Custom model endpoint ids are restricted to `[a-z0-9][a-z0-9_-]*`, always a
// valid bare TOML key, but the label/base URL a user types in are arbitrary
// strings that go inside a quoted TOML value - see
// `renderCustomModelEndpointTomlBlock` in the primary Modesto tree's
// `codexProcessEnv.ts` for the identical escaping this mirrors.
function escapeCustomModelEndpointTomlString(value: string): string {
...
export const makeCodexAdapter = Effect.fn("makeCodexAdapter")(function* (
  ...
  // Settings > Model Routers: a custom-endpoint model picked in the model
  // picker carries a `router:<endpointId>:<modelId>` slug (see
  // `appendCustomModelEndpointModels` in CodexProvider.ts, which is how those
  // models get into the picker's list in the first place). This resolves
  // that slug back into the matching `CustomModelEndpointConfig` plus the
  // bare `modelId` Codex itself needs to see (Codex must never receive our
  // synthetic UI-facing slug as its `model` value - only the plain id from
  // the remote catalog). No secret-store lookup here; that only happens in
  // `resolveCustomEndpointArgs` below, at session-start time, to avoid a
  // wasted lookup on every `sendTurn`.
  const resolveCustomEndpointForModelSelection = (
    ...
    Effect.gen(function* () {
      ...
      const customModelEndpoints = settings?.customModelEndpoints ?? [];
      const parsed = parseCustomModelEndpointModelSlug(
        modelSelection.model,
        customModelEndpoints.map((entry) => entry.id),
      );
      if (!parsed) {
        return undefined;
      }
      const endpoint = customModelEndpoints.find((entry) => entry.id === parsed.endpointId);
      if (!endpoint) {
        yield* Effect.logWarning("Custom model endpoint not found; ignoring.", {
          endpointId: parsed.endpointId,
        });
        return undefined;
      }
      return { endpoint, modelId: parsed.modelId };
    });

  // Extra `-c model_providers.<id>.*` app-server args plus an API-key env
  // var, mirroring the `-c mcp_servers.t3-code.*` pattern already used below
  // for the built-in MCP server. Spawn-time only (the process's `-c` flags
  // apply once, at launch) - switching endpoints mid-thread does not
  // live-update an already-running app-server process, the same
  // "unsupported" simplification this port has used for every other
  // spawn-time-only option so far.
  const resolveCustomEndpointArgs = (modelSelection: ModelSelection | null | undefined) =>
    Effect.gen(function* () {
      ...
      const apiKeyBytes = yield* secretStore
  ...
  const startSession: CodexAdapterShape["startSession"] = (input) =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const customEndpoint = yield* resolveCustomEndpointArgs(input.modelSelection);
        // Merged by hand rather than two independent conditional spreads:
        // `environment`/`appServerArgs` must combine when both the MCP
        // session and a custom endpoint are active for the same session -
        // two separate `...(cond ? {environment: ..., appServerArgs: ...} :
        // {})` blocks would let whichever came last silently clobber the
        // other's `environment`/`appServerArgs` key instead of merging.
        const extraEnvironment: NodeJS.ProcessEnv = {
          ...(mcpSession
            ? { T3_MCP_BEARER_TOKEN: mcpSession.authorizationHeader.replace(/^Bearer\s+/, "") }
            : {}),
          ...customEndpoint?.env,
        };
        const extraAppServerArgs: ReadonlyArray<string> = [
          ...(mcpSession
            ? [
                "-c",
        ...
        const runtimeInput: CodexSessionRuntimeOptions = {
          ...
          ...(input.modelSelection?.instanceId === boundInstanceId
            ? { model: customEndpoint?.modelId ?? input.modelSelection.model }
            : {}),
          ...(serviceTier ? { serviceTier } : {}),
          ...(hasExtraEnvironment
            ? { environment: { ...(options?.environment ?? process.env), ...extraEnvironment } }
            : {}),
          ...(extraAppServerArgs.length > 0 ? { appServerArgs: extraAppServerArgs } : {}),
        };
  ...
  const sendTurn: CodexAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
    ...
    const serviceTier =
      input.modelSelection?.instanceId === boundInstanceId
        ? getCodexServiceTierOptionValue(input.modelSelection)
        : undefined;
    const customEndpoint = yield* resolveCustomEndpointForModelSelection(input.modelSelection);
