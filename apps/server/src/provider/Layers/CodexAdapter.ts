...
import { modestoSkillsDir } from "../skillsCatalog.ts";
...
const makeCodexAdapter = (options?: CodexAdapterLiveOptions) =>
  Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        return (
          ...
          new CodexAppServerManager(services, {
            modestoSkillsDir: modestoSkillsDir(serverConfig.baseDir),
    ...
    const setPluginInstalled: NonNullable<CodexAdapterShape["setPluginInstalled"]> = (input) =>
      Effect.tryPromise({
        try: () =>
          manager.setPluginInstalled({
          ...
          new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: input.installed ? "plugin add" : "plugin remove",
            detail: toMessage(cause, `plugin ${input.installed ? "add" : "remove"} failed`),
            cause,
          }),
      }).pipe(Effect.map((result) => result satisfies ProviderSetPluginInstalledResult));
