...
import { customModelEndpointSecretName } from "../customModelEndpoints.ts";
...
const customEndpointRuntimeFactory = makeRuntimeFactory();
const customEndpointLayer = it.layer(
  Layer.effect(
    CodexAdapter,
    Effect.gen(function* () {
      const codexConfig = decodeCodexSettings({});
      return yield* makeCodexAdapter(codexConfig, {
        makeRuntime: customEndpointRuntimeFactory.factory,
      });
    }),
  ).pipe(
    Layer.provideMerge(ServerConfig.layerTest(process.cwd(), process.cwd())),
...
customEndpointLayer("CodexAdapterLive custom model endpoints", (it) => {
  it.effect(
    "resolves a router:<endpointId>:<modelId> selection into model_providers -c args and the bare model id",
    () =>
      Effect.gen(function* () {
        customEndpointRuntimeFactory.factory.mockClear();
        const secretStore = yield* ServerSecretStore.ServerSecretStore;
        yield* secretStore.set(
          customModelEndpointSecretName("test-router"),
          new TextEncoder().encode("sk-test-key"),
        );
        const adapter = yield* CodexAdapter;
        ...
        NodeAssert.equal(callArgs?.model, "llama-3.3-70b");
        NodeAssert.deepStrictEqual(callArgs?.appServerArgs, [
          "-c",
          'model_providers.test-router.name="Test Router"',
          "-c",
          'model_providers.test-router.base_url="http://localhost:8000/v1"',
          "-c",
  ...
  it.effect("ignores a plain (non-router) model selection", () =>
    Effect.gen(function* () {
      customEndpointRuntimeFactory.factory.mockClear();
      const adapter = yield* CodexAdapter;
...
sessionErrorLayer("CodexAdapterLive session errors", (it) => {
  ...
  it.effect("uses MODESTO_CODEX_LAUNCH_ARGS for the session runtime", () => {
    const runtimeFactory = makeRuntimeFactory();
    const layer = Layer.effect(
      CodexAdapter,
      Effect.gen(function* () {
        ...
        return yield* makeCodexAdapter(codexConfig, {
          environment: { MODESTO_CODEX_LAUNCH_ARGS: " --strict-config --enable env-feature " },
          makeRuntime: runtimeFactory.factory,
        });
      }),
    ).pipe(
