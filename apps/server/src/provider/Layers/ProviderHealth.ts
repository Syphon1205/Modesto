...
import { ServerProviderCredentialError, ServerProviderUpdateError } from "@modesto/contracts";
import { parseCodexConfigModelProvider } from "@modesto/shared/codexConfig";
import { decodeJsonResult } from "@modesto/shared/schemaJson";
...
const PROVIDER_NPM_INSTALL_PACKAGES: Partial<Record<ProviderKind, string>> = {
  codex: "@openai/codex",
...
import {
  API_KEY_SUPPORTED_PROVIDERS,
  applyStoredApiKeyToProcessEnv,
  hasProviderApiKeyEnv,
  isApiKeySupportedProvider,
  isSignInSupportedProvider,
  providerApiKeySecretName,
  resolveProviderSignInCommand,
...
const DISABLED_PROVIDER_STATUS_MESSAGE = "Provider is disabled in Modesto settings.";
...
function parseCursorAuthStatusFromOutput(result: CommandResult): {
  ...
  if (result.code === 0) {
    return {
      status: "warning",
      authStatus: "unknown",
      message: "Cursor Agent is installed, but Modesto could not verify authentication status.",
    };
  }
...
> =>
  Effect.gen(function* () {
    ...
    const runtimeResolution = resolveProviderRuntimeExecutable({
      provider: "codex",
      configuredPath: nonEmptyTrimmed(binaryPath) ?? null,
      defaultBinary: "codex",
    });
    const executable = runtimeResolution.executable;
    ...
    if (Result.isFailure(versionProbe)) {
      ...
      return {
        ...
        message: isCommandMissingCause(error)
          ? providerRuntimeMissingMessage(runtimeResolution, "Codex CLI (`codex`)")
  ...
  Effect.gen(function* () {
    ...
    if (Result.isFailure(versionProbe)) {
      ...
      return {
        ...
        checkedAt,
        message: isCommandMissingCause(error)
          ? providerRuntimeMissingMessage(runtimeResolution, "Claude Agent CLI (`claude`)")
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      ...(hasApiKey
        ...
        : {
            message:
              "Droid CLI is installed. Modesto can use the CLI's cached device-pairing login; run `droid` to authenticate locally if needed, or set FACTORY_API_KEY.",
          }),
    } satisfies ServerProviderStatus;
  });
...
export const checkPiProviderStatus = (
  ...
  Effect.gen(function* () {
    ...
    // Pi itself is SDK-backed in Modesto. Keep this CLI probe advisory so health
    ...
    if (Result.isFailure(versionProbe)) {
      ...
      return {
        ...
        checkedAt,
        message: isCommandMissingCause(error)
          ? "Pi SDK is bundled, but the Pi CLI (`pi`) is not on PATH, so Modesto could not verify the installed CLI version."
    ...
    if (Option.isNone(versionProbe.success)) {
      return {
        ...
        checkedAt,
        message:
          "Pi SDK is bundled, but the CLI health check timed out before Modesto could verify the installed version.",
      } satisfies ServerProviderStatus;
    }
    ...
    return {
      ...
      message: configuredAgentDir
        ? `Pi CLI is installed. Modesto will use Pi agent dir ${configuredAgentDir}.`
...
export const makeCheckCursorProviderStatus = (
  ...
  Effect.gen(function* () {
    ...
    const runtimeResolution = resolveProviderRuntimeExecutable({
      provider: "cursor",
      configuredPath: nonEmptyTrimmed(binaryPath) ?? null,
      defaultBinary: DEFAULT_CURSOR_AGENT_BINARY,
    });
    const executable = resolveCursorAgentBinaryPath(runtimeResolution.executable);
    ...
    if (Result.isFailure(versionProbe)) {
      ...
      return {
        ...
        checkedAt,
        message: isCommandMissingCause(error)
          ? providerRuntimeMissingMessage(runtimeResolution, "Cursor Agent CLI (`cursor-agent`)")
    ...
    // `cursor-agent status` only reports the CLI's persisted login state — it does
    // not consult CURSOR_API_KEY (confirmed against the real CLI: `status` says "Not
    // logged in" even with the env var set, while actual invocations do use it). Skip
    // straight to authenticated when a key is present rather than misreporting it.
    if (hasProviderApiKeyEnv("cursor")) {
      return {
        provider: CURSOR_PROVIDER,
        status: "ready" as const,
        available: true,
    ...
    if (Option.isNone(modelsProbe.success)) {
      return {
        ...
        checkedAt,
        message:
          "Cursor Agent is authenticated, but model discovery timed out before Modesto could verify available models.",
      } satisfies ServerProviderStatus;
    }
...
export const ProviderHealthLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getStoredApiKey = (provider: ProviderKind): Effect.Effect<string | null> =>
      secretStore.get(providerApiKeySecretName(provider)).pipe(
        Effect.map((bytes) => (bytes && bytes.length > 0 ? new TextDecoder().decode(bytes) : null)),
        Effect.orElseSucceed(() => null),
      );

    const resolveStoredApiKeys = (): Effect.Effect<ReadonlyMap<ProviderKind, string>> =>
      Effect.forEach(
        API_KEY_SUPPORTED_PROVIDERS,
        (provider) => getStoredApiKey(provider).pipe(Effect.map((key) => [provider, key] as const)),
        { concurrency: "unbounded" },
      ).pipe(
        Effect.map(
          (entries) =>
            new Map(entries.filter((entry): entry is [ProviderKind, string] => entry[1] !== null)),
        ),
      );

    // Hydrate process.env from previously-saved keys on boot, without clobbering
    // an env var the user already set outside Modesto.
    yield* Effect.forEach(
      API_KEY_SUPPORTED_PROVIDERS,
      (provider) =>
        getStoredApiKey(provider).pipe(
          Effect.map((apiKey) => {
            if (apiKey) {
              applyStoredApiKeyToProcessEnv({ provider, apiKey, onlyIfUnset: true });
            }
          }),
        ),
    ...
    const loadProviderStatuses = serverSettings.ready
      ...
      .pipe(
        ...
        Effect.flatMap((statuses) =>
          resolveStoredApiKeys().pipe(
            Effect.map((storedApiKeys) =>
              statuses.map((status) => ({
                ...status,
                apiKeySupported: isApiKeySupportedProvider(status.provider),
                apiKeyConfigured: storedApiKeys.has(status.provider),
                signInSupported: isSignInSupportedProvider(status.provider),
              })),
            ),
          ),
        ),
    ...
    const updateProvider: ProviderHealthShape["updateProvider"] = Effect.fn(
      ...
      if (!isProviderEnabledForSettings(provider, settings)) {
        return yield* new ServerProviderUpdateError({
          provider,
          reason: "Provider is disabled in Modesto settings.",
        });
      }
      const installPackage =
        input.action === "install" ? PROVIDER_NPM_INSTALL_PACKAGES[provider] : undefined;
      const capabilities = yield* getProviderMaintenanceCapabilities(provider).pipe(
        Effect.mapError(toUpdateError),
      );
      const update = installPackage
        ? {
            executable: "npm",
        ...
        return yield* new ServerProviderUpdateError({
          ...
          reason:
            input.action === "install"
              ? "This provider does not support a safe in-app install yet."
      ...
      const run = Effect.gen(function* () {
        ...
        yield* setProviderUpdateState(
          ...
          makeUpdateState({
            ...
            startedAt,
            finishedAt: null,
            message: input.action === "install" ? "Installing provider." : "Updating provider.",
          }),
        );
        ...
        const finalProviders = yield* setProviderUpdateState(
          ...
          makeUpdateState({
            ...
            message: stillOutdated
              ? "Update command completed, but Modesto still detects an outdated provider version."
              : input.action === "install"
                ? "Provider installed."
                : "Provider updated.",
            output: output ? output.slice(0, UPDATE_OUTPUT_MAX_BYTES) : null,
    ...
    const toCredentialError = (provider: ProviderKind) => (cause: unknown) =>
      new ServerProviderCredentialError({
        provider,
        reason: cause instanceof Error ? cause.message : String(cause),
      });

    const setApiKey: ProviderHealthShape["setApiKey"] = Effect.fn("ProviderHealth.setApiKey")(
      function* (input) {
        if (!isApiKeySupportedProvider(input.provider)) {
          return yield* new ServerProviderCredentialError({
            provider: input.provider,
            reason: "This provider does not accept a pasted API key.",
          });
        }
        yield* secretStore
          .set(providerApiKeySecretName(input.provider), new TextEncoder().encode(input.apiKey))
          .pipe(Effect.mapError(toCredentialError(input.provider)));
        applyStoredApiKeyToProcessEnv({ provider: input.provider, apiKey: input.apiKey });
        const providers = yield* refreshNow.pipe(
          Effect.mapError(toCredentialError(input.provider)),
        );
        return { providers };
      },
    );

    const clearApiKey: ProviderHealthShape["clearApiKey"] = Effect.fn("ProviderHealth.clearApiKey")(
      function* (input) {
        yield* secretStore
          .remove(providerApiKeySecretName(input.provider))
          .pipe(Effect.mapError(toCredentialError(input.provider)));
        applyStoredApiKeyToProcessEnv({ provider: input.provider, apiKey: null });
        const providers = yield* refreshNow.pipe(
          Effect.mapError(toCredentialError(input.provider)),
        );
        return { providers };
    ...
    const signIn: ProviderHealthShape["signIn"] = Effect.fn("ProviderHealth.signIn")(
      function* (input) {
        const settings = yield* serverSettings.getSettings.pipe(
          Effect.mapError(toCredentialError(input.provider)),
        );
        const binaryPath = getProviderBinaryPath(input.provider, settings);
        const signInCommand = resolveProviderSignInCommand(input.provider, binaryPath);
        if (!signInCommand) {
          return yield* new ServerProviderCredentialError({
            provider: input.provider,
            reason:
              "Modesto can't trigger sign-in for this provider yet. Run its CLI's login command in a terminal, then refresh.",
          });
        }
        return yield* Effect.callback<{ started: boolean }, ServerProviderCredentialError>(
          (resume) => {
            const child = spawn(signInCommand.command, signInCommand.args, {
              detached: true,
              stdio: "ignore",
              env: process.env,
            });
            ...
            child.once("spawn", () => {
              child.unref();
              resume(Effect.succeed({ started: true }));
            });
          },
        );
      },
