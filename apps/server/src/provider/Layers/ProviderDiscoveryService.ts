...
const make = Effect.gen(function* () {
  ...
  const getComposerCapabilities: ProviderDiscoveryServiceShape["getComposerCapabilities"] = (
    ...
    Effect.gen(function* () {
      ...
      // The unified Modesto skills catalog backs skill discovery for every
    ...
    Effect.gen(function* () {
      ...
      const nativeResult: ProviderListSkillsResult | null = adapter.listSkills
        ? yield* adapter
            ...
            .pipe(
              Effect.catch((error) =>
                Effect.logWarning(
                  "provider-native skill discovery failed; serving the Modesto skills catalog only",
      ...
      const catalogSkills = yield* Effect.tryPromise(() =>
        discoverSkillsCatalog({
          ...
          modestoBaseDir: serverConfig.baseDir,
          provider: parsed.provider,
          ...(parsed.forceReload !== undefined ? { forceReload: parsed.forceReload } : {}),
        }),
      ).pipe(
        Effect.catchCause((cause) =>
          Effect.logWarning("modesto skills catalog discovery failed", {
            provider: parsed.provider,
            cause,
      ...
      return {
        ...
        source: nativeResult?.source ? `${nativeResult.source}+modesto.catalog` : "modesto.catalog",
  ...
  const setPluginInstalled: ProviderDiscoveryServiceShape["setPluginInstalled"] = (input) =>
    Effect.gen(function* () {
      const parsed = yield* decodeInputOrValidationError({
        operation: "ProviderDiscoveryService.setPluginInstalled",
        schema: ProviderSetPluginInstalledInput,
        payload: input,
      });
      const adapter = yield* registry.getByProvider(parsed.provider);
      if (!adapter.setPluginInstalled) {
        return yield* new ProviderValidationError({
          operation: "ProviderDiscoveryService.setPluginInstalled",
          issue: `Plugin installation is unavailable for provider '${parsed.provider}'.`,
        });
      }
      return yield* adapter.setPluginInstalled(parsed);
    });
