...
import { PREFERRED_DEFAULT_CODEX_MODELS, ServerSettingsError } from "@modesto/contracts";
...
export function appendCustomModelEndpointModels(
  ...
  for (const endpoint of customModelEndpoints) {
    ...
    for (const modelId of modelIds) {
      const slug = customModelEndpointModelSlug(endpoint.id, modelId);
      if (seen.has(slug)) {
        continue;
      }
      seen.add(slug);
      entries.push({
        slug,
        name: modelId,
        subProvider: endpoint.label,
        isCustom: true,
        capabilities: fallbackCapabilities,
      });
    }
  }
  return entries.length === 0 ? models : [...models, ...entries];
}
...
const probeCodexAppServerProvider = Effect.fn("probeCodexAppServerProvider")(function* (input: {
  ...
  readonly customModelEndpoints?: ReadonlyArray<CustomModelEndpointConfig>;
  ...
  if (!accountResponse.account && accountResponse.requiresOpenaiAuth) {
    return {
      ...
      version,
      models: appendCustomModelEndpointModels(
        appendCustomCodexModels([], input.customModels ?? []),
        input.customModelEndpoints ?? [],
      ),
      skills: [],
...
const emptyCodexModelsFromSettings = (
  ...
  return appendCustomModelEndpointModels(
...
const makePendingCodexProvider = (
  ...
  Effect.gen(function* () {
    ...
    const models = emptyCodexModelsFromSettings(codexSettings, customModelEndpoints);
    ...
    if (!codexSettings.enabled) {
      return buildServerProvider({
        ...
        probe: {
          ...
          status: "warning",
          auth: { status: "unknown" },
          message: "Codex is disabled in Modesto settings.",
        },
      });
    }
...
> {
  ...
  const emptyModels = emptyCodexModelsFromSettings(codexSettings, customModelEndpoints);
