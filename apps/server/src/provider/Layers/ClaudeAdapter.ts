...
import { parseCliArgs } from "@modesto/shared/cliArgs";
import {
  ApprovalRequestId,
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleSystemMessage = Effect.fn("handleSystemMessage")(function* (
    ...
    switch (message.subtype) {
      ...
      // New in SDK 0.3.x, no dedicated surface yet - same deliberate no-op
      // treatment as the rest of this bucket until product need emerges.
      case "background_tasks_changed":
  ...
  const handleSdkMessage = Effect.fn("handleSdkMessage")(function* (
    ...
    switch (message.type) {
      ...
      case "prompt_suggestion":
        return;
      // New in SDK 0.3.x. Carries a fresh new_conversation_id - plausibly
      // relevant to thread/session continuity, but no confirmed product
      // requirement here yet, so treated as a deliberate no-op like the
      // rest of this bucket rather than guessing at new tracking behavior.
      // Revisit if conversation resets need to update session state.
  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const handleAskUserQuestion = Effect.fn("handleAskUserQuestion")(function* (
        ...
        // expected lookup key. See https://github.com/pingdotgg/modesto/issues/2388
