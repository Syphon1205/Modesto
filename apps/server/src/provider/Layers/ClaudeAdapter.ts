...
import { buildClaudeSubagentPrompt } from "@modesto/shared/agentMentions";
import {
  Cause,
  DateTime,
  Deferred,
...
import { buildClaudeProcessEnv, resolveBundledClaudeBinaryPath } from "../claudeProcessEnv.ts";
import {
...
const EMBEDDED_CLAUDE_SYSTEM_PROMPT_APPEND = [
  "You are running inside Modesto, a coding app that embeds the Claude Agent SDK.",
    ...
    const startSession: ClaudeAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        const settings = {
          ...
          // Modesto's safer 200k budget explicit unless the thread opts into 1M.
        ...
        // An explicit user-configured path always wins; otherwise prefer the
        // launcher Modesto's desktop shell bootstraps outside app.asar. Left
        // fully unset, the SDK's own bundled-runtime resolution has no notion
        // of asar and ends up trying to spawn a path inside the archive in a
        // packaged app (surfaces as `spawn ENOTDIR`) — see
        // resolveBundledClaudeBinaryPath for the full explanation.
        const resolvedClaudeBinaryPath =
          providerOptions?.binaryPath || resolveBundledClaudeBinaryPath();
        ...
        const queryOptions: ClaudeQueryOptions = {
          ...
          ...(resolvedClaudeBinaryPath
            ? { pathToClaudeCodeExecutable: resolvedClaudeBinaryPath }
            : {}),
    ...
    const listCommands: NonNullable<ClaudeAdapterShape["listCommands"]> = (
      ...
      Effect.gen(function* () {
        ...
        const discoveryPromise =
          ...
          discoverCommandsViaTemporaryProcess(input.cwd, claudeSdkEnv, input.binaryPath);
