...
import type { ServerProviderStatus } from "@modesto/contracts";
import { DEFAULT_SERVER_SETTINGS, ServerProviderUpdateError } from "@modesto/contracts";
...
import { MODESTO_CODEX_HOME_OVERLAY_DIR } from "../../codexHomePaths";
...
function assertCommandName(command: string, expectedName: string): void {
  assert.strictEqual(command.replaceAll("\\", "/").split("/").at(-1), expectedName);
}
...
const disabledProviderHealthServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "provider-health-disabled-",
});
...
const disabledProviderHealthLayer = ProviderHealthLive.pipe(
  ...
  Layer.provideMerge(disabledProviderHealthServerConfigLayer),
  Layer.provideMerge(
    ServerSecretStoreLive.pipe(Layer.provide(disabledProviderHealthServerConfigLayer)),
  ),
);
...
function withTempCodexHome(configContent?: string) {
  return Effect.gen(function* () {
    ...
    const tmpDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "modesto-test-codex-" });
    const runtimeDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "modesto-test-runtime-",
    });
  ...
  describe("disabled provider handling", () => {
    it("builds an inert status for disabled providers", () => {
      assert.deepStrictEqual(makeDisabledProviderStatus("kilo", "2026-06-16T12:00:00.000Z"), {
        ...
        authStatus: "unknown",
        checkedAt: "2026-06-16T12:00:00.000Z",
        message: "Provider is disabled in Modesto settings.",
      });
    });
    ...
    it.effect("does not expose cached ready statuses for disabled providers", () =>
      Effect.gen(function* () {
        ...
        const layer = ProviderHealthLive.pipe(
          ...
          Layer.provideMerge(ServerSecretStoreLive.pipe(Layer.provide(serverConfigLayer))),
        );
    ...
    it.effect("projects cached ready status when a disabled provider is re-enabled", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.strictEqual(disabledCodex?.message, "Provider is disabled in Modesto settings.");
          ...
          assert.notStrictEqual(currentCodex?.message, "Provider is disabled in Modesto settings.");
    ...
    it.effect("rejects one-click updates for disabled providers", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(error.reason, "Provider is disabled in Modesto settings.");
      }).pipe(Effect.provide(disabledProviderHealthLayer)),
    );
  });
  ...
  describe("checkCodexProviderStatus", () => {
    ...
    it.effect("uses configured codex home for version, config, and auth probes", () => {
      ...
      return Effect.gen(function* () {
        ...
        const configuredHome = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "modesto-configured-codex-",
        });
        yield* fileSystem.writeFileString(
          path.join(configuredHome, "config.toml"),
          'model_provider = "openai"\n',
        );
        expectedCodexHome = path.join(runtimeDir, MODESTO_CODEX_HOME_OVERLAY_DIR);
    ...
    it.effect("returns unavailable when codex is missing", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          status.message,
          "Codex CLI (`codex`) is not installed, not embedded in a detected desktop app, or not on PATH.",
        );
  ...
  describe("checkClaudeProviderStatus", () => {
          ...
          const status = yield* makeCheckClaudeProviderStatus(undefined, "claude", homeDir).pipe(
            Effect.provide(
              mockSpawnerLayer((args, command, env) => {
                assertCommandName(command, "claude");
    ...
    it.effect("returns unavailable when claude is missing", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          status.message,
          "Claude Agent CLI (`claude`) is not installed, not embedded in a detected desktop app, or not on PATH.",
        );
  ...
  describe("checkPiProviderStatus", () => {
    ...
    it.effect("uses configured Pi binary and agent dir without SDK registry reads", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          status.message,
          "Pi CLI is installed. Modesto will use Pi agent dir /tmp/pi-agent.",
        );
      }).pipe(
        Effect.provide(
          mockSpawnerLayer((args, command) => {
  ...
  describe("checkCursorProviderStatus", () => {
    it.effect("returns ready when Cursor Agent is authenticated and has models", () =>
      Effect.gen(function* () {
        ...
        Effect.provide(
          mockSpawnerLayer((args, command, env) => {
            assertCommandName(command, "cursor-agent");
    ...
    it.effect("returns unavailable when Cursor Agent is missing", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          status.message,
          "Cursor Agent CLI (`cursor-agent`) is not installed, not embedded in a detected desktop app, or not on PATH.",
        );
        ...
        Effect.provide(
          Layer.succeed(
            ...
            ChildProcessSpawner.make((command) => {
              ...
              assertCommandName(cmd.command, "cursor-agent");
