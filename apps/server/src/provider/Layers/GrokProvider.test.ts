...
import { GrokSettings } from "@modesto/contracts";
...
describe("buildInitialGrokProviderSnapshot", () => {
  ...
  it.effect("returns a pending snapshot by default — Grok is enabled", () =>
    Effect.gen(function* () {
    ...
    Effect.gen(function* () {
      ...
      const snapshot = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const dir = yield* fs.makeTempDirectoryScoped({ prefix: "modesto-grok-version-" });
    ...
    Effect.gen(function* () {
      const snapshot = yield* Effect.scoped(
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          const dir = yield* fs.makeTempDirectoryScoped({ prefix: "modesto-grok-success-" });
          const grokPath = path.join(dir, "grok");
          yield* fs.writeFileString(
            grokPath,
