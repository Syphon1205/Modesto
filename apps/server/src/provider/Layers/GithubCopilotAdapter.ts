// FILE: GithubCopilotAdapter.ts
// Purpose: ProviderAdapter implementation for GitHub Copilot CLI's ACP server
//          (`copilot --acp`). Adapted from GrokAdapter.ts, which implements
//          the identical generic ACP session-lifecycle contract against the
//          same AcpSessionRuntime - the concurrency-sensitive machinery here
...
//          generic, not Grok-specific, so it carries over unchanged.
//
//          What's genuinely different from Grok, both confirmed live against
//          a real installed `copilot` binary (v1.0.80, unauthenticated - see
//          GithubCopilotAcpSupport.ts's header for exactly what that did and
//          didn't verify):
//          - No custom ask-user-question extension (Grok's XAiAcpExtension
//            has no Copilot equivalent - `initialize` advertised nothing
//            beyond the base ACP capabilities plus an
//            `available_commands_update` notification for slash-command
//            discovery, which this v1 doesn't consume).
//          - No live model switching (see GithubCopilotAcpSupport.ts) - the
//            model is fixed for the life of a session, so there is no
//            per-turn "requested model differs from current" branch to
//            reconcile the way Grok's applyGrokAcpModelSelection does.
// @module provider/Layers/GithubCopilotAdapter
...
interface GithubCopilotSessionContext {
  ...
  // Never populated - Copilot has no ask-user-question style extension - but
...
export function makeGithubCopilotAdapter(
  ...
  return Effect.gen(function* () {
    ...
    const startSession: GithubCopilotAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const githubCopilotSpawnFlags = githubCopilotAcpSpawnFlagsFromModelSelection(
                ...
                Effect.mapError(
                  (cause) =>
                    new EffectAcpErrors.AcpTransportError({
                      detail: "Failed to process GitHub Copilot ACP elicitation callback.",
                      cause,
                    }),
                ),
              ),
            ...
            yield* acp.handleRequestPermission((params) =>
              Effect.gen(function* () {
                ...
                Effect.mapError(
                  (cause) =>
                    new EffectAcpErrors.AcpTransportError({
                      detail: "Failed to process GitHub Copilot ACP permission callback.",
                      cause,
                    }),
                ),
              ),
          ...
          const session: ProviderSession = {
            ...
            cwd,
            ...(githubCopilotSpawnFlags.model ? { model: githubCopilotSpawnFlags.model } : {}),
            threadId: input.threadId,
            resumeCursor: {
          ...
          const ctx: GithubCopilotSessionContext = {
            ...
            promptsInFlight: 0,
            modelId: githubCopilotModelSelection?.model,
            stopped: false,
          };
    ...
    // Copilot has no ask-user-question style extension - pendingUserInputs is
    ...
    return {
      provider: PROVIDER,
      // No live model switch - see the module header. A model change on a
      // started thread restarts the session instead (Modesto's orchestration
      // layer already does this whenever this capability isn't "in-session").
