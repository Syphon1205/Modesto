...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  const recoverSessionForThread = Effect.fn("recoverSessionForThread")(function* (input: {
    ...
    readonly restartForMcpCredential?: boolean;
    readonly operation: string;
  }) {
    ...
    return yield* Effect.gen(function* () {
      ...
      if (hasActiveSession && !input.restartForMcpCredential) {
    ...
    function* (threadId, rawInput) {
      ...
      return yield* Effect.gen(function* () {
        ...
        if (!instanceInfo.enabled) {
          return yield* toValidationError(
            "ProviderService.startSession",
            `Provider instance '${resolvedInstanceId}' is disabled in Modesto settings.`,
          );
        }
  ...
  const sendTurn: ProviderServiceMethod<"sendTurn"> = Effect.fn("sendTurn")(function* (rawInput) {
    ...
    return yield* Effect.gen(function* () {
      ...
      const refreshed = yield* (
        options?.touchMcpCredential ?? McpSessionRegistry.touchActiveMcpThread
      )(input.threadId);
      if (refreshed === false && McpProviderSession.readMcpProviderSession(input.threadId)) {
        // An idle session can outlive its credential. Resume the same provider
        // thread with a fresh process so its immutable environment receives the
        // new token. Never revive an expired credential or retry a sent turn.
        const binding = yield* directory.getBinding(input.threadId);
        if (Option.isNone(binding)) {
          return yield* toValidationError(
            "ProviderService.sendTurn",
            "Cannot refresh MCP access without a persisted provider session.",
          );
        }
        yield* recoverSessionForThread({
          binding: binding.value,
          operation: "ProviderService.sendTurn",
          restartForMcpCredential: true,
        });
      }
