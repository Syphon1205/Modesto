// FILE: CustomAcpAdapter.ts
// Purpose: ProviderAdapter implementation for the "Custom ACP Agent" driver -
//          an arbitrary, user-configured ACP-speaking agent (command + args
//          + optional auth method id, all typed in by the user in this
//          instance's settings; see CustomAcpSupport.ts's header for what
//          that does and doesn't assume). Adapted from
//          GithubCopilotAdapter.ts, which implements the identical generic
//          ACP session-lifecycle contract against the same
//          AcpSessionRuntime - the concurrency-sensitive machinery here
//          (thread-locked prompt steering, turn settlement races, permission
//          request plumbing, notification-stream translation) is protocol-
//          generic, not provider-specific, so it carries over unchanged.
//
//          What's genuinely different from every other ACP provider in this
//          tree: there is no fixed model concept at all. Modesto doesn't
//          know what models (if any) the configured agent exposes, so no
//          model is ever passed to spawn or reported on the session/turn
//          events - if the user's agent needs a model flag, they bake it
//          into the `args` setting themselves. There's also no custom
//          extension handling (ask-user-question, slash commands, etc.) -
//          only base ACP is spoken, since Modesto has no way to know what a
//          given custom agent's extensions look like ahead of time.
// @module provider/Layers/CustomAcpAdapter
...
import { type CustomAcpAdapterShape } from "../Services/CustomAcpAdapter.ts";
...
export interface CustomAcpAdapterLiveOptions {
...
interface CustomAcpSessionContext {
  ...
  // Never populated - a custom agent's extensions (if any) are unknown, so
  // there's nothing to route into this map - but it's kept so
  // respondToUserInput can report "unknown request" honestly instead of
  // throwing on a map that doesn't exist.
...
const resolveNotificationTurnId = (ctx: CustomAcpSessionContext): TurnId | undefined =>
  ctx.activeTurnId;

const resolveCallbackTurnId = (ctx: CustomAcpSessionContext): TurnId | undefined =>
  ctx.activeTurnId;

const resolveSessionCallbackTurnId = (
  sessions: ReadonlyMap<ThreadId, CustomAcpSessionContext>,
  threadId: ThreadId,
): TurnId | undefined => {
...
function parseCustomAcpResume(raw: unknown): { sessionId: string } | undefined {
...
export function customAcpPromptSettlementBelongsToContext(input: {
...
export function makeCustomAcpAdapter(
  ...
  options?: CustomAcpAdapterLiveOptions,
) {
  return Effect.gen(function* () {
    const boundInstanceId = options?.instanceId ?? ProviderInstanceId.make("customAcp");
    ...
    const sessions = new Map<ThreadId, CustomAcpSessionContext>();
      ...
      Effect.mapError(
        (cause) =>
          new ProviderAdapterRequestError({
            ...
            detail: "Failed to generate Custom ACP Agent runtime identifier.",
            cause,
          }),
      ),
    );
    ...
    const settlePromptInFlight = (
      ...
      Effect.gen(function* () {
        ...
        const settlementBelongsToLiveContext = customAcpPromptSettlementBelongsToContext({
    ...
    const logNative = (threadId: ThreadId, method: string, payload: unknown) =>
      Effect.gen(function* () {
        ...
        Effect.catchCause((cause) =>
          Effect.logWarning("Failed to write native Custom ACP Agent notification log.", {
            cause,
            threadId,
            method,
          }),
    ...
    const requireSession = (
      threadId: ThreadId,
    ): Effect.Effect<CustomAcpSessionContext, ProviderAdapterSessionNotFoundError> => {
      const ctx = sessions.get(threadId);
      if (!ctx || ctx.stopped) {
        return Effect.fail(
    ...
    const stopSessionInternal = (ctx: CustomAcpSessionContext) =>
      Effect.gen(function* () {
    ...
    const startSession: CustomAcpAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          if (!customAcpSettings.command?.trim()) {
            return yield* new ProviderAdapterValidationError({
              provider: PROVIDER,
              operation: "startSession",
              issue:
                "No command configured for this Custom ACP Agent instance. Set one in its settings.",
            });
          }
          ...
          const resumeSessionId = parseCustomAcpResume(input.resumeCursor)?.sessionId;
                ...
                Effect.mapError(
                  (cause) =>
                    new EffectAcpErrors.AcpTransportError({
                      detail: "Failed to process Custom ACP ACP elicitation callback.",
                      cause,
                    }),
                ),
              ),
            ...
            yield* acp.handleRequestPermission((params) =>
              Effect.gen(function* () {
                ...
                Effect.mapError(
                  (cause) =>
                    new EffectAcpErrors.AcpTransportError({
                      detail: "Failed to process Custom ACP Agent permission callback.",
                      cause,
                    }),
                ),
              ),
          ...
          const ctx: CustomAcpSessionContext = {
            threadId: input.threadId,
          ...
          const nf = yield* Stream.runDrain(
            ...
            Effect.catch((cause) =>
              Effect.logError("Failed to process Custom ACP Agent runtime notification.", {
                cause,
              }),
            ),
          ...
          yield* offerRuntimeEvent({
            ...
            provider: PROVIDER,
            threadId: input.threadId,
            payload: { state: "ready", reason: "Custom ACP Agent session ready" },
          });
          yield* offerRuntimeEvent({
            type: "thread.started",
            ...(yield* makeEventStamp()),
            ...
            return yield* Effect.gen(function* () {
              ...
              if (ctx.interruptedTurnIds.has(turnId)) {
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "Custom ACP Agent prompt was interrupted during preparation.",
                });
              }
              ...
              Effect.tapCause(() =>
                Effect.gen(function* () {
                  ...
                  yield* settlePromptInFlight(input.threadId, turnId, liveCtx.acpSessionId, {
                    errorMessage: "Custom ACP Agent prompt preparation failed.",
                    emitTurnCompletion: false,
                  });
                }),
              ),
            ...
            Effect.gen(function* () {
              ...
              if (ctx.acpSessionId !== prepared.acpSessionId) {
                yield* settlePromptInFlight(
                  ...
                  {
                    errorMessage: "Custom ACP Agent session changed before the turn completed.",
                    settleAllPrompts: true,
                  },
                );
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "Custom ACP Agent session changed before the turn completed.",
                });
              }
            ...
            Effect.gen(function* () {
                  ...
                  Effect.gen(function* () {
                    ...
                    if (ctx.acpSessionId !== prepared.acpSessionId) {
                      yield* settlePromptInFlight(
                        ...
                        {
                          errorMessage:
                            "Custom ACP Agent session changed before the turn completed.",
                          settleAllPrompts: true,
                        },
                      );
                      return;
              ...
              yield* withThreadLock(
                ...
                settlePromptInFlight(input.threadId, prepared.turnId, prepared.acpSessionId, {
                  errorMessage: errorMessage ?? "Custom ACP Agent prompt request failed.",
                }),
              );
    ...
    const interruptTurn: CustomAcpAdapterShape["interruptTurn"] = (threadId, turnId) =>
      Effect.gen(function* () {
    ...
    // No known extension surface for an arbitrary custom agent - pendingUserInputs
    // is always empty, so this always reports "unknown request" honestly rather
    ...
    const rollbackThread: CustomAcpAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: "Custom ACP Agent sessions do not support provider-side rollback yet.",
        });
      });
    ...
    const listSessions: CustomAcpAdapterShape["listSessions"] = () =>
      Effect.sync(() => Array.from(sessions.values(), (c) => ({ ...c.session })));

    const hasSession: CustomAcpAdapterShape["hasSession"] = (threadId) =>
      Effect.sync(() => {
    ...
    return {
      provider: PROVIDER,
      // No model concept exists here - see the module header. Instance-level
      // config never changes mid-session in a way this driver can detect, so
      // there is no live-switch to attempt.
