...
// Purpose: ProviderAdapter implementation for Gemini CLI's ACP server
//          (`gemini --acp`). Adapted from GithubCopilotAdapter.ts, which
...
//          Modesto tree's real Gemini adapter source (see
//          GeminiAcpSupport.ts's header for exactly what did and didn't come
//          across, and what was deliberately cut) - NOT verified live, no
//          `gemini` binary installed in this migration tree:
//          - Live model switching, the same `session/set_model` mechanism
//            Grok uses (`applyGeminiAcpModelSelection`, mirroring Grok's
//            `applyGrokAcpModelSelection`) - unlike Copilot, where the model
//            is fixed for the session's lifetime.
//          - No custom ask-user-question extension (Gemini's `initialize`
//            never advertised one in the primary tree's source, same as
//            Copilot - Grok's XAiAcpExtension has no Gemini equivalent).
// @module provider/Layers/GeminiAdapter
...
interface GeminiSessionContext {
  ...
  // Never populated - Gemini has no ask-user-question style extension - but
...
export function makeGeminiAdapter(
  ...
  return Effect.gen(function* () {
    ...
    const startSession: GeminiAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
                ...
                Effect.mapError(
                  (cause) =>
                    new EffectAcpErrors.AcpTransportError({
                      detail: "Failed to process Gemini ACP elicitation callback.",
                      cause,
                    }),
                ),
              ),
            ...
            yield* acp.handleRequestPermission((params) =>
              Effect.gen(function* () {
                ...
                Effect.mapError(
                  (cause) =>
                    new EffectAcpErrors.AcpTransportError({
                      detail: "Failed to process Gemini ACP permission callback.",
                      cause,
                    }),
                ),
              ),
          ...
          const requestedStartModelId = resolveGeminiAcpModelId(geminiModelSelection?.model);
            ...
            return yield* Effect.gen(function* () {
              const turnModelSelection =
                input.modelSelection?.instanceId === boundInstanceId
                  ...
                  : undefined;
              const requestedTurnModelId = resolveGeminiAcpModelId(turnModelSelection?.model);
    ...
    // Gemini has no ask-user-question style extension - pendingUserInputs is
