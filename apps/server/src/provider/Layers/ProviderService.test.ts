...
let mcpCredentialIsAlive = true;
let mcpCredentialNumber = 0;
const mcpRecovery = makeProviderServiceLayer({
  issueMcpCredential: (request) =>
    Effect.sync(() => ({
      config: {
        environmentId: EnvironmentId.make("mcp-recovery-environment"),
        threadId: request.threadId,
        providerInstanceId: request.providerInstanceId,
        providerSessionId: `mcp-${++mcpCredentialNumber}`,
        endpoint: "http://127.0.0.1:12345/mcp",
        authorizationHeader: `Bearer test-credential-${mcpCredentialNumber}`,
      },
    })),
  touchMcpCredential: () => Effect.succeed(mcpCredentialIsAlive),
});

mcpRecovery.layer("MCP credential recovery", (it) => {
  it.effect("resumes an idle provider with a new credential before sending the turn once", () =>
    Effect.gen(function* () {
      const provider = yield* ProviderService.ProviderService;
      const threadId = asThreadId("mcp-expired-thread");
      const session = yield* provider.startSession(threadId, {
        provider: CODEX_DRIVER,
        providerInstanceId: codexInstanceId,
        threadId,
      ...
      const originalCredential = McpProviderSession.readMcpProviderSession(threadId);
      mcpCredentialIsAlive = false;
      yield* provider.sendTurn({
        threadId,
        input: "continue",
        attachments: [],
      ...
      assert.equal(
        mcpRecovery.codex.stopSession.mock.calls.filter(([id]) => id === threadId).length,
        1,
      );
      const starts = mcpRecovery.codex.startSession.mock.calls.filter(
        ([input]) => input.threadId === threadId,
      );
      assert.equal(starts.length, 2);
      assert.deepEqual(starts[1]?.[0].resumeCursor, session.resumeCursor);
      assert.equal(starts[1]?.[0].cwd, "/tmp/mcp-project");
      assert.notEqual(
        McpProviderSession.readMcpProviderSession(threadId)?.authorizationHeader,
        originalCredential?.authorizationHeader,
      );
      assert.equal(
        mcpRecovery.codex.sendTurn.mock.calls.filter(([input]) => input.threadId === threadId)
          .length,
        1,
      );
    }),
  ...
  it.effect("keeps the same process and credential while the credential is live", () =>
    Effect.gen(function* () {
      const provider = yield* ProviderService.ProviderService;
      const threadId = asThreadId("mcp-live-thread");
      yield* provider.startSession(threadId, {
        provider: CODEX_DRIVER,
        providerInstanceId: codexInstanceId,
        threadId,
      ...
      assert.equal(
        mcpRecovery.codex.startSession.mock.calls.filter(([input]) => input.threadId === threadId)
          .length,
        1,
      );
      assert.equal(
      ...
      assert.equal(McpProviderSession.readMcpProviderSession(threadId), originalCredential);
    }),
  );
});
