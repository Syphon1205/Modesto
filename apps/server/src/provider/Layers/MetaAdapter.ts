// FILE: MetaAdapter.ts
// Purpose: ProviderAdapter implementation for Muse Code's ACP server
//          (`muse --acp`). Adapted from GithubCopilotAdapter.ts, which
//          implements the identical generic ACP session-lifecycle contract
//          against the same AcpSessionRuntime - the concurrency-sensitive
//          machinery here (thread-locked prompt steering, turn settlement
//          races, permission request plumbing, notification-stream
//          translation) is protocol-generic, not Copilot-specific, so it
//          carries over unchanged.
//
//          What's genuinely different from Copilot, traced from the primary
//          Modesto tree's real Meta adapter source (see
//          MetaAcpSupport.ts's header for exactly what did and didn't come
//          across, and what was deliberately cut) - NOT verified live, no
//          `muse` binary installed in this migration tree:
//          - Live model switching, the same `session/set_model` mechanism
//            Grok uses (`applyMetaAcpModelSelection`, mirroring Grok's
//            `applyGrokAcpModelSelection`) - unlike Copilot, where the model
//            is fixed for the session's lifetime.
//          - No custom ask-user-question extension (Meta's `initialize`
//            never advertised one in the primary tree's source, same as
//            Copilot - Grok's XAiAcpExtension has no Meta equivalent).
// @module provider/Layers/MetaAdapter
...
import { acpElicitationExchange } from "../acp/AcpElicitation.ts";
...
import {
  ...
  currentMetaModelIdFromSessionSetup,
  makeMetaAcpRuntime,
  resolveMetaAcpModelId,
} from "../acp/MetaAcpSupport.ts";
import { type MetaAdapterShape } from "../Services/MetaAdapter.ts";
import { type EventNdjsonLogger, makeEventNdjsonLogger } from "./EventNdjsonLogger.ts";

const encodeUnknownJsonStringExit = Schema.encodeUnknownExit(Schema.fromJsonString(Schema.Unknown));

const PROVIDER = ProviderDriverKind.make("meta");
const META_RESUME_VERSION = 1 as const;
...
export interface MetaAdapterLiveOptions {
...
interface MetaSessionContext {
  ...
  // Never populated - Meta has no ask-user-question style extension - but
  // kept so respondToUserInput can report "unknown request" honestly rather
  // than throwing on a map that doesn't exist.
...
const resolveNotificationTurnId = (ctx: MetaSessionContext): TurnId | undefined => ctx.activeTurnId;

const resolveCallbackTurnId = (ctx: MetaSessionContext): TurnId | undefined => ctx.activeTurnId;

const resolveSessionCallbackTurnId = (
  sessions: ReadonlyMap<ThreadId, MetaSessionContext>,
  threadId: ThreadId,
): TurnId | undefined => {
...
function parseMetaResume(raw: unknown): { sessionId: string } | undefined {
  if (!isRecord(raw)) return undefined;
  if (raw.schemaVersion !== META_RESUME_VERSION) return undefined;
...
export function metaPromptSettlementBelongsToContext(input: {
...
export function makeMetaAdapter(metaSettings: MetaSettings, options?: MetaAdapterLiveOptions) {
  return Effect.gen(function* () {
    const boundInstanceId = options?.instanceId ?? ProviderInstanceId.make("meta");
    ...
    const sessions = new Map<ThreadId, MetaSessionContext>();
      ...
      Effect.mapError(
        (cause) =>
          new ProviderAdapterRequestError({
            ...
            detail: "Failed to generate Meta runtime identifier.",
            cause,
          }),
      ),
    );
    ...
    const settlePromptInFlight = (
      ...
      Effect.gen(function* () {
        ...
        const settlementBelongsToLiveContext = metaPromptSettlementBelongsToContext({
    ...
    const logNative = (threadId: ThreadId, method: string, payload: unknown) =>
      Effect.gen(function* () {
        ...
        Effect.catchCause((cause) =>
          Effect.logWarning("Failed to write native Meta notification log.", {
            cause,
            threadId,
            method,
          }),
    ...
    const requireSession = (
      threadId: ThreadId,
    ): Effect.Effect<MetaSessionContext, ProviderAdapterSessionNotFoundError> => {
      const ctx = sessions.get(threadId);
      if (!ctx || ctx.stopped) {
        return Effect.fail(
    ...
    const stopSessionInternal = (ctx: MetaSessionContext) =>
      Effect.gen(function* () {
    ...
    const startSession: MetaAdapterShape["startSession"] = (input) =>
      withThreadLock(
        input.threadId,
        Effect.gen(function* () {
          ...
          const resumeSessionId = parseMetaResume(input.resumeCursor)?.sessionId;
          ...
          const started = yield* Effect.gen(function* () {
            // ACP elicitation is how an ACP agent asks the user a question
            // mid-turn. Registering it routes those into the same picker
            // Claude's and Codex's questions already use.
            yield* acp.handleElicitation((params) =>
              Effect.gen(function* () {
                ...
                return yield* acpElicitationExchange({
                  request: params,
                  emitRequested: (questions) =>
                    Effect.gen(function* () {
                      yield* offerRuntimeEvent({
                  ...
                  awaitResolution: Deferred.await(resolution),
                });
              }).pipe(
                Effect.mapError(
                  (cause) =>
                    new EffectAcpErrors.AcpTransportError({
                      detail: "Failed to process Meta ACP elicitation callback.",
                      cause,
                    }),
                ),
              ),
            ...
            yield* acp.handleRequestPermission((params) =>
              Effect.gen(function* () {
                ...
                Effect.mapError(
                  (cause) =>
                    new EffectAcpErrors.AcpTransportError({
                      detail: "Failed to process Meta ACP permission callback.",
                      cause,
                    }),
                ),
              ),
          ...
          const requestedStartModelId = resolveMetaAcpModelId(metaModelSelection?.model);
          const boundModelId = yield* applyMetaAcpModelSelection({
            runtime: acp,
            currentModelId: currentMetaModelIdFromSessionSetup(started.sessionSetupResult),
          ...
          const session: ProviderSession = {
            ...
            threadId: input.threadId,
            resumeCursor: {
              schemaVersion: META_RESUME_VERSION,
          ...
          const nf = yield* Stream.runDrain(
            ...
            Effect.catch((cause) =>
              Effect.logError("Failed to process Meta runtime notification.", { cause }),
            ),
            // Fork into the session scope, not the calling fiber - see
            // GrokAdapter.ts's identical comment for why: `forkChild` would
            // make this a child of `startSession` and get interrupted the
            // moment it returns, silently dropping every later notification.
          ...
          yield* offerRuntimeEvent({
            ...
            provider: PROVIDER,
            threadId: input.threadId,
            payload: { state: "ready", reason: "Meta ACP session ready" },
          });
          yield* offerRuntimeEvent({
            type: "thread.started",
            ...(yield* makeEventStamp()),
    ...
    const sendTurn: MetaAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              const turnModelSelection =
                input.modelSelection?.instanceId === boundInstanceId
                  ...
                  : undefined;
              const requestedTurnModelId = resolveMetaAcpModelId(turnModelSelection?.model);
              const currentModelId = yield* applyMetaAcpModelSelection({
                runtime: ctx.acp,
              ...
              if (ctx.interruptedTurnIds.has(turnId)) {
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "Meta prompt was interrupted during preparation.",
                });
              }
              ...
              Effect.tapCause(() =>
                Effect.gen(function* () {
                  ...
                  yield* settlePromptInFlight(input.threadId, turnId, liveCtx.acpSessionId, {
                    errorMessage: "Meta prompt preparation failed.",
                    emitTurnCompletion: false,
                  });
                }),
              ),
            ...
            Effect.gen(function* () {
              ...
              if (ctx.acpSessionId !== prepared.acpSessionId) {
                yield* settlePromptInFlight(
                  ...
                  {
                    errorMessage: "Meta session changed before the turn completed.",
                    settleAllPrompts: true,
                  },
                );
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "Meta session changed before the turn completed.",
                });
              }
            ...
            Effect.gen(function* () {
              ...
              yield* withThreadLock(
                ...
                settlePromptInFlight(input.threadId, prepared.turnId, prepared.acpSessionId, {
                  errorMessage: errorMessage ?? "Meta prompt request failed.",
                }),
              );
    ...
    const interruptTurn: MetaAdapterShape["interruptTurn"] = (threadId, turnId) =>
      Effect.gen(function* () {
    ...
    const respondToRequest: MetaAdapterShape["respondToRequest"] = (
      threadId,
      requestId,
      decision,
    ) =>
    ...
    // Meta has no ask-user-question style extension - pendingUserInputs is
    // always empty, so this always reports "unknown request" honestly rather
    // than silently accepting an answer nothing is waiting on.
    const respondToUserInput: MetaAdapterShape["respondToUserInput"] = (
      threadId,
      requestId,
      _answers,
    ) =>
    ...
    const readThread: MetaAdapterShape["readThread"] = (threadId) =>
      Effect.gen(function* () {
        const ctx = yield* requireSession(threadId);
        return { threadId, turns: ctx.turns };
      });

    const rollbackThread: MetaAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: "Meta ACP sessions do not support provider-side rollback yet.",
        });
      });

    const stopSession: MetaAdapterShape["stopSession"] = (threadId) =>
      withThreadLock(
        threadId,
        Effect.gen(function* () {
          const ctx = yield* requireSession(threadId);
    ...
    const listSessions: MetaAdapterShape["listSessions"] = () =>
      Effect.sync(() => Array.from(sessions.values(), (c) => ({ ...c.session })));

    const hasSession: MetaAdapterShape["hasSession"] = (threadId) =>
      Effect.sync(() => {
        const c = sessions.get(threadId);
        return c !== undefined && !c.stopped;
      });

    const stopAll: MetaAdapterShape["stopAll"] = () =>
