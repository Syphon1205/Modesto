...
const runtimeMock = {
  state: {
    ...
    pendingLiveEvents: [] as unknown[],
    liveEventWaiters: [] as Array<(event: unknown | null) => void>,
  ...
  reset() {
    ...
    this.state.liveEventsEnabled = false;
    this.state.pendingLiveEvents = [];
    for (const waiter of this.state.liveEventWaiters) {
      waiter(null);
    }
    this.state.liveEventWaiters = [];
...
const pushOpenCodeLiveEvent = (event: unknown): void => {
  const waiter = runtimeMock.state.liveEventWaiters.shift();
  if (waiter) {
    waiter(event);
    return;
  }
  runtimeMock.state.pendingLiveEvents.push(event);
};
      ...
      event: {
        subscribe: async () => ({
          stream: (async function* () {
            ...
            if (!runtimeMock.state.liveEventsEnabled) {
              return;
            }
            while (true) {
              const queued = runtimeMock.state.pendingLiveEvents.shift();
              if (queued !== undefined) {
                yield queued;
                continue;
              }
              const next = await new Promise<unknown | null>((resolve) => {
                runtimeMock.state.liveEventWaiters.push(resolve);
              });
              if (next === null) {
                return;
              }
...
it.layer(OpenCodeAdapterTestLayer)("OpenCodeAdapterLive", (it) => {
  ...
  it.effect("interruptTurn clears the active turn so a follow-up send is a new turn", () =>
    Effect.gen(function* () {
      const adapter = yield* OpenCodeAdapter;
      const threadId = asThreadId("thread-interrupt-then-send");
      yield* adapter.startSession({
        provider: ProviderDriverKind.make("opencode"),
        threadId,
        runtimeMode: "full-access",
      ...
      const sessionsAfterInterrupt = yield* adapter.listSessions();
      const interrupted = sessionsAfterInterrupt.find((entry) => entry.threadId === threadId);
      ...
      NodeAssert.notEqual(String(nextTurn.turnId), String(firstTurn.turnId));

      const sessionsAfterSend = yield* adapter.listSessions();
      const running = sessionsAfterSend.find((entry) => entry.threadId === threadId);
      NodeAssert.equal(running?.status, "running");
      NodeAssert.equal(String(running?.activeTurnId), String(nextTurn.turnId));
      NodeAssert.equal(runtimeMock.state.promptCalls.length, 2);
    }),
  );

  it.effect("does not emit runtime.error when OpenCode reports abort after interrupt", () =>
    Effect.gen(function* () {
      runtimeMock.state.liveEventsEnabled = true;
      const adapter = yield* OpenCodeAdapter;
      const threadId = asThreadId("thread-interrupt-session-error");
      const eventTypes = yield* Ref.make<string[]>([]);
      yield* adapter.streamEvents.pipe(
        Stream.filter((event) => event.threadId === threadId),
        Stream.tap((event) => Ref.update(eventTypes, (types) => [...types, event.type])),
        Stream.runDrain,
        Effect.forkChild,
      );
      ...
      const types = yield* Ref.get(eventTypes);
      NodeAssert.equal(types.includes("turn.aborted"), true);
      NodeAssert.equal(types.includes("runtime.error"), false);
    }),
  );
  ...
  it.effect("classifies abort-like OpenCode session errors as user interrupts", () =>
    Effect.sync(() => {
      NodeAssert.equal(isOpenCodeAbortLikeMessage("The operation was aborted."), true);
      NodeAssert.equal(isOpenCodeAbortLikeMessage("Interrupted by user."), true);
      NodeAssert.equal(isOpenCodeAbortLikeMessage("Request cancelled"), true);
      NodeAssert.equal(isOpenCodeAbortLikeMessage("provider overloaded"), false);
    }),
  );
