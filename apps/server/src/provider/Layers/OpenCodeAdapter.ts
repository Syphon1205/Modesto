...
interface OpenCodeSessionContext {
  ...
  /**
   * Set by `interruptTurn` so a follow-up `session.error` / idle from the
   * aborted turn cannot look like a new failure or complete a later turn.
   */
...
export function isOpenCodeAbortLikeMessage(message: string): boolean {
...
function isStaleOpenCodeAbortEvent(context: OpenCodeSessionContext): boolean {
  return (
    context.activeTurnId !== undefined &&
    context.interruptedTurnId !== undefined &&
    context.activeTurnId !== context.interruptedTurnId
  );
}
...
export function makeOpenCodeAdapter(
  ...
  return Effect.gen(function* () {
    ...
    /**
     * Reports what an OpenCode turn has consumed so far.
     *
     * OpenCode revises an assistant message in place as the turn runs, so this
     * fires repeatedly with growing totals; each event supersedes the last
     * rather than accumulating. Messages with no tokens yet (the initial empty
     * revision) are skipped so the meter never flashes a zeroed reading.
     *
     * `tokens.total` is optional in the SDK, so the total is reconstructed from
     * the parts when absent - otherwise usage would read as zero on a provider
     * that in fact reported every component.
     */
    const emitOpenCodeTokenUsage = Effect.fn("emitOpenCodeTokenUsage")(function* (
      ...
      const usedTokens =
        tokens.total ?? tokens.input + tokens.output + tokens.reasoning + cacheRead + cacheWrite;
      ...
      switch (event.type) {
        ...
        case "session.status": {
          ...
          if (event.properties.status.type === "idle") {
            if (context.userInterrupted && isStaleOpenCodeAbortEvent(context)) {
              break;
            }
            if (context.userInterrupted && !context.activeTurnId) {
              context.userInterrupted = false;
              context.interruptedTurnId = undefined;
        ...
        case "session.error": {
          ...
          const abortLike = context.userInterrupted || isOpenCodeAbortLikeMessage(message);
          if (abortLike) {
            if (isStaleOpenCodeAbortEvent(context)) {
              context.userInterrupted = false;
              break;
            }
            context.activeTurnId = undefined;
