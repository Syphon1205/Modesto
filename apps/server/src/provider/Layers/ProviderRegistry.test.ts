...
import { deepMerge } from "@modesto/shared/Struct";
import { createModelCapabilities } from "@modesto/shared/model";
import { applyServerSettingsPatch } from "@modesto/shared/serverSettings";
...
process.env.MODESTO_CURSOR_ENABLED = "1";
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          const missingBinary = `modesto_codex_missing_`;
          ...
          const providerRegistryLayer = ProviderRegistryLive.pipe(
            ...
            Layer.provideMerge(
              ServerSecretStore.layer.pipe(
                Layer.provide(
                  ServerConfig.layerTest(process.cwd(), { prefix: "t3-provider-registry-" }),
                ),
              ),
            ),
      ...
      it.effect("re-probes when settings change the codex binaryPath", () =>
        Effect.gen(function* () {
          const firstMissing = `modesto_codex_first_`;
          const secondMissing = `modesto_codex_second_`;
          ...
          yield* Effect.gen(function* () {
            ...
            // Kilo, Gemini, and Meta (like Codex/Claude, unlike Cursor/Grok/OpenCode)
            // are enabled by default, so they also probe their binaries at
            // boot - order between them isn't meaningful here.
            assert.deepStrictEqual(
              spawnedCommands.toSorted(),
              [firstMissing, "kilo", "gemini", "muse"].toSorted(),
            );
            ...
            assert.deepStrictEqual(
              spawnedCommands.toSorted(),
              [firstMissing, "kilo", "gemini", "muse", secondMissing].toSorted(),
            );
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(
                ...
                "Cursor is disabled in Modesto settings.",
              );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in Modesto settings.");
        }),
      );
    });
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("runs Claude status probes with the configured CLAUDE_CONFIG_DIR", () => {
        const claudeConfigDir = "/tmp/modesto-claude-home";
