...
//          walking) plus the unified cross-provider skills catalog backing Modesto
//          portable skills. Aggregates `~/.modesto/skills` with every provider-native
...
//          mergeSkillsIntoCatalog, filterDisabledSkills, ensureModestoSkillsDir

import * as fs from "node:fs/promises";
import * as nodePath from "node:path";

import type { ProviderKind, ProviderSkillDescriptor } from "@modesto/contracts";
...
export interface SkillsCatalogDiscoveryInput {
  ...
  /** Modesto base dir (usually `~/.modesto`); skills live in `{base}/skills`. */
  readonly modestoBaseDir: string;
...
export interface SkillsCatalogRootInput extends SkillsCatalogDiscoveryInput {
  /** Native provider scans can opt out; the catalog itself always includes Modesto. */
  readonly includeModestoRoot?: boolean;
}
...
const ensuredModestoSkillsDirs = new Set<string>();

export function clearSkillsCatalogCacheForTests(): void {
  skillsCatalogCache.clear();
  skillsCatalogInflight.clear();
  ensuredModestoSkillsDirs.clear();
}

export function modestoSkillsDir(modestoBaseDir: string): string {
  return nodePath.join(modestoBaseDir, "skills");
}

// Creates the portable skills folder on first use so users have a drop-in target.
export async function ensureModestoSkillsDir(modestoBaseDir: string): Promise<string> {
  const dir = modestoSkillsDir(modestoBaseDir);
  if (ensuredModestoSkillsDirs.has(dir)) {
    return dir;
  }
  try {
    await fs.mkdir(dir, { recursive: true });
    ensuredModestoSkillsDirs.add(dir);
  } catch {
...
const SKILL_ORIGIN_ROOTS = {
  modesto: {
    homeRoots: (input) => [modestoSkillsDir(input.modestoBaseDir)],
    projectRootNames: [".modesto"],
  },
  codex: {
    // Keep Modesto's existing Codex-local root. Official Codex discovery uses
...
// Native copies first so an agent keeps using its own skill, then Modesto as the
...
function orderedOriginsForProvider(
  provider: ProviderKind | null | undefined,
  includeModestoRoot = true,
  includeRemainingOrigins = true,
): SkillsHomeOrigin[] {
  const preferred = preferredOriginsForProvider(provider);
  const ordered = [...preferred];
  if (includeModestoRoot && !ordered.includes("modesto")) {
    ordered.push("modesto");
  }
  if (!includeRemainingOrigins) {
    return ordered.filter((origin) => includeModestoRoot || origin !== "modesto");
  }
  for (const origin of HOME_ORIGIN_ORDER) {
    if (!includeModestoRoot && origin === "modesto") {
      continue;
    }
...
export function skillsCatalogRoots(input: SkillsCatalogRootInput): SkillRoot[] {
  return rootsForOrderedOrigins(
    input,
    orderedOriginsForProvider(input.provider, input.includeModestoRoot !== false),
  );
}
...
export async function discoverSkillsCatalog(
  ...
  const scan = (async () => {
    await ensureModestoSkillsDir(input.modestoBaseDir);
