...
> {
  ...
  const applySnapshotBase = Effect.fn("applySnapshot")(function* (
    ...
    // A refresh that discovers nothing must not empty the model picker: the UI
    // derives its options purely from this snapshot, so publishing an empty
    // list leaves the user with no models until the next successful refresh -
    // a full health interval away. See `preserveDiscoveredModels` for exactly
    // when the previous list is carried forward.
    const previousSnapshot = yield* Ref.get(snapshotStateRef).pipe(
      Effect.map((state) => state.snapshot),
    );
    const nextSnapshot = preserveDiscoveredModels(previousSnapshot, checkedSnapshot);
