 ...
 */
import { CodexSettings, ProviderDriverKind, type ServerProvider } from "@modesto/contracts";
...
export const CodexDriver: ProviderDriver<CodexSettings, CodexDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      // `makeManagedServerProvider.checkProvider`'s `R = never`. Custom model
      // router endpoints are fetched live from `serverSettings` on every
      // check (rather than baked into `effectiveConfig`, which is Codex's own
      // settings and doesn't change when a router endpoint is edited) so
      // adding/editing one doesn't require restarting this instance.
