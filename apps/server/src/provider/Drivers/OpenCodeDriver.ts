 ...
 */
import { OpenCodeSettings, ProviderDriverKind, type ServerProvider } from "@modesto/contracts";
...
export const OpenCodeDriver: ProviderDriver<OpenCodeSettings, OpenCodeDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      // Settings > Providers > Custom endpoints reach OpenCode/Kilo through
      // their own `provider` config block, injected here so both the adapter's
      // server and the status probe below spawn with the same catalog. The
      // resulting models surface with no extra work: `OpenCodeProvider.ts`
      // builds its model list from the live server's own provider list, so a
      // custom endpoint appears as `<endpointId>/<modelId>` with the
      // endpoint's label as its `subProvider` - OpenCode's native namespacing,
      // no synthetic slug needed (unlike Codex's `router:` scheme).
