// FILE: GeminiDriver.ts
// Purpose: ProviderDriver registration for Gemini CLI, adapted from
//          GrokDriver.ts. Uses `makeUnsupportedTextGeneration` for
//          commit-message/PR-content/branch-name/thread-title generation -
//          the primary Modesto tree has no dedicated Gemini text-generation
//          path either (confirmed by grepping its GeminiAdapter.ts for the
//          four generate* operations - none exist there), so this isn't a
//          scope cut relative to the source, just an honest reflection of
//          what Gemini support actually includes today.
// @module provider/Drivers/GeminiDriver
import { GeminiSettings, ProviderDriverKind, type ServerProvider } from "@modesto/contracts";
