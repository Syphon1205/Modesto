...
//          that makes Modesto catalog skills usable on every provider.
...
import type { ProviderKind, ProviderSkillReference } from "@modesto/contracts";
...
export function shouldInlineSkillForProvider(provider: ProviderKind, skillPath: string): boolean {
  ...
  switch (provider) {
    case "codex":
      ...
      // folders plus `~/.modesto/skills`, which Modesto registers at session start
    ...
    case "cursor":
      ...
      // only Modesto-owned paths need inlining.
      return segments.has(".modesto");
    case "claudeAgent":
