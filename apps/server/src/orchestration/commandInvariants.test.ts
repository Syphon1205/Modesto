...
describe("commandInvariants", () => {
  ...
  it("does not treat chats as project-owned threads", () => {
    const chatThread = {
      ...readModel.threads[0]!,
      id: ThreadId.make("chat-1"),
      conversationMode: "chat" as const,
    };
    const withChat: OrchestrationReadModel = {
      ...readModel,
      threads: [...readModel.threads, chatThread],
    };
    expect(
      listProjectOwnedActiveThreads(withChat, ProjectId.make("project-a")).map(
        (thread) => thread.id,
      ),
