...
import { normalizeProjectPathForComparison } from "@modesto/shared/path";
...
/** Code/project threads are owned by a project. Chats are a separate surface. */
export function isProjectOwnedThread(
  thread: Pick<OrchestrationThread, "conversationMode" | "deletedAt">,
): boolean {
  return thread.deletedAt === null && thread.conversationMode !== "chat";
}

export function listProjectOwnedActiveThreads(
  readModel: OrchestrationReadModel,
  projectId: ProjectId,
): ReadonlyArray<OrchestrationThread> {
  return listThreadsByProjectId(readModel, projectId).filter(isProjectOwnedThread);
}
