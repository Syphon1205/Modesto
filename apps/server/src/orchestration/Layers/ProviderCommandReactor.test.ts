...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir = input?.baseDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "modesto-reactor-"));
  ...
  it("renames temporary worktree branches and keeps associated worktree metadata in sync", async () => {
    ...
    await waitFor(async () => {
      ...
      return (
        thread?.branch === "modesto/app-startup-crash" &&
        thread.associatedWorktreeBranch === "modesto/app-startup-crash" &&
        thread.associatedWorktreeRef === "modesto/app-startup-crash"
      );
    });
    ...
    expect(thread).toMatchObject({
      branch: "modesto/app-startup-crash",
      worktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreeBranch: "modesto/app-startup-crash",
      associatedWorktreeRef: "modesto/app-startup-crash",
    });
  });
  ...
  it("falls back to prompt-based worktree branch names when the provider cannot generate one", async () => {
    ...
    expect(harness.renameBranch.mock.calls[0]?.[0]).toMatchObject({
      oldBranch: "modesto/cb661f0d",
      newBranch: "modesto/fix-provider-startup-timeouts",
    });
    ...
    await waitFor(async () => {
      ...
      return thread?.branch === "modesto/fix-provider-startup-timeouts";
    });
  });
  ...
  it("replaces the provider session when a later turn changes provider", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
