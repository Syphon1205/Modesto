...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir =
      input?.baseDir ?? NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "modesto-reactor-"));
  ...
  it("hands off to a different provider mid-thread, replaying prior context", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    // Handoff stops the prior provider session before starting the new one.
    ...
    // The second sendTurn call is the handoff turn: it must carry the prior
    // conversation as a replayed transcript, not just the new message alone.
    const handoffTurnInput = harness.sendTurn.mock.calls[1]?.[0] as { input?: string };
    expect(handoffTurnInput.input).toContain("Handoff from codex to claudeAgent");
    expect(handoffTurnInput.input).toContain("## Core context");
    expect(handoffTurnInput.input).toContain("User: first");
    expect(handoffTurnInput.input).toContain("second");
    expect(thread?.activities.some((activity) => activity.kind === "thread.core-context")).toBe(
      true,
    );
  });

  it("parks the previous thread's idle provider session when a different thread starts", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.create",
        commandId: CommandId.make("cmd-thread-create-park-2"),
        threadId: ThreadId.make("thread-2"),
        projectId: asProjectId("project-1"),
        title: "Thread 2",
        modelSelection: {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-park-1"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-park-1"),
          role: "user",
          text: "first thread",
          attachments: [],
        },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-park-2"),
        threadId: ThreadId.make("thread-2"),
        message: {
          messageId: asMessageId("user-message-park-2"),
          role: "user",
          text: "second thread",
          attachments: [],
        },
    ...
    expect(harness.runtimeSessions.map((session) => String(session.threadId))).toEqual([
      "thread-2",
    ]);

    const readModel = await harness.readModel();
    const parkedThread = readModel.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(parkedThread?.session?.status).toBe("stopped");
  });

  it("compacts conversation context and resets usage when switching models", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    const compactTurnInput = harness.sendTurn.mock.calls[1]?.[0] as { input?: string };
    expect(compactTurnInput.input).toContain(
      "Context compacted after switching from gpt-5-codex to gpt-5.4",
    );
    expect(compactTurnInput.input).toContain("## Core context");
    expect(compactTurnInput.input).toContain("User: first");
    expect(compactTurnInput.input).toContain("second");
    ...
    expect(
      thread?.activities.some(
        (activity) =>
          ...
          (activity.payload as { usedTokens?: number }).usedTokens === 0,
      ),
    ).toBe(true);
  });

  it("hands off to a different provider after the existing thread session has stopped", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    expect(thread?.session?.threadId).toBe("thread-1");
    expect(thread?.session?.providerName).toBe("claudeAgent");
    // No prior messages existed before the stopped session, so there is
    // nothing to replay - the turn text goes through unprefixed.
    const turnInput = harness.sendTurn.mock.calls[0]?.[0] as { input?: string };
    expect(turnInput.input).toBe("continue with claude");
  });
  ...
  it("records a failure activity when provider interrupt fails", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-session-set-interrupt-failed"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "running",
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.interrupt",
        commandId: CommandId.make("cmd-turn-interrupt-failed"),
        threadId: ThreadId.make("thread-1"),
        turnId: asTurnId("turn-1"),
        createdAt: now,
      }),
  ...
  it("restarts a provider session after a projected error so the next turn can send", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-before-error"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-before-error"),
          role: "user",
          text: "first",
          attachments: [],
        },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-after-error"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-after-error"),
          role: "user",
          text: "second",
          attachments: [],
        },
