...
} from "@modesto/shared/conversationEdit";
import { isTemporaryWorktreeBranch, WORKTREE_BRANCH_PREFIX } from "@modesto/shared/git";
import { claudeSelectionRequiresRestart } from "@modesto/shared/model";
import { buildStalePendingRequestFailureDetail } from "@modesto/shared/threadSummary";
import { resolveThreadWorkspaceState } from "@modesto/shared/threadEnvironment";
...
const RESEARCH_PROVIDER_INSTRUCTIONS = `<modesto_research_mode>
...
type ProviderContextTag =
  | "handoff_context"
  | "handoff_return_context"
  | "sidechat_context"
  | "thread_context";
...
function buildGeneratedWorktreeBranchName(raw: string): string {
  ...
  const withoutPrefix = normalized.replace(/^modesto\//, "");
...
const make = Effect.gen(function* () {
  ...
  const ensureSessionForThread = Effect.fnUntraced(function* (
    ...
    // Providers own incompatible native session formats. A cross-provider
    // selection starts a fresh native session for this same Modesto thread;
    // dispatch then gives that session the existing transcript as context.
    const providerChanged =
      requestedModelSelection !== undefined && requestedModelSelection.provider !== threadProvider;
    const preferredProvider: ProviderKind = requestedModelSelection?.provider ?? threadProvider;
    ...
    if (providerChanged) {
      // The replacement adapter cannot resume another provider's native state,
      // so seed its first turn with the Modesto transcript instead.
    ...
    if (existingSessionThreadId) {
      ...
      if (providerChanged) {
        // Stop and unbind the old adapter before binding the replacement. This
        // avoids leaving a live session behind under the same thread id.
  ...
  const dispatchTurnForThread = Effect.fnUntraced(function* (input: {
    ...
    readonly workspaceMode?: "research";
    ...
    const handoffBootstrapText =
      shouldBootstrapHandoff && handoffBootstrapAvailableChars > 0
        ...
        : null;
    const shouldBootstrapHandoffReturn =
      thread.handoffReturn !== null &&
      !shouldBootstrapHandoff &&
      !hasNativeAssistantMessagesBefore(thread, input.messageId);
    const handoffReturnBootstrapAvailableChars = availableProviderContextChars({
      tag: "handoff_return_context",
      messageText: boundaryMessageText,
      wrapLatestUserMessage: true,
    });
    const handoffReturnForBootstrap = thread.handoffReturn;
    const handoffReturnBootstrapText =
      shouldBootstrapHandoffReturn &&
      handoffReturnForBootstrap !== null &&
      handoffReturnForBootstrap !== undefined &&
      handoffReturnBootstrapAvailableChars > 0
        ? buildHandoffReturnBootstrapText(
            handoffReturnForBootstrap,
            handoffReturnBootstrapAvailableChars,
          )
        : null;
    ...
    const priorTranscriptBootstrapText =
      shouldBootstrapPriorTranscriptContext && priorTranscriptBootstrapAvailableChars > 0
        ...
        : null;
    const contextualProviderInput = handoffBootstrapText
      ? wrapProviderContext({
          tag: "handoff_context",
      ...
      : handoffReturnBootstrapText
        ? wrapProviderContext({
            tag: "handoff_return_context",
            contextText: handoffReturnBootstrapText,
            messageText: boundaryMessageText,
            wrapLatestUserMessage: true,
          })
        : sidechatBootstrapText
          ...
          : priorTranscriptBootstrapText
            ...
            : boundaryMessageText;
    const providerInput =
      input.workspaceMode === "research"
        ? `${RESEARCH_PROVIDER_INSTRUCTIONS}\n\n${contextualProviderInput}`
        : contextualProviderInput;
      ...
      const sentTurn = yield* sendQueuedProviderTurn(normalizedInput).pipe(
        Effect.catch((error) =>
          Effect.gen(function* () {
            ...
            // dead cursor and replay once with Modesto transcript context.
            yield* clearStaleProviderResumeState({
              threadId: input.threadId,
              cause: error,
            });
  ...
  const processTurnStartRequested = Effect.fnUntraced(function* (
    ...
    yield* dispatchTurnForThread({
      ...
      ...(event.payload.workspaceMode !== undefined
    ...
    try {
      ...
      yield* orchestrationEngine
        .dispatch({
          ...
          ...(nextQueuedTurn.workspaceMode !== undefined
            ? { workspaceMode: nextQueuedTurn.workspaceMode }
            : {}),
