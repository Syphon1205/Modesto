...
const ACTIVITY_DATA_TRUNCATION_MARKER = "__modestoTruncated";
    ...
    Effect.gen(function* () {
      ...
      if (event.type === "runtime.error") {
        ...
        if (shouldApplyRuntimeError) {
          // Terminal runtime errors must clear the active turn so projection
          // finalizes open turns, the session reaper can reclaim idle processes,
          // and Recover session does not rehydrate a stuck "Working" state.
          yield* orchestrationEngine.dispatch({
            type: "thread.session.set",
      ...
      if (event.type === "turn.diff.updated") {
        const turnId = toTurnId(event.turnId);
        // Live diffs a provider reports directly (not derived from git) work
        // in any workspace, git repo or not — the real, git-checkpointed
        // diff from CheckpointReactor stays authoritative when it exists
        // (see the `existingProviderPlaceholder` check below); this is only
        // ever a preview until that real checkpoint lands.
        if (turnId) {
          ...
          if (existingCheckpoint && !existingProviderPlaceholder) {
            ...
            if (canParseLiveDiffPatch) {
              ...
              // Stats alone aren't enough to render a real diff. Persist the
              // provider's raw patch so "Review" can show it later without a
              // git checkpoint to diff against (git-backed threads never read
              // this — CheckpointDiffQuery only falls back to it for
              // `provider-diff:` checkpoints, which git supersedes).
              yield* checkpointDiffBlobStore
                .upsert({
                  threadId: thread.id,
