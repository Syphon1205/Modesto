...
const REASONING_BY_TURN_CACHE_CAPACITY = 10_000;
const REASONING_BY_TURN_TTL = Duration.minutes(120);
/**
 * Reasoning is stored whole, once per turn, so it needs its own bound. Larger
 * than an assistant buffer because thinking routinely runs longer than the
 * answer it produces; truncation is marked rather than silent.
 */
const MAX_REASONING_CHARS = 60_000;
/**
 * How long assistant text may sit in memory before it is written to the event
 * store, whatever else is happening in the turn.
 *
 * Buffered delivery exists so the client is not re-rendering the transcript per
 * token, but the buffer is process memory: until it spills, a server restart
 * takes the whole answer with it. Spilling on a timer bounds that loss to this
 * interval instead of a whole turn, at one write per interval per message.
 */
const BUFFERED_ASSISTANT_SPILL_INTERVAL_MS = 3_000;
const STRICT_PROVIDER_LIFECYCLE_GUARD = process.env.MODESTO_STRICT_PROVIDER_LIFECYCLE_GUARD !== "0";
...
const make = Effect.gen(function* () {
  ...
  /**
   * Reasoning text accumulated for the turn currently producing it.
   *
   * Adapters emit reasoning as `content.delta` with a `reasoning_text` stream
   * kind, one fragment at a time, and ingestion used to drop every one - which
   * is why "Thinking" could never be opened. Emitting an activity per fragment
   * would flood the event stream on a long think, so fragments accumulate here
   * and are written once, when the turn ends.
   */
  const reasoningTextByTurnId = yield* Cache.make<TurnId, string>({
    capacity: REASONING_BY_TURN_CACHE_CAPACITY,
    timeToLive: REASONING_BY_TURN_TTL,
  ...
  /**
   * When each message's buffer is next due to spill. Absent means "not armed
   * yet" - the next append arms it, and a spill disarms it again.
   */
  const bufferedAssistantSpillDueAtByMessageId = yield* Cache.make<MessageId, number>({
    capacity: BUFFERED_MESSAGE_TEXT_BY_MESSAGE_ID_CACHE_CAPACITY,
    timeToLive: BUFFERED_MESSAGE_TEXT_BY_MESSAGE_ID_TTL,
  ...
  const appendTurnReasoning = (turnId: TurnId, delta: string) =>
    Cache.getOption(reasoningTextByTurnId, turnId).pipe(
      Effect.flatMap((current) =>
        Effect.gen(function* () {
          const existing = Option.getOrElse(current, () => "");
          if (existing.length >= MAX_REASONING_CHARS) {
            return;
          }
          const next = existing + delta;
          yield* Cache.set(
            reasoningTextByTurnId,
            turnId,
            next.length > MAX_REASONING_CHARS ? next.slice(0, MAX_REASONING_CHARS) : next,
          );
        }),
      ),
    );
  ...
  /**
   * Writes a turn's reasoning out as a single activity, once.
   *
   * The cache entry is cleared first: a turn is flushed from more than one
   * place (completion, interruption), and clearing up front is what stops a
   * second flush appending the same thinking twice.
   */
  const flushTurnReasoning = (input: {
    ...
    Effect.gen(function* () {
      const stored = yield* Cache.getOption(reasoningTextByTurnId, input.turnId);
      const text = Option.getOrElse(stored, () => "").trim();
      if (text.length === 0) {
        return;
      }
      yield* Cache.invalidate(reasoningTextByTurnId, input.turnId);
      const truncated = text.length >= MAX_REASONING_CHARS;
      yield* orchestrationEngine.dispatch({
        type: "thread.activity.append",
        commandId: yield* providerCommandId(input.event, "reasoning"),
        threadId: input.thread.id,
        activity: {
          id: EventId.make(`${input.turnId}:reasoning`),
          tone: "info",
          kind: "reasoning.completed",
          summary: "Thinking",
          // `detail` on purpose: the timeline's expandable-row machinery
          // already renders an entry's detail, so naming it this way makes
          // "Thinking" open with no UI change and no second code path.
          payload: { detail: text, truncated },
          turnId: input.turnId,
          createdAt: input.now,
        },
        createdAt: input.now,
    ...
    Cache.getOption(bufferedAssistantTextByMessageId, messageId).pipe(
      Effect.flatMap((existingText) =>
        Effect.gen(function* () {
          ...
          const dueAt = Option.getOrUndefined(
            yield* Cache.getOption(bufferedAssistantSpillDueAtByMessageId, messageId),
          );
          // Over the memory cap, or the buffer has been held long enough that a
          // restart would cost the reader real text.
          const shouldSpill =
            nextText.length > MAX_BUFFERED_ASSISTANT_CHARS || (dueAt !== undefined && now >= dueAt);

          if (!shouldSpill) {
            yield* Cache.set(bufferedAssistantTextByMessageId, messageId, nextText);
            if (dueAt === undefined) {
              yield* Cache.set(
                bufferedAssistantSpillDueAtByMessageId,
                messageId,
                now + BUFFERED_ASSISTANT_SPILL_INTERVAL_MS,
              );
            }
            return "";
          }
          ...
          yield* Cache.invalidate(bufferedAssistantSpillDueAtByMessageId, messageId);
          return nextText;
        }),
      ),
    );
    ...
    Cache.getOption(bufferedAssistantTextByMessageId, messageId).pipe(
      Effect.flatMap((existingText) =>
        Cache.invalidate(bufferedAssistantTextByMessageId, messageId).pipe(
          Effect.andThen(Cache.invalidate(bufferedAssistantSpillDueAtByMessageId, messageId)),
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      // A turn ending is the moment its thinking is complete and can be
      // written out. `turn.completed` covers interruption and failure too -
      // the reasoning up to that point is still what the model was thinking,
      // and dropping it on a stopped turn would hide exactly the case a user
      // most wants to inspect.
      if (event.type === "turn.completed" || event.type === "turn.aborted") {
        const completedTurnId = toTurnId(event.turnId);
        if (completedTurnId) {
          yield* flushTurnReasoning({
            thread,
      ...
      // Reasoning accumulates rather than dispatching per fragment; see
      // `reasoningTextByTurnId`. Written out when the turn ends.
      ...
      if (reasoningDelta && reasoningDelta.length > 0) {
        ...
        if (reasoningTurnId) {
          yield* appendTurnReasoning(reasoningTurnId, reasoningDelta);
        }
      }
