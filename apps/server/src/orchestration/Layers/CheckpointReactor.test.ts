...
describe("CheckpointReactor", () => {
  ...
  it("captures pre-turn baseline on turn.started and post-turn checkpoint on turn.completed", async () => {
    ...
    const thread = await waitForThread(
      ...
      (entry) =>
        ...
        entry.activities.some((activity) => activity.kind === "thread.core-context"),
    );
    expect(thread.checkpoints[0]?.checkpointTurnCount).toBe(1);
    expect(
      thread.activities.some(
        (activity) =>
          activity.kind === "thread.core-context" &&
          (activity.payload as { reason?: string }).reason === "turn",
      ),
    ).toBe(true);
    expect(
  ...
  it("refreshes core context when a proposed plan is upserted", async () => {
    const harness = await createHarness({ seedFilesystemCheckpoints: false });
    const createdAt = "2026-01-01T00:00:00.000Z";
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        commandId: CommandId.make("cmd-plan-upsert-core-context"),
        threadId: ThreadId.make("thread-1"),
        proposedPlan: {
          id: "plan-core-context",
          turnId: asTurnId("turn-plan"),
          planMarkdown: "Next: Update resumeProviderSession()\nIncomplete: legacy auth remains",
          implementedAt: null,
    ...
    const thread = await waitForThread(harness.readModel, (entry) =>
      entry.activities.some(
        (activity) =>
          activity.kind === "thread.core-context" &&
          (activity.payload as { reason?: string }).reason === "plan",
      ),
    );
    expect((thread.activities.at(-1)?.payload as { nextStep?: string }).nextStep).toBe(
  ...
  it("adopts a drifted checkout as the thread branch on a dedicated worktree", async () => {
    const harness = await createHarness({
      seedFilesystemCheckpoints: false,
      threadBranch: "modesto/original-branch",
      localStatusRefName: "modesto/renamed-by-agent",
    });
    ...
    await waitForEvent(
      ...
      (event) =>
        ...
        (event as unknown as { payload: { branch?: string } }).payload.branch ===
          "modesto/renamed-by-agent",
    );

    const snapshot = await harness.readModel();
    const thread = snapshot.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.branch).toBe("modesto/renamed-by-agent");
  });
  ...
  it("does not adopt a drifted checkout when the worktree is shared by another thread", async () => {
    ...
    const snapshot = await harness.readModel();
    const thread = snapshot.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.branch).toBe("modesto/original-branch");
  });
  ...
  it("does not adopt a temporary placeholder checkout as the thread branch", async () => {
    const harness = await createHarness({
      seedFilesystemCheckpoints: false,
      threadBranch: "modesto/original-branch",
      localStatusRefName: "modesto/0a1b2c3d",
    });
