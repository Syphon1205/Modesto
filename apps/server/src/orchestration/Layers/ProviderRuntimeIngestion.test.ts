...
import { CheckpointDiffBlobStoreLive } from "../../persistence/Layers/CheckpointDiffBlobs.ts";
...
describe("ProviderRuntimeIngestion", () => {
  ...
  async function createHarness(options: { withGitRepo?: boolean } = {}) {
    const { withGitRepo = true } = options;
    const workspaceRoot = makeTempDir("modesto-provider-project-");
    if (withGitRepo) {
    ...
    const layer = ProviderRuntimeIngestionLive.pipe(
      ...
      Layer.provideMerge(CheckpointDiffBlobStoreLive),
  ...
  it("bounds large tool activity data while keeping command metadata", async () => {
    ...
    expect(data.__modestoTruncated).toBe(true);
  ...
  it("still produces a live diff checkpoint when the workspace is not a git repo", async () => {
    const harness = await createHarness({ withGitRepo: false });
    const now = new Date().toISOString();

    harness.emit({
      type: "turn.diff.updated",
      eventId: asEventId("evt-non-git-diff"),
      provider: "codex",
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-non-git"),
      payload: {
        unifiedDiff: [
          "diff --git a/scratch-note.txt b/scratch-note.txt",
          "new file mode 100644",
          "index 0000000..3333333",
          "--- /dev/null",
          "+++ b/scratch-note.txt",
    ...
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.checkpoints.some(
        (checkpoint: ProviderRuntimeTestCheckpoint) => checkpoint.turnId === "turn-non-git",
      ),
    );

    const checkpoint = thread.checkpoints.find(
      (entry: ProviderRuntimeTestCheckpoint) => entry.turnId === "turn-non-git",
    );
    expect(checkpoint).toMatchObject({
      checkpointRef: "provider-diff:evt-non-git-diff",
      status: "missing",
      files: [{ path: "scratch-note.txt", kind: "modified", additions: 1, deletions: 0 }],
    });
  });
