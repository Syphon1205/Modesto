...
const make = Effect.gen(function* () {
  ...
  const handleRevertRequested = Effect.fnUntraced(function* (
    ...
    // Not gated on staleCheckpointRefs: a provider-diff placeholder checkpoint
    // has no real git ref to delete above, but can still have a stored blob.
    // Not required for correctness (an unreachable turn count's blob is never
    // looked up again, and a reused one is safely overwritten on upsert) —
    // this just bounds storage growth across revert/redo cycles.
...
export const CheckpointReactorLive = Layer.effect(CheckpointReactor, make).pipe(
  ...
  Layer.provide(CheckpointDiffBlobStoreLive),
);
