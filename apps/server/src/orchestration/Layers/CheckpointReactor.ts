...
import { isTemporaryWorktreeBranch } from "@modesto/shared/git";
import { coreContextFromThread } from "../coreContext.ts";
...
const make = Effect.gen(function* () {
  ...
  const appendCoreContextActivity = (input: {
    readonly thread: {
    ...
    const coreContext = coreContextFromThread(
      input.thread,
      input.reason,
      input.latestCheckpoint ?? null,
    );
    const hasReplayableMessages = input.thread.messages.some(
      (message) =>
        (message.role === "user" || message.role === "assistant") && message.text.trim().length > 0,
    );
    if (!shouldCarryCoreContext(coreContext, hasReplayableMessages)) {
      return Effect.void;
    }
    return Effect.all({
      ...
      Effect.flatMap(({ commandId, activityId }) =>
        orchestrationEngine.dispatch({
          ...
          activity: {
            ...
            summary: coreContextActivitySummary(input.reason),
            payload: coreContext,
            turnId: input.turnId ?? input.thread.latestTurn?.turnId ?? null,
            createdAt: input.createdAt,
          },
          createdAt: input.createdAt,
        }),
  ...
  const captureAndDispatchCheckpoint = Effect.fn("captureAndDispatchCheckpoint")(function* (input: {
    ...
    const threadDetail = yield* resolveThreadDetail(input.threadId);
    if (threadDetail) {
      yield* appendCoreContextActivity({
        thread: threadDetail,
        reason: coreContextReasonFromHandoffTurnId(input.turnId),
        createdAt: input.createdAt,
        latestCheckpoint: {
          checkpointTurnCount: input.turnCount,
          checkpointRef: targetCheckpointRef,
