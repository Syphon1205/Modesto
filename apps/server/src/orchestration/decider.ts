...
import { doThreadMarkerRangesOverlap } from "@modesto/shared/threadMarkers";
import {
  collectTailTurnIds,
...
import { hasNativeHandoffMessages, resolveHandoffDiffAckStatus } from "./handoff.ts";
...
> {
  switch (command.type) {
      ...
      const createdEvent: Omit<OrchestrationEvent, "sequence"> = {
        ...
        payload: {
          ...
          handoff: {
            ...
            checkpointRef: command.checkpointRef ?? null,
            baseCheckpointRef: command.baseCheckpointRef ?? null,
            baseHeadSha: command.baseHeadSha ?? null,
            checkpointStatus: command.checkpointStatus ?? "not_applicable",
          },
          handoffReturn: null,
          createdAt: command.createdAt,
          updatedAt: command.createdAt,
      ...
      const importedMessageEvents: ReadonlyArray<Omit<OrchestrationEvent, "sequence">> =
        command.importedMessages.map((message) => ({
          ...
          payload: {
            ...
            provenance: {
              sourceKind: "imported-conversation",
              freshness: "stale",
              path: null,
              label: `Imported from ${sourceThread.modelSelection.provider}`,
              contentHash: null,
    ...
    case "thread.handoff.return": {
      const sourceThread = yield* requireThread({
        readModel,
        command,
        threadId: command.sourceThreadId,
      });
      const fromThread = yield* requireThread({
        readModel,
        command,
        threadId: command.fromThreadId,
      });
      if (fromThread.handoff === null) {
        return yield* new OrchestrationCommandInvariantError({
          commandType: command.type,
          detail: `Thread '${command.fromThreadId}' is not a handoff target.`,
        });
      }
      if (fromThread.handoff.sourceThreadId !== command.sourceThreadId) {
        return yield* new OrchestrationCommandInvariantError({
          commandType: command.type,
          detail: `Handoff thread '${command.fromThreadId}' does not return to '${command.sourceThreadId}'.`,
        });
      }
      if (sourceThread.projectId !== fromThread.projectId) {
        return yield* new OrchestrationCommandInvariantError({
          commandType: command.type,
          detail: `Handoff return threads must belong to the same project.`,
        });
      }
      ...
      return {
        ...
        payload: {
          ...
          handoffReturn: {
            ...
            fromProvider: fromThread.modelSelection.provider,
            summary: command.summary,
            repoSnapshot: command.repoSnapshot ?? null,
            completedStepIds: command.completedStepIds ?? [],
            returnedAt: command.createdAt,
          },
          updatedAt: command.createdAt,
        },
      };
    ...
    case "thread.meta.update": {
      ...
      return {
        ...
        payload: {
          ...
          ...(command.handoffReturn !== undefined ? { handoffReturn: command.handoffReturn } : {}),
