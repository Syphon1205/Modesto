...
> {
  switch (command.type) {
    ...
    case "project.delete": {
      ...
      const projectOwnedThreads = listProjectOwnedActiveThreads(readModel, command.projectId);
      if (projectOwnedThreads.length > 0 && command.force !== true) {
        return yield* new OrchestrationCommandInvariantError({
          commandType: command.type,
          detail: `Project '${command.projectId}' still has code threads and cannot be deleted without force=true.`,
        });
      }
      if (projectOwnedThreads.length > 0) {
        return yield* decideCommandSequence({
          readModel,
          commands: [
            ...projectOwnedThreads.map(
      ...
      return {
        ...
        payload: {
          ...
          ...(command.modelSelection !== undefined
            ? { modelSelection: command.modelSelection }
            : {}),
          ...(command.conversationMode !== undefined
            ? { conversationMode: command.conversationMode }
            : {}),
