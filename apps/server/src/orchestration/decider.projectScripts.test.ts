...
describe("decider project scripts", () => {
  ...
  it("Fremont: handoff.create stores repo snapshot and return writes handoffReturn", async () => {
    const now = new Date().toISOString();
    const initial = createEmptyReadModel(now);
    const withProject = await Effect.runPromise(
      projectEvent(initial, {
        sequence: 1,
        eventId: asEventId("evt-project-create-fremont"),
        aggregateKind: "project",
        aggregateId: asProjectId("project-fremont"),
        type: "project.created",
        occurredAt: now,
        commandId: CommandId.makeUnsafe("cmd-project-create-fremont"),
        causationEventId: null,
        correlationId: CommandId.makeUnsafe("cmd-project-create-fremont"),
        metadata: {},
        payload: {
          projectId: asProjectId("project-fremont"),
          kind: "project",
          title: "Fremont",
          workspaceRoot: "/tmp/fremont",
          defaultModelSelection: null,
          scripts: [],
          isPinned: false,
          createdAt: now,
    ...
    const withSource = await Effect.runPromise(
      projectEvent(withProject, {
        sequence: 2,
        eventId: asEventId("evt-thread-create-fremont-source"),
        aggregateKind: "thread",
        aggregateId: ThreadId.makeUnsafe("thread-fremont-source"),
        type: "thread.created",
        occurredAt: now,
        commandId: CommandId.makeUnsafe("cmd-thread-create-fremont-source"),
        causationEventId: null,
        correlationId: CommandId.makeUnsafe("cmd-thread-create-fremont-source"),
        metadata: {},
        payload: {
          threadId: ThreadId.makeUnsafe("thread-fremont-source"),
          projectId: asProjectId("project-fremont"),
          title: "Cursor task",
          modelSelection: {
            provider: "cursor",
    ...
    const withSourceMessage = await Effect.runPromise(
      projectEvent(withSource, {
        sequence: 3,
        eventId: asEventId("evt-fremont-source-message"),
        aggregateKind: "thread",
        aggregateId: ThreadId.makeUnsafe("thread-fremont-source"),
        type: "thread.message-sent",
        occurredAt: now,
        commandId: CommandId.makeUnsafe("cmd-fremont-source-message"),
        causationEventId: null,
        correlationId: CommandId.makeUnsafe("cmd-fremont-source-message"),
        metadata: {},
        payload: {
          threadId: ThreadId.makeUnsafe("thread-fremont-source"),
          messageId: asMessageId("message-fremont-source-1"),
          role: "user",
          text: "Edit three files then stop before tests",
          turnId: null,
          streaming: false,
          source: "native",
          createdAt: now,
    ...
    const createResult = await Effect.runPromise(
      decideOrchestrationCommand({
        command: {
          type: "thread.handoff.create",
          commandId: CommandId.makeUnsafe("cmd-fremont-handoff"),
          threadId: ThreadId.makeUnsafe("thread-fremont-codex"),
          sourceThreadId: ThreadId.makeUnsafe("thread-fremont-source"),
          projectId: asProjectId("project-fremont"),
          title: "Cursor task",
          modelSelection: {
            provider: "codex",
          ...
          branch: "feature/fremont",
          worktreePath: null,
          summary: "Cursor edited three files; tests not run.",
          objective: "Finish and test without redoing Cursor's edits.",
          unfinishedSteps: [{ id: "tests", text: "Run tests", status: "todo" }],
          repoSnapshot: {
            headSha: "abc",
            branch: "feature/fremont",
            worktreePath: null,
            ...
            changedFiles: [
              ...
              { path: "c.ts", insertions: 3, deletions: 0 },
            ],
            statusSummary: "3 files changed",
            diffSummaryMarkdown: "three file edits",
            capturedAt: now,
          },
          importedMessages: [
            {
              messageId: asMessageId("message-fremont-imported-1"),
              role: "user",
              text: "Edit three files then stop before tests",
              createdAt: now,
              updatedAt: now,
          ...
          checkpointRef: CheckpointRef.makeUnsafe(
            "refs/modesto/agent-checkpoints/src/dst/tree",
          ),
          baseCheckpointRef: CheckpointRef.makeUnsafe(
            "refs/modesto/agent-checkpoints/src/dst/base",
          ),
          baseHeadSha: "abc",
          checkpointStatus: "captured",
          createdAt: now,
        },
        readModel: withSourceMessage,
      }),
    );

    const createEvents = Array.isArray(createResult) ? createResult : [createResult];
    expect(createEvents[0]?.type).toBe("thread.created");
    if (createEvents[0]?.type !== "thread.created") {
      throw new Error("expected thread.created");
    }
    expect(createEvents[0].payload.handoff?.summary).toBe(
      "Cursor edited three files; tests not run.",
    );
    expect(createEvents[0].payload.handoff?.diffAckStatus).toBe("pending");
    expect(createEvents[0].payload.handoff?.repoSnapshot?.changedFiles).toHaveLength(3);
    expect(createEvents[0].payload.handoff?.checkpointStatus).toBe("captured");
    expect(createEvents[0].payload.handoff?.checkpointRef).toContain("/tree");
    expect(createEvents[0].payload.handoff?.baseCheckpointRef).toContain("/base");
    expect(createEvents[1]?.type).toBe("thread.message-sent");
    if (createEvents[1]?.type === "thread.message-sent") {
      expect(createEvents[1].payload.provenance?.sourceKind).toBe("imported-conversation");
    }

    let afterHandoff = withSourceMessage;
    for (const [index, event] of createEvents.entries()) {
      afterHandoff = await Effect.runPromise(
        projectEvent(afterHandoff, {
          ...event,
          sequence: 4 + index,
          eventId: asEventId(`evt-fremont-create-${index}`),
          occurredAt: now,
          commandId: CommandId.makeUnsafe("cmd-fremont-handoff"),
          causationEventId: null,
          correlationId: CommandId.makeUnsafe("cmd-fremont-handoff"),
          metadata: {},
        }),
      );
    }

    const returnResult = await Effect.runPromise(
      decideOrchestrationCommand({
        command: {
          type: "thread.handoff.return",
          commandId: CommandId.makeUnsafe("cmd-fremont-return"),
          sourceThreadId: ThreadId.makeUnsafe("thread-fremont-source"),
          fromThreadId: ThreadId.makeUnsafe("thread-fremont-codex"),
          summary: "Codex finished tests; do not redo file edits.",
          completedStepIds: ["tests"],
          repoSnapshot: {
            headSha: "def",
            branch: "feature/fremont",
            worktreePath: null,
        ...
        readModel: afterHandoff,
      }),
    );

    const returnEvent = Array.isArray(returnResult) ? returnResult[0] : returnResult;
    expect(returnEvent?.type).toBe("thread.meta-updated");
    if (returnEvent?.type === "thread.meta-updated") {
      expect(returnEvent.payload.handoffReturn?.summary).toContain("Codex finished tests");
      expect(returnEvent.payload.handoffReturn?.fromProvider).toBe("codex");
      expect(returnEvent.payload.handoffReturn?.completedStepIds).toEqual(["tests"]);
    }
  });
});
