...
// Purpose: Build a ZIP archive that exports a single Modesto thread so a user
...
// The export-eligibility guard lives in @modesto/shared/threadExport so the
// web composer and the HTTP route share one predicate.

import zlib from "node:zlib";

import type { OrchestrationMessage, OrchestrationThread } from "@modesto/contracts";
...
export function threadArchiveFileName(input: {
  ...
  return `modesto-thread-${slugifyTitle(input.title)}-${dateBucket}.zip`;
}
