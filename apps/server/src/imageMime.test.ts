...
describe("inferAttachmentExtension", () => {
  it("keeps raster image extensions", () => {
    expect(inferAttachmentExtension({ mimeType: "image/png", fileName: "shot.png" })).toBe(".png");
  });

  it("maps document mime types and filenames", () => {
    expect(inferAttachmentExtension({ mimeType: "application/pdf", fileName: "brief.pdf" })).toBe(
      ".pdf",
    );
    expect(
