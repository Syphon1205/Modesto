/**
 * IosSimulatorBackend - CoreSimulator DeviceBackend.
 *
 * Split by capability:
 *
...
} from "@modesto/shared/deviceHelperCache";
...
import { type DeviceTypeCatalogue } from "./deviceTypeCatalogue.ts";
import {
 ...
 * that fail. Watch, TV, and Vision runtimes are dropped too — this pane only
 * frames phones and tablets.
 */
export function isPhoneOrTabletSimulator(device: {
  readonly name: string;
  readonly family?: DeviceDescriptor["family"];
}): boolean {
  if (device.family === "phone" || device.family === "tablet") return true;
  if (device.family) return false;
  const name = device.name.toLowerCase();
  if (/(watch|apple tv|\btv\b|reality|vision)/u.test(name)) return false;
  return /iphone|ipad|ipod/u.test(name);
}

/** `com.apple.CoreSimulator.SimDeviceType.iPhone-17-Pro` -> phone, without reading a plist. */
export function familyFromDeviceTypeIdentifier(
  identifier: string | null,
): DeviceDescriptor["family"] | undefined {
  if (!identifier) return undefined;
  if (/iPhone|iPod/iu.test(identifier)) return "phone";
  if (/iPad/iu.test(identifier)) return "tablet";
  return undefined;
}
  ...
  for (const [runtimeIdentifier, rawList] of Object.entries(
    ...
    for (const raw of rawList as readonly SimctlDevice[]) {
      ...
      const typeIdentifier =
        typeof raw.deviceTypeIdentifier === "string" ? raw.deviceTypeIdentifier : null;
      const deviceType = typeIdentifier ? catalogue.get(typeIdentifier) : undefined;
      const family = deviceType?.family ?? familyFromDeviceTypeIdentifier(typeIdentifier);
      if (!isPhoneOrTabletSimulator({ name, family })) continue;
      devices.push({
        ...
        ...(family ? { family } : {}),
        ...(deviceType ? { geometry: deviceType.geometry } : {}),
      });
    }
  }
  return devices;
...
/**
 * `xcodebuild -downloadPlatform iOS` is a no-op on Xcode 26 when the iOS SDK
 * already lives inside Xcode.app: it prints a UUID and exits without fetching a
 * simulator runtime. The architecture variant is the flag Apple's own usage
 * line documents; Settings → Components is the GUI that actually installs one.
 */
export function iosSimulatorRuntimeInstallCommand(arch: string = process.arch): string {
  const variant = arch === "arm64" ? "arm64" : "universal";
  return `xcodebuild -downloadPlatform iOS -architectureVariant ${variant}`;
}

/** True when CoreSimulator already has an iOS simulator runtime disk image. */
export function hasAvailableIosSimulatorRuntime(json: string): boolean {
  let parsed: unknown;
  try {
    parsed = JSON.parse(json);
  } catch {
  ...
  const runtimes = (parsed as { runtimes?: unknown }).runtimes;
  if (!Array.isArray(runtimes)) return false;
  return runtimes.some((raw) => {
    ...
    const identifier = typeof runtime.identifier === "string" ? runtime.identifier : "";
    const name = typeof runtime.name === "string" ? runtime.name : "";
    return identifier.includes(".iOS-") || /^iOS\s/u.test(name);
  });
}
...
export function parseXcodebuildVersion(stdout: string): string | null {
  const match = /^Xcode\s+(\S+)/mu.exec(stdout);
  return match?.[1] ?? null;
}
...
export class IosSimulatorBackend implements DeviceBackend {
  ...
  /** Availability does not change unless the helper or toolchain does. */
  ...
  private async probeAvailability(): Promise<DeviceAvailability> {
    ...
    const selectedDir = await this.xcodeSelectRaw();
    const selectedIsXcode = selectedDir !== null && !selectedDir.includes("CommandLineTools");
    const discoveredDir = selectedIsXcode ? null : await this.discoverXcodeDeveloperDir();
    const developerDir = selectedIsXcode ? selectedDir : discoveredDir;
    const xcodeInstalled = developerDir !== null;
    const xcodeVersion = xcodeInstalled ? await this.readXcodeVersion(developerDir) : null;
    steps.push({
      id: "install-xcode",
    ...
    const selectTarget =
      developerDir ?? path.join("/Applications", "Xcode.app", "Contents", "Developer");
    // A discovered Xcode.app is enough to pin DEVELOPER_DIR for simctl. The
    // machine-wide `xcode-select` can stay on the CLT; booting must not wait
    // on `sudo xcode-select` or on opening Xcode.app.
    steps.push({
      id: "select-xcode-command-line-tools",
      label: "Select Xcode",
      done: xcodeInstalled,
      detail: xcodeInstalled ? undefined : `sudo xcode-select -s ${selectTarget}`,
    });
    ...
    // simctl list is public and does not need the license or a project. Gating
    // it on `xcodebuild -version` hid every iPhone whenever the CLT still owned
    // `xcode-select` or the license card was unfinished.
    const devices = await this.listDevicesUnchecked();
    let runtimeInstalled = hasBootableIosRuntime(devices);
    if (!runtimeInstalled) {
      runtimeInstalled = await this.hasIosRuntimeInstalled();
    }
    steps.push({
      id: "install-ios-runtime",
      label: "Simulator installed",
      done: runtimeInstalled,
      detail: runtimeInstalled ? undefined : iosSimulatorRuntimeInstallCommand(),
    });
    ...
    steps.push({
      ...
      label: "Build the Modesto device helper",
      done: helperBuilt,
  ...
  warmupAttach(_udid: string): void {
    if (this.osPlatform !== "darwin") return;
    // Fire-and-forget: the compile is shared, and boot must not wait on it.
    void this.compileHelperIfNeeded().catch(() => undefined);
  }

  async boot(udid: string): Promise<DeviceDescriptor> {
    // Helper compile overlaps boot and must not gate it: `simctl boot` talks to
    // CoreSimulator directly. Never open Xcode.app, never `xcodebuild` a user
    // `.xcodeproj` / `.xcworkspace`, and never wait for the Swift helper.
    this.warmupAttach(udid);
    ...
    const alreadyBooted = /current state: Booted/iu.test(result.stderr);
    if (result.code !== 0 && !alreadyBooted) {
      throw this.simctlError("boot", result);
    }
    // Do not wait on `bootstatus` here. That call can sit for a minute while
    // SpringBoard comes up, which froze the pane on "Starting…" before attach
    // could even start. `DeviceManager.attach` already retries until the
    // display exists ("is not booted" / no framebuffer) and reports phases.
    const devices = await this.listDevicesUnchecked();
    ...
    return {
      ...device,
      state: alreadyBooted || device.state === "booted" ? "booted" : "booting",
    };
  }
  ...
  async screenshot(
    ...
    const directory = await mkdtemp(path.join(tmpdir(), "modesto-device-"));
   ...
   * Modesto, SimulatorKit dropped the connection) the injection silently
  ...
  private async simctl(
    ...
    const env = await this.toolchainEnv();
    const developerDir = env?.DEVELOPER_DIR ?? null;
    const simctlBin = this.toolchainBinary("simctl", developerDir);
    // simctl lives inside Xcode.app. Invoke it directly so a CLT `xcrun` on
    // PATH cannot intercept the boot. `/usr/bin/xcrun simctl` is the fallback
    // when no full Xcode was discovered.
    if (simctlBin !== "simctl") {
      return await this.run(simctlBin, args, {
    ...
    return await this.run("/usr/bin/xcrun", ["simctl", ...args], {
      timeoutMs: options.timeoutMs ?? SIMCTL_TIMEOUT_MS,
      allowNonZeroExit: true,
      outputMode: "truncate",
      env,
  ...
  private async listDevicesUnchecked(): Promise<readonly DeviceDescriptor[]> {
    if (this.osPlatform !== "darwin") return [];
    // Family comes from the type identifier. Do not kick off the ~120 `plutil`
    // profile reads here: they used to saturate the machine during `simctl boot`.
  ...
  private async hasIosRuntimeInstalled(): Promise<boolean> {
    const result = await this.simctl(["list", "runtimes", "--json"]).catch(() => null);
    if (!result || result.code !== 0) return false;
    return hasAvailableIosSimulatorRuntime(result.stdout);
  }
  ...
  /**
   * What `xcode-select -p` actually points at, without falling back to a
   * discovered Xcode.app. The CLT is the macOS default; boot still pins
   * `DEVELOPER_DIR` to Xcode.app when that bundle is installed.
   */
  private async xcodeSelectRaw(): Promise<string | null> {
  ...
  private async xcodeSelectPath(): Promise<string | null> {
    const selected = await this.xcodeSelectRaw();
  ...
  /**
   * `simctl` / `xcodebuild` from the Xcode bundle, not whatever the CLT put on
   * PATH. `xcrun` itself lives at `/usr/bin/xcrun` and is not inside the
   * Developer dir; `simctl` and `xcodebuild` are.
   */
  private toolchainBinary(name: "simctl" | "xcodebuild", developerDir: string | null): string {
    if (developerDir && !developerDir.includes("CommandLineTools")) {
      return path.join(developerDir, "usr", "bin", name);
    }
    return name;
  }

  /**
   * Marketing/build version only. Always `-version`, always the helper's
   * directory as cwd, so a user's repo cannot be picked up as an implied
   * `.xcodeproj` / `.xcworkspace`.
   */
  private async xcodebuildVersion(developerDir?: string | null): Promise<ProcessRunResult | null> {
    const resolved = developerDir === undefined ? await this.xcodeSelectPath() : developerDir;
    return await this.run(this.toolchainBinary("xcodebuild", resolved), ["-version"], {
      timeoutMs: 20_000,
      allowNonZeroExit: true,
      cwd: this.helperSourceDir,
      env: resolved === null ? this.processEnv : { ...this.processEnv, DEVELOPER_DIR: resolved },
    }).catch(() => null);
  }

  private async readXcodeVersion(developerDir: string): Promise<string | null> {
    const result = await this.xcodebuildVersion(developerDir);
    if (!result || result.code !== 0) return null;
    return parseXcodebuildVersion(result.stdout);
  }
  ...
  private async xcodeLicenseAccepted(): Promise<boolean> {
    const result = await this.xcodebuildVersion();
  ...
  private async attachedHelper(
    ...
    try {
      ...
      // A device can also be rebooted outside Modesto (Simulator.app, or simctl
  ...
  private async compileHelper(): Promise<string> {
    ...
    const result = await this.run("/bin/sh", [buildScript, outputDirectory], {
      ...
      allowNonZeroExit: true,
      outputMode: "truncate",
      // Modesto's Swift helper, never the user's app. `build.sh` compiles the
      // sources next to this script with `xcrun swiftc`; cwd is the helper
      // package so a stray `xcodebuild` cannot pick up a repo `.xcodeproj`.
      cwd: this.helperSourceDir,
  ...
  private async cachedHelperPath(): Promise<string | null> {
    ...
    const key = await this.tryXcodeBuildKey();
  ...
  private async tryXcodeBuildKey(): Promise<string | null> {
    const result = await this.xcodebuildVersion();
