 ...
 *   already the `@modesto/contracts` device-frame envelope, so this module only
...
import { decodeDeviceFrame } from "@modesto/shared/deviceFrame";
...
export class HelperClient {
  ...
  async startStream(udid: string, onFrame: (frame: DeviceStreamFrame) => void): Promise<void> {
    ...
    const directory = await mkdtemp(path.join(tmpdir(), "modesto-device-frames-"));
