 ...
 * - The Modesto boot cap (`DEVICE_MODESTO_BOOT_LIMIT`). Boot past the cap is
...
/** How long a Modesto-booted device stays up with no thread attached. */
...
/**
 * Prefer an already-booted simulator of the same model over launching a second
 * copy. Name + platform is the marketing model the picker groups by.
 */
export function matchingBootedDevice(
  devices: readonly DeviceDescriptor[],
  requestedUdid: string,
): DeviceDescriptor | null {
  const known = devices.find((device) => device.udid === requestedUdid);
  if (known?.state === "booted") return known;
  const name = known?.name;
  const platform = known?.platform;
  if (!name || !platform) {
    const booted = devices.find(
      (device) => device.udid === requestedUdid && device.state === "booted",
    );
    return booted ?? null;
  }
  return (
    devices.find(
      (device) => device.state === "booted" && device.name === name && device.platform === platform,
    ) ?? null
  );
}
...
export interface DeviceManagerOptions {
  ...
  /** Remembers Modesto's boots across a crash; defaults to remembering nothing. */
...
export class DeviceManager {
  ...
  private readonly modestoBooted = new Set<string>();
  ...
  constructor(options: DeviceManagerOptions) {
    ...
    this.bootLimit = options.bootLimit ?? DEVICE_MODESTO_BOOT_LIMIT;
  ...
  private async recordBootOwnership(): Promise<void> {
    await this.bootOwnership.write([...this.modestoBooted]).catch(() => undefined);
  }
  ...
  private async discover(
    ...
    // The wire schema caps the array; one extra simulator must not fail the
    // whole getThreadState RPC and leave the pane on fallback setup cards.
    return devices.slice(0, 256).map((device) => this.describe(device));
  }
  ...
  async modestoBootedDevices(): Promise<readonly DeviceDescriptor[]> {
    const devices = await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
    this.reconcileModestoBooted(devices);
    return devices
      .filter((device) => this.modestoBooted.has(device.udid))
   ...
   *
   * The set is Modesto's own bookkeeping, but the simulators are not Modesto's to
   ...
   */
  private reconcileModestoBooted(devices: readonly DeviceDescriptor[]): void {
    ...
    for (const udid of this.modestoBooted) {
      // Boot lists one platform at a time. A missing UDID is the other
      // platform, not a phantom — dropping it here would leak Android slots
      // the next time someone starts an iPhone.
      if (!known.has(udid)) continue;
      if (running.has(udid)) continue;
      this.modestoBooted.delete(udid);
  ...
  async boot(udid: string): Promise<DeviceBootResult> {
    // Helper compile overlaps CoreSimulator boot; it must not run first.
    this.backend.warmupAttach?.(udid);
    const devices = await this.devicesForBoot(udid);
    // Devices that stopped without Modesto doing it still held their slots, so
    // three shutdowns from a shell were enough to make every later boot refuse.
    this.reconcileModestoBooted(devices);
    const already = matchingBootedDevice(devices, udid);
    // Viewing an already-booted device is uncapped: the cap exists to stop
    // Modesto from accumulating simulators, not to limit what the user watches.
    // A shutdown twin of a running model attaches to the running one.
    if (already) {
      return { kind: "booted", device: this.describe(already) };
    }
    if (this.modestoBooted.size >= this.bootLimit) {
      return {
        kind: "boot-limit-reached",
        limit: this.bootLimit,
        modestoBooted: await this.modestoBootedDevices(),
      };
    }
    ...
    // exists to stop Modesto accumulating multi-gigabyte simulators would be
    // exceeded by however many requests arrived inside that window.
    this.modestoBooted.add(udid);
    ...
    this.clearIdleTimer(udid);
    // Other panes can catch up in the background; the boot RPC must not wait
    // on Android availability probes before the client can attach.
    void this.publishAllThreads();
    return { kind: "booted", device: { ...device, bootSource: "modesto" } };
  }
  ...
  /**
   * Inject the gravity vector the guest's accelerometer should report.
   *
   * Backends that have no sensor surface — the iOS simulator, which exposes no
   * CoreMotion at all — do not implement this, and the call is refused rather
   * than silently accepted: a free-look pane that reports success while the
   * guest never moves is worse than one that says the runtime cannot do it.
   */
  async setGravity(udid: string, gravity: DeviceGravity): Promise<void> {
    const setter = this.backend.setGravity;
    if (!setter) {
      throw new DeviceBackendError("This device runtime cannot simulate motion.");
    }
    await setter.call(this.backend, udid, gravity);
  }
  ...
  /**
   * App quit: shut down everything Modesto booted, leave the user's devices
  ...
  async dispose(): Promise<void> {
    ...
    const booted = Array.from(this.modestoBooted);
  ...
  private describe(device: DeviceDescriptor): DeviceDescriptor {
    const bootSource = this.modestoBooted.has(device.udid) ? "modesto" : device.bootSource;
   ...
   * the device down or start the idle countdown if Modesto booted it.
   *
   * A switch shuts down immediately rather than waiting out the idle window.
   * The cap is three Modesto-booted devices, and each one costs a couple of GB
  ...
  private async releaseDevice(
    ...
    if (!this.modestoBooted.has(udid)) return;
    if (reason === "switched") {
      this.clearIdleTimer(udid);
  ...
  private async shutdownIfStillIdle(udid: string): Promise<void> {
    ...
    if (this.isAttachedAnywhere(udid) || !this.modestoBooted.has(udid)) return;
    await this.shutdown(udid).catch(() => undefined);
  }
  ...
  private async devicesForBoot(udid: string): Promise<readonly DeviceDescriptor[]> {
    if (this.backend.listRelatedDevices) {
      return await this.backend.listRelatedDevices(udid, { includeShutdown: true }).catch(() => []);
    }
    return await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
  }

  private async snapshot(threadId: string): Promise<ThreadDeviceState> {
    const attachment = this.threadState(threadId);
    // Listing and availability are independent; waiting on Android SDK probes
    // before `simctl list` used to hold the pane on fallback setup cards.
    const [availability, devices] = await Promise.all([
      this.backend.availability(),
      this.backend.listDevices({ includeShutdown: true }).catch(() => [] as const),
    ]);
    const visible =
      availability.kind === "unsupported-platform"
        ? []
        : devices.slice(0, 256).map((device) => this.describe(device));
    return {
