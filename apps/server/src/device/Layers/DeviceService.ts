 ...
 * iOS needs darwin + Xcode; Android needs the official AOSP emulator/SDK.
 * `supported` stays true so the pane can render the setup checklist instead
 * of hiding the surface.
 *
 ...
 * directory so both land in the same place under a custom MODESTO_HOME.
 *
...
function defaultBootOwnershipPath(): string {
  const baseDir = process.env.MODESTO_HOME?.trim() || path.join(homedir(), ".modesto");
        ...
        if (reclaimed.length > 0) {
          console.info(
            `[device] shut down ${reclaimed.length} simulator(s) left booted by a previous ` +
              `Modesto run: ${reclaimed.join(", ")}`,
          );
        }
      });

      // App quit shuts down every simulator Modesto booted and leaves the
