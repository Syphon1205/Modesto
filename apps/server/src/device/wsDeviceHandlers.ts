 ...
 * platform-gated: this module answers every method whether or not a
 * backend exists, so the RPC group stays exhaustively handled everywhere and
 * machines without Xcode or an Android SDK get one clear refusal instead of a
 * transport error.
 *
...
const UNSUPPORTED_MESSAGE =
  "The device pane needs a local iOS Simulator (macOS + Xcode) or Android Emulator (Android SDK).";

function unsupported<A>(): Effect.Effect<A, DeviceError> {
  return Effect.fail(new DeviceError({ message: UNSUPPORTED_MESSAGE }));
}

function attempt<A>(
  promise: () => Promise<A>,
  fallbackMessage: string,
): Effect.Effect<A, DeviceError> {
...
export interface WsDeviceHandlers {
  ...
  ) => Effect.Effect<DeviceListResult, DeviceError>;
  readonly [DEVICE_WS_METHODS.boot]: (
    input: DeviceBootInput,
  ) => Effect.Effect<DeviceBootResult, DeviceError>;
  ...
  ) => Effect.Effect<ThreadDeviceState, DeviceError>;
  ...
  readonly [DEVICE_WS_METHODS.tap]: (input: DeviceTapInput) => Effect.Effect<void, DeviceError>;
  readonly [DEVICE_WS_METHODS.swipe]: (input: DeviceSwipeInput) => Effect.Effect<void, DeviceError>;
  ...
  readonly [DEVICE_WS_METHODS.setGravity]: (
    input: DeviceSetGravityInput,
  ) => Effect.Effect<void, DeviceError>;
  readonly [DEVICE_WS_METHODS.installApp]: (
    input: DeviceInstallAppInput,
  ) => Effect.Effect<DeviceInstallAppResult, DeviceError>;
  readonly [DEVICE_WS_METHODS.launchApp]: (
    input: DeviceLaunchAppInput,
  ) => Effect.Effect<DeviceLaunchAppResult, DeviceError>;
...
export function makeWsDeviceHandlers(
  ...
  if (!deviceService?.supported) {
    return {
      ...
      [DEVICE_WS_METHODS.setGravity]: () => unsupported(),
  ...
  const threadState = (state: ThreadDeviceState): ThreadDeviceState => ({
    ...state,
    // Schema brands expose `.make`, not `.makeUnsafe`. Using the missing
    // constructor threw after snapshot() and left the pane on fallback cards.
    threadId: ThreadId.make(state.threadId),
  });
  ...
  return {
    ...
    [DEVICE_WS_METHODS.setGravity]: (input) =>
      attempt(
        () => manager.setGravity(input.udid, { x: input.x, y: input.y, z: input.z }),
        "Failed to set device orientation",
      ),
