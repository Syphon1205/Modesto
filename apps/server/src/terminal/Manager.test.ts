...
> =>
  Effect.flatMap(Effect.service(FileSystem.FileSystem), (fs) =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "modesto-terminal-" });
...
it.layer(
  ...
  it.effect("attaches to exited sessions without restarting them", () =>
    Effect.gen(function* () {
      ...
      const unsubscribe = yield* manager.attachStream(
        openInput({
          env: {
            MODESTO_WORKTREE_PATH: "/tmp/should-not-restart",
          },
      ...
      const unsubscribe = yield* manager.attachStream(
        {
          ...openInput({
            env: {
              MODESTO_WORKTREE_PATH: "/tmp/restart-requested",
            },
  ...
  it.effect("publishes closed events when terminals are explicitly closed", () =>
    Effect.gen(function* () {
      ...
      expect(closedEvents.map((event) => event.terminalId).toSorted()).toEqual([
        "default",
        "sidecar",
      ]);
    }),
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      expect(spawnInput.env.MODESTO_PORT).toBeUndefined();
  ...
  it.effect("strips AppImage runtime env from terminal sessions", () =>
    Effect.gen(function* () {
      const appDir = "/tmp/.mount_Modestoabc123";
      const { manager, ptyAdapter } = yield* createManager(5, {
        env: {
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            MODESTO_PROJECT_ROOT: "/repo",
            MODESTO_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
          },
        }),
      );
      ...
      assert.equal(spawnInput.env.MODESTO_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.MODESTO_WORKTREE_PATH, "/repo/worktree-a");
