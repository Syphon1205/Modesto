...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "modesto-terminal-"));
  ...
  it("keeps a running terminal alive when open reattaches with different cwd or env", async () => {
    ...
    const snapshot = await manager.open(
      openInput({ cwd: logsDir, env: { MODESTO_TERMINAL_TEST: "changed" } }),
    );
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    expect(spawnInput.env.MODESTO_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.MODESTO_WORKTREE_PATH).toBe("/repo/worktree-a");
