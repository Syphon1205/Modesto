...
import { modestoReviewCommand } from "./review/modestoReviewCli";
...
interface CliInput {
  ...
  readonly modestoHome: Option.Option<string>;
...
const CliEnvConfig = Config.all({
  mode: Config.string("MODESTO_MODE").pipe(
    Config.option,
    Config.map(
  ...
  modestoHome: Config.string("MODESTO_HOME").pipe(Config.option, Config.map(Option.getOrUndefined)),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: Config.boolean("MODESTO_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  authToken: Config.string("MODESTO_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  ...
  logProviderEvents: Config.boolean("MODESTO_LOG_PROVIDER_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
...
const ServerConfigLive = (input: CliInput) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const configuredHome = Option.getOrUndefined(input.modestoHome) ?? env.modestoHome;
...
const makeServerProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("Modesto running", {
      ...safeConfig,
...
const modestoHomeFlag = Flag.string("home-dir").pipe(
  Flag.withDescription("Base directory for all Modesto data (equivalent to MODESTO_HOME)."),
  Flag.optional,
);
...
const logProviderEventsFlag = Flag.boolean("log-provider-events").pipe(
  Flag.withDescription(
    "Emit native/canonical provider NDJSON logs for debugging (equivalent to MODESTO_LOG_PROVIDER_EVENTS).",
  ),
  Flag.optional,
);
...
const modestoServerCommand = Command.make("modesto", {
  mode: modeFlag,
  port: portFlag,
  host: hostFlag,
  modestoHome: modestoHomeFlag,
  devUrl: devUrlFlag,
  ...
  Command.withDescription("Run Modesto Server or invoke a repository command."),
  Command.withHandler((input) => Effect.scoped(makeServerProgram(input))),
);

export const modestoCli = modestoServerCommand.pipe(
  Command.withSubcommands([modestoReviewCommand]),
);
