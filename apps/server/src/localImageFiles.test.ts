...
describe("resolveAllowedLocalPreviewFile", () => {
  it("allows images inside the current workspace", async () => {
    const workspace = makeTempDir("modesto-image-workspace-");
  ...
  it("allows images inside Codex generated_images without a cwd", async () => {
    const codexHome = makeTempDir("modesto-codex-home-");
  ...
  it("allows images written to the MODESTO_HOME codex-home-overlay generated_images root", async () => {
    // Codex app-server is launched with CODEX_HOME pointing at a Modesto overlay
    // directory (see resolveModestoCodexHomeOverlayPath). Generated images therefore
    // live under <MODESTO_HOME>/codex-home-overlay/generated_images/<thread>/<call>.png,
    ...
    const modestoHome = path.join(fakeRoot, "modesto", "runtime");
    ...
    process.env.MODESTO_HOME = modestoHome;
    try {
      const result = await resolveAllowedLocalPreviewFile({
        requestedPath: imagePath,
        cwd: null,
      ...
      if (previousModestoHome === undefined) {
        delete process.env.MODESTO_HOME;
      } else {
        process.env.MODESTO_HOME = previousModestoHome;
      }
  ...
  it("allows PDFs inside the current workspace", async () => {
    const workspace = makeTempDir("modesto-pdf-workspace-");
    writeFileSync(path.join(workspace, ".git"), "gitdir: .git");
  ...
  it("allows PDFs inside a per-thread scratch workspace without a cwd", async () => {
    ...
    // <tmpdir>/modesto-codex-workspaces/<threadId>; files agents create there
    // are workspace-equivalent, so documents must be servable from that root.
    const scratchRoot = path.join(os.tmpdir(), "modesto-codex-workspaces");
  ...
  it("rejects PDFs outside the workspace even under the temp-dir image roots", async () => {
    ...
    const tempDir = makeTempDir("modesto-pdf-outside-");
  ...
  it("still allows images under the temp-dir roots without a workspace", async () => {
    const tempDir = makeTempDir("modesto-image-tmp-root-");
