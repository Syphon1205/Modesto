...
import { sanitizeBranchFragment } from "@modesto/shared/git";
import {
  detectSourceControlProviderFromRemoteUrl,
  isSshRemoteUrl,
} from "@modesto/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("MODESTO_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("MODESTO_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("MODESTO_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("MODESTO_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
function checkoutBranchName(input: {
  ...
  return `modesto/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set MODESTO_BITBUCKET_EMAIL and MODESTO_BITBUCKET_API_TOKEN, or MODESTO_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
