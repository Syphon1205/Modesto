...
export interface GitHubListedRepository {
  readonly nameWithOwner: string;
  readonly url: string;
  readonly sshUrl: string;
...
export class GitHubCli extends Context.Service<
  GitHubCli,
  {
    ...
    readonly listRepositories: (input: {
      readonly cwd: string;
      readonly limit?: number;
    }) => Effect.Effect<ReadonlyArray<GitHubListedRepository>, GitHubCliError>;
...
const LIST_REPOSITORIES_DEFAULT_LIMIT = 50;
const LIST_REPOSITORIES_MAX_LIMIT = 100;

const RawGitHubListedRepositorySchema = Schema.Struct({
  nameWithOwner: TrimmedNonEmptyString,
  url: TrimmedNonEmptyString,
  sshUrl: TrimmedNonEmptyString,
...
const decodeRawGitHubListedRepositories = Schema.decodeEffect(
  Schema.fromJsonString(Schema.Array(RawGitHubListedRepositorySchema)),
);

function clampRepositoryListLimit(limit: number | undefined): number {
  if (limit === undefined) return LIST_REPOSITORIES_DEFAULT_LIMIT;
  return Math.min(Math.max(1, Math.trunc(limit)), LIST_REPOSITORIES_MAX_LIMIT);
}
    ...
    listRepositories: (input) =>
      execute({
        ...
        args: [
          ...
          "list",
          "--limit",
          String(clampRepositoryListLimit(input.limit)),
          "--json",
        ...
        Effect.flatMap((raw) =>
          raw.length === 0
            ? Effect.succeed([])
            : decodeRawGitHubListedRepositories(raw).pipe(
                Effect.mapError(
                  (cause) =>
