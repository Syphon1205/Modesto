...
import { VcsProcessExitError, VcsProcessSpawnError } from "@modesto/contracts";
...
describe("GitHubCli.layer", () => {
  ...
  it.effect("keeps pull requests from gh versions without headRepository.nameWithOwner", () =>
    ...
    Effect.gen(function* () {
            ...
            JSON.stringify([
              {
                ...
                baseRefName: "main",
                headRefName: "modesto/codex-turn-mapping",
                state: "OPEN",
                mergedAt: null,
      ...
      const result = yield* gh.listOpenPullRequests({
        cwd: "/repo",
        headSelector: "modesto/codex-turn-mapping",
      });
  ...
  it.effect("lists repositories for the authenticated GitHub account", () =>
    Effect.gen(function* () {
      mockRun.mockReturnValueOnce(
        Effect.succeed(
          processOutput(
