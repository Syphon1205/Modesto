...
import { GitCommandError, SourceControlProviderError } from "@modesto/contracts";
...
const CLONE_URLS = {
  nameWithOwner: "octocat/modesto",
  url: "https://github.com/octocat/modesto",
  sshUrl: "git@github.com:octocat/modesto.git",
};
...
it.effect("lists GitHub repositories through gh", () =>
  Effect.gen(function* () {
    const service = yield* SourceControlRepositoryService.SourceControlRepositoryService;
    const result = yield* service.listRepositories({ provider: "github", limit: 10 });
...
it.effect("refuses repository listing for non-GitHub providers", () =>
  Effect.gen(function* () {
    const service = yield* SourceControlRepositoryService.SourceControlRepositoryService;
    const error = yield* Effect.flip(service.listRepositories({ provider: "gitlab" }));

    assert.strictEqual(error.provider, "gitlab");
    assert.strictEqual(error.operation, "listRepositories");
    assert.include(error.detail, "GitHub");
...
it.effect("looks up repositories through the requested provider without search", () => {
  ...
  return Effect.gen(function* () {
    ...
    const result = yield* service.lookupRepository({
      provider: "github",
      repository: "octocat/modesto",
      cwd: "/workspace",
    });

    assert.deepStrictEqual(result, { provider: "github", ...CLONE_URLS });
    assert.deepStrictEqual(calls, [{ cwd: "/workspace", repository: "octocat/modesto" }]);
...
it.effect("clones a looked-up repository into the requested destination", () =>
  Effect.gen(function* () {
    ...
    const destinationPath = `${parent}/modesto`;
    ...
    yield* Effect.gen(function* () {
      ...
      assert.deepStrictEqual(cloneCalls, [
        {
          cwd: parent,
          args: ["clone", CLONE_URLS.url, "modesto"],
        },
      ]);
    }).pipe(
      Effect.provide(
...
it.effect("preserves destination probe failures instead of treating them as missing paths", () => {
  const fileSystemCause = PlatformError.systemError({
    ...
    module: "FileSystem",
    method: "exists",
    pathOrDescriptor: "/restricted/modesto",
  });
  ...
  return Effect.gen(function* () {
    ...
    const error = yield* Effect.flip(
      service.cloneRepository({
        ...
        destinationPath: "/restricted/modesto",
      }),
    );
...
it.effect("publishes by creating the repository, adding a remote, and pushing upstream", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(createCalls, [
      { cwd: "/workspace", repository: "octocat/modesto", visibility: "private" },
    ]);
