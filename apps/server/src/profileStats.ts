...
// Purpose: Compute Profile-page stats from Modesto's local projection DB only.
...
import { isBuiltInComposerSlashCommandName } from "@modesto/shared/composerSlashCommands";
...
// Builds profile skill rows from every stored Modesto user message, plus the
...
>()("modesto/profileStats/ProfileStatsQuery") {}
...
const makeProfileStatsQuery = Effect.gen(function* () {
  ...
  // Activity = days/hours the user actually sent a Modesto prompt. One day-hour
  ...
  // Token usage for EVERY provider, straight from Modesto's own DB (no external
  ...
  const getProfileStats = (
    ...
    Effect.gen(function* () {
      ...
      // ── Identity ──
      const homeDirBasename = nodePath.basename(config.homeDir) || "modesto";
