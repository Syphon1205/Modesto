import { EnvironmentHttpApi } from "@modesto/contracts";
...
import { listeningArtworkRouteLayer } from "./listening/listeningArtworkRoute.ts";
...
import { AutomationSchedulerLive } from "./automation/AutomationScheduler.ts";
import { AutomationServiceLive } from "./automation/AutomationService.ts";
import { WebhookServiceLive } from "./automation/WebhookService.ts";
import { webhookIncomingRouteLayer } from "./automation/webhookRoute.ts";
...
import { makeDeviceFrameRouteLayer } from "./device/deviceFrameRoute.ts";
...
import { disableTailscaleServe, ensureTailscaleServe } from "@modesto/tailscale";
...
const DeviceLayerLive = makeDeviceServiceLayer();
...
const deviceFrameRouteLayer = makeDeviceFrameRouteLayer({
  authorizeUpgrade: (request) =>
    Effect.flatMap(EnvironmentAuth.EnvironmentAuth, (auth) =>
      auth.authenticateWebSocketUpgrade(request),
    ).pipe(
      Effect.as(true),
...
const ProviderRuntimeLayerLive = ProviderSessionReaperLive.pipe(
  Layer.provideMerge(AutomationServiceLive),
  Layer.provideMerge(AutomationSchedulerLive.pipe(Layer.provideMerge(WebhookServiceLive))),
...
const RuntimeCoreDependenciesLive = ReactorLayerLive.pipe(
  ...
  Layer.provideMerge(VercelService.layer.pipe(Layer.provide(ServerSecretStore.layer))),
...
const RuntimeDependenciesLive = RuntimeCoreDependenciesLive.pipe(
  ...
  Layer.provideMerge(FolderPicker.layer),
...
export const makeRoutesLayer = Layer.mergeAll(
  Layer.mergeAll(
    ...
    otlpTracesProxyRouteLayer,
    assetRouteLayer,
    listeningArtworkRouteLayer,
    webhookIncomingRouteLayer,
    staticAndDevRouteLayer,
