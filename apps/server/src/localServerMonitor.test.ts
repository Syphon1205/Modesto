...
describe("localServerMonitor", () => {
  ...
  it("keeps dev servers and ignores Electron/Modesto-style application listeners", () => {
    expect(
      isLikelyDevServerProcess({
        command: "node",
    ...
    expect(
      isIgnoredLocalServerProcess({
        command: "Electron",
        args: "/Applications/Modesto.app/Contents/MacOS/Modesto",
        ports: [61449],
      }),
    ).toBe(true);
    expect(
    ...
    expect(
      isLikelyDevServerProcess({
        command: "node",
        args: "node /Users/dev/modesto/apps/web/node_modules/.bin/vite",
        ports: [5733],
      }),
    ).toBe(true);
    expect(
  ...
  it("uses parent command lines when a dev-tool child owns the listening port", () => {
    const processInfo = new Map<number, LocalServerProcessInfo>([
      ...
      [
        ...
        {
          ppid: 12064,
          commandLine: "node /Users/dev/modesto-website/node_modules/.bin/next dev",
        },
      ],
      [
        12064,
