import { resolveServerBackgroundActivitySettings } from "@modesto/shared/backgroundActivitySettings";
...
import {
  ...
  type ServerSetCustomModelEndpointInput,
  type ServerDeleteCustomModelEndpointInput,
  type CustomModelEndpointStatus,
  ThreadId,
  ...
  WS_METHODS,
  LISTENING_WS_METHODS,
  ListeningLibraryError,
  WsRpcGroup,
} from "@modesto/contracts";
import { detectListeningLibrariesOnThisMachine } from "@modesto/shared/listeningLibrary";
import {
...
import * as WebhookService from "./automation/WebhookService.ts";
import * as ServerConfig from "./config.ts";
import * as Keybindings from "./keybindings.ts";
import * as ExternalLauncher from "./process/externalLauncher.ts";
import * as FolderPicker from "./process/folderPicker.ts";
import {
...
} from "./provider/claudePluginBundle.ts";
import {
  installClaudePlugin,
  listInstalledClaudePlugins,
  uninstallClaudePlugin,
  type InstalledClaudePlugin,
} from "./provider/claudePluginInstall.ts";
import {
  SkillPackBundleError,
  resolveSkillPackBundle,
  SKILL_PACK_PROVIDERS,
} from "./provider/skillPackBundle.ts";
import {
  installSkillPack,
  installedSkillPackSummary,
  listInstalledSkillPacks,
  previewSkillPackEligibility,
  defaultSkillPackEligibility,
  uninstallSkillPack,
} from "./provider/skillPackInstall.ts";
...
import * as DeviceService from "./device/Services/DeviceService.ts";
import { makeWsDeviceHandlers } from "./device/wsDeviceHandlers.ts";
...
import * as GitHubAccountAuth from "./git/githubAccountAuth.ts";
...
import * as VercelService from "./vercel/VercelService.ts";
...
import * as RelayClient from "@modesto/shared/relayClient";
...
/** claudePluginBundle.ts's stable code strings match ClaudePluginFailureCode exactly (see that file's module header) - any other thrown value is a genuine unexpected failure. */
function claudePluginRequestErrorFromCause(cause: unknown): ClaudePluginRequestError {
  if (cause instanceof ClaudePluginBundleError) {
    return new ClaudePluginRequestError({
      code: cause.code as ClaudePluginFailureCode,
  ...
  return new ClaudePluginRequestError({
    ...
    message: cause instanceof Error ? cause.message : "Failed to resolve the plugin.",
    cause,
  });
}

function installedClaudePluginSummary(plugin: InstalledClaudePlugin) {
  return {
...
function skillPackRequestErrorFromCause(cause: unknown): SkillPackRequestError {
  if (cause instanceof SkillPackBundleError) {
    return new SkillPackRequestError({ code: cause.code, message: cause.message, cause });
  }
  return new SkillPackRequestError({
    code: "source_fetch_failed",
    message: cause instanceof Error ? cause.message : "Failed to resolve the skill pack.",
    cause,
  });
}
...
const makeWsRpcLayer = (
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const folderPicker = yield* FolderPicker.FolderPicker;
      ...
      const localVoice = yield* LocalVoice.LocalVoice;
      ...
      const coalesceShellEvents = (
        ...
        Effect.gen(function* () {
          ...
          const survivors = Array.from(latestByAggregate.values()).toSorted(
          ...
          const bootstrapProgram = Effect.gen(function* () {
            if (bootstrap?.createThread) {
              yield* dispatchFromClient({
                ...
                conversationMode: bootstrap.createThread.conversationMode,
      ...
      const toCustomModelEndpointError = (cause: unknown) =>
        new ServerCustomModelEndpointError({
          reason: cause instanceof Error ? cause.message : String(cause),
        });

      // Enriches each persisted definition with a derived `apiKeyConfigured`
      // flag - the raw key never leaves ServerSecretStore.
      const listCustomModelEndpointStatuses: Effect.Effect<
        ReadonlyArray<CustomModelEndpointStatus>,
        ServerCustomModelEndpointError
      > = Effect.gen(function* () {
        const settings = yield* serverSettings.getSettings.pipe(
          Effect.mapError(toCustomModelEndpointError),
        );
        return yield* Effect.forEach(
          settings.customModelEndpoints,
          (endpoint) =>
            secretStore.get(customModelEndpointSecretName(endpoint.id)).pipe(
              Effect.map((bytes) => ({
                ...endpoint,
                apiKeyConfigured: Option.isSome(bytes) && bytes.value.length > 0,
              })),
              Effect.orElseSucceed(() => ({ ...endpoint, apiKeyConfigured: false })),
            ),
      ...
      const setCustomModelEndpoint = Effect.fn("setCustomModelEndpoint")(function* (
        ...
        const existingIds = new Set(settings.customModelEndpoints.map((entry) => entry.id));
        const isUpdate = input.id !== undefined && existingIds.has(input.id);
        const id =
          input.id ??
        ...
        const nextEndpoints = isUpdate
          ? settings.customModelEndpoints.map((entry) => (entry.id === id ? nextEndpoint : entry))
          : [...settings.customModelEndpoints, nextEndpoint];

        yield* serverSettings
          .updateSettings({ customModelEndpoints: nextEndpoints })
          .pipe(Effect.mapError(toCustomModelEndpointError));

        if (input.apiKey) {
          yield* secretStore
            .set(customModelEndpointSecretName(id), new TextEncoder().encode(input.apiKey))
            .pipe(Effect.mapError(toCustomModelEndpointError));
        }
      ...
      const deleteCustomModelEndpoint = Effect.fn("deleteCustomModelEndpoint")(function* (
        ...
        yield* serverSettings
          .updateSettings({
            customModelEndpoints: settings.customModelEndpoints.filter(
              (entry) => entry.id !== input.id,
            ),
          })
          .pipe(Effect.mapError(toCustomModelEndpointError));
        yield* secretStore
          .remove(customModelEndpointSecretName(input.id))
          .pipe(Effect.mapError(toCustomModelEndpointError));
      ...
      const loadServerConfig = Effect.gen(function* () {
        ...
        const customModelEndpoints = yield* listCustomModelEndpointStatuses.pipe(
          Effect.catchCause((cause) =>
            Effect.logWarning("Failed to resolve custom model endpoint statuses.", { cause }).pipe(
              Effect.as([]),
            ),
          ),
        );
      ...
      return WsRpcGroup.of({
        [DEVICE_WS_METHODS.list]: (input) =>
          observeRpcEffect(DEVICE_WS_METHODS.list, deviceHandlers[DEVICE_WS_METHODS.list](input), {
            "rpc.aggregate": "device",
          }),
        [DEVICE_WS_METHODS.boot]: (input) =>
          observeRpcEffect(DEVICE_WS_METHODS.boot, deviceHandlers[DEVICE_WS_METHODS.boot](input), {
            "rpc.aggregate": "device",
          }),
        [DEVICE_WS_METHODS.shutdown]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.shutdown,
            deviceHandlers[DEVICE_WS_METHODS.shutdown](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.attach]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.attach,
            deviceHandlers[DEVICE_WS_METHODS.attach](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.detach]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.detach,
            deviceHandlers[DEVICE_WS_METHODS.detach](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.getThreadState]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.getThreadState,
            deviceHandlers[DEVICE_WS_METHODS.getThreadState](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.tap]: (input) =>
          observeRpcEffect(DEVICE_WS_METHODS.tap, deviceHandlers[DEVICE_WS_METHODS.tap](input), {
            "rpc.aggregate": "device",
          }),
        [DEVICE_WS_METHODS.swipe]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.swipe,
            deviceHandlers[DEVICE_WS_METHODS.swipe](input),
            {
              "rpc.aggregate": "device",
            },
          ),
        [DEVICE_WS_METHODS.typeText]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.typeText,
            deviceHandlers[DEVICE_WS_METHODS.typeText](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.keyEvent]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.keyEvent,
            deviceHandlers[DEVICE_WS_METHODS.keyEvent](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.pressButton]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.pressButton,
            deviceHandlers[DEVICE_WS_METHODS.pressButton](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.setGravity]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.setGravity,
            deviceHandlers[DEVICE_WS_METHODS.setGravity](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.installApp]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.installApp,
            deviceHandlers[DEVICE_WS_METHODS.installApp](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.launchApp]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.launchApp,
            deviceHandlers[DEVICE_WS_METHODS.launchApp](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.openUrl]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.openUrl,
            deviceHandlers[DEVICE_WS_METHODS.openUrl](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.screenshot]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.screenshot,
            deviceHandlers[DEVICE_WS_METHODS.screenshot](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.startRecording]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.startRecording,
            deviceHandlers[DEVICE_WS_METHODS.startRecording](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.stopRecording]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.stopRecording,
            deviceHandlers[DEVICE_WS_METHODS.stopRecording](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.describeUi]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.describeUi,
            deviceHandlers[DEVICE_WS_METHODS.describeUi](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.scrollToElement]: (input) =>
          observeRpcEffect(
            DEVICE_WS_METHODS.scrollToElement,
            deviceHandlers[DEVICE_WS_METHODS.scrollToElement](input),
            { "rpc.aggregate": "device" },
          ),
        [DEVICE_WS_METHODS.subscribeEvents]: (_input) =>
          observeRpcStream(
            DEVICE_WS_METHODS.subscribeEvents,
            Stream.callback<DeviceEvent>((queue) =>
              Effect.acquireRelease(
                Effect.sync(() =>
                  deviceService.manager.onEvent((event) => {
        ...
        [WS_METHODS.serverTranscribeVoice]: (input) =>
          observeRpcEffect(
            ...
            localVoice.transcribe(input).pipe(Effect.orDie),
            { "rpc.aggregate": "server" },
          ),
        [WS_METHODS.serverVoiceStatus]: (_input) =>
          observeRpcEffect(WS_METHODS.serverVoiceStatus, localVoice.status, {
            "rpc.aggregate": "server",
          }),
        [WS_METHODS.serverInstallVoiceModel]: (input) =>
          observeRpcEffect(
            WS_METHODS.serverInstallVoiceModel,
            localVoice.installModel(input.modelId),
            { "rpc.aggregate": "server" },
          ),
        [LISTENING_WS_METHODS.listLibraries]: (_input) =>
          observeRpcEffect(
            LISTENING_WS_METHODS.listLibraries,
            Effect.try({
              try: () => ({ libraries: detectListeningLibrariesOnThisMachine() }),
              catch: (cause) =>
                new ListeningLibraryError({
                  message:
                    cause instanceof Error ? cause.message : "Could not detect listening apps.",
                }),
            }),
            { "rpc.aggregate": "listening" },
          ),
        [LISTENING_WS_METHODS.listPlaylists]: (input) =>
          observeRpcEffect(
            LISTENING_WS_METHODS.listPlaylists,
            Effect.tryPromise({
              try: () => listNativeListeningPlaylists(input.id),
              catch: (cause) =>
                new ListeningLibraryError({
                  message:
                    cause instanceof Error
                      ? cause.message
                      : "Could not read this library. Allow Modesto to control Music in System Settings.",
                }),
            }),
            { "rpc.aggregate": "listening" },
          ),
        [LISTENING_WS_METHODS.listTracks]: (input) =>
          observeRpcEffect(
            LISTENING_WS_METHODS.listTracks,
            Effect.tryPromise({
              try: () => listNativeListeningTracks(input.id, input.playlistId),
              catch: (cause) =>
                new ListeningLibraryError({
                  message: cause instanceof Error ? cause.message : "Could not load songs.",
                }),
            }),
            { "rpc.aggregate": "listening" },
          ),
        [LISTENING_WS_METHODS.control]: (input) =>
          observeRpcEffect(
            LISTENING_WS_METHODS.control,
            Effect.tryPromise({
              try: async () => {
                await controlNativeListeningLibrary(input);
                return { ok: true };
              },
              catch: (cause) =>
                new ListeningLibraryError({
                  message: cause instanceof Error ? cause.message : "Playback failed.",
                }),
            }),
            { "rpc.aggregate": "listening" },
          ),
        [LISTENING_WS_METHODS.playback]: (input) =>
          observeRpcEffect(
            LISTENING_WS_METHODS.playback,
            Effect.tryPromise({
              try: () => nativeListeningPlayback(input.id),
              catch: (cause) =>
                new ListeningLibraryError({
                  message: cause instanceof Error ? cause.message : "Could not read now playing.",
                }),
            }),
            { "rpc.aggregate": "listening" },
          ),
        ...
        [WS_METHODS.serverSetCustomModelEndpoint]: (input) =>
        ...
        [WS_METHODS.serverDeleteCustomModelEndpoint]: (input) =>
          observeRpcEffect(
            ...
            deleteCustomModelEndpoint(input),
            {
              "rpc.aggregate": "server",
            },
          ),
        ...
        [WS_METHODS.vercelAuthStatus]: () =>
          observeRpcEffect(WS_METHODS.vercelAuthStatus, vercel.authStatus(), {
            "rpc.aggregate": "vercel",
          }),
        [WS_METHODS.vercelListProjects]: () =>
          observeRpcEffect(WS_METHODS.vercelListProjects, vercel.listProjects(), {
            "rpc.aggregate": "vercel",
          }),
        [WS_METHODS.vercelSetToken]: (input) =>
          observeRpcEffect(WS_METHODS.vercelSetToken, vercel.setToken(input), {
            "rpc.aggregate": "vercel",
          }),
        [WS_METHODS.vercelClearToken]: () =>
          observeRpcEffect(WS_METHODS.vercelClearToken, vercel.clearToken(), {
            "rpc.aggregate": "vercel",
          }),
        [WS_METHODS.vercelInstallMcp]: () =>
          observeRpcEffect(WS_METHODS.vercelInstallMcp, vercel.installMcp(), {
            "rpc.aggregate": "vercel",
          }),
        [WS_METHODS.claudePluginPreview]: (input) =>
          observeRpcEffect(
            WS_METHODS.claudePluginPreview,
            Effect.tryPromise({
              try: () => resolveClaudePluginBundle(input),
              catch: (cause) => claudePluginRequestErrorFromCause(cause),
            }).pipe(Effect.map(({ preview }) => ({ preview }))),
            { "rpc.aggregate": "claude-plugin" },
          ),
        [WS_METHODS.claudePluginInstall]: (input) =>
          observeRpcEffect(
            WS_METHODS.claudePluginInstall,
            Effect.tryPromise({
              try: () => resolveClaudePluginBundle(input),
              catch: (cause) => claudePluginRequestErrorFromCause(cause),
            }).pipe(
              Effect.flatMap((bundle) =>
                installClaudePlugin({ bundle, baseDir: config.baseDir }).pipe(
                  Effect.mapError(
                    (cause) =>
                      new ClaudePluginRequestError({
                        code: "install_failed",
                        message: cause.message,
                        cause,
                      }),
              ...
              Effect.map((installed) => ({ plugin: installedClaudePluginSummary(installed) })),
            ),
            { "rpc.aggregate": "claude-plugin" },
          ),
        [WS_METHODS.automationList]: (_input) =>
          observeRpcEffect(WS_METHODS.automationList, automationService.list(), {
            "rpc.aggregate": "automation",
          }),
        [WS_METHODS.automationCreate]: (input) =>
          observeRpcEffect(
            WS_METHODS.automationCreate,
            automationService.create(input).pipe(Effect.map((automation) => ({ automation }))),
            { "rpc.aggregate": "automation" },
          ),
        [WS_METHODS.automationSetState]: (input) =>
          observeRpcEffect(
            WS_METHODS.automationSetState,
            automationService.setState(input).pipe(Effect.map((automation) => ({ automation }))),
            { "rpc.aggregate": "automation" },
          ),
        [WS_METHODS.automationRuns]: (input) =>
          observeRpcEffect(
            WS_METHODS.automationRuns,
            automationService.runs(input).pipe(Effect.map((runs) => ({ runs }))),
            { "rpc.aggregate": "automation" },
          ),
        [WS_METHODS.claudePluginList]: (_input) =>
          observeRpcEffect(
            WS_METHODS.claudePluginList,
            listInstalledClaudePlugins(config.baseDir).pipe(
              Effect.map((plugins) => ({
                plugins: plugins.map(installedClaudePluginSummary),
              })),
            ),
            { "rpc.aggregate": "claude-plugin" },
          ),
        [WS_METHODS.claudePluginUninstall]: (input) =>
          observeRpcEffect(
            WS_METHODS.claudePluginUninstall,
            uninstallClaudePlugin({ pluginId: input.pluginId, baseDir: config.baseDir }).pipe(
              Effect.map(() => ({ ok: true })),
              Effect.mapError(
                (cause) =>
                  new ClaudePluginRequestError({
        ...
        [WS_METHODS.skillPackPreview]: (input) =>
          observeRpcEffect(
            WS_METHODS.skillPackPreview,
            Effect.tryPromise({
              try: () => resolveSkillPackBundle(input),
              catch: skillPackRequestErrorFromCause,
            }).pipe(
              Effect.flatMap((bundle) =>
                previewSkillPackEligibility({
                  bundle,
                  ...
                  Effect.orElseSucceed(() => defaultSkillPackEligibility(SKILL_PACK_PROVIDERS)),
                  Effect.map((eligibility) => ({
                    preview: { ...bundle.preview, eligibility },
                  })),
                ),
              ),
            ),
            { "rpc.aggregate": "skill-pack" },
          ),
        [WS_METHODS.skillPackInstall]: (input) =>
          observeRpcEffect(
            WS_METHODS.skillPackInstall,
            Effect.tryPromise({
              try: () => resolveSkillPackBundle({ ...input, includeFiles: true }),
              catch: skillPackRequestErrorFromCause,
            }).pipe(
              Effect.flatMap((bundle) =>
                installSkillPack({
                  bundle,
                  baseDir: config.baseDir,
                  providers: input.providers?.length ? input.providers : SKILL_PACK_PROVIDERS,
                }).pipe(
                  Effect.mapError(
                    (cause) =>
                      new SkillPackRequestError({
                        code: "install_failed",
                        message: cause.message,
                        cause,
                      }),
              ...
              Effect.map((pack) => ({ pack: installedSkillPackSummary(pack) })),
            ),
            { "rpc.aggregate": "skill-pack" },
          ),
        [WS_METHODS.skillPackList]: (_input) =>
          observeRpcEffect(
            WS_METHODS.skillPackList,
            listInstalledSkillPacks(config.baseDir).pipe(
              Effect.map((packs) => ({ packs: packs.map(installedSkillPackSummary) })),
            ),
            { "rpc.aggregate": "skill-pack" },
          ),
        [WS_METHODS.skillPackUninstall]: (input) =>
          observeRpcEffect(
            WS_METHODS.skillPackUninstall,
            uninstallSkillPack({ packId: input.packId, baseDir: config.baseDir }).pipe(
              Effect.as({ ok: true }),
              Effect.mapError(
                (cause) =>
                  new SkillPackRequestError({
        ...
        [WS_METHODS.webhookList]: (_input) =>
          observeRpcEffect(WS_METHODS.webhookList, webhookService.list(), {
            "rpc.aggregate": "webhook",
          }),
        [WS_METHODS.webhookCreate]: (input) =>
          observeRpcEffect(WS_METHODS.webhookCreate, webhookService.create(input), {
            "rpc.aggregate": "webhook",
          }),
        [WS_METHODS.webhookSetEnabled]: (input) =>
          observeRpcEffect(
            WS_METHODS.webhookSetEnabled,
            webhookService.setEnabled(input).pipe(Effect.map((webhook) => ({ webhook }))),
            { "rpc.aggregate": "webhook" },
          ),
        [WS_METHODS.webhookDelete]: (input) =>
          observeRpcEffect(
            WS_METHODS.webhookDelete,
            webhookService.delete(input.webhookId).pipe(Effect.map((ok) => ({ ok }))),
            { "rpc.aggregate": "webhook" },
          ),
        ...
        [WS_METHODS.filesystemPickFolder]: (input) =>
          observeRpcEffect(WS_METHODS.filesystemPickFolder, folderPicker.pickFolder(input), {
            "rpc.aggregate": "workspace",
          }),
        ...
        [WS_METHODS.gitGithubAuthStatus]: () =>
          observeRpcEffect(WS_METHODS.gitGithubAuthStatus, GitHubAccountAuth.status, {
            "rpc.aggregate": "git",
          }),
        [WS_METHODS.gitGithubSignIn]: () =>
          observeRpcEffect(WS_METHODS.gitGithubSignIn, GitHubAccountAuth.signIn, {
            "rpc.aggregate": "git",
          }),
        [WS_METHODS.gitGithubSignOut]: () =>
          observeRpcEffect(WS_METHODS.gitGithubSignOut, GitHubAccountAuth.signOut, {
            "rpc.aggregate": "git",
          }),
        ...
        const rpcWebSocketHttpEffect = yield* RpcServer.toHttpEffectWebsocket(WsRpcGroup, {
          ...
          Effect.provide(
            makeWsRpcLayer(session, clientOrigin, previewAutomationBroker).pipe(
              ...
              Layer.provide(LocalVoice.layer),
