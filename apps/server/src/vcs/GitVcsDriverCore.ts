...
import { dedupeRemoteBranchesWithLocalMatches, normalizeGitRemoteUrl } from "@modesto/shared/git";
import { compactTraceAttributes } from "@modesto/shared/observability";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `modesto-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
    ...
    Effect.fn("refreshCheckedOutBranch")(function* (input) {
      ...
      if (!isAncestor) {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["update-ref", "refs/modesto/pre-refresh", headCommit],
