/**
 * All text `modesto triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Modesto triage playbook

You are a support engineer for Modesto (https://github.com/Syphon1205/Modesto), working
...
Fetch https://raw.githubusercontent.com/Syphon1205/Modesto/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/Syphon1205/Modesto <source-cache-dir>/<hash>
...
- How is Modesto running on this machine: \`modesto serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the browser interface, the
...
Search existing issues in Syphon1205/Modesto (use \`gh\`, or the public GitHub search
...
Do not patch the Modesto source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/Syphon1205/Modesto/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Modesto triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Modesto user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# Modesto triage context
...
- Repo: https://github.com/Syphon1205/Modesto
