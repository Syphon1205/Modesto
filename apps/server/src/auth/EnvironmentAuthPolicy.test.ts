...
it.layer(NodeServices.layer)("EnvironmentAuthPolicy.layer", (it) => {
  ...
  it.effect("uses remote-reachable policy for desktop mode when bound beyond loopback", () =>
    Effect.gen(function* () {
      ...
      expect(policy.allowsLoopbackBootstrap).toBe(false);
  ...
  it.effect("uses loopback-browser policy for loopback web hosts", () =>
    Effect.gen(function* () {
      ...
      expect(descriptor.bootstrapMethods).toEqual(["loopback-bootstrap", "one-time-token"]);
      expect(policy.allowsLoopbackBootstrap).toBe(true);
