 ...
 *
 * IPv4-mapped IPv6 peers (`::ffff:127.0.0.1`) are normalized before the check,
 * because that is the form a dual-stack Node listener reports for a plain
...
/** The peer address of an HTTP request, or `undefined` when the server cannot see one. */
...
/**
 * Whether a request came from the app's own page rather than from another site.
 *
 * This is the check that makes credential-free loopback bootstrap safe: a
 * loopback server is reachable by every page the user has open, so without it
 * any website could POST to `localhost` and be handed a session. Browsers
 * label their own cross-site requests, and cannot be talked out of it:
 *
 * - `Sec-Fetch-Site` is set by the browser and unforgeable by page script.
 *   `same-origin` is the app calling itself; `none` is a user-initiated
 *   navigation. Anything else is another site and is refused.
 * - Older browsers omit the header, so `Origin` is compared to the host the
 *   request was addressed to. A cross-site caller still sends its own origin
 *   there, including under DNS rebinding, where the page origin stays the
 *   attacker's.
 *
 * A request with neither header is not a browser request at all and is refused:
 * non-browser clients pair with a token.
 */
export function isSameOriginBrowserRequest(request: HttpServerRequest.HttpServerRequest): boolean {
  const fetchSite = normalizeNonEmptyString(request.headers["sec-fetch-site"])?.toLowerCase();
  if (fetchSite !== undefined) {
    return fetchSite === "same-origin" || fetchSite === "none";
  }

  const origin = normalizeNonEmptyString(request.headers.origin);
  const host = normalizeNonEmptyString(request.headers.host);
  if (!origin || !host) {
    return false;
  }
  try {
