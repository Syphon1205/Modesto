...
import type { AuthEnvironmentScope } from "@modesto/contracts";
import { parseAllowedOAuthScope } from "@modesto/shared/oauthScope";
...
export const authHttpApiLayer = HttpApiBuilder.group(
  ...
  Effect.fnUntraced(function* (handlers) {
    ...
    const authPolicy = yield* EnvironmentAuthPolicy.EnvironmentAuthPolicy;

    /**
     * Set the browser session cookie for a session that was just issued.
     *
     * Shared by the two ways a browser can obtain one (pairing token and
     * loopback bootstrap) so they cannot drift apart on cookie attributes -
     * `sameSite: "lax"` in particular is load-bearing for CSRF.
     */
    const attachSessionCookie = (input: {
      readonly sessionToken: string;
      readonly expiresAt: DateTime.Utc;
    }) =>
      Effect.gen(function* () {
        const sessionCookies = yield* Effect.fromResult(
          Cookies.set(Cookies.empty, sessions.cookieName, input.sessionToken, {
    ...
    return handlers
      ...
      .handle(
        "loopbackSession",
        Effect.fn("environment.auth.loopbackSession")(
          function* (args) {
            yield* annotateEnvironmentRequest(args.endpoint.name);
            const request = yield* HttpServerRequest.HttpServerRequest;

            // Three independent conditions, all required. The environment must
            // be loopback-bound (nothing off-machine can reach it), the peer
            // must actually be loopback (a proxy in front would not be), and
            // the request must come from the app's own page rather than from
            // another site the user has open.
            if (!authPolicy.allowsLoopbackBootstrap) {
              return yield* failEnvironmentAuthInvalid("invalid_credential");
            }
            if (!isLoopbackAddress(readRequestRemoteAddress(request))) {
              return yield* failEnvironmentAuthInvalid("invalid_credential");
            }
            ...
            const result = yield* serverAuth.createLoopbackSession(
              deriveAuthClientMetadata({ request }),
            );
            yield* attachSessionCookie({
              sessionToken: result.sessionToken,
