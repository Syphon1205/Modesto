...
import { encodeOAuthScope } from "@modesto/shared/oauthScope";
...
/**
 * Subject recorded for sessions claimed by a same-machine browser with no
 * credential. Distinct from the pairing subjects so the Clients list can say
 * where a session actually came from.
 */
export const LOOPBACK_BOOTSTRAP_SUBJECT = "loopback-bootstrap";
...
export class EnvironmentAuth extends Context.Service<
  EnvironmentAuth,
  {
    ...
    >;
    /**
     * Issue a browser session for a caller that has already proven it is on
     * this machine, with no credential to exchange.
     *
     * The proof is the caller's business, not this method's: the HTTP layer
     * only reaches here after checking the environment is loopback-bound, the
     * peer address is loopback, and the request is same-origin. Scopes match
     * the startup pairing token, so every local window can manage pairing and
     * settings rather than being a second-class client.
     */
    readonly createLoopbackSession: (requestMetadata: AuthClientMetadata) => Effect.Effect<
      {
        readonly response: AuthBrowserSessionResult;
        readonly sessionToken: string;
      },
...
export const make = Effect.gen(function* () {
  ...
  const createLoopbackSession: EnvironmentAuth["Service"]["createLoopbackSession"] = (
    requestMetadata,
  ) =>
    sessions
      .issue({
        method: "browser-session-cookie",
        subject: LOOPBACK_BOOTSTRAP_SUBJECT,
      ...
      .pipe(
        ...
        Effect.withSpan("EnvironmentAuth.createLoopbackSession"),
      );
