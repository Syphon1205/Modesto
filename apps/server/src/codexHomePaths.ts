...
export const MODESTO_CODEX_HOME_OVERLAY_DIR = "codex-home-overlay";
...
export function resolveModestoCodexHomeOverlayPath(
  env: NodeJS.ProcessEnv,
  sourceHomePath: string,
): string {
  const runtimeHome = env.MODESTO_HOME?.trim();
  const overlayRoot = runtimeHome || path.join(path.dirname(sourceHomePath), ".modesto", "runtime");
  return path.join(overlayRoot, MODESTO_CODEX_HOME_OVERLAY_DIR);
}

/**
 * Returns the home directory that the codex app-server child process actually
 * writes under. Modesto keeps its generated config isolated from the user's
...
export function resolveActiveCodexHomeWritePath(input: CodexHomePathsInput = {}): string {
  ...
  const overlay = resolveModestoCodexHomeOverlayPath(env, source);
