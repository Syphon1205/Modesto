...
/**
 * Shown only when a restart left a thread with nothing to resume from. A thread
 * whose binding still carries a resume cursor is recoverable, so it settles
 * quietly instead of wearing an error it would immediately shrug off - see
 * `reconcileProviderSessions`.
 */
export const UNRECOVERABLE_PROVIDER_SESSION_ERROR =
  "Provider session did not survive a server restart, and no resume state was saved. Send a new message to start a fresh session.";
...
export const reconcileProviderSessions = Effect.gen(function* () {
  ...
  for (const thread of orphanedThreads) {
    ...
    // A restart kills the provider process, not the conversation. The binding
    // keeps the resume cursor, the cwd and the model selection, so the next
    // message resumes the provider thread where it left off
    // (`recoverSessionForThread`). Only a thread with nothing to resume from is
    // genuinely lost.
    const isRecoverable = yield* Effect.gen(function* () {
      ...
      return binding.value.resumeCursor !== null && binding.value.resumeCursor !== undefined;
    }).pipe(
    ...
    yield* Effect.gen(function* () {
      ...
      yield* orchestrationEngine.dispatch({
        ...
        session: {
          ...session,
          // "stopped" settles the interrupted turn and leaves the thread ready
          // to resume; "error" would raise a banner on a thread that is fine.
          status: isRecoverable ? "stopped" : "error",
          activeTurnId: null,
          lastError: isRecoverable ? null : UNRECOVERABLE_PROVIDER_SESSION_ERROR,
          ...
          if (serverConfig.startupPresentation === "headless") {
            ...
            if (serverConfig.mode !== "desktop") {
              yield* Effect.logInfo(
                "Authentication required. Open Modesto using the pairing URL.",
