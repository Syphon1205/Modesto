...
import { ApprovalRequestId, ThreadId } from "@modesto/contracts";
...
import {
  ...
  codexPluginMutationArgs,
  isRecoverableThreadResumeError,
  normalizeCodexModelSlug,
  readCodexAccountSnapshot,
  readCodexPluginBundledSkillSummaries,
  resolveCodexPluginArtworkPath,
...
describe("codex plugin commands", () => {
  it("builds install and remove commands without shell interpolation", () => {
    expect(
      codexPluginMutationArgs({
    ...
    ).toEqual(["plugin", "add", "figma", "--marketplace", "openai-curated", "--json"]);
    expect(
      codexPluginMutationArgs({
        marketplaceName: "openai-curated",
        pluginName: "figma",
    ...
    ).toEqual(["plugin", "remove", "figma", "--marketplace", "openai-curated", "--json"]);
  });

  it("resolves artwork inside the plugin root without allowing traversal", () => {
    const pluginRoot = path.join(os.tmpdir(), "plugins", "figma");
    expect(resolveCodexPluginArtworkPath(pluginRoot, "./assets/logo.png")).toBe(
      path.join(pluginRoot, "assets/logo.png"),
    );
    expect(resolveCodexPluginArtworkPath(pluginRoot, "../other/logo.png")).toBeUndefined();
    expect(resolveCodexPluginArtworkPath(pluginRoot, "https://example.com/logo.svg")).toBe(
      "https://example.com/logo.svg",
    );
  });

  it("reads installable skill summaries from a plugin manifest", async () => {
    const pluginRoot = mkdtempSync(path.join(os.tmpdir(), "modesto-plugin-skills-"));
    try {
      const skillRoot = path.join(pluginRoot, "agent-skills", "review-code");
      ...
      writeFileSync(
        ...
        [
          ...
          "short-description: Find bugs before they ship.",
          "---",
          "",
      ...
      await expect(
        readCodexPluginBundledSkillSummaries({
          pluginRoot,
          pluginId: "review-tools@personal",
          manifest: { skills: "./agent-skills" },
        }),
      ).resolves.toEqual([
        {
          name: "review-code",
          ...
          interface: {
            ...
            shortDescription: "Find bugs before they ship.",
          },
        },
      ]);
    } finally {
...
describe("buildCodexProcessEnv", () => {
  it("hydrates the active custom provider env_key from the effective CODEX_HOME", () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "modesto-codex-env-"));
    try {
      writeFileSync(
  ...
  it("allows the configured desktop browser-use socket in the Codex sandbox", () => {
    const env = buildCodexProcessEnv({
      env: {
        MODESTO_BROWSER_USE_PIPE_PATH: "/tmp/codex-browser-use/modesto.sock",
    ...
    expect(env.NODE_REPL_SANDBOX_ALLOWED_UNIX_SOCKETS).toBe(
      "/tmp/existing.sock,/tmp/codex-browser-use/modesto.sock",
    );
  });
  ...
  it("resolves the browser-use pipe path from desktop env aliases", () => {
    expect(
      resolveCodexBrowserUsePipePath({
        env: { MODESTO_BROWSER_USE_PIPE_PATH: "/tmp/codex-browser-use/modesto.sock" },
        platform: "darwin",
      }),
    ).toBe("/tmp/codex-browser-use/modesto.sock");
  });

  it("applies durable section suppressions inside Modesto's Codex overlay", () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "modesto-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "modesto-runtime-home-"));
    try {
      writeFileSync(
        path.join(tempDir, "config.toml"),
        [
      ...
      writeFileSync(
        path.join(overlayHome, "modesto-config-suppressions-v1.json"),
        `${JSON.stringify({
          version: 1,
  ...
  it("seeds markerless suppressions for conflicting local browser plugins", () => {
    ...
    try {
      ...
      const suppressionMarker = JSON.parse(
        readFileSync(path.join(overlayHome, "modesto-config-suppressions-v1.json"), "utf8"),
  ...
  it("repairs stale real files in Modesto's Codex home overlay", () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "modesto-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "modesto-runtime-home-"));
    try {
  ...
  it("repairs stale auth.json files in Modesto's Codex home overlay", () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "modesto-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "modesto-runtime-home-"));
    try {
  ...
  it("preserves real generated image directories in Modesto's Codex home overlay", () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "modesto-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "modesto-runtime-home-"));
    try {
      writeFileSync(path.join(tempDir, "config.toml"), 'model = "gpt-5.5"', "utf8");
...
describe("handleStdoutLine", () => {
  ...
  it("ignores multiline and standalone JSON leaked from command output", () => {
    ...
    for (const line of ["{", "[", '{"scripts": {', "{}", "[]", '{"name":"modesto"}']) {
      handleStdoutLine(context, line);
    }
...
describe("startSession", () => {
  ...
  it("uses an isolated scratch workspace path when no cwd is provided", () => {
    ...
    expect(cwd).toContain(`${path.sep}modesto-codex-workspaces${path.sep}thread-1`);
  });
  ...
  it("fails fast with an upgrade message when codex is below the minimum supported version", async () => {
    ...
    const versionCheck = vi
      ...
      .mockImplementation(() => {
        throw new Error(
          "Codex CLI v0.36.0 is too old for Modesto. Upgrade to v0.37.0 or newer and restart Modesto.",
        );
      });
