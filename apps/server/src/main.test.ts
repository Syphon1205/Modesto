...
import type { OrchestrationReadModel } from "@modesto/contracts";
...
import { CliConfig, recordStartupHeartbeat, modestoCli, type CliConfigShape } from "./main";
...
let defaultModestoHome = "";
const tempHomes = new Set<string>();

function makeTempHome(prefix = "modesto-main-test-"): string {
...
const testLayer = Layer.mergeAll(
  Layer.succeed(CliConfig, {
    cwd: "/tmp/modesto-test-workspace",
    fixPath: Effect.void,
...
const runCli = (args: ReadonlyArray<string>, env: Record<string, string> = {}) => {
  const program = Command.runWith(modestoCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            MODESTO_HOME: defaultModestoHome,
            MODESTO_NO_BROWSER: "true",
            ...env,
          },
        }),
      ),
...
beforeEach(() => {
  vi.clearAllMocks();
  defaultModestoHome = makeTempHome();
  resolvedConfig = null;
...
it.layer(testLayer)("server CLI command", (it) => {
  it.effect("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("modesto-main-flag-");
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      const envHome = makeTempHome("modesto-main-env-");

      yield* runCli([], {
        MODESTO_MODE: "desktop",
        MODESTO_PORT: "4999",
        MODESTO_HOST: "100.88.10.4",
        MODESTO_HOME: envHome,
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        MODESTO_NO_BROWSER: "true",
        MODESTO_AUTH_TOKEN: "env-token",
      });
  ...
  it.effect("prefers --mode over MODESTO_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over MODESTO_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        MODESTO_NO_BROWSER: "false",
      });
  ...
  it.effect("supports CLI and env for bootstrap/provider-log/websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        MODESTO_MODE: "desktop",
        MODESTO_LOG_PROVIDER_EVENTS: "true",
        MODESTO_LOG_WS_EVENTS: "false",
        MODESTO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        MODESTO_NO_BROWSER: "true",
      });
