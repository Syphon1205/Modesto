...
layer("032_ReconcileImportedSchemaLineage", (it) => {
  // Simulates a legacy ~/.modesto import where the imported `effect_sql_migrations`
  // tracker has IDs 17-31 recorded under unrelated Modesto names. The 17-31
  ...
  // references env_mode.
  it.effect("heals an imported Modesto DB whose tracker skipped 17-31", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;

      // Bring the imported schema to the last shared migration boundary.
      yield* runMigrations({ toMigrationInclusive: 16 });

      // Mark IDs 17-31 applied under Modesto's old names so the migrator
      // skips Modesto's renumbered 17-23. Names are illustrative; only the
      ...
      // Seed a thread row with the Modesto-era column set so the data-rewrite
  ...
  it.effect("is a no-op on a fresh Modesto install", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
