...
import { MODEL_OPTIONS_BY_PROVIDER } from "@modesto/contracts";
...
// Imported instance ids may be runtime names rather than Modesto provider literals.
...
export function normalizePersistedModelSelection(input: unknown): unknown {
  ...
  // Newer Modesto writes provider-less selections as { instanceId, model } and
  // option rows as [{ id, value }]; Modesto stores canonical provider/options objects.
