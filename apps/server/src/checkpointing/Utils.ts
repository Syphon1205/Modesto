...
import { resolveThreadWorkspaceCwd as resolveSharedThreadWorkspaceCwd } from "@modesto/shared/threadEnvironment";

export const CHECKPOINT_REFS_PREFIX = "refs/modesto/checkpoints";
export const AGENT_CHECKPOINT_REFS_PREFIX = "refs/modesto/agent-checkpoints";
...
function agentHandoffCheckpointFamilyPrefix(
...
export function checkpointRefForAgentHandoffTree(
  ...
  return CheckpointRef.makeUnsafe(
    `${agentHandoffCheckpointFamilyPrefix(sourceThreadId, destThreadId)}/tree`,
  );
}

export function checkpointRefForAgentHandoffBase(
  sourceThreadId: ThreadId,
  destThreadId: ThreadId,
): CheckpointRef {
  return CheckpointRef.makeUnsafe(
    `${agentHandoffCheckpointFamilyPrefix(sourceThreadId, destThreadId)}/base`,
  );
}
