...
describe("managed checkpoint refs", () => {
  ...
  it("creates canonical Modesto refs", () => {
    expect(checkpointRefForThreadTurn(threadId, 4)).toMatch(/^refs\/modesto\/checkpoints\//);
  });
  ...
  it("recognizes a structurally valid persisted ref for the same thread", () => {
    ...
    const historical = canonical.replace("refs/modesto/", "refs/historical/");
