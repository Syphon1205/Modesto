...
export interface CheckpointStoreShape {
  ...
  /**
   * Resolve the tree oid pointed to by a checkpoint ref (or null if missing).
   */
  readonly resolveCheckpointTreeOid: (
    input: Omit<RestoreCheckpointInput, "fallbackToHead">,
  ) => Effect.Effect<string | null, CheckpointStoreError>;

  /**
   * Resolve the current HEAD commit oid (or null when HEAD is unavailable).
   */
  readonly resolveHeadCommitOid: (cwd: string) => Effect.Effect<string | null, CheckpointStoreError>;

  /**
   * Compute the working-tree oid the same way capture would, without writing a ref.
   *
   * Used for idempotent agent/handoff checkpoint declarations: if the tree oid
   * matches an existing checkpoint, capture can no-op.
   */
  readonly resolveWorkingTreeOid: (cwd: string) => Effect.Effect<string | null, CheckpointStoreError>;
...
export class CheckpointStore extends ServiceMap.Service<CheckpointStore, CheckpointStoreShape>()(
  "modesto/checkpointing/Services/CheckpointStore",
) {}
