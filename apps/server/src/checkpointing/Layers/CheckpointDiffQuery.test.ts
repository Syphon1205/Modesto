import { CheckpointRef, ProjectId, ThreadId, TurnId, type ProjectKind } from "@modesto/contracts";
...
// None of the existing scenarios below exercise a `provider-diff:` checkpoint,
// so the blob store is never actually read from/written to for them.
const unusedCheckpointDiffBlobStore: CheckpointDiffBlobStoreShape = {
  upsert: () => Effect.die("unused"),
  getByTurnRange: () => Effect.die("unused"),
  deleteAfterTurnCount: () => Effect.die("unused"),
};
...
describe("CheckpointDiffQueryLive", () => {
  it("prefers exact turn-start checkpoints for single-turn diffs", async () => {
    ...
    const toCheckpointRef = CheckpointRef.makeUnsafe(
      checkpointRefForThreadTurn(threadId, 1).replace("refs/modesto/", "refs/historical/"),
    );
  ...
  it("serves the stored provider diff blob for a provider-diff placeholder, without touching git", async () => {
    const projectId = ProjectId.makeUnsafe("project-non-git");
    const threadId = ThreadId.makeUnsafe("thread-non-git");
    const toCheckpointRef = CheckpointRef.makeUnsafe("provider-diff:evt-1");

    const threadCheckpointContext = makeThreadCheckpointContext({
      projectId,
      threadId,
      workspaceRoot: "/tmp/non-git-workspace",
      envMode: "local",
      worktreePath: null,
      checkpointTurnCount: 1,
      checkpointRef: toCheckpointRef,
      // Provider-diff placeholders are always dispatched with `status: "missing"`
      // and, in a non-git workspace, never get superseded by a real checkpoint.
      status: "missing",
    });
    ...
    const checkpointStore: CheckpointStoreShape = {
      ...
      diffCheckpoints: () => Effect.die("git must not be consulted for a provider-diff checkpoint"),
      deleteCheckpointRefs: () => Effect.die("unused"),
    };

    const getByTurnRangeCalls: Array<{
      readonly threadId: ThreadId;
      readonly fromTurnCount: number;
      readonly toTurnCount: number;
    }> = [];
    const checkpointDiffBlobStore: CheckpointDiffBlobStoreShape = {
      upsert: () => Effect.die("unused"),
      getByTurnRange: (input) =>
        Effect.sync(() => {
          getByTurnRangeCalls.push(input);
          return Option.some({
            threadId: input.threadId,
            fromTurnCount: input.fromTurnCount,
            toTurnCount: input.toTurnCount,
            diff: "provider-reported diff patch",
    ...
    const layer = CheckpointDiffQueryLive.pipe(
      Layer.provideMerge(Layer.succeed(CheckpointStore, checkpointStore)),
      Layer.provideMerge(Layer.succeed(CheckpointDiffBlobStore, checkpointDiffBlobStore)),
      Layer.provideMerge(
        Layer.succeed(ProjectionSnapshotQuery, {
          getSnapshot: () => Effect.die("unused"),
          getCommandReadModel: () => Effect.die("unused"),
    ...
    expect(getByTurnRangeCalls).toEqual([{ threadId, fromTurnCount: 0, toTurnCount: 1 }]);
    expect(result).toEqual({
      threadId,
      fromTurnCount: 0,
      toTurnCount: 1,
  ...
  it("falls back to git when no diff blob was stored for a provider-diff placeholder", async () => {
    const projectId = ProjectId.makeUnsafe("project-no-blob");
    const threadId = ThreadId.makeUnsafe("thread-no-blob");
    const toCheckpointRef = CheckpointRef.makeUnsafe("provider-diff:evt-2");
    ...
    const checkpointDiffBlobStore: CheckpointDiffBlobStoreShape = {
      upsert: () => Effect.die("unused"),
      getByTurnRange: () => Effect.succeed(Option.none()),
      deleteAfterTurnCount: () => Effect.die("unused"),
    };
