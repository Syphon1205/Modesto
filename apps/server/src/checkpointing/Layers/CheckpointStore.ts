...
import { CheckpointRef } from "@modesto/contracts";
...
const makeCheckpointStore = Effect.gen(function* () {
  ...
  const captureCheckpointOnce: CheckpointStoreShape["captureCheckpoint"] = (input) =>
    Effect.gen(function* () {
      ...
      yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "modesto-fs-checkpoint-" }),
        (tempDir) =>
          Effect.gen(function* () {
            ...
            const commitEnv: NodeJS.ProcessEnv = {
              ...
              GIT_AUTHOR_NAME: "Modesto",
              GIT_AUTHOR_EMAIL: "modesto@users.noreply.github.com",
              GIT_COMMITTER_NAME: "Modesto",
              GIT_COMMITTER_EMAIL: "modesto@users.noreply.github.com",
            };
            ...
            const message = `Modesto checkpoint ref=${input.checkpointRef}`;
  ...
  const resolveCheckpointTreeOid: CheckpointStoreShape["resolveCheckpointTreeOid"] = (input) =>
    git
      .execute({
        operation: "CheckpointStore.resolveCheckpointTreeOid",
        cwd: input.cwd,
        args: ["rev-parse", "--verify", "--quiet", `${input.checkpointRef}^{tree}`],
        allowNonZeroExit: true,
      })
      .pipe(
        Effect.map((result) => {
  ...
  const resolveHeadCommitOid: CheckpointStoreShape["resolveHeadCommitOid"] = (cwd) =>
    resolveHeadCommit(cwd);

  const resolveWorkingTreeOid: CheckpointStoreShape["resolveWorkingTreeOid"] = (cwd) =>
    Effect.acquireUseRelease(
      fs.makeTempDirectory({ prefix: "modesto-fs-checkpoint-tree-" }),
      (tempDir) =>
        Effect.gen(function* () {
          const operation = "CheckpointStore.resolveWorkingTreeOid";
          const tempIndexPath = path.join(tempDir, `index-${randomUUID()}`);
          const commitEnv: NodeJS.ProcessEnv = {
            ...process.env,
            GIT_INDEX_FILE: tempIndexPath,
      ...
      Effect.catchTags({
        PlatformError: (error) =>
          Effect.fail(
            new CheckpointInvariantError({
              operation: "CheckpointStore.resolveWorkingTreeOid",
              detail: "Failed to resolve working-tree oid.",
              cause: error,
            }),
          ),
      }),
  ...
  const reverseCheckpointDiff: CheckpointStoreShape["reverseCheckpointDiff"] = (input) =>
    Effect.gen(function* () {
      ...
      return yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "modesto-checkpoint-undo-" }),
        (tempDir) =>
          Effect.gen(function* () {
  ...
  return {
    ...
    copyCheckpointRef,
    hasCheckpointRef,
    resolveCheckpointTreeOid,
    resolveHeadCommitOid,
    resolveWorkingTreeOid,
    restoreCheckpoint,
    diffCheckpoints,
