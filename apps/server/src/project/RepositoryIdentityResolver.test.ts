...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:octocat/example-repo.git"]);
      ...
      expect(identity?.canonicalKey).toBe("github.com/octocat/example-repo");
      expect(normalizeResolvedPath(resolvedIdentityRoot)).toBe(normalizeResolvedPath(resolvedCwd));
      expect(identity?.displayName).toBe("octocat/example-repo");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("octocat");
      expect(identity?.name).toBe("example-repo");
  ...
  it.effect("returns the git top-level root path when resolving from a nested workspace", () =>
    Effect.gen(function* () {
      ...
      yield* git(repoRoot, ["remote", "add", "origin", "git@github.com:octocat/example-repo.git"]);
  ...
  it.effect("prefers upstream over origin when both remotes are configured", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/example-repo.git"]);
      yield* git(cwd, ["remote", "add", "upstream", "git@github.com:octocat/example-repo.git"]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, [
        ...
        "add",
        "origin",
        "git@gitlab.com:octocat/platform/example-repo.git",
      ]);
      ...
      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/octocat/platform/example-repo");
      expect(identity?.displayName).toBe("octocat/platform/example-repo");
      expect(identity?.owner).toBe("octocat");
      expect(identity?.name).toBe("example-repo");
    }).pipe(Effect.provide(RepositoryIdentityResolver.layer)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/octocat/example-repo");
        expect(refreshedIdentity?.name).toBe("example-repo");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/octocat/example-repo");

      yield* git(cwd, [
        "remote",
        "set-url",
        "origin",
        "git@github.com:octocat/example-repo-next.git",
      ]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/octocat/example-repo");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/octocat/example-repo-next");
      expect(refreshedIdentity?.displayName).toBe("octocat/example-repo-next");
      expect(refreshedIdentity?.name).toBe("example-repo-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),
