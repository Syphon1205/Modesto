...
import { MessageId, ThreadId, TurnId } from "@modesto/contracts";
...
const checkpointStoreTestLayer = Layer.succeed(CheckpointStore, {
  ...
  resolveCheckpointTreeOid: () => Effect.die("unused checkpoint store test method"),
  resolveHeadCommitOid: () => Effect.die("unused checkpoint store test method"),
  resolveWorkingTreeOid: () => Effect.die("unused checkpoint store test method"),
...
const testLayer = Layer.mergeAll(ProfileStatsQueryLive, ProfileStatsArchiveLive).pipe(
  ...
  Layer.provide(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "modesto-profile-stats-archive-test-",
    }),
  ),
