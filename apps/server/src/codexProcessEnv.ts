...
// Purpose: Builds the exact environment used when Modesto launches Codex subprocesses.
...
import { readActiveCodexProviderEnvKey } from "@modesto/shared/codexConfig";
import {
  readEnvironmentFromLoginShell,
  resolveLoginShell,
  type ShellEnvironmentReader,
} from "@modesto/shared/shell";

import { resolveBaseCodexHomePath, resolveModestoCodexHomeOverlayPath } from "./codexHomePaths.ts";
import {
  resolveCustomModelEndpointsSync,
  type ResolvedCustomModelEndpoint,
} from "./provider/customModelEndpointsSync.ts";
...
const MODESTO_CONFIG_SUPPRESSIONS_FILE = "modesto-config-suppressions-v1.json";
...
export function resolveCodexBrowserUsePipePath(
  ...
  const configured = env.MODESTO_BROWSER_USE_PIPE_PATH?.trim();
  if (configured) {
    return configured;
  }
...
export function readModestoConfigSuppressions(markerPath: string): readonly string[] {
  try {
...
function writeModestoConfigSuppressions(
  markerPath: string,
  sectionHeaders: readonly string[],
): void {
...
// TOML section names are restricted to CustomModelEndpointId's pattern
// (^[a-z0-9][a-z0-9_-]*$ — see packages/contracts/src/settings.ts), which is always
// a valid bare TOML key, so `[model_providers.<id>]` never needs quoting. String
// values still get escaped defensively.
function escapeTomlString(value: string): string {
  return value.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
}

function renderCustomModelEndpointTomlBlock(endpoint: ResolvedCustomModelEndpoint): string {
  return [
    `[model_providers.${endpoint.id}]`,
    `name = "${escapeTomlString(endpoint.label)}"`,
    `base_url = "${escapeTomlString(endpoint.baseUrl)}"`,
    `env_key = "${escapeTomlString(endpoint.envVar)}"`,
    `wire_api = "${endpoint.wireApi}"`,
  ].join("\n");
}

function prepareModestoCodexHomeOverlay(input: {
  readonly env: NodeJS.ProcessEnv;
  readonly homePath?: string;
  readonly customModelEndpoints?: ReadonlyArray<ResolvedCustomModelEndpoint>;
}): string | undefined {
  const sourceHomePath = resolveBaseCodexHomePath(input.env, input.homePath);
  const overlayHomePath = resolveModestoCodexHomeOverlayPath(input.env, sourceHomePath);
  ...
  const suppressionMarkerPath = path.join(overlayHomePath, MODESTO_CONFIG_SUPPRESSIONS_FILE);
  const suppressedSections = [
    ...new Set([
      ...findConflictingLocalBrowserPluginSections(sourceConfig),
      ...readModestoConfigSuppressions(suppressionMarkerPath),
    ]),
  ].slice(0, MAX_CONFIG_SUPPRESSION_SECTIONS);
  const configWithSuppressions = disableCodexConfigSections(sourceConfig, suppressedSections, true);
  // Append Settings → Model Routers custom endpoint definitions. This never
  // touches the real config.toml (sourceConfigPath) — only the overlay, which is
  // regenerated from scratch on every spawn, so edited/removed endpoints can't
  // leave stale sections behind.
  const customEndpointBlocks = (input.customModelEndpoints ?? [])
    .map(renderCustomModelEndpointTomlBlock)
    .join("\n\n");
  const nextConfig = customEndpointBlocks
    ? `${configWithSuppressions.replace(/\n*$/, "\n")}\n${customEndpointBlocks}\n`
    : configWithSuppressions;
  writeFileSync(path.join(overlayHomePath, "config.toml"), nextConfig, "utf8");
  writeModestoConfigSuppressions(suppressionMarkerPath, suppressedSections);
...
export function buildCodexProcessEnv(
  input: {
    ...
    // Settings → Model Routers: Modesto's own settings.json/secrets dir, used to
    // resolve custom endpoint definitions and their API keys. Omitted entirely
    // when the caller doesn't know Modesto's state dir (e.g. isolated tests).
    readonly settingsPath?: string;
    readonly secretsDir?: string;
  } = {},
): NodeJS.ProcessEnv {
  const baseEnv = { ...(input.env ?? process.env) };
  const customModelEndpoints =
    input.settingsPath && input.secretsDir
      ? resolveCustomModelEndpointsSync({
          settingsPath: input.settingsPath,
          secretsDir: input.secretsDir,
        })
      : [];
  const overlayHomePath = prepareModestoCodexHomeOverlay({
    env: baseEnv,
  ...
  for (const endpoint of customModelEndpoints) {
    if (endpoint.apiKey) {
      baseEnv[endpoint.envVar] = endpoint.apiKey;
    }
  }
  const effectiveEnv =
