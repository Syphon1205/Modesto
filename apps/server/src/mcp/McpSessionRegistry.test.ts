...
import { EnvironmentId, ProviderInstanceId, ThreadId } from "@modesto/contracts";
...
it.effect("reports expired credentials without reviving them when the provider wakes", () =>
  Effect.gen(function* () {
    let timestamp = 1_000;
    const registry = yield* makeRegistry(() => timestamp);
    const threadId = ThreadId.make("thread-waking");
    const issued = yield* registry.issue({
      threadId,
      providerInstanceId: ProviderInstanceId.make("codex"),
    });
    const token = issued.config.authorizationHeader.replace(/^Bearer\s+/, "");
    expect(yield* registry.touch(threadId)).toBe(true);
    timestamp += 101;
    expect(yield* registry.touch(threadId)).toBe(false);
    expect(yield* registry.resolve(token)).toBeUndefined();
  }),
);
