...
import type { ProviderRuntimeEvent } from "@modesto/contracts";
...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  ...
  afterEach(() => {
    if (previousModestoHome === undefined) delete process.env.MODESTO_HOME;
    else process.env.MODESTO_HOME = previousModestoHome;
  });

  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.MODESTO_HOME = "/modesto-test/runtime";
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/modesto-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });
  ...
  it("collapses to a single root when overlay equals source", () => {
    ...
    // The overlay falls under `<dirname(source)>/.modesto/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.modesto/runtime/codex-home-overlay";
