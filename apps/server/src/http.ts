...
import { EDITOR_ICON_ROUTE_PATH } from "@modesto/shared/editorIcons";
...
import { OPENCLAW_INGRESS_SECRET_NAME } from "./openclaw/Layers/OpenClawService";
...
const decodeOpenClawSubmitTaskInput = Schema.decodeUnknownEffect(OpenClawSubmitTaskInput);
...
export function makeEffectHttpRouteLayer(readiness: ServerReadiness) {
  return Layer.mergeAll(
    ...
    openClawIngressEffectRouteLayer,
...
const openClawIngressEffectRouteLayer = HttpRouter.add(
  "POST",
  "/api/integrations/openclaw/tasks",
  Effect.gen(function* () {
    const request = yield* HttpServerRequest.HttpServerRequest;
    const secrets = yield* ServerSecretStore;
    const openClaw = yield* OpenClawService;
    const stored = yield* secrets.get(OPENCLAW_INGRESS_SECRET_NAME);
    const authorization = request.headers.authorization ?? "";
    const token = authorization.startsWith("Bearer ") ? authorization.slice(7).trim() : "";
    const expected = stored ? Buffer.from(stored).toString("base64url") : "";
    if (!token || !expected || !constantTimeTokenEquals(token, expected)) {
      return HttpServerResponse.jsonUnsafe(
        { message: "Invalid OpenClaw integration credential." },
        { status: 401 },
      );
    }
    const decoded = yield* readEffectJson(request, "Invalid OpenClaw task payload.").pipe(
      Effect.flatMap(decodeOpenClawSubmitTaskInput),
      Effect.mapError((cause) => ({
        message: "Invalid OpenClaw task payload.",
        status: 400 as const,
        cause,
      })),
    );
    return yield* openClaw.submitTask(decoded).pipe(
      Effect.map((result) => HttpServerResponse.jsonUnsafe(result, { status: 202 })),
      Effect.catch((error) =>
        Effect.succeed(HttpServerResponse.jsonUnsafe({ message: error.message }, { status: 403 })),
      ),
    );
  }).pipe(
    Effect.catch((error) =>
...
const threadExportEffectRouteLayer = HttpRouter.add(
  ...
  Effect.gen(function* () {
    ...
    // app fetches cross-origin (modesto://app), and without them the browser masks
