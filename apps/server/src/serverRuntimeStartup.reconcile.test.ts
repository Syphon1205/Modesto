  ...
  return runReconciliation({
    ...
    Effect.tap(() =>
      Effect.sync(() => {
        ...
        // Every binding here carries a resume cursor, so each thread settles as
        // "stopped" (turn interrupted, thread resumable) rather than wearing an
        // error banner it would shrug off on the next message.
        assert.deepStrictEqual(
...
it.effect("errors only the orphans that have no resume state to come back from", () => {
  const resumable = makeThread("thread-resumable", "running", TurnId.make("turn-resumable"));
  const unresumable = makeThread("thread-unresumable", "running", TurnId.make("turn-unresumable"));
  const dispatched: OrchestrationCommand[] = [];

  return runReconciliation({
    threads: [resumable, unresumable],
    directory: {
      getBinding: (candidate) =>
        Effect.succeed(
          Option.some({
            ...
            providerInstanceId,
            status: "running" as const,
            ...(candidate === resumable.id ? { resumeCursor: { cursor: candidate } } : {}),
          }),
        ),
      upsert: () => Effect.void,
      getProvider: () => Effect.die("unused"),
    ...
    Effect.tap(() =>
      Effect.sync(() => {
        assert.deepStrictEqual(
          ...
          [
            { threadId: resumable.id, status: "stopped" as const },
            { threadId: unresumable.id, status: "error" as const },
          ],
        );
        const errored = dispatched.find(
          (command) => command.type === "thread.session.set" && command.threadId === unresumable.id,
        );
        assert.equal(
          errored?.type === "thread.session.set" ? errored.session.lastError : null,
          ServerRuntimeStartup.UNRECOVERABLE_PROVIDER_SESSION_ERROR,
        );
      }),
    ),
  );
