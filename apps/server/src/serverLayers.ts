...
import { CheckpointDiffBlobStoreLive } from "./persistence/Layers/CheckpointDiffBlobs";
import { IntegrationRepositoryLive } from "./persistence/Layers/IntegrationRepository";
...
export function makeServerRuntimeServicesLayer() {
  ...
  const incomingTaskRouterLayer = IncomingTaskRouterLive.pipe(
    Layer.provideMerge(IntegrationRepositoryLive),
    Layer.provideMerge(GitCoreLive),
    Layer.provideMerge(ServerSettingsLive),
    Layer.provideMerge(runtimeServicesLayer),
  );
  const openClawServiceLayer = OpenClawServiceLive.pipe(
    Layer.provideMerge(IntegrationRepositoryLive),
  ...
  const reviewServiceLayer = ReviewServiceLive.pipe(
    Layer.provideMerge(ReviewRepositoryLive),
    Layer.provideMerge(runtimeServicesLayer),
  );
