...
layer("GitHubPullRequestCli.layer", (it) => {
  ...
  it.effect("carries every repository and every qualifier into one search", () =>
    Effect.gen(function* () {
      ...
      yield* cli.searchPullRequests({
        cwd: "/w",
        host: "github.com",
        repositories: ["acme/web", "pingdotgg/modesto"],
        state: "closed",
        involvement: "reviewing",
        viewer: "bilal",
        limit: 10,
      ...
      assert.strictEqual(
        ...
        'is:pr is:closed is:unmerged review-requested:bilal "pull requests page" ' +
          "updated:<=2026-07-02T00:00:00Z sort:updated-desc repo:acme/web repo:pingdotgg/modesto",
      );
    }),
  );
    ...
    Effect.gen(function* () {
      mockedExecute.mockReturnValue(
        Effect.succeed(
          searchPage([
            ...
            searchItem(9, "pingdotgg/modesto", "2026-07-02T00:00:00Z"),
      ...
      assert.deepStrictEqual(
        ...
        [
          ...
          ["pingdotgg/modesto", 9, "https://avatars/octocat"],
        ],
      );
  ...
  it.effect('resolves assignee, mentions, and involves "me" the way notification filters do', () =>
    Effect.gen(function* () {
      mockedExecute.mockReturnValue(Effect.succeed(output("[]")));
      const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
      ...
      yield* cli.listPullRequests({
        ...
        viewer: "bilal",
        limit: 10,
        filters: { assignee: "me", mentions: "me", involves: "me" },
      });

      expect(searchOfCall(0)).toBe(
        'assignee:"bilal" mentions:"bilal" involves:"bilal" sort:updated-desc',
      );
    }),
  );
