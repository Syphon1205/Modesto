...
import { HostProcessEnvironment, HostProcessPlatform } from "@modesto/shared/hostProcess";
...
import {
  ...
  type DpopPublicJwk,
} from "@modesto/shared/dpop";
import { RELAY_HEALTH_REQUEST_TYP, RELAY_MINT_REQUEST_TYP } from "@modesto/shared/relayJwt";
...
import { IosSimulatorBackend } from "./device/IosSimulatorBackend.ts";
import { AutomationService } from "./automation/AutomationService.ts";
import { WebhookService } from "./automation/WebhookService.ts";
...
const buildAppUnderTest = (options?: {
  ...
  layers?: {
    ...
    folderPicker?: Partial<FolderPicker.FolderPicker["Service"]>;
    ...
    >;
    vercelService?: Partial<VercelService.VercelService["Service"]>;
      ...
      .pipe(
        ...
        Layer.provide(
          Layer.mergeAll(
            ...
            Layer.mock(FolderPicker.FolderPicker)({
              pickFolder: () => Effect.succeed(null),
              ...options?.layers?.folderPicker,
            }),
        ...
        Layer.provide(
          Layer.mock(VercelService.VercelService)({
            authStatus: () =>
              Effect.succeed({
                authenticated: false,
                username: null,
            ...
            clearToken: () => Effect.succeed({ cleared: true }),
            installMcp: () => Effect.succeed({ installed: true, name: "vercel" }),
            ...options?.layers?.vercelService,
          }),
        ),
    ...
    const appLayer = servedRoutesLayer.pipe(
      ...
      Layer.provide(
        Layer.succeed(DeviceService.DeviceService, {
          supported: false,
          manager: new DeviceManager({ backend: new IosSimulatorBackend({ platform: "linux" }) }),
        }),
      ),
      ...
      // The live service needs SqlClient, which this stack does not provide
      // here; these tests exercise the router seam, not automations.
      Layer.provideMerge(
        Layer.succeed(AutomationService, {
          list: () => Effect.succeed({ automations: [] }),
          create: () => Effect.die("unused automation create"),
          setState: () => Effect.die("unused automation setState"),
          runs: () => Effect.succeed([]),
        }),
      ),
      Layer.provideMerge(
        Layer.succeed(WebhookService, {
          list: () => Effect.succeed({ webhooks: [] }),
          create: () => Effect.die("unused webhook create"),
          setEnabled: () => Effect.die("unused webhook setEnabled"),
          delete: () => Effect.die("unused webhook delete"),
          admit: () => Effect.die("unused webhook admission"),
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  for (const desktopOrigin of ["modesto://app", "modesto-dev://app"]) {
      ...
      Effect.gen(function* () {
        ...
        const refreshStatus = vi.fn((_: string) =>
          Effect.succeed({
            ...
            hasPrimaryRemote: true,
            isDefaultRef: false,
            refName: "modesto/bootstrap-refName",
            hasWorkingTreeChanges: false,
            workingTree: {
              files: [],
              insertions: 0,
            ...
            client[ORCHESTRATION_WS_METHODS.dispatchCommand]({
              ...
              bootstrap: {
                ...
                prepareWorktree: {
                  ...
                  baseBranch: "main",
                  branch: "modesto/bootstrap-refName",
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          ...
          newRefName: "modesto/bootstrap-refName",
          baseRefName: "main",
          path: null,
        });
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const reportPath = yield* Config.string("MODESTO_TRANSFER_BUDGET_REPORT_PATH").pipe(
        Config.option,
      );
      ...
      const resultPath = yield* Config.string("MODESTO_TRANSFER_BUDGET_RESULT_PATH").pipe(
        Config.option,
      );
