...
import type { GitActionProgressEvent } from "@modesto/contracts";
import type {
  GitPullRequestCheck,
...
function makeManager(input?: {
  ...
  const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
    prefix: "modesto-git-manager-test-",
  });
...
const GitManagerTestLayer = GitCoreLive.pipe(
  Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "modesto-git-manager-test-" })),
  Layer.provideMerge(NodeServices.layer),
);
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const status = yield* manager.status({ cwd: repoDir });
        expect(status.branch).toBe("modesto/pr-488/statemachine");
        expect(status.pr).toEqual({
          number: 488,
          title: "Rebase this PR on latest main",
          url: "https://github.com/example-org/sample-repo/pull/488",
      ...
      const { manager } = yield* makeManager({
        ghScenario: {
          pullRequest: {
            ...
            url: "https://github.com/example-org/modesto/pull/642",
            baseRefName: "main",
