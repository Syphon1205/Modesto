...
import { DEFAULT_GIT_TEXT_GENERATION_MODEL } from "@modesto/contracts";
import { sanitizeGeneratedThreadTitle } from "@modesto/shared/chatThreads";
...
const makeCodexTextGeneration = Effect.gen(function* () {
  ...
  const writeTempFile = (
    ...
    const filePath = path.join(tempDir, `modesto-${prefix}-${process.pid}-${randomUUID()}.tmp`);
  ...
  const prepareIsolatedCodexHome = (
    ...
    Effect.gen(function* () {
      ...
      const isolatedHomePath = path.join(
        tempDir,
        `modesto-codex-home-${process.pid}-${randomUUID()}`,
      );
