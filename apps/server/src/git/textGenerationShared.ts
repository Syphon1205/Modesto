...
import { MAX_CHAT_THREAD_TITLE_WORDS } from "@modesto/shared/chatThreads";
...
export function buildThreadRecapPrompt(input: {
  ...
  return {
    prompt: [
      "You are writing a compact live recap for Modesto's chat side panel.",
...
export function buildAutomationIntentPrompt(input: {
  ...
  return {
    prompt: [
      "You extract structured Modesto automation creation intents.",
      ...
      "- Modesto automations run a saved prompt on a schedule.",
...
export function buildAutomationCompletionEvaluationPrompt(input: {
  ...
  return {
    prompt: [
      "You evaluate whether a completed Modesto heartbeat automation should stop.",
