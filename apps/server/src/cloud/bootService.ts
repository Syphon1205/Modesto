...
const BOOT_SERVICE_NAME = "modesto";
export const BOOT_SERVICE_UNIT_FILE = `${BOOT_SERVICE_NAME}.service`;
// `.service` suffix keeps the label distinct from the desktop app's bundle id
// (com.fabweavr.modesto), so launchd and TCC records never collide.
export const BOOT_SERVICE_LAUNCHD_LABEL = "com.fabweavr.modesto.service";
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=MODESTO_HOME=${quoteSystemdValue(plan.baseDir)}`,
...
export class BootServiceInstallError extends Schema.TaggedErrorClass<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the Modesto background service.";
  }
}
