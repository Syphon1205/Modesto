...
} from "@modesto/shared/connectAuth";
import { clerkFrontendApiUrlFromPublishableKey } from "@modesto/shared/relayAuth";
import { normalizeSecureRelayUrl } from "@modesto/shared/relayUrl";
...
declare const __MODESTO_BUILD_RELAY_URL__: string | undefined;
declare const __MODESTO_BUILD_CLERK_PUBLISHABLE_KEY__: string | undefined;
declare const __MODESTO_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: string | undefined;
declare const __MODESTO_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: string | undefined;
declare const __MODESTO_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: string | undefined;
declare const __MODESTO_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: string | undefined;
...
export const buildTimeRelayUrl =
  typeof __MODESTO_BUILD_RELAY_URL__ === "undefined"
    ? ""
    : (normalizeSecureRelayUrl(__MODESTO_BUILD_RELAY_URL__) ?? "");
export const buildTimeClerkPublishableKey = readBuildTimeValue(
  typeof __MODESTO_BUILD_CLERK_PUBLISHABLE_KEY__ === "undefined"
    ? undefined
    : __MODESTO_BUILD_CLERK_PUBLISHABLE_KEY__,
);
export const buildTimeClerkCliOAuthClientId = readBuildTimeValue(
  typeof __MODESTO_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__ === "undefined"
    ? undefined
    : __MODESTO_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__,
);
export const buildTimeRelayClientTracing = {
  tracesUrl: readBuildTimeValue(
    typeof __MODESTO_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__ === "undefined"
      ? undefined
      : __MODESTO_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__,
  ),
  tracesDataset: readBuildTimeValue(
    typeof __MODESTO_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__ === "undefined"
      ? undefined
      : __MODESTO_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__,
  ),
  tracesToken: readBuildTimeValue(
    typeof __MODESTO_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__ === "undefined"
      ? undefined
      : __MODESTO_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__,
  ),
} as const;
...
export function resolveRelayClientTracingConfig(
  ...
  const tracesUrl = env.MODESTO_RELAY_CLIENT_OTLP_TRACES_URL?.trim() || fallback.tracesUrl;
  const tracesDataset =
    env.MODESTO_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() || fallback.tracesDataset;
  const tracesToken = env.MODESTO_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() || fallback.tracesToken;
...
export function makeRelayUrlConfig(fallback = buildTimeRelayUrl) {
  const runtimeConfig = Config.nonEmptyString("MODESTO_RELAY_URL");
...
export function makeCloudCliOAuthConfig({
  ...
  return Config.all({
    clerkPublishableKey: makePublicValueConfig(
      "MODESTO_CLERK_PUBLISHABLE_KEY",
      clerkPublishableKeyFallback,
    ),
    clientId: makePublicValueConfig(
      "MODESTO_CLERK_CLI_OAUTH_CLIENT_ID",
...
export const hasCloudPublicConfig = Boolean(
  (normalizeSecureRelayUrl(process.env.MODESTO_RELAY_URL ?? "") ?? buildTimeRelayUrl) &&
  (process.env.MODESTO_CLERK_PUBLISHABLE_KEY?.trim() || buildTimeClerkPublishableKey) &&
  (process.env.MODESTO_CLERK_CLI_OAUTH_CLIENT_ID?.trim() || buildTimeClerkCliOAuthClientId),
);
