...
import type { ChatAttachment } from "@modesto/contracts";
...
const INITIAL_THREAD_TITLE_PROMPT = `Generate a title that will help the user recognize this Modesto thread weeks later.
...
function regenerateThreadTitlePrompt(previousTitle: string): string {
  return `Regenerate the title for an existing Modesto thread so the user can recognize it weeks later.
