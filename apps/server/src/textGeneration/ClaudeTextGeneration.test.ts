...
import { ClaudeSettings, ProviderInstanceId } from "@modesto/contracts";
import { isHostWindows } from "@modesto/shared/hostProcess";
...
const ClaudeTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "modesto-claude-text-generation-test-",
...
function withFakeClaudeEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "modesto-claude-text-" });
