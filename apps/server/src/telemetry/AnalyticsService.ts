 ...
 */
import { HostProcessArchitecture, HostProcessPlatform } from "@modesto/shared/hostProcess";
...
const TelemetryEnvConfig = Config.all({
  posthogKey: Config.string("MODESTO_POSTHOG_KEY").pipe(
    Config.withDefault("phc_XOWci4oZP4VvLiEyrFqkFjP4CZn55mjYYBMREK5Wd6m"),
  ),
  posthogHost: Config.string("MODESTO_POSTHOG_HOST").pipe(
    Config.withDefault("https://us.i.posthog.com"),
  ),
  enabled: Config.boolean("MODESTO_TELEMETRY_ENABLED").pipe(Config.withDefault(true)),
  flushBatchSize: Config.number("MODESTO_TELEMETRY_FLUSH_BATCH_SIZE").pipe(Config.withDefault(20)),
  maxBufferedEvents: Config.number("MODESTO_TELEMETRY_MAX_BUFFERED_EVENTS").pipe(
    ...
    const payload = {
      ...
      batch: events.map((event) => ({
        ...
        properties: {
          ...
          modestoVersion: packageJson.version,
          clientType,
        },
