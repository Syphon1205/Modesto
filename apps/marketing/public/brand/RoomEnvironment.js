/*
 * three.js RoomEnvironment (MIT), vendored for the marketing site.
 *
 * Identical to `/announcements/modesto-saas-assets/vendor/RoomEnvironment.js`
 * except that the bare "three" specifier is rewritten to the explicit vendor
 * path. The original needs an <script type="importmap"> on every page that
 * loads it; that map is JSON inside a <script> tag, which the linter parses as
 * JavaScript and rejects. Resolving the path here removes the import map — and
 * the per-page setup it required — entirely.
 */
import {
  BackSide,
  BoxGeometry,
  ...
  Scene,
  Object3D,
} from "/announcements/modesto-saas-assets/vendor/three.module.min.js";
