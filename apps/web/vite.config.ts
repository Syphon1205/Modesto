...
import { DEV_PROXIED_PATH_PREFIXES } from "@modesto/shared/devProxy";
...
const isSingleOriginDev = process.env.MODESTO_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_MODESTO_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.MODESTO_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: MODESTO_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); MODESTO_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.MODESTO_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // MODESTO_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
  const port = Number(backendPort?.trim());
  if (Number.isInteger(port) && port > 0) {
    // The backend binds explicitly to 127.0.0.1 (see apps/server/src/bin.ts),
    // not "localhost" - on hosts where "localhost" resolves to the IPv6
    // loopback (::1) first, proxying to "localhost" here connects to nothing
    // and every request fails with ECONNREFUSED even though the backend is
    // up. Target the literal bind address instead.
...
const devProxyTarget = resolveDevProxyTarget(process.env.MODESTO_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.MODESTO_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_MODESTO_RELAY_URL": JSON.stringify(configuredRelayUrl),
