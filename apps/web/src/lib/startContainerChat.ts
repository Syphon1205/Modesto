...
import type { NewThreadNavigationOptions } from "../hooks/useHandleNewThread";
...
export async function startContainerChat(input: {
  ...
  readonly navigation?: NewThreadNavigationOptions;
  ...
  try {
    ...
    const threadId = await input.handleNewThread(projectId, threadOptions, input.navigation);
