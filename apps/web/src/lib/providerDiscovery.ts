...
import { resolveThreadBranchSourceCwd } from "@modesto/shared/threadEnvironment";
import type {
  ProviderKind,
...
export type ProviderDiscoveryTab = "plugins" | "skills";
export type ProviderDiscoveryCapabilities = { plugins: boolean; skills: boolean };

export function supportsProviderDiscoveryTab(options: {
  provider: ProviderKind;
  tab: ProviderDiscoveryTab;
  capabilities: ProviderDiscoveryCapabilities;
}): boolean {
  if (options.tab === "plugins") {
    return options.capabilities.plugins;
  }

  // Codex marketplace plugins can bundle installable skills even when no
  // workspace is available for scanning provider-local skill directories.
  return (
    options.capabilities.skills || (options.provider === "codex" && options.capabilities.plugins)
  );
}
...
export function isInstalledProviderPlugin(
  ...
  // Codex can report an available marketplace entry as enabled even before it
  // is installed. Installation state is authoritative; the policy covers
  // bundled plugins that Codex installs implicitly.
