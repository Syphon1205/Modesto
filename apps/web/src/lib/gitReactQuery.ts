...
export const GITHUB_DEVICE_SIGN_IN_URL = "https://github.com/login/device";

interface GitHubSignInApi {
  readonly git: Pick<NativeApi["git"], "githubSignIn">;
  readonly shell: Pick<NativeApi["shell"], "openExternal">;
}

export async function startGitHubSignIn(
  api: GitHubSignInApi = ensureNativeApi(),
): Promise<Awaited<ReturnType<NativeApi["git"]["githubSignIn"]>>> {
  const result = await api.git.githubSignIn({});
  if (result.userCode) {
    await copyTextToClipboard(result.userCode).catch(() => undefined);
  }
  await api.shell.openExternal(GITHUB_DEVICE_SIGN_IN_URL);
  return result;
}

export function githubDeviceSignInDescription(userCode: string | null): string {
  return userCode
    ? `Enter code ${userCode} on GitHub. It has also been copied to your clipboard.`
    : "Complete the device authorization flow in the GitHub browser window.";
}

export const gitQueryKeys = {
  all: ["git"] as const,
  githubAuth: () => ["git", "github-auth"] as const,
...
export function gitGithubAuthQueryOptions(input: { refetchInterval?: number | false } = {}) {
  return queryOptions({
    queryKey: gitQueryKeys.githubAuth(),
...
export function gitGithubSignInMutationOptions() {
  return mutationOptions({
    mutationKey: ["git", "mutation", "github-sign-in"] as const,
    mutationFn: () => startGitHubSignIn(),
  });
}

export function gitGithubSignOutMutationOptions(input: {
  queryClient: QueryClient;
  onSuccess?: () => void;
}) {
  return mutationOptions({
    mutationKey: ["git", "mutation", "github-sign-out"] as const,
    mutationFn: () => ensureNativeApi().git.githubSignOut({}),
    onSuccess: async () => {
      await Promise.all([
        input.queryClient.invalidateQueries({ queryKey: gitQueryKeys.githubAuth() }),
        input.queryClient.invalidateQueries({
          queryKey: ["server", "profileCommitActivity"] as const,
        }),
      ]);
      input.onSuccess?.();
    },
