...
import { isSupportedLocalPreviewFilePath } from "@modesto/shared/localPreviewFiles";
import {
  isLocalAbsolutePath,
...
const MODESTO_PUBLIC_ASSET_PATH_PREFIXES = [
  "/central-icons-reversed/",
  "/central-icons-fill/",
] as const;
const MODESTO_WEB_PUBLIC_WORKSPACE_DIR = "apps/web/public";

function resolveModestoPublicAssetOpenTarget(path: string, workspaceRoot: string | null) {
  if (!workspaceRoot) {
    return null;
  }
  const normalizedPath = path.replace(/\\/g, "/");
  if (!MODESTO_PUBLIC_ASSET_PATH_PREFIXES.some((prefix) => normalizedPath.startsWith(prefix))) {
    return null;
  }
  const relativePath = `${MODESTO_WEB_PUBLIC_WORKSPACE_DIR}${normalizedPath}`;
...
export function resolveWorkspaceFileOpenTarget(
  ...
  return resolveModestoPublicAssetOpenTarget(withoutPosition, workspaceRoot);
}
