...
describe("openPullRequestLink", () => {
  it("opens the requested pull request URL", async () => {
    ...
    const targetUrl = "https://github.com/pingdotgg/modesto/pull/123";
  ...
  it("reports bridge failures with a safe target origin", async () => {
    ...
    const targetUrl = "https://github.com/pingdotgg/modesto/pull/123?token=secret";
...
describe("parseChangeRequestUrl", () => {
  it("reads a GitHub pull request", () => {
    expect(parseChangeRequestUrl("https://github.com/octocat/example-repo/pull/123")).toEqual({
      host: "github.com",
  ...
  it("reads a GitLab merge request, nested groups and all", () => {
    expect(
      parseChangeRequestUrl("https://gitlab.com/octocat/platform/example-repo/-/merge_requests/42"),
    ).toEqual({
      host: "gitlab.com",
      repository: "octocat/platform/example-repo",
      number: 42,
    });
  });
  ...
  it("reads both Azure DevOps URL forms, keeping `_git` in the repository path", () => {
    expect(
      parseChangeRequestUrl("https://dev.azure.com/acme/platform/_git/modesto/pullrequest/17"),
    ).toEqual({
      host: "dev.azure.com",
      repository: "acme/platform/_git/modesto",
      number: 17,
    });
    expect(
      parseChangeRequestUrl("https://acme.visualstudio.com/platform/_git/modesto/pullrequest/17"),
    ).toEqual({
      host: "acme.visualstudio.com",
      repository: "platform/_git/modesto",
      number: 17,
    });
  });
  ...
  it("survives trailing segments, a trailing slash and a query string", () => {
    expect(
      parseChangeRequestUrl("https://github.com/octocat/example-repo/pull/123/files?w=1"),
    ).toEqual({
      host: "github.com",
      repository: "octocat/example-repo",
      number: 123,
  ...
  it("claims nothing it cannot be sure of, so the link goes to the browser", () => {
    for (const link of [
      "https://github.com/octocat/example-repo/issues/123",
      "https://github.com/octocat/example-repo/commit/0a1b2c3",
      "https://github.com/octocat/example-repo",
      "https://github.com/octocat/example-repo/pull/abc",
      "https://gitlab.com/octocat/example-repo/-/snippets/12",
      "https://gitlab.com/octocat/example-repo/-/issues/12",
      ...
      "javascript:alert(1)//github.com/octocat/example-repo/pull/1",
      "not a url",
    ]) {
...
describe("findProjectForChangeRequest", () => {
  ...
  it("matches a nested GitLab group by the whole path below the host", () => {
    ...
    // two-segment owner/name form would look for `octocat/example-repo` and find nothing.
    const projects = [
      project({
        canonicalKey: "gitlab.com/octocat/platform/example-repo",
        provider: "gitlab",
        displayName: "octocat/platform/example-repo",
        owner: "octocat",
        name: "modesto",
      }),
    ];
  ...
  it("keeps two hosts apart, so an Enterprise link does not open the public one", () => {
    const projects = [
      project({
        canonicalKey: "github.com/pingdotgg/modesto",
        provider: "github",
        owner: "pingdotgg",
        name: "modesto",
      }),
