  ...
  return queryOptions({
    ...
    queryFn: async () => {
      ...
      return api.provider.listPlugins({
        ...
        ...(input.cwd ? { cwd: input.cwd } : {}),
        ...(input.threadId ? { threadId: input.threadId } : {}),
        ...(input.forceRemoteSync ? { forceRemoteSync: true, forceReload: true } : {}),
      });
    },
    enabled: input.enabled ?? true,
    staleTime: input.forceRemoteSync ? 0 : 30_000,
    ...(input.forceRemoteSync ? { refetchOnMount: "always" as const } : {}),
