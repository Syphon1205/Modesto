...
import { DEFAULT_AUTOMATION_FAST_INTERVAL_MAX_ITERATIONS } from "@modesto/contracts";
import type {
  AutomationMode,
  AutomationSchedule,
...
export function buildAutomationDraftWarnings(input: {
    ...
    warnings.push({
      ...
      detail:
        input.worktreeMode === "auto"
          ? "If Modesto cannot create a worktree, runs may fall back to editing the active project checkout."
  ...
  if (
    ...
    warnings.push({
      ...
      detail: "Modesto was not fully confident about the parsed automation fields.",
      requiresAcknowledgement: false,
    });
  }
