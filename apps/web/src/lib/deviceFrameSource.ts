...
// Depends on: @modesto/shared/deviceFrame for the binary envelope, wsTransport for URL resolution
...
import {
  ...
  decodeDeviceFrame,
  type DeviceFrame,
} from "@modesto/shared/deviceFrame";
import { isLoopbackHost } from "@modesto/shared/preview";
 ...
 *
 * The prepared connection often points at `127.0.0.1` while the browser page is
 * on `localhost` (Vite proxy). Session cookies are host-bound, so a frame socket
 * opened against the other loopback name arrives unauthenticated and closes
 * immediately. Keep ticket query params from the prepared URL, but ride the
 * page origin whenever both ends are loopback so the cookie (and the proxy)
 * stay in play.
 */
export function deviceFrameSocketUrl(input: {
  ...
  const fallback = `${pageProtocol}//${window.location.host}/ws`;
  const url = new URL(input.explicitUrl ?? fallback);
