...
const APP_SETTINGS_STORAGE_KEY = "modesto:app-settings:v1";
const SERVER_SETTINGS_MIGRATION_STORAGE_KEY = "modesto:server-settings-migrated:v1";
...
export const TerminalTheme = Schema.Literals([
  "modesto",
  "vscode-dark",
  "vscode-light",
...
export const DEFAULT_TERMINAL_THEME: TerminalTheme = "modesto";
...
export const ReviewDefaultScope = Schema.Literals([
  "currentFile",
  "uncommittedChanges",
  "selectedFiles",
  "repository",
]);
export type ReviewDefaultScope = typeof ReviewDefaultScope.Type;
export const ReviewDisplaySeverity = Schema.Literals([
  "critical",
  "warning",
  "suggestion",
  "informational",
]);
export type ReviewDisplaySeverity = typeof ReviewDisplaySeverity.Type;

const PersistedReviewProvider = Schema.String.pipe(
  Schema.decodeTo(ReviewProvider, {
    decode: SchemaGetter.transform((provider) =>
      provider === "coderabbit" || provider === "greptile" ? provider : "modesto",
    ),
    encode: SchemaGetter.transform((provider) => provider),
  }),
);

const PersistedReviewRuntime = ProviderKind.pipe(
  Schema.decodeTo(ReviewRuntime, {
    decode: SchemaGetter.transform((runtime) => (runtime === "cursor" ? "cursor" : "codex")),
    encode: SchemaGetter.transform((runtime) => runtime),
  }),
);
...
export const AppSettingsSchema = Schema.Struct({
  ...
  terminalTheme: TerminalTheme.pipe(withDefaults(() => DEFAULT_TERMINAL_THEME)),
  ...
  reviewProvider: PersistedReviewProvider.pipe(withDefaults(() => "modesto" as const)),
  modestoReviewRuntime: PersistedReviewRuntime.pipe(withDefaults(() => "codex" as const)),
  modestoReviewModel: Schema.String.check(Schema.isMaxLength(256)).pipe(withDefaults(() => "")),
  modestoReviewDepth: ReviewDepth.pipe(withDefaults(() => "standard" as const)),
  modestoReviewScope: ReviewDefaultScope.pipe(withDefaults(() => "uncommittedChanges" as const)),
  reviewMinimumSeverity: ReviewDisplaySeverity.pipe(withDefaults(() => "suggestion" as const)),
  reviewIncludeSecurity: Schema.Boolean.pipe(withDefaults(() => true)),
  reviewIncludePerformance: Schema.Boolean.pipe(withDefaults(() => true)),
  reviewIncludeArchitecture: Schema.Boolean.pipe(withDefaults(() => true)),
  reviewIncludeTestCoverage: Schema.Boolean.pipe(withDefaults(() => true)),
  reviewAllowFixSuggestions: Schema.Boolean.pipe(withDefaults(() => true)),
  reviewInstructionFiles: Schema.String.check(Schema.isMaxLength(4_096)).pipe(
    withDefaults(() => "AGENTS.md\nCONTRIBUTING.md"),
  ),
  // Local-only sidebar preferences. The Studio/Workspace values remain in the
  // persisted schema solely so older settings payloads continue to decode; those
  // legacy tabs are no longer rendered.
...
// Overlays a thread's selected custom Codex endpoint (chosen in the model picker,
// carried on ModelSelection.options) onto the process-level ProviderStartOptions
// sent with `providers.startSession` — that's the payload codexAppServerManager
// reads to decide which `-c model_provider=` override to spawn the session with.
export function withCodexCustomEndpointOverride(
  providerOptions: ProviderStartOptions | undefined,
  modelSelection: ModelSelection | null | undefined,
): ProviderStartOptions | undefined {
  const customEndpointId =
    modelSelection?.provider === "codex" ? modelSelection.options?.customEndpointId : undefined;
  if (!customEndpointId) {
