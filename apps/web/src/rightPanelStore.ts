...
// v15 re-runs migrate so an empty launcher (no surfaces) starts closed; click the
// right-sidebar toggle or rail to open it, matching Cursor.
...
const singletonSurface = (
  ...
  switch (kind) {
    ...
    case "music":
      return { id: "music", kind };
    case "slides":
      return { id: "slides", kind };
    case "docs":
      return { id: "docs", kind };
    case "sheets":
      return { id: "sheets", kind };
    case "canvas":
      ...
      ? Object.fromEntries(
          Object.entries(persistedState.byThreadKey as Record<string, ThreadRightPanelState>)
            ...
            .map(([threadKey, threadState]) => {
              ...
              const uniqueSurfaces = surfaces.filter((surface) => {
                if (seenSurfaceIds.has(surface.id)) return false;
                seenSurfaceIds.add(surface.id);
                return true;
              });
              const surfacesForThread = uniqueSurfaces;
              const rawActiveSurfaceId =
                validThreadState?.activeSurfaceId === "slides" ||
                validThreadState?.activeSurfaceId === "docs" ||
                validThreadState?.activeSurfaceId === "sheets"
                  ? "canvas"
                  : validThreadState?.activeSurfaceId;
              const persistedActiveSurfaceId = surfacesForThread.some(
                (surface) => surface.id === rawActiveSurfaceId,
              )
                ? (rawActiveSurfaceId ?? null)
                : rawActiveSurfaceId === "pull-request"
                  ? (surfacesForThread.find((surface) => surface.kind === "pull-request")?.id ??
                    null)
                  : null;
              // A migration that dropped every surface (e.g. plan-only panels
              // in v9) must not reopen an empty panel.
              const isOpen =
                surfacesForThread.length > 0 &&
              ...
              const activeSurfaceId =
                persistedActiveSurfaceId ?? (isOpen ? (surfacesForThread[0]?.id ?? null) : null);
              return [threadKey, { isOpen, surfaces: surfacesForThread, activeSurfaceId }];
            }),
        )
      : {};
  return { byThreadKey };
