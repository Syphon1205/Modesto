...
describe("AppSettingsSchema", () => {
  it("adds safe review defaults to settings created before code review existed", () => {
    const decode = Schema.decodeSync(Schema.fromJsonString(AppSettingsSchema));

    expect(decode("{}")).toMatchObject({
      reviewProvider: "modesto",
      modestoReviewRuntime: "codex",
      modestoReviewDepth: "standard",
      modestoReviewScope: "uncommittedChanges",
      reviewMinimumSeverity: "suggestion",
      reviewIncludeSecurity: true,
      reviewIncludePerformance: true,
      reviewIncludeArchitecture: true,
      reviewIncludeTestCoverage: true,
      reviewAllowFixSuggestions: true,
      reviewInstructionFiles: "AGENTS.md\nCONTRIBUTING.md",
    });
  });

  it("migrates legacy review providers and runtimes to Modesto Review", () => {
    const decode = Schema.decodeSync(Schema.fromJsonString(AppSettingsSchema));

    expect(
      decode(
        JSON.stringify({
          reviewProvider: "retired-provider",
          modestoReviewRuntime: "claudeAgent",
        }),
      ),
    ).toMatchObject({
      reviewProvider: "modesto",
