...
describe("rightPanelStore", () => {
  ...
  it("opens a singleton music surface", () => {
    useRightPanelStore.getState().open(refA, "music");
    expect(selectActiveRightPanel(useRightPanelStore.getState().byThreadKey, refA)).toBe("music");
    useRightPanelStore.getState().open(refA, "music");
    expect(
      selectThreadRightPanelState(useRightPanelStore.getState().byThreadKey, refA).surfaces,
    ).toHaveLength(1);
  ...
  it("opens singleton docs and sheets surfaces", () => {
    useRightPanelStore.getState().open(refA, "docs");
    expect(selectActiveRightPanel(useRightPanelStore.getState().byThreadKey, refA)).toBe("docs");
    useRightPanelStore.getState().open(refA, "sheets");
    expect(selectActiveRightPanel(useRightPanelStore.getState().byThreadKey, refA)).toBe("sheets");
    expect(
  ...
  it("opens a singleton canvas surface", () => {
    useRightPanelStore.getState().open(refA, "canvas");
    expect(selectActiveRightPanel(useRightPanelStore.getState().byThreadKey, refA)).toBe("canvas");
    useRightPanelStore.getState().open(refA, "canvas");
    expect(
      selectThreadRightPanelState(useRightPanelStore.getState().byThreadKey, refA).surfaces,
    ).toHaveLength(1);
  ...
  it("opens a singleton device surface", () => {
  ...
  it("tracks one surface per pull request", () => {
    const first = { projectId: "project-a", repository: "pingdotgg/modesto", number: 4909 };
    const second = { projectId: "project-a", repository: "pingdotgg/modesto", number: 4910 };
  ...
  describe("updatePullRequestTabStatus", () => {
    ...
    it("keys a status under the same id a thread surface with no environment carries", () => {
      const target = { projectId: "project-a", repository: "pingdotgg/modesto", number: 4909 };
      useRightPanelStore.getState().openPullRequest(refA, target);
      const surface = selectSelectedRightPanelSurface(
        useRightPanelStore.getState().byThreadKey,
        refA,
