...
describe("hydrateImagesFromPersisted", () => {
  it("keeps svg and photoshop as files even when the mime starts with image/", () => {
    const svgPayload = btoa("<svg xmlns='http://www.w3.org/2000/svg'></svg>");
    const [svg] = hydrateImagesFromPersisted([
      {
        id: "att-svg",
        name: "mark.svg",
    ...
    expect(svg?.previewUrl).toBe("");

    const [psd] = hydrateImagesFromPersisted([
      {
        id: "att-psd",
        name: "poster.psd",
        mimeType: "image/vnd.adobe.photoshop",
        sizeBytes: 4,
        dataUrl: "data:image/vnd.adobe.photoshop;base64,AAAA",
      },
    ]);
    expect(psd?.type).toBe("file");
  });
});
...
describe("composerDraftStore clearComposerContent", () => {
  ...
  it("keeps chat mode after a message clears the composer", () => {
    const store = useComposerDraftStore.getState();
    store.setConversationMode(threadRef, "chat");
    store.setPrompt(threadRef, "Hello");

    store.clearComposerContent(threadRef);

    expect(draftFor(threadId, TEST_ENVIRONMENT_ID)?.conversationMode).toBe("chat");
  });

  it("persists chat mode but treats code as the removable default", () => {
    const store = useComposerDraftStore.getState();
    store.setConversationMode(threadRef, "chat");
    ...
    const persisted = persistApi.getOptions().partialize(useComposerDraftStore.getState()) as {
      draftsByThreadKey?: Record<string, { conversationMode?: string }>;
    };

    expect(
      persisted.draftsByThreadKey?.[threadKeyFor(threadId, TEST_ENVIRONMENT_ID)]?.conversationMode,
    ).toBe("chat");

    useComposerDraftStore.getState().setConversationMode(threadRef, "code");
    expect(draftFor(threadId, TEST_ENVIRONMENT_ID)).toBeUndefined();
  });

  it("keeps a New Chat landing selectable after switching to Work", () => {
    const store = useComposerDraftStore.getState();
    store.setConversationMode(threadRef, "chat", { selectable: true });
    store.setConversationMode(threadRef, "code");

    const draft = draftFor(threadId, TEST_ENVIRONMENT_ID);
    expect(draft?.conversationMode).toBe("code");
    expect(draft?.conversationModeSelectable).toBe(true);
    ...
    const persisted = persistApi.getOptions().partialize(useComposerDraftStore.getState()) as {
      draftsByThreadKey?: Record<
        string,
        { conversationMode?: string; conversationModeSelectable?: boolean }
      >;
    };
    expect(
...
describe("composerDraftStore appshot contexts", () => {
  const threadId = ThreadId.make("thread-appshot");
  const threadRef = scopeThreadRef(TEST_ENVIRONMENT_ID, threadId);
  const baseSelection = {
    appName: "Safari",
  ...
  it("adds an appshot context and stamps id + threadId", () => {
    useComposerDraftStore.getState().addAppshotContext(threadRef, baseSelection);
    const draft = draftFor(threadId, TEST_ENVIRONMENT_ID);
    expect(draft?.appshotContexts).toHaveLength(1);
    const entry = draft?.appshotContexts[0]!;
    expect(entry.id.startsWith("appshot_")).toBe(true);
    expect(entry.threadId).toBe(threadId);
    expect(entry.appName).toBe("Safari");
    expect(entry.accessibilityText).toBe("fetch(url)");
  });

  it("removeAppshotContext drops by id", () => {
    const store = useComposerDraftStore.getState();
    store.addAppshotContext(threadRef, baseSelection);
    store.addAppshotContext(threadRef, { ...baseSelection, windowTitle: "Other" });
    const ids = draftFor(threadId, TEST_ENVIRONMENT_ID)!.appshotContexts.map((c) => c.id);
    store.removeAppshotContext(threadRef, ids[0]!);
    const remaining = draftFor(threadId, TEST_ENVIRONMENT_ID)?.appshotContexts;
    expect(remaining?.map((c) => c.id)).toEqual([ids[1]]);
  });

  it("clearComposerContent wipes appshot contexts", () => {
    const store = useComposerDraftStore.getState();
    store.addAppshotContext(threadRef, baseSelection);
    store.clearComposerContent(threadRef);
    expect(draftFor(threadId, TEST_ENVIRONMENT_ID)).toBeUndefined();
