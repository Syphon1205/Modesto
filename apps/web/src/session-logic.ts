...
import { isBackgroundTaskActivity } from "@modesto/client-runtime/state/subagentRuntime";
import {
  ApprovalRequestId,
...
/** True while a turn has started but not yet settled — the "actively running" signal. */
 ...
 * that raised them can still receive the answer. Once the session has stopped or
...
export function deriveWorkLogEntries(
  ...
  for (const activity of ordered) {
    ...
    if (activity.kind === "thread.core-context") {
      const reason =
        activity.payload && typeof activity.payload === "object"
          ? (activity.payload as { reason?: unknown }).reason
          : undefined;
      if (reason === "turn") continue;
    }
