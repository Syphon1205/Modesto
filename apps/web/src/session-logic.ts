...
} from "@modesto/shared/subagents";
import { summarizeToolRawOutput } from "@modesto/shared/toolOutputSummary";
...
} from "./components/chat/activityTimelineAdapters";
...
export const PROVIDER_OPTIONS: Array<{
  ...
  { value: "poolside", label: "Poolside", available: true },
...
export interface ThreadActivitySummary {
  /** Short label for the most recent real work-log entry, e.g. "Searching the web for…". */
  readonly activityLabel: string | null;
  readonly activityTone: WorkLogEntry["tone"] | null;
  /** Completed/total percent from the current turn's task list; null when no task
   * list has been reported yet (not "0%"). */
  readonly taskProgressPercent: number | null;
}

/** Shared "what is this thread doing right now" derivation used by any live task
 * card (Teams/Research launcher cards, Kanban cards) — single source so every
 * surface reads the same activity label and progress figure from a thread's
 * activities. Cheap enough to call per visible card, but callers should still
 * avoid it for idle/done threads where there is nothing to show. */
export function deriveThreadActivitySummary(
  activities: ReadonlyArray<OrchestrationThreadActivity>,
  latestTurnId: TurnId | undefined,
): ThreadActivitySummary {
  const workLog = deriveWorkLogEntries(activities, latestTurnId);
  const lastEntry = workLog[workLog.length - 1];
  const activeTaskList = deriveActiveTaskListState(activities, latestTurnId);
  const taskProgressPercent =
    activeTaskList && activeTaskList.tasks.length > 0
      ...
      : null;
  return {
    activityLabel: lastEntry?.toolTitle ?? lastEntry?.label ?? null,
    activityTone: lastEntry?.tone ?? null,
    taskProgressPercent,
  };
}
...
function shouldKeepActivityForWorkLog(
  ...
  if (activity.kind === "automation.created" || isUnifiedThreadLevelActivity(activity.kind)) {
    return true;
  }
...
function toDerivedWorkLogEntry(activity: OrchestrationThreadActivity): DerivedWorkLogEntry {
  ...
  const adaptedActivity = adaptUnifiedTimelineActivity(activity);
  const entry: DerivedWorkLogEntry = {
    id: activity.id,
    createdAt: activity.createdAt,
    ...(activity.turnId !== null ? { turnId: activity.turnId } : {}),
    label: adaptedActivity?.label ?? activity.summary,
    tone: adaptedActivity?.tone ?? (activity.tone === "approval" ? "info" : activity.tone),
    activityKind: activity.kind,
    ...(toolName ? { toolName } : {}),
    ...(toolCallId ? { toolCallId } : {}),
  };
  if (adaptedActivity?.detail) {
    entry.detail = adaptedActivity.detail;
  }
