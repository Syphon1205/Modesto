...
import { settlePromise, squashAtomCommandFailure } from "@modesto/client-runtime/state/runtime";
import { canSettle, canSnooze, threadWokeAt } from "@modesto/client-runtime/state/thread-settled";
import { EnvironmentId, type ScopedThreadRef, ThreadId } from "@modesto/contracts";
  ...
  const confirmAndDeleteThread = useCallback(
    async (target: ScopedThreadRef) => {
      ...
      if (confirmThreadDelete && localApi) {
        const isChat = resolved?.thread.conversationMode === "chat";
        const noun = isChat ? "chat" : "thread";
        const title = resolved?.thread.title ?? `this ${noun}`;
        const confirmationResult = await settlePromise(() =>
          localApi.dialogs.confirm(
            [
              `Delete ${noun} "${title}"?`,
