...
describe("activateThreadFromSidebarIntent", () => {
  beforeEach(() => {
    useCoworkThreadStore.setState({ coworkThreadIds: [], threadKindById: {} });
  });

  it("always reopens Research threads inside the Research conversation surface", () => {
    useCoworkThreadStore.getState().markCoworkThread(THREAD_B, "research");
