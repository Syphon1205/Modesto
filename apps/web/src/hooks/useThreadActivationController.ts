...
import { isResearchThreadId, useCoworkThreadStore } from "../coworkThreadStore";
...
export function activateThreadFromSidebarIntent(
  ...
  if (isResearchThreadId(useCoworkThreadStore.getState().threadKindById, threadId)) {
    activateResearchThread(input, threadId);
    return;
  }
...
function activateResearchThread(input: ThreadActivationControllerInput, threadId: ThreadId): void {
