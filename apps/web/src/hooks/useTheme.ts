import type { DesktopBridge } from "@modesto/contracts";
...
import { playThemeColorWash, startThemeWashPointerTracking, themeWashColors } from "../themeWash";
...
const STORAGE_KEY = "modesto:theme";
...
function playUserThemeWash(theme: Theme) {
  ...
  playThemeColorWash(themeWashColors(resolveThemeHalf(theme, themeHalves, appearance), appearance));
}
...
export function useTheme() {
  ...
  const setAppearanceMode = useCallback((nextAppearanceMode: ThemePreferenceMode): boolean => {
    if (typeof window === "undefined") return false;
    const previousMode = readAppearanceModePreference(getStored());
    try {
    ...
    if (previousMode !== nextAppearanceMode) playUserThemeWash(nextTheme);
