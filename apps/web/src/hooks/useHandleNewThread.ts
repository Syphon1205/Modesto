...
export interface NewThreadNavigationOptions {
  /** Route the new draft into a non-Code conversation surface. */
  surface?: "code" | "research";
  /** Runs after the draft is registered and before route navigation starts. */
  ...
  const handleNewThread = useCallback(
    (
      ...
      const navigateToThread = (threadId: ThreadId) => {
        navigation?.onThreadReady?.(threadId);
        if (navigation?.surface === "research") {
          return navigate({
            to: "/research",
            search: { threadId },
          });
      ...
      return runDraftNavigationOnce(draftNavigationSlotKey(projectId, entryPoint), async () => {
        ...
        const committed = await stageDraftNavigation({
          ...
          navigate: () => navigateToThread(threadId),
          ...
          isDestinationActive: () =>
            navigation?.surface === "research"
              ? router.state.location.pathname === "/research" &&
                (router.state.location.search as Record<string, unknown>).threadId === threadId
              : router.state.location.pathname === `/${threadId}`,
