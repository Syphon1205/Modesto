...
export const MODESTO_WS_TRANSPORT_STATE_EVENT = "modesto:ws-transport-state";
...
export function emitWsTransportState(state: WsTransportState): void {
  ...
  window.dispatchEvent(
    new CustomEvent<WsTransportStateEventDetail>(MODESTO_WS_TRANSPORT_STATE_EVENT, {
      detail: { state },
    }),
  );
}
...
export function addWsTransportStateListener(
  ...
  window.addEventListener(MODESTO_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  return () => {
    window.removeEventListener(MODESTO_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  };
}
