...
import { ProjectId, ThreadId, TurnId } from "@modesto/contracts";
...
describe("splitViewStore", () => {
  ...
  it("keeps writing split views to the Modesto v1 storage key so persisted state can migrate", async () => {
    vi.resetModules();
    globalThis.localStorage = createMemoryStorage();
    ...
    const persisted = globalThis.localStorage.getItem("modesto:split-view-state:v1");
    expect(persisted).not.toBeNull();
    expect(globalThis.localStorage.getItem("modesto:split-view-state:v2")).toBeNull();
