...
export interface ProviderModelOption {
  ...
  // Set only for Settings → Model Routers custom endpoint entries (codex only
  // today) — carried into ModelSelection.options so the session spawns against
  // that endpoint instead of the default provider.
...
/**
 * Settings → Model Routers: each configured custom endpoint (self-hosted local
 * server, or a router like OpenRouter/LiteLLM) becomes one selectable Codex
 * entry per model it declares, or a single fallback entry keyed by the
 * endpoint id so it's still pickable before the user adds explicit model
 * names. Every surface with a Codex model picker must apply this — it's not
 * baked into codexOptions itself since custom endpoints are runtime/user
 * config, not part of the static model list.
 */
export function applyCustomModelEndpoints(
  codexOptions: ReadonlyArray<ProviderModelOption & { isCustom?: boolean }>,
  customModelEndpoints: ReadonlyArray<CustomModelEndpointStatus>,
): ReadonlyArray<ProviderModelOption & { isCustom?: boolean }> {
  if (customModelEndpoints.length === 0) {
    return codexOptions;
  }
  const customEndpointOptions: ProviderModelOption[] = customModelEndpoints.flatMap((endpoint) =>
    (endpoint.models.length > 0 ? endpoint.models : [endpoint.id]).map((modelSlug) => ({
      slug: modelSlug,
      name: endpoint.models.length > 0 ? modelSlug : endpoint.label,
      upstreamProviderId: endpoint.id,
      upstreamProviderName: endpoint.label,
      customEndpointId: endpoint.id,
    })),
  );
  return [...codexOptions, ...customEndpointOptions];
}
