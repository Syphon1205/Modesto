...
const openClawEventListeners = new Set<(payload: OpenClawStreamEvent) => void>();
const reviewEventListeners = new Set<(payload: ReviewStreamEvent) => void>();
...
export function createWsNativeApi(): NativeApi {
  ...
  transport.subscribe(WS_CHANNELS.openClawEvent, (message) => {
    for (const listener of openClawEventListeners) {
      try {
  ...
  transport.subscribe(WS_CHANNELS.reviewEvent, (message) => {
    for (const listener of reviewEventListeners) {
      try {
        listener(message.data);
      } catch {
        // Swallow listener errors
  ...
  const api: NativeApi = {
    ...
    repoMemory: {
      list: (input) => transport.request(WS_METHODS.repoMemoryList, input),
      suggest: (input) => transport.request(WS_METHODS.repoMemorySuggest, input),
      applyWrite: (input) => transport.request(WS_METHODS.repoMemoryApplyWrite, input),
    },
    studio: {
    ...
    git: {
      githubAuthStatus: (input) => transport.request(WS_METHODS.gitGithubAuthStatus, input),
      githubSignIn: (input) => transport.request(WS_METHODS.gitGithubSignIn, input),
      githubSignOut: (input) => transport.request(WS_METHODS.gitGithubSignOut, input),
    ...
    server: {
      ...
      listMcpServers: () => transport.request(WS_METHODS.serverListMcpServers),
      setMcpServerEnabled: (input) =>
        transport.request(WS_METHODS.serverSetMcpServerEnabled, input),
      upsertMcpServer: (input) => transport.request(WS_METHODS.serverUpsertMcpServer, input),
      removeMcpServer: (input) => transport.request(WS_METHODS.serverRemoveMcpServer, input),
      listHooks: () => transport.request(WS_METHODS.serverListHooks),
      setHook: (input) => transport.request(WS_METHODS.serverSetHook, input),
      listAgentRules: () => transport.request(WS_METHODS.serverListAgentRules),
      ...
      setProviderApiKey: (input) => transport.request(WS_METHODS.serverSetProviderApiKey, input),
      clearProviderApiKey: (input) =>
        transport.request(WS_METHODS.serverClearProviderApiKey, input),
      providerSignIn: (input) => transport.request(WS_METHODS.serverProviderSignIn, input),
      setCustomModelEndpoint: (input) =>
        transport.request(WS_METHODS.serverSetCustomModelEndpoint, input),
      deleteCustomModelEndpoint: (input) =>
        transport.request(WS_METHODS.serverDeleteCustomModelEndpoint, input),
    ...
    stats: {
      ...
      getProfileCommitActivity: (input) =>
        transport.request(WS_METHODS.statsGetProfileCommitActivity, input),
    ...
    provider: {
      ...
      listPlugins: (input) =>
        transport.request(WS_METHODS.providerListPlugins, input, { timeoutMs: null }),
      readPlugin: (input) => transport.request(WS_METHODS.providerReadPlugin, input),
      setPluginInstalled: (input) =>
        transport.request(WS_METHODS.providerSetPluginInstalled, input, { timeoutMs: null }),
    ...
    orchestration: {
      ...
      getHandoffCheckpointDiff: (input) =>
        transport.request(ORCHESTRATION_WS_METHODS.getHandoffCheckpointDiff, input),
      restoreHandoffCheckpoint: (input) =>
        transport.request(ORCHESTRATION_WS_METHODS.restoreHandoffCheckpoint, input),
    ...
    openClaw: {
      ...
      setup: (input = {}) => transport.request(WS_METHODS.openClawSetup, input),
      testConnection: () => transport.request(WS_METHODS.openClawTestConnection, {}),
      onEvent: (callback) => {
        openClawEventListeners.add(callback);
        return () => openClawEventListeners.delete(callback);
      },
    },
    review: {
      providers: () => transport.request(WS_METHODS.reviewProviders, {}),
      install: (input) => transport.request(WS_METHODS.reviewInstall, input),
      list: (input) => transport.request(WS_METHODS.reviewList, input),
      start: (input) => transport.request(WS_METHODS.reviewStart, input),
      cancel: (input) => transport.request(WS_METHODS.reviewCancel, input),
      ignoreFinding: (input) => transport.request(WS_METHODS.reviewIgnoreFinding, input),
      onEvent: (callback) => {
        reviewEventListeners.add(callback);
        return () => reviewEventListeners.delete(callback);
      },
    },
    browser: {
      open: async (input) => {
...
export function resetWsNativeApiForTest(): void {
  ...
  openClawEventListeners.clear();
  reviewEventListeners.clear();
