import { ProjectId, ThreadId } from "@modesto/contracts";
...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.modesto/worktrees/modesto-mvp/modesto-4e609bb8",
    );
    expect(result).toBe("modesto-4e609bb8");
  });

  it("keeps legacy .modesto worktree paths readable after migration", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.modesto/worktrees/modesto-mvp/modesto-legacy123",
    );
    expect(result).toBe("modesto-legacy123");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.modesto\\worktrees\\modesto-mvp\\modesto-4e609bb8",
    );
    expect(result).toBe("modesto-4e609bb8");
  });

  it("uses the final segment even when outside ~/.modesto/worktrees", () => {
