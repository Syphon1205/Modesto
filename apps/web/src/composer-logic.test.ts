...
describe("parseStandaloneComposerSlashCommand", () => {
  ...
  it("does not treat /multiagent, /spawn, or /side as standalone mode commands", () => {
    expect(parseStandaloneComposerSlashCommand("/spawn fix the flaky test")).toBeNull();
    expect(parseStandaloneComposerSlashCommand("/multiagent fix the flaky test")).toBeNull();
    expect(parseStandaloneComposerSlashCommand("/side")).toBeNull();
    expect(parseStandaloneComposerSlashCommand("/side ask about the auth test")).toBeNull();
  });

  it("does not treat /slides as a plan or default mode command", () => {
    expect(parseStandaloneComposerSlashCommand("/slides")).toBeNull();
    expect(parseStandaloneComposerSlashCommand("/slides pitch the launch")).toBeNull();
  });

  it("does not treat /docs or /sheets as plan mode commands", () => {
    expect(parseStandaloneComposerSlashCommand("/docs")).toBeNull();
    expect(parseStandaloneComposerSlashCommand("/spreadsheets launch")).toBeNull();
    expect(parseStandaloneComposerSlashCommand("/sheets launch")).toBeNull();
    expect(parseStandaloneComposerSlashCommand("/canvas")).toBeNull();
  });
});
...
describe("parseMultiagentComposerCommand", () => {
  ...
  it("keeps /spawn as a silent alias of /multiagent", () => {
    expect(parseMultiagentComposerCommand(" /spawn fix the flaky auth test ")).toEqual({
      ok: true,
      task: "fix the flaky auth test",
    });
  });

  it("preserves multiline task text", () => {
    expect(parseMultiagentComposerCommand("/multiagent first line\nsecond line")).toEqual({
      ok: true,
      task: "first line\nsecond line",
    });
  });

  it("reports missing task for bare /multiagent or /spawn", () => {
    expect(parseMultiagentComposerCommand("/multiagent")).toEqual({
      ok: false,
      reason: "missing-task",
    });
    expect(parseMultiagentComposerCommand("/spawn   ")).toEqual({
      ok: false,
      reason: "missing-task",
    });
  });

  it("ignores non-multiagent prompts", () => {
    expect(parseMultiagentComposerCommand("/plan")).toBeNull();
    expect(parseMultiagentComposerCommand("spawn something")).toBeNull();
    expect(parseMultiagentComposerCommand("/spawned task")).toBeNull();
    expect(parseMultiagentComposerCommand("/side ask this")).toBeNull();
  });
});

describe("parseSideComposerCommand", () => {
  it("parses a bare /side as an empty sidechat", () => {
    expect(parseSideComposerCommand(" /side ")).toEqual({
      ok: true,
      task: null,
    });
  });

  it("parses /side with an opening message", () => {
  ...
  it("does not treat /sidebar as /side", () => {
    expect(parseSideComposerCommand("/sidebar open")).toBeNull();
  });
});
