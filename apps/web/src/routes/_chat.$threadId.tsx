...
import { isWorkspaceRelativePathSafe } from "@modesto/shared/path";
...
const ContextUsagePanel = lazy(() =>
  import("../components/chat/ContextUsagePanel").then((module) => ({
    ...
    <DiffWorkerPoolProvider>
      ...
      >
        <DiffPanel
          ...
          {...(props.hideHeader !== undefined ? { hideHeader: props.hideHeader } : {})}
          {...(props.hideReviewPanel !== undefined
            ? { hideReviewPanel: props.hideReviewPanel }
            : {})}
...
function DeferredChatView(props: {
  ...
  return (
    <ChatView
      ...
      {...(props.onOpenContext ? { onOpenContextPanel: props.onOpenContext } : {})}
...
function SingleChatSurface(props: {
  ...
  const handleOpenContextPane = useCallback(() => {
    requestImmediateDockHydration("context");
    openPane(props.threadId, { kind: "context" });
  }, [openPane, props.threadId, requestImmediateDockHydration]);
  ...
  const renderDockPane = useCallback(
    (
      ...
      switch (pane.kind) {
        ...
        case "context":
          return (
            <Suspense fallback={<PanelStateMessage>Loading context usage...</PanelStateMessage>}>
              <ContextUsagePanel threadId={props.threadId} />
            </Suspense>
          );
        case "terminal":
        ...
        >
          <RouteInsetSurface surfaceClassName={CHAT_BACKGROUND_CLASS_NAME}>
            <DeferredChatView
              ...
              onOpenContext={handleOpenContextPane}
