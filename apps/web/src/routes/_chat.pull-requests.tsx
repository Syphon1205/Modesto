...
import { pullRequestHostOf, ThreadId } from "@modesto/contracts";
import type {
  EnvironmentId,
  ProjectId,
...
import {
  PullRequestInboxSidebar,
  PullRequestInboxToolbar,
} from "../components/pullRequest/PullRequestInboxSidebar";
import {
  filterInboxEntries,
  groupInboxByDate,
  inboxEntryHasViewerStake,
  inboxReasonFilters,
  inboxRepositoryCounts,
  inboxUnreadCount,
  ...
  type PullRequestInboxReason,
  type PullRequestInboxSort,
  type PullRequestInboxTray,
} from "../components/pullRequest/pullRequestInbox.logic";
import { usePullRequestInboxStore } from "../components/pullRequest/pullRequestInboxStore";
import { PullRequestRow } from "../components/pullRequest/PullRequestRow";
import {
  derivePullRequestStacks,
  formatStackPosition,
} from "~/components/pullRequest/pullRequestStacks.logic";
...
export interface PullRequestsSearch {
  ...
  /** Inbox / Saved / Done. Omitted from the URL while it is the default inbox. */
  readonly tray?: PullRequestInboxTray;
  /** Why this thread is in the inbox — assigned, mentioned, and so on. */
  ...
  /**
   * Narrows the inbox to one repository. Separate from `repository`, which opens a pull request.
   */
...
export const Route = createFileRoute("/_chat/pull-requests")({
  validateSearch: (raw: Record<string, unknown>): PullRequestsSearch => ({
    ...
    ...(parseInboxTray(raw.tray) !== "inbox" ? { tray: parseInboxTray(raw.tray) } : {}),
    ...(parseInboxReason(raw.reason) ? { reason: parseInboxReason(raw.reason) } : {}),
    ...(parseInboxUnread(raw.unread) ? { unread: true } : {}),
    ...(parseInboxSort(raw.sort) !== "newest" ? { sort: parseInboxSort(raw.sort) } : {}),
    ...(parseInboxGroup(raw.group) !== "date" ? { group: parseInboxGroup(raw.group) } : {}),
    ...(typeof raw.listRepo === "string" && raw.listRepo
      ? { listRepo: raw.listRepo.slice(0, 200) }
      : {}),
  }),
...
function PullRequestsRouteView() {
  ...
  const inboxGroup = search.group ?? "date";
  const inboxUnreadOnly = search.unread === true;
  const effectiveInvolvement = search.reason
    ? involvementForInboxReason(search.reason)
    : search.involvement;
      ...
      void navigate({
        ...
        search: (previous: PullRequestsSearch): PullRequestsSearch => {
          ...
          return {
            ...
            ...(next.tray && next.tray !== "inbox" ? { tray: next.tray } : {}),
            ...(next.reason ? { reason: next.reason } : {}),
            ...(next.unread ? { unread: true } : {}),
            ...(next.sort && next.sort !== "newest" ? { sort: next.sort } : {}),
            ...(next.group && next.group !== "date" ? { group: next.group } : {}),
            ...(next.listRepo ? { listRepo: next.listRepo } : {}),
          };
        },
        replace: true,
      }),
  ...
  const menuFilters = useMemo(
    (): PullRequestListFilters => ({
      ...chromeFilters,
      ...inboxReasonFilters(search.reason),
    }),
    [chromeFilters, search.reason],
  );
  ...
  const scopeKey = `${environmentKey}:${assignmentKey}:${search.state}:${effectiveInvolvement}:${search.reason ?? ""}:${scopedProjectId ?? ""}:${search.host ?? ""}:${search.draft ?? ""}:${search.review ?? ""}:${search.checks ?? ""}`;
        ...
        return [
          {
            ...
            input: {
              ...
              involvement: effectiveInvolvement,
              limit: pageSize,
  ...
  const chromeFiltered = Object.keys(chromeFilters).length > 0;
  const censusTargets = useMemo(
    () =>
      environmentQueries.map(({ environmentId, projectIds }) => ({
        environmentId,
        input: {
          ...
          ...(projectIds ? { projectIds } : {}),
          ...(search.host ? { host: search.host } : {}),
          ...(chromeFiltered ? { filters: chromeFilters } : {}),
        } satisfies PullRequestListInput,
      })),
    [chromeFiltered, chromeFilters, environmentQueries, scopedProjectId, search.host, search.state],
  );
  const censusQuery = usePullRequestList(censusTargets);
  ...
  const inboxRecords = usePullRequestInboxStore((state) => state.records);
  const entries = useMemo(
    () =>
      filterInboxEntries(listed, {
        state: inboxRecords,
        tray: inboxTray,
        unreadOnly: inboxUnreadOnly,
        ...(search.listRepo ? { repository: search.listRepo } : {}),
        sort: inboxSort,
      }),
    [inboxRecords, inboxSort, inboxTray, inboxUnreadOnly, listed, search.listRepo],
  );
  const censusEntries = Array.isArray(censusQuery.data?.entries)
    ? censusQuery.data.entries
    : Array.isArray(listed)
      ? listed
      : [];
  const inboxUnread = inboxUnreadCount(censusEntries, inboxRecords);
  const participatingUnread = inboxUnreadCount(
    censusEntries.filter((entry) =>
      inboxEntryHasViewerStake(entry, pullRequestEntryViewer(entry, viewers)),
    ),
    inboxRecords,
  );
  const inboxRepositories = inboxRepositoryCounts(censusEntries, inboxRecords);
   ...
   */
  // Derived from the whole listing rather than per group: a stack can span
  // groups - one PR authored by you, the next awaiting your review - and
  // deriving inside a group would split it and mislabel the positions.
  const stackIndex = useMemo(() => derivePullRequestStacks(entries), [entries]);

  const groups = useMemo(() => {
    if (inboxGroup === "date") return groupInboxByDate(entries);
    if (effectiveInvolvement !== "all" || search.reason !== undefined) {
    ...
    const intoInbox = (rows: ReadonlyArray<EnvironmentPullRequestEntry> | undefined) =>
      rows === undefined
        ? undefined
        : filterInboxEntries(rows, {
            state: inboxRecords,
            tray: inboxTray,
            unreadOnly: inboxUnreadOnly,
            ...(search.listRepo ? { repository: search.listRepo } : {}),
    ...
    const authored = intoInbox(
      narrow(partitionsWanted ? (authoredQuery.data?.entries ?? held?.authored) : undefined),
    );
    const reviewing = intoInbox(
      narrow(partitionsWanted ? (reviewingQuery.data?.entries ?? held?.reviewing) : undefined),
    );
  ...
  const selectEntry = useCallback(
    (entry: EnvironmentPullRequestEntry) => {
      ...
      if (rightPanelRef === null) return;
      usePullRequestInboxStore.getState().markRead(entry);
  ...
  const saveInboxEntry = useCallback((entry: EnvironmentPullRequestEntry) => {
    usePullRequestInboxStore.getState().setTray(entry, "saved");
  }, []);
  const doneInboxEntry = useCallback((entry: EnvironmentPullRequestEntry) => {
    usePullRequestInboxStore.getState().setTray(entry, "done");
  }, []);
  const unreadInboxEntry = useCallback((entry: EnvironmentPullRequestEntry) => {
    usePullRequestInboxStore.getState().markUnread(entry);
  }, []);
  ...
  const searchInput = (
    <PullRequestSearchInput
      ...
      placeholder="Search notifications, or label:bug"
    ...
    <>
      {!capabilityKnown ? (
        ...
        <PullRequestsUnavailableState
          ...
          error="Update your Modesto servers to browse pull requests."
        />
      ) : firstLoad ? (
        <PullRequestListGhost rows={7} />
        ...
        <div className="overflow-hidden rounded-xl border border-border/70">
          {groups.map((group, index) => (
            <div
              key={group.key}
              className={cn("divide-y divide-border/60", index > 0 && "border-t border-border/60")}
            >
              {group.label ? (
                <h2 className="bg-muted/25 px-3 py-1.5 text-xs font-medium text-muted-foreground">
                  {group.label}
                </h2>
              ) : null}
              {group.entries.map((entry) => (
                <PullRequestRow
                  ...
                  {...(() => {
                    const node = stackIndex.nodeByNumber.get(entry.number);
                    const position = formatStackPosition(stackIndex, entry.number);
                    return node && position ? { stack: { depth: node.depth, position } } : {};
                  })()}
                  showProjectTitle
                  ...
                  unread={isInboxUnread(inboxRecords, entry)}
                  onSelect={selectEntry}
                  onSave={saveInboxEntry}
                  onDone={doneInboxEntry}
                  onMarkUnread={unreadInboxEntry}
                />
              ))}
            </div>
          ))}
      ...
      {listData?.truncated && listed.length > 0 ? (
  ...
  const columnProps = {
    ...
    inboxToolbar: (
      <PullRequestInboxToolbar
        ...
        sort={inboxSort}
        group={inboxGroup}
        onUnreadOnly={(unreadOnly) => updateSearch({ unread: unreadOnly || undefined })}
        onSort={(sort) => updateSearch({ sort: sort === "newest" ? undefined : sort })}
        onGroup={(group) => updateSearch({ group: group === "date" ? undefined : group })}
      />
    ),
      ...
      <div className="relative flex min-h-0 flex-1">
        ...
        {pullRequestsSupported ? (
          <PullRequestInboxSidebar
            ...
            repository={search.listRepo}
            inboxUnread={inboxUnread}
            participatingUnread={participatingUnread}
            ...
            onTray={(tray) => updateSearch({ tray: tray === "inbox" ? undefined : tray })}
            onReason={(reason) =>
              updateListScope({
                reason,
                involvement: involvementForInboxReason(reason),
              })
            }
            onRepository={(repository) => updateSearch({ listRepo: repository })}
          />
        ) : null}
        ...
        {rightPanelState.isOpen && activePullRequestSurface && panelEnvironmentId !== null ? (
          <RightPanelTabs
            mode="inline"
            widthStorageKey="modesto:pull-request-panel-width"
