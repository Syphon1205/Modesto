...
import { getModelOptions, normalizeModelSlug } from "@modesto/shared/model";
...
import { ThemeFontsEditor } from "../components/ThemePackEditor";
import { DebouncedSettingTextInput } from "../components/settings/DebouncedSettingTextInput";
import {
  AgentsSettingsPanel,
  BrowserNetworkSettingsPanel,
  CloudAgentsSettingsPanel,
  HooksSettingsPanel,
  IndexingSettingsPanel,
  PluginsSettingsPanel,
  RulesSkillsSettingsPanel,
  TabSettingsPanel,
  ToolsMcpsSettingsPanel,
} from "../components/settings/CursorParitySettingsPanels";
...
import { ModelRoutersSettingsPanel } from "../components/settings/ModelRoutersSettingsPanel";
import { ProfileSettingsPanel } from "../components/settings/ProfileSettingsPanel";
import { KeyboardShortcutsSettingsPanel } from "../components/settings/KeyboardShortcutsSettingsPanel";
import { DesktopUpdatesSettingsPanel } from "../components/settings/DesktopUpdatesSettingsPanel";
import { CodeReviewSettingsPanel } from "../components/settings/CodeReviewSettingsPanel";
...
const TERMINAL_THEME_OPTIONS = [
  { value: "modesto", label: "Match Modesto" },
  { value: "vscode-dark", label: "VS Code Dark" },
  { value: "vscode-light", label: "VS Code Light" },
  { value: "vscode-high-contrast", label: "VS Code High Contrast" },
] as const satisfies ReadonlyArray<{ value: TerminalTheme; label: string }>;
          ...
          {docs.map((doc) =>
            doc.label === "Install" && canInstall ? (
              ...
              >
                ...
                {installing ? "Installing" : "Install in Modesto"}
              </Button>
            ) : (
              <a
                key={`${doc.label}:${doc.href}`}
...
// Sign in / API key controls for one provider row in "Provider tools". Reads
// readiness from ServerProviderStatus.apiKeySupported / .signInSupported, which the
// server derives per-provider — this component never hardcodes which providers get
// which control.
function ProviderCredentialControls({
  provider,
  title,
  status,
}: {
  ...
  const invalidateProviderStatuses = useCallback(
  ...
  const setApiKeyMutation = useMutation({
    mutationFn: (apiKey: string) =>
      ensureNativeApi().server.setProviderApiKey({ provider, apiKey }),
    onSuccess: async () => {
      setApiKeyDraft("");
      await invalidateProviderStatuses();
      toastManager.add({
        type: "success",
        title: `${title} API key saved`,
      });
    },
    onError: (error) => {
      toastManager.add({
        type: "error",
        title: `Could not save ${title} API key`,
        description: error instanceof Error ? error.message : "The key was rejected.",
      });
    },
  });

  const clearApiKeyMutation = useMutation({
    mutationFn: () => ensureNativeApi().server.clearProviderApiKey({ provider }),
    onSuccess: async () => {
      await invalidateProviderStatuses();
      toastManager.add({ type: "info", title: `${title} API key removed` });
    },
    onError: (error) => {
      toastManager.add({
        type: "error",
        title: `Could not remove ${title} API key`,
  ...
  const signInMutation = useMutation({
    mutationFn: () => ensureNativeApi().server.providerSignIn({ provider }),
    onSuccess: () => {
      setAwaitingSignIn(true);
      toastManager.add({
        type: "info",
        title: `Finish signing in to ${title}`,
        description: "Complete the sign-in flow that just opened, then come back here.",
      });
    },
    onError: (error) => {
      toastManager.add({
        type: "error",
        title: `Could not start ${title} sign-in`,
        description: error instanceof Error ? error.message : "Sign-in failed to start.",
      });
    },
  });

  // Poll provider status while a sign-in flow is in flight, same cadence as GitHub
  // sign-in above: every 2s, capped at 120s or until this provider reports authenticated.
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      void ensureNativeApi()
        ...
        .then(() => invalidateProviderStatuses());
    }, 2_000);
    const timeout = window.setTimeout(() => setAwaitingSignIn(false), 120_000);
    return () => {
      window.clearInterval(interval);
      window.clearTimeout(timeout);
    };
  }, [awaitingSignIn, authenticated, invalidateProviderStatuses]);

  if (!status?.apiKeySupported && !status?.signInSupported) {
    return null;
  }
  ...
  return (
    <div className="space-y-2">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          {authenticated ? (
            ...
            <Button
              ...
              size="sm"
              variant="outline"
              disabled={signInMutation.isPending || awaitingSignIn}
      ...
      {status.apiKeySupported ? (
        <label className="block">
          <span className="block text-xs font-medium text-foreground">
            {title} API key
            {status.apiKeyConfigured ? (
              <span className="ml-1.5 text-muted-foreground">(saved)</span>
            ) : null}
          </span>
          <div className="mt-1 flex items-center gap-2">
            <SecretInput
              ...
              placeholder={status.apiKeyConfigured ? "Replace saved key" : "Paste API key"}
              spellCheck={false}
            />
            <Button
              type="button"
              size="sm"
              variant="outline"
              disabled={apiKeyDraft.trim().length === 0 || setApiKeyMutation.isPending}
              onClick={() => setApiKeyMutation.mutate(apiKeyDraft.trim())}
            >
...
function SettingsRouteView() {
  ...
  const requestedProvider =
    typeof routeSearch.provider === "string" &&
    INSTALL_PROVIDER_SETTINGS.some((entry) => entry.provider === routeSearch.provider)
      ? (routeSearch.provider as ProviderKind)
      : null;
  const activeSectionItem = resolveSettingsNavItem(activeSection);
  ...
  const githubAuthQuery = useQuery({
    queryKey: ["git", "github-auth"] as const,
    queryFn: () => ensureNativeApi().git.githubAuthStatus({}),
    staleTime: 15_000,
    refetchOnWindowFocus: true,
    refetchInterval: awaitingGitHubSignIn ? 2_000 : false,
  });
  ...
  useEffect(() => {
    if (activeSection !== "providers" || requestedProvider === null) return;
    setOpenInstallProviders((existing) => ({ ...existing, [requestedProvider]: true }));
  }, [activeSection, requestedProvider]);
  ...
  const changedSettingLabels = [
    ...
    ...(settings.terminalTheme !== defaults.terminalTheme ? ["Terminal theme"] : []),
  ...
  const runProviderUpdate = useCallback(
    async (provider: ProviderKind, action: "update" | "install" = "update") => {
      if (updatingProviders.has(provider)) {
        return;
      }
      setUpdatingProviders((current) => new Set(current).add(provider));
      try {
        const result = await ensureNativeApi().server.updateProvider({ provider, action });
        ...
        if (failureMessage) {
          ...
          toastManager.add({
            type: "error",
            title: `Could not ${action} ${PROVIDER_DISPLAY_NAMES[provider]}`,
  ...
  async function sendTestNotification() {
    ...
    const notification = new Notification(title, { body, tag: "modesto:test-notification" });
  ...
  const renderAppearancePanel = () => (
    <div className="space-y-6">
      <section className={SETTINGS_PANEL_SECTION_CLASS_NAME}>
        <h2 className={SETTINGS_SECTION_LABEL_CLASS_NAME}>Theme and layout</h2>
        <SettingsCard>
          <SettingsRow
            title="Theme"
            description="Choose how Modesto looks across the app."
            resetAction={
              theme !== "system" ? (
        ...
        <SettingsCard>
          <SettingsRow
            ...
            description="Use Modesto colors or a familiar VS Code terminal palette."
            resetAction={
              settings.terminalTheme !== defaults.terminalTheme ? (
                <SettingResetButton
                  label="terminal theme"
                  onClick={() => updateSettings({ terminalTheme: DEFAULT_TERMINAL_THEME })}
                />
              ) : null
            }
            control={
              <SettingsSelectControl
                ...
                onValueChange={(value) => {
                  ...
                  updateSettings({ terminalTheme: value });
                }}
                ariaLabel="Terminal theme"
                triggerClassName="w-full sm:w-52"
                valueContent={
                  TERMINAL_THEME_OPTIONS.find((option) => option.value === settings.terminalTheme)
                    ?.label
                }
              >
  ...
  const renderFontsPanel = () => (
    <div className="space-y-6">
      <section className={SETTINGS_PANEL_SECTION_CLASS_NAME}>
        <h2 className={SETTINGS_SECTION_LABEL_CLASS_NAME}>Theme fonts</h2>
        <div className="space-y-3">
          <ThemeFontsEditor variant="dark" />
          <ThemeFontsEditor variant="light" />
        </div>
      </section>

      <SettingsSection title="Font sizing">
        <SettingsRow
          ...
          resetAction={
            settings.chatFontSizePx !== defaults.chatFontSizePx ? (
              <SettingResetButton
                ...
                onClick={() => updateSettings({ chatFontSizePx: defaults.chatFontSizePx })}
              />
            ) : null
          }
          control={
            <div className="flex w-full items-center justify-end gap-2 sm:w-auto">
              <Input
                ...
                onChange={(event) => {
                  ...
                  updateSettings({ chatFontSizePx: normalizeChatFontSizePx(Number(nextValue)) });
                }}
          ...
          resetAction={
            settings.terminalFontSizePx !== defaults.terminalFontSizePx ? (
              <SettingResetButton
                ...
                onClick={() => updateSettings({ terminalFontSizePx: defaults.terminalFontSizePx })}
              />
            ) : null
          }
          control={
        ...
        <SettingsRow
          ...
          resetAction={
            settings.terminalFontFamily !== defaults.terminalFontFamily ? (
              <SettingResetButton
                label="terminal font"
                onClick={() => updateSettings({ terminalFontFamily: defaults.terminalFontFamily })}
              />
            ) : null
          }
          control={
            <div className="flex w-full items-center justify-end sm:w-auto">
              <Autocomplete
                ...
                onValueChange={(value) => {
                  updateSettings({ terminalFontFamily: normalizeTerminalFontFamily(value) });
                }}
              >
                <AutocompleteInput
                  size="sm"
                    ...
                    onHiddenChange={(hidden) => {
                      ...
                      void ensureNativeApi()
                        .server.updateSettings({
                          providers: { [option.provider]: { enabled: !hidden } },
                        })
                        .then(() => queryClient.invalidateQueries(serverSettingsQueryOptions()))
                        .catch((error) =>
                          toastManager.add({
                            title: "Provider setting was not saved",
    ...
    <div ref={providerUpdatesRef} id={SETTINGS_TARGETS.providerUpdates}>
      <SettingsSection title="Updates">
        ...
        <SettingsRow
          ...
          description="Review installed provider tools that Modesto can safely update."
          status={
        ...
        <SettingsRow
          ...
          status={
            githubAuthQuery.isPending
              ? "Checking connection…"
              : githubAuthQuery.data?.authenticated
                ? `Connected as @${githubAuthQuery.data.login}`
                : githubAuthQuery.data?.installed
                  ? awaitingGitHubSignIn
                    ? githubSignInMutation.data?.userCode
          ...
          control={
            githubAuthQuery.data?.installed === false ? (
              ...
              <Button
                ...
                onClick={() => void githubAuthQuery.refetch()}
              >
                {githubAuthQuery.isFetching ? "Checking…" : "Check again"}
              </Button>
            ) : (
              <Button
                type="button"
                size="sm"
                disabled={githubSignInMutation.isPending || awaitingGitHubSignIn}
                onClick={() => githubSignInMutation.mutate()}
              >
                {githubSignInMutation.isPending || awaitingGitHubSignIn
                  ? "Waiting for GitHub…"
                  : "Sign in with GitHub"}
              </Button>
            )
          ...
          <div className="mt-4">
            <div className={SETTINGS_INSET_LIST_CLASS_NAME}>
              {INSTALL_PROVIDER_SETTINGS.map((providerSettings) => {
                ...
                const providerReadinessLabel = !providerStatus
                  ? "Checking"
                  : providerStatus.available && providerStatus.authStatus !== "unauthenticated"
                    ? "Ready"
                    : providerStatus.authStatus === "unauthenticated"
                      ? "Sign in required"
                      : "Install required";
                const providerIsReady = providerReadinessLabel === "Ready";
                    ...
                    <div className="border-t border-border/70 first:border-t-0">
                      <div className="flex min-h-11 items-center gap-2 px-3 py-2">
                        ...
                        >
                          ...
                          <span
                            className={cn(
                              "shrink-0 text-[11px]",
                              providerIsReady ? "text-success" : "text-muted-foreground",
                            )}
                          >
                      ...
                      <CollapsibleContent>
                        <div className="border-t border-border/70 bg-muted/20 px-3 py-3">
                          <div className="space-y-3">
                            {providerStatus && !providerIsReady ? (
                              <div className="rounded-lg border border-border/70 bg-background/65 px-3 py-2 text-xs text-muted-foreground">
                                {providerStatus.message ??
                                  `${providerSettings.title} needs to be installed and signed in before it can run agents.`}
                              </div>
                            ) : null}
                            ...
                            <ProviderDocsLinks
                              ...
                              onInstall={() =>
                                void runProviderUpdate(providerSettings.provider, "install")
                              }
                            />
                            {showProviderUpdateStatus &&
                            updateAdvisory?.status === "behind_latest" ? (
                              <div className="text-xs text-muted-foreground">
                                {updateAdvisory.canUpdate && updateAdvisory.updateCommand ? (
                                  ...
                                  "A newer version is available, but Modesto could not identify a safe one-click update command for this installation."
                                )}
                              </div>
                            ) : null}
  ...
  const openAboutLink = (url: string) => void ensureNativeApi().shell.openExternal(url);

  const renderAboutPanel = () => (
    <div className="space-y-6">
      <SettingsSection title="Modesto">
        <SettingsRow
          title="About Modesto"
      ...
      <SettingsSection title="Community">
        <SettingsRow
          ...
          description="Source code, releases, and project development."
          control={
            <Button
              size="sm"
              variant="outline"
              onClick={() => openAboutLink("https://github.com/Syphon1205/Modesto")}
            >
              Open GitHub
            </Button>
          }
        ...
        <SettingsRow
          ...
          description="@trymodesto"
          control={
            <Button
              size="sm"
              variant="outline"
              onClick={() => openAboutLink("https://www.instagram.com/trymodesto/")}
            >
              Follow
            </Button>
          }
      ...
      <SettingsSection title="Resources">
        ...
        <SettingsRow
          title="Changelog"
          description="See what changed in every Modesto release."
          control={
      ...
      <SettingsSection title="Feedback">
        <SettingsRow
          ...
          control={
            <Button
              ...
              onClick={() =>
                openAboutLink(
                  "https://github.com/Syphon1205/Modesto/issues/new?labels=bug&template=bug_report.yml",
                )
              }
            >
              Report bug
        ...
        <SettingsRow
          ...
          description="Share an idea or suggest an improvement."
          control={
            <Button
              size="sm"
              variant="outline"
              onClick={() =>
                openAboutLink(
                  "https://github.com/Syphon1205/Modesto/issues/new?labels=enhancement&template=feature_request.yml",
                )
              }
            >
              Request feature
  ...
  const renderActivePanel = () => {
    switch (activeSection) {
      ...
      case "updates":
        return <DesktopUpdatesSettingsPanel />;
      case "behavior":
      ...
      case "modelRouters":
        return <ModelRoutersSettingsPanel />;
      case "providers":
      ...
      case "skills":
      case "rulesSkills":
        return <RulesSkillsSettingsPanel />;
      case "usage":
        return <ProviderUsageSettingsPanel />;
      case "agents":
        return <AgentsSettingsPanel />;
      case "cloudAgents":
        return <CloudAgentsSettingsPanel projects={projects} />;
      case "plugins":
        return <PluginsSettingsPanel />;
      case "toolsMcps":
        return <ToolsMcpsSettingsPanel />;
      case "hooks":
        return <HooksSettingsPanel />;
      case "browserNetwork":
        return <BrowserNetworkSettingsPanel />;
      case "tab":
        return <TabSettingsPanel />;
      case "indexing":
        return <IndexingSettingsPanel />;
      case "advanced":
