...
export type ParsedMultiagentComposerCommand =
  | { ok: true; task: string }
  | { ok: false; reason: "missing-task" };

/** @deprecated Use ParsedMultiagentComposerCommand. */
export type ParsedSpawnComposerCommand = ParsedMultiagentComposerCommand;

export type ParsedSideComposerCommand = { ok: true; task: string | null };
...
export function parseStandaloneComposerSlashCommand(
  ...
  "model" | "side" | "multiagent" | "canvas" | "slides" | "docs" | "spreadsheets"
> | null {
 ...
 * Bare `/multiagent` or `/spawn` returns a missing-task result.
 */
export function parseMultiagentComposerCommand(
  text: string,
): ParsedMultiagentComposerCommand | null {
  const match = /^\/(?:multiagent|spawn)(?:\s+([\s\S]*))?$/i.exec(text.trim());
  if (!match) {
    return null;
  }
...
/** @deprecated Use parseMultiagentComposerCommand. */
...
export function parseSideComposerCommand(text: string): ParsedSideComposerCommand | null {
  ...
  const task = match[1]?.trim() ?? "";
  return { ok: true, task: task.length > 0 ? task : null };
}
