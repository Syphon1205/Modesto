...
describe("deriveWorkLogEntries", () => {
  ...
  it("surfaces a turn's reasoning as an expandable Thinking entry", () => {
    // The whole point of capturing reasoning: it has to reach the timeline as
    // a row whose detail can be opened, or "Thinking" stays a dead label.
    const entries = deriveWorkLogEntries([
      makeActivity({
        ...
        payload: { detail: "First I check the config, then the loader.", truncated: false },
      }),
    ]);

    expect(entries).toHaveLength(1);
    expect(entries[0]?.label).toBe("Thinking");
    expect(entries[0]?.detail).toBe("First I check the config, then the loader.");
    expect(entries[0]?.sourceActivityKind).toBe("reasoning.completed");
  });
  ...
  it("keeps handoff core-context rows and omits per-turn refreshes", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "core-turn",
        createdAt: "2026-02-23T00:00:01.000Z",
        kind: "thread.core-context",
        summary: "Core context updated",
        tone: "info",
        payload: { reason: "turn" },
      }),
      makeActivity({
        ...
        createdAt: "2026-02-23T00:00:02.000Z",
        kind: "thread.core-context",
        summary: "Handoff checkpoint ready",
        tone: "info",
    ...
    const entries = deriveWorkLogEntries(activities);
    expect(entries.map((entry) => entry.id)).toEqual(["core-handoff"]);
  });
