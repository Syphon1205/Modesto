...
/** Byte channels of a #rrggbb string, for asserting near-neutrality. */
...
describe("theme files", () => {
  ...
  it("derives a readable palette from extreme simple-editor colors", () => {
    ...
    // Status colors fall back to Modesto's standard red and amber rather than
  ...
  it("merges a small user file onto the matching contrast-safe base palette", () => {
    ...
    // A role the file omits comes from the contrast-safe base rather than
    // being invented. Asserted against the base itself, so changing the
    // flagship palette does not require editing a magic constant here.
    expect(theme?.colors.placeholder).toBe(getDefaultThemeColors("dark").placeholder);
  });
  ...
  it("canonicalizes the explicitly exported theme", () => {
    const serialized = serializeThemeFile({
      ...MODESTO_THEME,
      colors: { ...MODESTO_THEME.colors, accent: "hsl(263 70% 58%)" },
    });
    expect(JSON.parse(serialized)).toMatchObject({
      version: THEME_FILE_VERSION,
      id: MODESTO_THEME.id,
      name: MODESTO_THEME.label,
      appearance: "light",
  ...
  it("suppresses sidebar artwork during a live custom-theme preview", () => {
    ...
    applyThemeColorPreview(MODESTO_THEME.colors, "light");
  ...
  it("keeps optional light and dark palettes under one theme id", () => {
    ...
    expect(getThemeModes(MODESTO_THEME)).toEqual(["light", "dark"]);
    expect(resolveThemeAppearance(MODESTO_THEME.id, true, true)).toBe("dark");
    expect(resolveDesktopTheme(MODESTO_THEME.id, true)).toBe("system");
    expect(resolveThemeAppearance(MODESTO_THEME.id, false, false, "dark")).toBe("dark");
    expect(resolveDesktopTheme(MODESTO_THEME.id, false, "dark")).toBe("dark");
  ...
  it("keeps the Signal palette faithful to the marketing brand and readable", () => {
    // Asserted as properties rather than exact hex: the literals are oklch and
    // the conversion rounds, so pinning hex tests the converter's arithmetic
    // instead of the palette. Signal is the Astro brand: ink field, paper
    // cream, one lime accent — not the zinc/indigo card it replaced.
    const light = MODESTO_THEME.colors;
    const dark = MODESTO_THEME.variants!.dark!;

    expect(MODESTO_THEME.label).toBe("Signal");
    const darkGround = channels(asHex(dark.canvas));
    const lightGround = channels(asHex(light.canvas));
    expect(Math.max(...darkGround)).toBeLessThan(24);
    expect(Math.min(...lightGround)).toBeGreaterThan(200);
    expect(Math.min(...lightGround)).toBeLessThan(235);
    // Ink has a green cast; paper is warm (red > blue).
    expect(darkGround[1]!).toBeGreaterThan(darkGround[0]!);
    expect(lightGround[0]!).toBeGreaterThan(lightGround[2]!);
    expect(asHex(light.accent)).toBe(asHex(dark.accent));

    expect(contrastRatio(dark.text, dark.canvas)).toBeGreaterThanOrEqual(12);
    expect(contrastRatio(light.text, light.canvas)).toBeGreaterThanOrEqual(12);
    expect(contrastRatio(dark.textMuted, dark.canvas)).toBeGreaterThanOrEqual(4.5);
    expect(contrastRatio(light.textMuted, light.canvas)).toBeGreaterThanOrEqual(4.5);
    expect(contrastRatio(light.accentForeground, light.accent)).toBeGreaterThanOrEqual(4.5);
    expect(contrastRatio(dark.accentForeground, dark.accent)).toBeGreaterThanOrEqual(4.5);
  ...
  it("includes the dual-mode maintainer themes", () => {
    for (const theme of [MODESTO_THEME, GROVE_THEME, OCEAN_THEME, EMBER_THEME, IRIS_THEME]) {
...
describe("stored theme preferences", () => {
  ...
  it("resolves the legacy t3-chat-dark preference to dark Modesto", () => {
    expect(getThemeDefinition("t3-chat-dark")).toBe(MODESTO_THEME);
  ...
  it("recognizes only preferences the runtime can render", () => {
    for (const preference of ["light", "dark", "system", MODESTO_THEME.id, GROVE_THEME.id]) {
