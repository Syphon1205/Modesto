...
import { EnvironmentHttpCommonError, PRIMARY_LOCAL_ENVIRONMENT_ID } from "@modesto/contracts";
import type { EnvironmentHttpCommonError as EnvironmentHttpCommonErrorType } from "@modesto/contracts";
...
function unreachableAuthGateState(
  error: unknown,
): Extract<ServerAuthGateState, { status: "unreachable" }> {
  if (isPrimaryEnvironmentRequestError(error)) {
    return {
      status: "unreachable",
      errorMessage: "Can't reach the Modesto server. Start it, then reload this page.",
    };
  }
  return {
    status: "unreachable",
    errorMessage: error instanceof Error ? error.message : "Can't reach the Modesto server.",
  };
}
...
/**
 * Claim a session on a loopback environment without a pairing token.
 *
 * Only attempted when the server itself advertises `loopback-bootstrap`; the
 * server re-checks every condition and refuses a cross-site caller, so a
 * rejection here is a normal answer rather than an error worth surfacing.
 */
async function claimLoopbackSession(): Promise<AuthBrowserSessionResult> {
  return retryTransientBootstrap(async () => {
    try {
      return await runPrimaryHttp(
        PrimaryEnvironmentHttpClient.pipe(
          Effect.flatMap((client) => client.auth.loopbackSession()),
        ),
      );
    } catch (error) {
      throw PrimaryEnvironmentRequestError.fromCause({
        operation: "claim-loopback-session",
        cause: error,
      });
    }
  });
...
async function bootstrapServerAuth(): Promise<ServerAuthGateState> {
  ...
  // A loopback environment trusts this machine, so a second window opens
  // instead of asking for a token. Falls through to the pairing surface if the
  // server declines, which is what happens when the request is not same-origin.
  if (!bootstrapCredential && currentSession.auth.bootstrapMethods.includes("loopback-bootstrap")) {
    try {
      await claimLoopbackSession();
...
export async function resolveInitialServerAuthGateState(): Promise<ServerAuthGateState> {
  ...
  const nextPromise = bootstrapServerAuth().catch(unreachableAuthGateState);
