...
} from "@modesto/contracts/relay";
import { EnvironmentRegistry } from "@modesto/client-runtime/connection";
import { request, runStream } from "@modesto/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@modesto/client-runtime/rpc";
import { ManagedRelay } from "@modesto/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Modesto cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Modesto cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "MODESTO_RELAY_URL is not configured.",
      });
    }
