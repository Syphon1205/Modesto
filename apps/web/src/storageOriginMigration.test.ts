...
describe("storageOriginMigration", () => {
  ...
  it("imports missing keys without overwriting current-origin state", async () => {
    globalThis.localStorage.setItem("modesto:theme", "current");
    const { importModestoStorageSnapshot } = await import("./storageOriginMigration");

    expect(
      importModestoStorageSnapshot({
        version: 1,
        exportedAt: "2026-07-09T00:00:00.000Z",
        entries: {
          "modesto:theme": "snapshot",
          "modesto:composer-drafts:v1": "draft",
        },
      }),
    ).toBe(true);
    expect(globalThis.localStorage.getItem("modesto:theme")).toBe("current");
    expect(globalThis.localStorage.getItem("modesto:composer-drafts:v1")).toBe("draft");
  });
  ...
  it("rejects an invalid snapshot before writing any entry", async () => {
    ...
    expect(globalThis.localStorage.getItem("modesto:theme")).toBeNull();
  });
  ...
  it("imports snapshots containing large composer drafts", async () => {
    ...
    expect(globalThis.localStorage.getItem("modesto:composer-drafts:v1")).toBe(largeDraft);
  });
  ...
  it("keeps the snapshot retryable after a partial storage failure", async () => {
    ...
    const snapshot = {
      version: 1 as const,
      exportedAt: "2026-07-09T00:00:00.000Z",
      entries: { "modesto:theme": "dark", "modesto:composer-drafts:v1": "draft" },
    };

    expect(importModestoStorageSnapshot(snapshot, storage)).toBe(false);
    storage.setItem = setItem;
    expect(importModestoStorageSnapshot(snapshot, storage)).toBe(true);
    expect(storage.getItem("modesto:composer-drafts:v1")).toBe("draft");
  });
  ...
  it("acknowledges the desktop snapshot only after a complete bootstrap import", async () => {
    ...
    expect(globalThis.localStorage.getItem("modesto:theme")).toBe("dark");
  });
