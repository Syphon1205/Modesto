...
const THEME_STORAGE_KEY = "modesto:theme";
...
describe("index.html boot script", () => {
  const parityCases: ReadonlyArray<{
    ...
    {
      name: "Modesto follows a dark OS",
      storage: { [THEME_STORAGE_KEY]: "modesto", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "an explicit global dark mode applies to Modesto",
      storage: {
        [THEME_STORAGE_KEY]: "modesto",
    ...
    {
      name: "a retired t3-chat preference resolves to Modesto",
    ...
    {
      name: "legacy t3-chat-dark resolves to dark Modesto",
  ...
  it("marks built-in and custom themes on the document element", () => {
    ...
    expect(chat.themeId).toBe("modesto");
  ...
  it("applies the matching half of an automatic mix to the splash", () => {
    const storage = {
      ...
      "modesto:theme-halves:v1": JSON.stringify({ dark: GROVE_THEME.id }),
    };
    ...
    expect(light.themeId).toBe("modesto");
    expect(light.bootVariables["--boot-background"]).toBe(
      getThemeColorsForMode(MODESTO_THEME, "light")!.canvas,
    );
  });
  ...
  it("resolves a legacy-prefixed mix half onto the renamed theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "modesto",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "modesto:theme-halves:v1": JSON.stringify({ dark: "t3-grove" }),
      },
      prefersDark: true,
    });
    expect(boot.isDark).toBe(true);
  ...
  it("ignores a mix half that names an unknown theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "modesto",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "modesto:theme-halves:v1": JSON.stringify({ dark: "gone-theme" }),
      },
      prefersDark: true,
    });
    expect(boot.themeId).toBe("modesto");
    expect(boot.isDark).toBe(true);
  });
