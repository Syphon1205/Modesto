...
export default function WhatsNewDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogPopup className="max-w-xl gap-0 overflow-hidden p-0" showCloseButton={false}>
        ...
        <DialogPanel className="max-h-[min(70vh,620px)] px-4 py-3">
          {view === "current" ? (
...
function CurrentHeader({
  ...
  return (
    <div className="flex items-center gap-3">
      <ModestoLogo aria-hidden className="size-8 shrink-0 text-foreground" />
      <div className="flex min-w-0 flex-col">
        <DialogTitle className="text-base">What&rsquo;s new?</DialogTitle>
        <DialogDescription className="text-xs">
          v{currentVersion}
          {currentVersion === "0.1.1" ? " - Milpitas" : ""}
