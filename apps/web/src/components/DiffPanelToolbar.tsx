...
import type { ThreadId, TurnId } from "@modesto/contracts";
    ...
    case "lastTurn":
      return (
        <span className="inline-flex size-3.5 shrink-0 items-center justify-center text-muted-foreground">
          <PlusMinusIcon className="size-2.25" />
        </span>
      );
    default:
...
export const DiffPanelToolbar = memo(function DiffPanelToolbar(props: DiffPanelToolbarProps) {
  ...
  const scopePickerIcon = useMemo((): ReactNode => {
    if (props.viewSource.kind === "turn") {
      return <PlusMinusIcon className="size-2.5 text-[var(--color-text-foreground)]" />;
    }
      ...
      <div className="ml-auto flex min-w-0 items-center gap-1.5">
        ...
        <Menu>
          ...
          >
            <EnvironmentRowBody
              compact
              icon={<PlusMinusIcon className="size-2.5 text-[var(--color-text-foreground)]" />}
              ...
              >
                ...
                {visibleTurnSummaries.map((summary) => (
                  <MenuRadioItem key={summary.turnId} value={summary.turnId}>
                    <PlusMinusIcon className="size-2.5 shrink-0 text-muted-foreground" />
