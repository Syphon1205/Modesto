...
describe("groupChildThreadsByParentId", () => {
  const child = (input: {
    id: string;
  ...
  it("groups live children under their parent, newest first", () => {
    const grouped = groupChildThreadsByParentId([
      child({
        id: "older",
        parentThreadId: "chat-1",
    ...
    expect(grouped.get("chat-1")?.map((thread) => thread.id)).toEqual(["newer", "older"]);
    expect(grouped.get("chat-2")?.map((thread) => thread.id)).toEqual(["other"]);
    expect(grouped.has("top-level")).toBe(false);
  });

  it("omits archived children", () => {
    const grouped = groupChildThreadsByParentId([
      child({
        id: "live",
        parentThreadId: "chat-1",
      ...
      child({
        ...
        parentThreadId: "chat-1",
        createdAt: "2026-09-08T13:00:00.000Z",
        archivedAt: "2026-09-08T14:00:00.000Z",
      }),
    ]);

    expect(grouped.get("chat-1")?.map((thread) => thread.id)).toEqual(["live"]);
  });
});

describe("selectTopLevelCodeThreads / selectNestedCodeThreads", () => {
  const thread = (input: {
    id: string;
  ...
  it("nests multiagent children under a code parent and keeps them out of top-level code", () => {
    const parent = thread({
      id: "code-parent",
    ...
    const threads = [parent, child, orphan];

    expect(selectTopLevelCodeThreads(threads).map((entry) => entry.id)).toEqual([
      "code-parent",
      "orphan",
    ]);
    expect(selectNestedCodeThreads(threads).map((entry) => entry.id)).toEqual(["multiagent-child"]);
  });
});
