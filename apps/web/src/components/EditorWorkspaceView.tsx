...
import type { ProjectId, ReviewFinding, ThreadId } from "@modesto/contracts";
...
type EditorActivityBarItem = EditorCenterMode | "search" | "review";
...
function EditorActivityBar(props: {
  ...
  const reviewActive = props.sidebarVisible && props.reviewActive;
  return (
    <nav
    ...
    >
      ...
      <ExplorerActivityBarButton
        label={reviewActive ? "Hide review sidebar" : "Review"}
        active={reviewActive}
        onClick={() => props.onSelectItem("review")}
      >
...
export function EditorWorkspaceView(props: EditorWorkspaceViewProps) {
  ...
  const handleActivityBarSelectItem = useCallback(
    (item: EditorActivityBarItem) => {
      const itemActive =
        ...
        (item === "review"
          ...
          : item === "search"
            ? searchPaneActive
            : !reviewOpen && !searchPaneActive && centerMode === item);
      if (itemActive) {
    ...
    [centerMode, onCenterModeChange, reviewOpen, searchPaneActive, sidebarVisible],
  );
  ...
  const handleOpenReviewLocation = useCallback(
    (finding: ReviewFinding) => {
      props.onSelectFile(finding.file);
      if (finding.startLine) {
        setReviewReveal({ line: finding.startLine, token: Date.now() });
      }
    },
    [props.onSelectFile],
  );
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-1 flex-col bg-[var(--color-background-root)] text-foreground">
      ...
      >
        ...
        >
          <div className="flex min-w-0 items-center gap-1.5">
            ...
            <span className="hidden truncate text-[11px] text-muted-foreground/70 md:inline">
              {props.selectedFilePath ?? props.workspaceRoot ?? "No file selected"}
            </span>
          </div>
        ...
        <div className="flex items-center overflow-hidden rounded-lg border border-border/70 bg-background/35">
          <ChatHeaderButton
            type="button"
            tone="plain"
        ...
        >
          ...
          <span className="truncate font-normal">Exit editor</span>
        </ChatHeaderButton>
      </div>
      <div className="flex min-h-0 min-w-0 flex-1 overflow-hidden">
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden lg:flex-row">
          {!sidebarVisible ? null : reviewOpen && props.threadId ? (
            <ReviewFindingsPanel
              threadId={props.threadId}
              selectedAgent="Modesto"
              enabled
              layout="sidebar"
              selectedFilePath={props.selectedFilePath}
              workspaceRoot={props.workspaceRoot}
              selectedCode={reviewSelection}
            ...
            {props.centerMode === "file" ? (
              <div className="flex min-h-0 min-w-0 flex-1">
                <WorkspaceFilePreview
                  ...
                  onCodeSelectionChange={setReviewSelection}
