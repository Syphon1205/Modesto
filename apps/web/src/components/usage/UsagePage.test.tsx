import { USAGE_CONTRACT_VERSION } from "@modesto/contracts";
import { mergeUsage } from "@modesto/shared/usageMerge";
...
// Two scanned providers is enough for the hourly-breakdown assertions below.
// `ALL_PROVIDER_KINDS` must still be stubbed because `UsageCoverageNotice`
// reads it (and each entry's `unsupported` flag) to explain providers that
// have no usage scanner.
vi.mock("./usageProviders", () => ({
  ALL_PROVIDER_KINDS: ["codex", "claude"],
  PROVIDER_ORDER: ["codex", "claude"],
  PROVIDER_PRESENTATION: {
    codex: { color: "white", label: "Codex", mark: "span", unsupported: false },
    claude: { color: "orange", label: "Claude Code", mark: "span", unsupported: false },
  },
}));
