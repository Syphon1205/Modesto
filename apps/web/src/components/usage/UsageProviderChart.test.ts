...
describe("buildDayColumns", () => {
  ...
  it("keeps band values absolute rather than cumulative", () => {
    ...
    // Every provider in PROVIDER_ORDER gets a band so band indices (and
    // therefore colors) stay stable across periods; providers with no usage
    // that period are zero-filled. Assert on the providers this fixture
    // actually has data for, so adding a provider to the roster can't break a
    // test that is really about stacking math.
