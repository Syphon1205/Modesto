...
import type { DailyTotals, HourlyTotals } from "@modesto/shared/usageMerge";
...
import {
  ...
  formatUsd,
  makeWindow,
} from "@modesto/shared/usageFormat";
...
import { ALL_PROVIDER_KINDS, PROVIDER_ORDER, PROVIDER_PRESENTATION } from "./usageProviders";
                  ...
                  <div className="flex min-w-0 flex-col gap-5">
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {metric === "cost"
                          ? `${formatCount(merged.sessions)} sessions · API estimate · Cursor cloud when available`
...
function UsageCoverageNotice({
  ...
  const unsupportedProviders = ALL_PROVIDER_KINDS.filter(
    (provider) => PROVIDER_PRESENTATION[provider].unsupported === true,
  );
  const sourceNotices = environments.flatMap((environment) =>
    (environment.summary?.sources ?? []).flatMap((source) => {
      if (source.message === null) return [];
      const showOkCursorNote = source.fingerprint.provider === "cursor" && source.status === "ok";
      const showProblem =
        source.status === "failed" || source.status === "missing" || source.status === "partial";
      if (!showOkCursorNote && !showProblem) return [];
      const label =
        PROVIDER_PRESENTATION[source.fingerprint.provider]?.label ?? source.fingerprint.provider;
      return [
        {
          key: `${environment.environmentId}:${source.fingerprint.provider}:${source.status}`,
          text:
            showOkCursorNote && source.fingerprint.provider === "cursor"
              ? source.message
  ...
  // Deduplicate identical Cursor cycle notes across environments.
  const seenNotices = new Set<string>();
  const uniqueSourceNotices = sourceNotices.filter((notice) => {
    if (seenNotices.has(notice.text)) return false;
    seenNotices.add(notice.text);
    return true;
  });
  ...
  return (
    <div className="flex flex-col gap-1 border border-border px-3 py-2 text-xs text-muted-foreground">
      ...
      {uniqueSourceNotices.map((notice) => (
        <span key={notice.key}>{notice.text}</span>
      ))}
      {unsupportedProviders.length > 0 ? (
        <span>
          {unsupportedProviders.map((provider) => PROVIDER_PRESENTATION[provider].label).join(", ")}{" "}
          {unsupportedProviders.length === 1 ? "does" : "do"} not expose local usage data, so{" "}
          {unsupportedProviders.length === 1 ? "it isn't" : "they aren't"} counted above.
        </span>
      ) : null}
    </div>
  );
