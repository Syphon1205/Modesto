...
import { hiddenToastActionProps, stackedThreadToast, updatesToastManager } from "./ui/toast";
import { useAtomCommand } from "../state/use-atom-command";

const seenProviderUpdateNotificationKeys = new Set<string>();
type ProviderUpdateToastId = ReturnType<typeof updatesToastManager.add>;
...
function addProviderUpdateToast(input: {
  ...
  if (input.view.type === "loading" || input.view.type === "success") {
    return updatesToastManager.add({
  ...
  toastId = updatesToastManager.add(
    stackedThreadToast({
      type: input.view.type,
      title: input.view.title,
      description: input.view.description,
  ...
  useEffect(() => {
    return () => {
      ...
      if (activeToast?.kind === "prompt") {
        updatesToastManager.close(activeToast.toastId);
      }
  ...
  const openProviderSettings = useCallback(
    (toastId?: ProviderUpdateToastId) => {
      ...
      if (toastId !== undefined) {
        updatesToastManager.close(toastId);
