...
import type { VcsStatusResult } from "@modesto/contracts";
...
/**
 * The compact activity indicator shown next to a thread's name in the sidebar
 * preview rows and in the chat tab strip.
 *
 * One component so those two surfaces cannot drift: they previously had the
 * same spinner JSX pasted into each, which is also how the "failed" case ended
 * up missing from both. `resolveThreadStatusPill` has no error branch, so a
 * thread whose session errored produced no pill and rendered nothing at all -
 * silently hiding exactly the state a user most needs to notice. That case is
 * handled here rather than by widening `resolveThreadStatusPill`, which is
 * shared with the legacy sidebar and the kanban cards and would change how
 * they render too.
 *
 * Returns `null` for a resting thread; a resting row should be quiet.
 */
export function ThreadActivityIndicator({ thread }: { readonly thread: SidebarThreadSummary }) {
  ...
  if (status === "working") {
    return (
      <Tooltip>
        ...
        >
          <ThreadRunningSpinner className="size-3 text-sky-600 dark:text-sky-400" />
        </TooltipTrigger>
        <TooltipPopup side="top">Working</TooltipPopup>
      </Tooltip>
    );
  }
      ...
      <Tooltip>
        <TooltipTrigger
          render={
            <span
              aria-label="Failed"
              className="inline-flex size-3.5 shrink-0 items-center justify-center text-red-600 dark:text-red-400"
            />
          }
        >
          <span className="size-[9px] rounded-full bg-red-500 dark:bg-red-400" />
        </TooltipTrigger>
        <TooltipPopup side="top">Failed</TooltipPopup>
      </Tooltip>
    );
  }
  ...
  return pill ? <ThreadStatusLabel status={pill} compact /> : null;
}
