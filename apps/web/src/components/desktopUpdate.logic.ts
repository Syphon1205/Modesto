...
export function isDesktopUpdateButtonDisabled(state: DesktopUpdateState | null): boolean {
  // Keep "available" clickable so the user can start (or re-trigger) the download
  // even while a background prepare is queued — Cursor-style click-to-update.
  return state?.status === "downloading" || state?.status === "checking";
}

/** Blue notification badge on the download icon when an update exists but isn't installed yet. */
export function shouldShowDesktopUpdateAvailableBadge(state: DesktopUpdateState | null): boolean {
  if (!state?.enabled) return false;
  if (shouldHighlightDesktopUpdateError(state)) return false;
  return (
    state.status === "available" ||
    (state.status === "downloading" && getDesktopUpdateDownloadPercent(state) === null)
  );
}
...
export function getArm64IntelBuildWarningDescription(state: DesktopUpdateState): string {
  ...
  const action = resolveDesktopUpdateButtonAction(state);
  if (action === "download") {
    return "This Mac has Apple Silicon, but Modesto is still running the Intel build under Rosetta. Click Download to fetch the native Apple Silicon update.";
  }
  if (action === "install") {
    return "This Mac has Apple Silicon, but Modesto is still running the Intel build under Rosetta. Click Update to restart into the native Apple Silicon build.";
  }
...
export function getDesktopUpdateButtonTooltip(
  ...
  if (state.errorContext === "install" && !state.downloadedVersion && state.availableVersion) {
    return `Modesto restarted, but update ${state.availableVersion} was not installed. Click to try again.`;
  }
  ...
  if (state.status === "available") {
    return `Update ${state.availableVersion ?? ""} available. Click to download.`.trim();
  }
  if (state.status === "downloading") {
    const progress =
