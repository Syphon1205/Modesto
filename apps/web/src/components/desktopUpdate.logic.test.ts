...
describe("desktop update button state", () => {
  ...
  it("shows a download action when an update is available", () => {
    ...
    expect(getDesktopUpdateButtonLabel(state)).toBe("Download");
    expect(getDesktopUpdateButtonTooltip(state)).toContain("Click to download");
    expect(shouldShowDesktopUpdateAvailableBadge(state)).toBe(true);
  });
  ...
  it("rebuilds updater state after a failed install restart", () => {
    const state: DesktopUpdateState = {
      ...
      availableVersion: "1.1.0",
      downloadedVersion: null,
      message: "Modesto restarted before the update was installed.",
      errorContext: "install",
      canRetry: true,
    ...
    expect(getDesktopUpdateButtonTooltip(state)).toBe(
      "Modesto restarted, but update 1.1.0 was not installed. Click to try again.",
    );
  });
...
describe("desktop update UI helpers", () => {
  ...
  it("changes the warning copy when a native build update is being prepared", () => {
    ...
    expect(getArm64IntelBuildWarningDescription(state)).toContain("Click Download");
  });
});
