...
function createBaseServerConfig(): ServerConfig {
  return {
    ...
    keybindingsConfigPath: "/repo/project/.modesto-keybindings.json",
    keybindings: [],
    issues: [],
    providers: [
      {
  ...
  return {
    ...
    projects: [
      ...snapshot.projects,
      {
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/Users/tester/Documents/Modesto/Studio",
        defaultModelSelection: {
          provider: "codex",
          model: "gpt-5",
        },
...
describe("ChatView timeline estimator parity (full app)", () => {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            cwd: "/repo/project",
            env: {
              MODESTO_PROJECT_ROOT: "/repo/project",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              MODESTO_PROJECT_ROOT: "/repo/project",
              MODESTO_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
  ...
  it("toggles plan mode with Shift+Tab only while the composer is focused", async () => {
    ...
    try {
      ...
      const composerEditorAfterPlan = await waitForComposerEditor();
      composerEditorAfterPlan.focus();
      composerEditorAfterPlan.dispatchEvent(
        new KeyboardEvent("keydown", {
          key: "Tab",
          shiftKey: true,
          bubbles: true,
    ...
    const mounted = await mountChatView({
      ...
      configureFixture: (nextFixture) => {
        nextFixture.welcome = {
          ...
          chatWorkspaceRoot: "/Users/tester/Documents/Modesto",
    ...
    try {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(createWorktreeRequest?.newBranch).toMatch(/^modesto\/[0-9a-f]{8}$/);
  ...
  it("runs the setup action from the newly-created worktree before starting the turn", async () => {
    ...
    try {
      ...
      expect(terminalOpenRequest).toMatchObject({
        ...
        env: {
          MODESTO_PROJECT_ROOT: "/repo/project",
          MODESTO_WORKTREE_PATH: worktreePath,
        },
      });
  ...
  it("shows the skinny inline plan card for active turn plans", async () => {
    ...
    try {
      await vi.waitFor(
        () => {
          expect(document.body.textContent).toContain("1/3 tasks");
          expect(document.body.textContent).toMatch(/\d+ Working/);
          expect(document.querySelector('[data-testid="composer-chrome-bubbles"]')).not.toBeNull();
        },
        { timeout: 8_000, interval: 16 },
      );

      const transcriptPane = document.querySelector<HTMLElement>("[data-chat-transcript-pane]");
      const chromeBubbles = document.querySelector<HTMLElement>(
        '[data-testid="composer-chrome-bubbles"]',
      );
      const composerShell = document.querySelector<HTMLElement>(
        'form[data-chat-composer-form="true"] .chat-composer-shell',
      );
      expect(transcriptPane).not.toBeNull();
      expect(chromeBubbles).not.toBeNull();
      expect(composerShell).not.toBeNull();
      expect(transcriptPane!.getBoundingClientRect().bottom).toBeGreaterThan(
        chromeBubbles!.getBoundingClientRect().top + 1,
      );
      ...
      const openPlanButton = await waitForElement(
        () => document.querySelector<HTMLButtonElement>('button[title="Open task list"]'),
        "Unable to find composer tasks bubble.",
      );
    ...
    try {
      await vi.waitFor(
        () => {
          ...
          expect(document.body.textContent).toContain("1/3 tasks");
          expect(document.body.textContent).not.toMatch(/\d+ Working/);
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
  ...
  it("hides a completed task list once the latest turn is settled", async () => {
    ...
    try {
      await vi.waitFor(
        () => {
          expect(document.body.textContent).toContain("Finished the investigation.");
          expect(document.body.textContent).not.toContain("3/3 tasks");
          expect(
            document.querySelector('button[aria-label="3 of 3 tasks"]'),
          ).toBeNull();
        },
        { timeout: 8_000, interval: 16 },
      );
