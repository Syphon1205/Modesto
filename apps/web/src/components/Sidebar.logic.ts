...
import type { SidebarProjectSortOrder, SidebarThreadSortOrder } from "@modesto/contracts/settings";
import {
...
/**
 * Child conversations (sidechats and threads started from a chat) keyed by
 * the parent they belong to. Archived rows stay out of the live sidebar tree.
 */
export function groupChildThreadsByParentId<
  T extends {
    readonly id: string;
...
>(threads: readonly T[]): Map<string, T[]> {
  ...
  for (const [parentId, list] of map) {
    map.set(parentId, sortThreadsForSidebar(list));
  }
  return map;
}

/**
 * Top-level code/project threads in the sidebar. Any live child with a
 * resolvable parentThreadId is excluded so multiagent spawns nest under
 * their parent instead of looking like brand-new chats.
 */
export function selectTopLevelCodeThreads<
  T extends {
    readonly id: string;
    readonly conversationMode?: string | undefined;
    readonly parentThreadId?: string | null | undefined;
  },
>(threads: readonly T[]): T[] {
  const knownIds = new Set(threads.map((thread) => thread.id));
  return threads.filter((thread) => {
    if (thread.conversationMode === "chat") return false;
    const parentId = thread.parentThreadId ?? null;
    return parentId === null || !knownIds.has(parentId);
  });
}

/**
 * Live code children with a resolvable parent (chat or code). Used for
 * search indexing and for nesting under non-chat parents.
 */
export function selectNestedCodeThreads<
  T extends {
    readonly id: string;
    readonly conversationMode?: string | undefined;
    readonly archivedAt: string | null;
...
>(threads: readonly T[]): T[] {
  const knownIds = new Set(threads.map((thread) => thread.id));
  return threads.filter((thread) => {
    if (thread.conversationMode === "chat" || thread.archivedAt !== null) return false;
    const parentId = thread.parentThreadId ?? null;
    return parentId !== null && knownIds.has(parentId);
  });
}
...
export {
  ...
  pinOrderKeyBetween,
  planPinnedReorder,
} from "@modesto/client-runtime/state/thread-sort";
export { sortPinnedThreadsByOrderKey as sortPinnedThreadsForSidebar } from "@modesto/client-runtime/state/thread-sort";
