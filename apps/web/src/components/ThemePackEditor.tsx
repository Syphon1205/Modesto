...
// Exports: ThemePackEditor and ThemeFontsEditor
...
export function ThemePackEditor({
  ...
  const defaultTheme = defaultPack.theme;
  ...
  const isPristine = areThemePacksEqual(
    { ...pack, theme: { ...theme, fonts: defaultTheme.fonts } },
    defaultPack,
  );
        ...
        <div className="flex items-center gap-2">
          ...
          {!isPristine ? (
            <button
              type="button"
              onClick={() => {
                updateThemePack(variant, { ...defaultTheme, fonts: theme.fonts });
                setCodeThemeId(variant, defaultPack.codeThemeId);
              }}
...
export function ThemeFontsEditor({ variant }: { variant: ThemeVariant }) {
  const { darkTheme, lightTheme, updateThemeFonts } = useTheme();
  const theme = variant === "dark" ? darkTheme.theme : lightTheme.theme;
  const titleLabel = variant === "dark" ? "Dark theme fonts" : "Light theme fonts";
  const ariaLabel = variant === "dark" ? "Dark theme" : "Light theme";

  return (
    <div className={cn(SETTINGS_CARD_CLASS_NAME, "overflow-hidden")}>
      <div className="border-b border-[color:var(--color-border)] px-4 py-3 sm:py-3.5">
        <h3 className="text-sm font-medium text-foreground">{titleLabel}</h3>
      </div>
      <div className="divide-y divide-[color:var(--color-border)]">
