...
import type { DeviceUdid } from "@modesto/contracts";
import type { DeviceFrame } from "@modesto/shared/deviceFrame";
...
export function useDeviceVideoStream(input: {
  ...
  /** Authenticated RPC socket URL for the device's environment. */
  readonly socketUrl?: string | null;
  /**
   * Called after each decoded frame has been painted.
   *
   * Read through a ref rather than captured, so a consumer can react to every
   * frame without a state update per frame — the 3D stage uses it to schedule a
   * redraw at video rate while free look is engaged.
   */
  readonly onFrame?: (() => void) | undefined;
}): { readonly status: DeviceVideoStatus; readonly dimensions: DeviceVideoDimensions | null } {
  const { canvasRef, udid, enabled, socketUrl } = input;
  ...
  useEffect(() => {
    ...
    const openFrameSource = () =>
      createDeviceFrameSource({
        udid,
        // Spread rather than assigned: the option is optional and typed
        // `string | null`, so passing an explicit `undefined` is a type error
        // under exactOptionalPropertyTypes.
        ...(socketUrl !== undefined ? { explicitUrl: socketUrl } : {}),
    ...
    const configureDecoder = (frame: DeviceFrame) => {
      ...
      if (!codec) {
        failStream("The simulator stream sent parameters Modesto could not read.");
        return;
      }
      teardownDecoder();
    ...
    const submit = (frame: DeviceFrame, keyframe: boolean) => {
      ...
      try {
        ...
        // A static simulator framebuffer can sit on one IDR for a long time.
        // With `optimizeForLatency`, Chromium may hold that first output until
        // more AU data or an explicit flush — without this the pane stays on
        // "Connecting…" even though the socket already delivered a keyframe.
        if (keyframe) {
      ...
      setStatus({
        ...
        message:
          reason === "decode-failed"
            ? "The simulator stream sent a frame Modesto could not read."
  ...
  }, [canvasRef, udid, enabled, socketUrl]);
