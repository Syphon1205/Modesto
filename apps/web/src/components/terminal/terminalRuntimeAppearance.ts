...
export function terminalThemeFromApp(): ITheme {
  const configuredTheme = document.documentElement.getAttribute("data-terminal-theme");
  if (configuredTheme && configuredTheme in VS_CODE_TERMINAL_THEMES) {
    return VS_CODE_TERMINAL_THEMES[configuredTheme as keyof typeof VS_CODE_TERMINAL_THEMES];
  }
