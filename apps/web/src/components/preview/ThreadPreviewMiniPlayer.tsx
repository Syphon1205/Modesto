...
const COMPACT_MEDIA_PLAYER_SIZE = { width: 320, height: 92 } as const;
...
export function ThreadPreviewMiniPlayer({ threadRef, tabId, bottomInset }: Props) {
  ...
  const compactMediaCard = shouldShowCompactMediaCard({
    audible: desktopOverlay?.audible ?? false,
    pageUrl,
  ...
  const size =
    miniPlayer?.tabId === tabId && miniPlayer.size
      ? miniPlayer.size
      : PREVIEW_MINI_PLAYER_DEFAULT_SIZE;
  const displayedSize = compactMediaCard ? COMPACT_MEDIA_PLAYER_SIZE : size;
  const close = () => {
  ...
  useEffect(() => {
    if (!desktopOverlay?.audible || !desktopOverlay.hasWebContents || !previewBridge) {
  ...
  }, [desktopOverlay?.audible, desktopOverlay?.hasWebContents, pageTitle, pageUrl, runtimeTabId]);
  ...
  useLayoutEffect(() => {
    const clampAndMove = () => {
      ...
      const nextSize = compactMediaCard
        ? COMPACT_MEDIA_PLAYER_SIZE
        : clampPreviewMiniPlayerSize(
  ...
  }, [bottomInset, compactMediaCard, position, tabId, threadRef]);
  ...
  return (
    ...
    >
      ...
      <div className="relative h-full min-h-0">
        ...
        {compactMediaCard && mediaMetadata ? (
          <div className="absolute inset-0 z-[30] flex items-center gap-3 overflow-hidden rounded-xl bg-popover/96 p-2.5 pr-9 text-left shadow-xl backdrop-blur-xl">
            <div className="flex size-[72px] shrink-0 items-center justify-center overflow-hidden rounded-lg bg-muted ring-1 ring-inset ring-border/60">
              {mediaMetadata.artworkUrl || desktopOverlay?.favicon?.dataUrl ? (
                <img
                  src={mediaMetadata.artworkUrl ?? desktopOverlay?.favicon?.dataUrl}
                  alt=""
            ...
            <div className="min-w-0 flex-1">
              ...
              {mediaMetadata.artist || mediaMetadata.album ? (
                <div className="mt-0.5 truncate text-xs text-muted-foreground">
                  {[mediaMetadata.artist, mediaMetadata.album].filter(Boolean).join(" · ")}
                </div>
              ) : null}
              <div className="mt-2 truncate text-[11px] font-medium text-muted-foreground/80">
                Playing from {mediaMetadata.sourceLabel}
              </div>
            </div>
          </div>
        ) : (
