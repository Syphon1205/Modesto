...
const PREVIEW_PANEL_WIDTH_STORAGE_KEY = "modesto:preview-panel-width";
...
export function PreviewPanelShell(props: {
  ...
  /** Inline panels stay mounted while closed so their exit motion can finish. */
  open?: boolean;
  maximized?: boolean;
  /**
  ...
  return (
    <div
      ref={hostRef}
      className={cn(
        "relative h-full min-h-0 min-w-0 max-w-full self-stretch bg-background",
        isInline
          ? open && props.maximized
            ? "flex-1 border-l border-border"
            : cn("shrink-0", open && "border-l border-border")
          : "w-full",
        isInline && disclosurePanelClassName(open, "right"),
      )}
      style={isInline && !props.maximized ? { width: open ? `${width}px` : "0px" } : undefined}
    ...
    >
      <div
        className="relative flex h-full min-h-0 max-w-full flex-col"
        style={isInline && !props.maximized ? { width: `${width}px` } : undefined}
      >
        {isInline && !props.maximized && open ? (
