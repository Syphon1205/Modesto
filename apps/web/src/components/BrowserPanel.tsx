...
import { localServerPrimaryLabel } from "@modesto/shared/localServers";
import {
  BROWSER_BLANK_URL,
  isBlankBrowserTabUrl,
  resolveCopyableBrowserTabUrl,
} from "@modesto/shared/browserSession";
import {
  BROWSER_COPY_LINK_TOAST_TITLE,
  isBrowserCopyLinkChord,
} from "@modesto/shared/browserShortcuts";
...
const BROWSER_WEBVIEW_PARTITION = "persist:modesto-browser";
...
function isBrowserPerfLoggingEnabled(): boolean {
  ...
  try {
    return window.localStorage.getItem("modesto:browser-perf") === "1";
  } catch {
    return false;
  }
}
...
export function BrowserPanel({
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      console.info(`[${MODESTO_BROWSER_LABEL} panel perf]`, {
        threadId,
