...
async function moveEffortSlider(keys: string): Promise<void> {
...
describe("TraitsPicker (Claude)", () => {
  ...
  it("shows prompt-controlled Ultrathink state with disabled effort controls", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(document.querySelector('.effort-slider[data-rainbow="true"]')).not.toBeNull();
    });
  });
  ...
  it("persists sticky claude model options when traits change", async () => {
    ...
    await page.getByRole("button").click();
    await moveEffortSlider("{ArrowRight>2/}");
...
describe("TraitsPicker (Codex)", () => {
  ...
  it("closes after committing a slider stop", async () => {
    await using _ = await mountCodexPicker({
    ...
    await moveEffortSlider("{ArrowRight}");
