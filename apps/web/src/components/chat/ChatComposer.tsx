...
import {
  inferProviderSendTurnFileMimeType,
  isProviderSendTurnSupportedFileAttachment,
...
import type { EnvironmentConnectionPresentation } from "@modesto/client-runtime/connection";
import { serializeComposerFileLink } from "@modesto/shared/composerTrigger";
import { createModelSelection, normalizeModelSlug } from "@modesto/shared/model";
import {
  memo,
  type ReactNode,
  useCallback,
...
import { type AppshotContextDraft } from "../../lib/appshotContext";
import { ComposerPendingElementContexts } from "./ComposerPendingElementContexts";
import { ComposerPendingAppshotContexts } from "./ComposerPendingAppshotContexts";
...
import { builtInComposerSlashCommandItems } from "./composerSlashCommands";
import {
  nativeAppMentionLabel,
  NATIVE_APPS,
  searchNativeAppMentions,
  type NativeApp,
} from "@modesto/shared/nativeApps";
import type { WebApp } from "~/connections/webApps";
import { searchWebAppMentions, WEB_APPS, webAppMentionLabel } from "~/connections/webApps";
import { webAppSlashLabel } from "~/connections/webAppSlash";
import {
...
import type { UnifiedSettings } from "@modesto/contracts/settings";
...
import { formatProviderSkillDisplayName } from "@modesto/client-runtime/providerSkills";
import { searchProviderSkills } from "../../providerSkillSearch";
import { useMediaQuery } from "../../hooks/useMediaQuery";
import type { ReviewCommentContext } from "../../reviewCommentContext";

// Color-coded by how much a mode does without asking: green (always asks) up
// through red (never asks) - the same low-to-high-risk ladder as the "danger"
// glyphs used elsewhere for permissions, so the trigger icon reads at a glance
    ...
    <>
      ...
      <Tooltip>
        ...
        >
          ...
          >
            <ComposerControlIcon icon={RuntimeModeIcon} className={runtimeModeOption.tone} />
                  ...
                  <div className="flex min-w-0 items-center gap-3">
                    <div className="grid min-w-0 flex-1 gap-0.5">
                      <span className="inline-flex items-center gap-1.5 font-medium text-foreground">
                        <OptionIcon className={cn("size-3.5 shrink-0", option.tone)} />
                        {option.label}
                      </span>
...
export interface ChatComposerHandle {
  ...
  getSendContext: () => {
    ...
    appshotContexts: AppshotContextDraft[];
...
export interface ChatComposerProps {
  ...
  /** Chat keeps the composer conversational; Code exposes project controls. */
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const composerAppshotContexts = composerDraft.appshotContexts;
  ...
  const removeComposerDraftAppshotContext = useComposerDraftStore(
    (store) => store.removeAppshotContext,
  );
  ...
  const insertComposerTextAtEndRef = useRef<
    ((text: string, options?: { ensureLeadingBoundary?: boolean }) => boolean) | null
  >(null);
  ...
  const composerVoice = useComposerVoiceController({
    onTranscript: useCallback((transcript: string) => {
      insertComposerTextAtEndRef.current?.(transcript, { ensureLeadingBoundary: true });
    }, []),
  });
  ...
  const activeContextWindow = useMemo(
    () =>
      resolveVisibleContextWindowSnapshot({
        ...
        persistedModelSelection: activeThreadModelSelection,
  ...
  const composerMenuItems = useMemo<ComposerCommandItem[]>(() => {
    ...
    if (composerTrigger.kind === "path") {
      const nativeAppItems = searchNativeAppMentions(composerTrigger.query).map((match) => ({
        id: `native-app:${match.app.id}`,
        type: "native-app" as const,
        app: match.app,
        label: nativeAppMentionLabel(match.app),
        description: match.app.capabilityHint,
        didYouMean: match.didYouMean,
      }));
      const webAppItems = searchWebAppMentions(composerTrigger.query).map((match) => ({
        id: `web-app:${match.app.id}`,
        type: "web-app" as const,
        app: match.app,
        label: webAppMentionLabel(match.app),
        description: match.app.capabilityHint,
        didYouMean: match.didYouMean,
      }));
      ...
      return [...nativeAppItems, ...webAppItems, ...pathItems];
    }
    if (composerTrigger.kind === "slash-command") {
      const builtInSlashCommandItems = builtInComposerSlashCommandItems({
      ...
      const nativeAppItems =
        query.length === 0
          ? []
          : NATIVE_APPS.map((app) => ({
              id: `native-app:${app.id}`,
              type: "native-app" as const,
              app,
              label: nativeAppMentionLabel(app),
              description: app.capabilityHint,
            }));
      const webAppItems =
        query.length === 0
          ? []
  ...
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      const persistableImages = composerImages.filter((attachment) => attachment.type === "image");
      if (persistableImages.length === 0) {
        if (composerImages.length === 0) {
          clearComposerDraftPersistedAttachments(composerDraftTarget);
        } else {
          syncComposerDraftPersistedAttachments(composerDraftTarget, []);
        }
        return;
      }
      ...
      try {
        ...
        await Promise.all(
          persistableImages.map(async (image) => {
            try {
  ...
  const onSelectComposerItem = useCallback(
    (item: ComposerCommandItem) => {
      ...
      if (item.type === "native-app") {
        const replacement = `${nativeAppMentionLabel(item.app)} `;
        const replacementRangeEnd = extendReplacementRangeForTrailingSpace(
          snapshot.value,
          trigger.rangeEnd,
          replacement,
        ...
        onOpenNativeApp(item.app);
        return;
      }
      if (item.type === "web-app") {
        const replacement = `${webAppMentionLabel(item.app)} `;
        const replacementRangeEnd = extendReplacementRangeForTrailingSpace(
          snapshot.value,
          trigger.rangeEnd,
          replacement,
        ...
        onOpenWebApp(item.app);
        return;
      }
      if (item.type === "slash-command") {
        if (item.command === "multiagent") {
          const replacement = "/multiagent ";
          const replacementRangeEnd = extendReplacementRangeForTrailingSpace(
            snapshot.value,
            trigger.rangeEnd,
            replacement,
  ...
  const stashCurrentPrompt = useCallback(async () => {
    ...
    try {
      ...
      for (const image of images) {
        if (image.type === "file") {
          // File bytes are not written to localStorage; the stash only
          // round-trips images. Names are reported so restore can say why.
  ...
  const addComposerImages = async (files: File[]) => {
    ...
    const acceptedDocuments: File[] = [];
    let error: string | null = null;
    for (const file of files) {
      const isSupportedImage = isProviderSendTurnSupportedImageMimeType(file.type);
      ...
      if (!isSupportedImage && !isSupportedFile) {
        error = `'${file.name}' is not a supported attachment.`;
        continue;
      }
      if (!isSupportedImage && file.size > PROVIDER_SEND_TURN_MAX_FILE_BYTES) {
        error = `'${file.name}' is too large to attach.`;
        continue;
      }
      ...
      if (isSupportedImage) {
        ...
        acceptedDocuments.push(file);
      }
      reservedCount += 1;
    }
    setThreadError(threadId, error);
    const acceptedCount = acceptedImages.length + acceptedDocuments.length;
    if (acceptedCount === 0) return;

    pendingImageCompressionsRef.current.set(threadId, pendingCount + acceptedCount);
    try {
      const nextAttachments: ComposerImageAttachment[] = acceptedDocuments.map((file) => {
        const mimeType = inferProviderSendTurnFileMimeType({
          mimeType: file.type,
          fileName: file.name,
        });
        return {
      ...
      const remaining = (pendingImageCompressionsRef.current.get(threadId) ?? 0) - acceptedCount;
      if (remaining > 0) {
  ...
  // A voice note lands exactly like a dropped mention: appended to whatever is
  // already typed, with a space inserted if needed, rather than replacing it.
  // Routed through a ref because `insertComposerTextAtEnd` is rebuilt every
  // render, and depending on it directly would rebuild the controller's
  // callbacks (and so its identity) on every keystroke.
            ...
            >
              ...
              {!isComposerCollapsedMobile &&
                ...
                composerAppshotContexts.length > 0 && (
                  <ComposerPendingAppshotContexts
                    contexts={composerAppshotContexts}
                    onRemove={(contextId) =>
                      removeComposerDraftAppshotContext(composerDraftTarget, contextId)
                    }
                    className="mb-3"
                  />
                )}
                    ...
                    {composerImages
                      ...
                      .map((attachment) =>
                        attachment.type === "file" ? (
                          <div
                            key={attachment.id}
                            className="relative flex max-w-[220px] items-center gap-2 rounded-lg border border-border/80 bg-background px-2.5 py-2"
                          >
                            <ComposerFileAttachmentIcon
