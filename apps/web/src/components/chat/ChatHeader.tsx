...
export const ChatHeader = memo(function ChatHeader({
  ...
  // The right-side panel toggle stays in the header in both layouts — beside the
  // Environment button when that is enabled, and inside the legacy cluster otherwise —
  // so collapsing or reopening the dock is always one click away. When the thread
  // already has dock tabs, this controls dock visibility; otherwise it opens Review.
  const rightPanelTogglePressed = hasRightDockPanes ? rightDockOpen : diffOpen;
  const handleRightPanelToggle = useCallback(() => {
    if (hasRightDockPanes) {
      onToggleRightDock?.();
      return;
    }
    onToggleDiff();
  }, [hasRightDockPanes, onToggleDiff, onToggleRightDock]);
      ...
      <TooltipTrigger
        render={
          <Toggle
            ...
            pressed={rightPanelTogglePressed}
            onPressedChange={handleRightPanelToggle}
            aria-label={hasRightDockPanes ? "Toggle side panel" : "Toggle diff panel"}
            variant="default"
            size="xs"
            disabled={
              !hasRightDockPanes && (!isGitRepo || (diffDisabledReason !== null && !diffOpen))
            }
          >
            {showDiffTotals ? (
            ...
            <TooltipTrigger
              render={
                <ChatHeaderButton
                  type="button"
                  tone="outline"
                  className={cn("gap-1.5", monitorAction.active && "bg-secondary")}
                  aria-pressed={monitorAction.active}
                  aria-label={
                    monitorAction.active ? "Hide research monitor" : "Show research monitor"
                  }
                  onClick={monitorAction.onClick}
                >
                  <TelescopeIcon className="size-3.5" />
                  {!compact ? <span className="font-normal">Monitor</span> : null}
                </ChatHeaderButton>
              }
            />
            <TooltipPopup side="bottom">
              {monitorAction.active ? "Hide research monitor" : "See what the model is viewing"}
            </TooltipPopup>
          </Tooltip>
        ) : null}
              ...
              <TooltipTrigger
                render={
                  ...
                  >
                    ...
                    {!compact ? <span className="truncate font-normal">Open in</span> : null}
                  </MenuTrigger>
                }
              />
            ...
            <ComposerPickerMenuPopup align="end" side="bottom" className="w-48 min-w-48">
              {handoffActionTargetProviders.map((provider) => (
                <MenuItem key={provider} onClick={() => onCreateHandoff(provider)}>
                  ...
                  <span>Open in {PROVIDER_DISPLAY_NAMES[provider]}</span>
                </MenuItem>
              ))}
            </ComposerPickerMenuPopup>
          </Menu>
            ...
            <TooltipTrigger
              render={
                <ChatHeaderIconButton
                  ...
                  className={
                    terminalOpen ? "bg-[var(--color-background-button-secondary)]" : undefined
                  }
        ...
        {environment ? (
          ...
          <>
            {/* Open in editor: dedicated split-button with an editor switcher; the project
                action control now lives beside Open in as its own project command surface. */}
            {activeProjectName ? (
              <OpenInPicker
                keybindings={keybindings}
