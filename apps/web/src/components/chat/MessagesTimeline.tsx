...
import { resolveLatestTailUserMessageEditTarget } from "@modesto/shared/conversationEdit";
...
import { MessageProvenanceChip } from "./MessageProvenanceChip";
...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  const renderRowContent = (row: MessagesTimelineRow) => (
    ...
    >
          ...
          return (
            <div className="flex w-full justify-end">
              ...
              >
                ...
                />
                {(row.message.provenance ||
                  row.message.source === "handoff-import" ||
                  row.message.source === "fork-import") && (
                  <div className="mb-1 self-end">
                    <MessageProvenanceChip
                      source={row.message.source}
                      provenance={row.message.provenance}
                    />
                  </div>
                )}
      ...
      {row.kind === "message" &&
        ...
        (() => {
          ...
          const showProvenanceChip =
            Boolean(row.message.provenance) ||
            row.message.source === "handoff-import" ||
            row.message.source === "fork-import";
            ...
            <>
              ...
              <div className="group min-w-0 py-0.5">
                ...
                {(() => {
                  ...
                  const singleEditedFile =
                    checkpointFiles.length === 1 ? checkpointFiles[0] : undefined;
                  const changedFilesTitle = singleEditedFile
                    ? `Edited ${basename(singleEditedFile.path)}`
                    : editedFilesLabel;
                  const canExpandFileList = checkpointFiles.length > 1;
                  ...
                  const renderCheckpointFileRow = (
                    ...
                    <button
                      ...
                      type="button"
                      className={cn(
                        "group/file-row flex w-full min-w-0 items-center gap-2 border-t border-[color:var(--color-border-light)] bg-transparent px-3 py-2.5 text-left transition-colors hover:bg-[var(--color-background-button-secondary-hover)] dark:bg-transparent dark:hover:bg-transparent",
                    ...
                    >
                      ...
                      />
                      <span
                        className="font-system-ui min-w-0 flex-1 truncate font-normal text-[var(--color-text-foreground)] underline-offset-2 group-hover/file-row:underline group-focus-visible/file-row:underline"
                        style={{ fontSize: chatTypographyStyle.fontSize }}
                      >
                        {file.path}
                      </span>
                  ...
                  return (
                    <div
                      className="mt-3 mb-4 overflow-hidden rounded-[0.9rem] border border-[color:var(--color-border-light)] bg-[color:color-mix(in_srgb,var(--app-user-message-background)_24%,transparent)] shadow-[0_1px_0_rgb(255_255_255/0.025)_inset] dark:border-[color:color-mix(in_srgb,var(--color-border-light)_65%,transparent)]"
                      data-edited-files-card="true"
                    >
                      <div
                        className={cn(
                      ...
                      >
                        <div
                          ...
                          title={singleEditedFile?.path}
                        >
                          <span className="flex size-10 shrink-0 items-center justify-center rounded-[0.7rem] border border-[color:var(--color-border-light)] bg-background/45 shadow-sm">
                            {singleEditedFile ? (
                              <FileEntryIcon
                                pathValue={singleEditedFile.path}
                                kind="file"
                                theme={resolvedTheme}
                                className="size-5"
                              />
                            ) : (
                              <ChangesIcon className="size-5 text-muted-foreground/75" />
                            )}
                          </span>
                          <div className="min-w-0">
                            <div
                              className="truncate font-system-ui font-medium text-foreground/95"
      ...
      {row.kind === "working-header" && (
        <div>
          {/* Compact live timer — the durable Working bubble above the composer is
              the primary status chrome; this stays a thin transcript cue. */}
          <div
            className="-ml-0.5 pb-1.5 text-muted-foreground/55"
            style={{ fontSize: chatTypographyStyle.fontSize }}
          >
            Working for{" "}
            {nowIso ? (
            ...
            if (canOpenEditedDiff) {
              return (
                <button
                  ...
                  onClick={() => onOpenTurnDiff!(turnId!, changedFilePath)}
                >
                  {editedRowChildren}
                </button>
              );
