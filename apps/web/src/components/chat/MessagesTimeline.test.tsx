import { CheckpointRef, EnvironmentId, MessageId, TurnId } from "@modesto/contracts";
import { codexFeedbackMessage } from "@modesto/client-runtime/state/threads";
...
describe("MessagesTimeline", () => {
  ...
  it("renders long user messages in full", () => {
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
    ...
    expect(markup).toContain("deep hidden detail only after expand");
    expect(markup).toContain('data-maintain-scroll-at-end="enabled"');
  ...
  it("renders long assistant markdown in full in chat mode", () => {
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        conversationMode="chat"
        timelineEntries={[buildAssistantTimelineEntry(buildLongUserMessageText())]}
      />,
    );

    expect(markup).not.toContain("Show full message");
    expect(markup).toContain("deep hidden detail only after expand");
    expect(markup).toContain('data-assistant-message-collapsed="false"');
    expect(markup).toContain('data-assistant-message-collapsible="false"');
  });

  it("renders long assistant markdown in full in code mode", () => {
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        ...
        timelineEntries={[
          {
            ...
            entry: {
              ...
              label: "Updated files",
              tone: "tool",
              changedFiles: ["C:/Users/mike/dev-stuff/modesto/apps/web/src/session-logic.ts"],
            },
          },
        ]}
        workspaceRoot="C:/Users/mike/dev-stuff/modesto"
      />,
    );

    expect(markup).toContain("Changed 1 file");
    expect(markup).not.toContain("C:/Users/mike/dev-stuff/modesto/apps/web/src/session-logic.ts");
  });
  ...
  it("renders the Thinking row with its dot-matrix flicker icon alongside the working timer", () => {
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        isWorking
    ...
    // The Thinking row now carries the same flicker icon every tool-call row
    // does (see MessagesTimeline.tsx's LiveActivityContent flickerGrids prop),
    // so it takes the icon-bearing padding (px-0.5), not the iconless one.
