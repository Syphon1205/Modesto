...
/**
 * Cursor 3 keeps its workspace panels (Changes, Browser, Context Usage, …)
 * visible as a persistent tab strip at the top of the right panel, whether or
 * not the panel is open yet — clicking an unopened tab opens it. These are the
 * kinds that always appear in the strip; everything else stays behind "+".
 */
const RIGHT_DOCK_STRIP_KINDS: readonly RightDockPaneKind[] = [
  "diff",
  "browser",
  "terminal",
  "explorer",
...
export function RightDock(props: RightDockProps) {
  ...
  // Cursor-style persistent strip: workspace kinds without an open pane render
  // as inactive tabs that open on click; the "+" menu only offers the rest.
  const openPaneKinds = new Set(props.state.panes.map((pane) => pane.kind));
  const stripGhostKinds = RIGHT_DOCK_STRIP_KINDS.filter(
    (kind) => props.addMenuKinds.includes(kind) && !openPaneKinds.has(kind),
  );
  const addMenuKinds = props.addMenuKinds.filter((kind) => !RIGHT_DOCK_STRIP_KINDS.includes(kind));
  ...
  useEffect(() => {
    ...
    const targetWidth = Math.max(minWidth, Math.round(shell.getBoundingClientRect().width / 2));
    if (targetWidth <= 0) {
      return;
    }
