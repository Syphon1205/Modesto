...
function SpawnedThreadRow({
  thread,
  onOpen,
}: {
  thread: SpawnedThreadLike;
  onOpen?: ((threadId: ThreadId) => void) | undefined;
}) {
  const kind = spawnedThreadKind(thread);
  const live = spawnedThreadIsLive(thread);
  const status = spawnedThreadStatusLabel(thread);
  const Icon = kind === "side" ? MessagesSquare : Split;
  const role = spawnedThreadRoleLabel(kind);
  ...
  return (
    ...
    >
      <span className="col-start-1 row-start-1 flex items-center">
        <span
          ...
          className={cn(
            ...
            live ? "bg-info" : "bg-muted-foreground/50",
          )}
        />
      </span>
      <span className="col-start-2 row-start-1 flex min-w-0 items-baseline gap-2">
      ...
      <span className="col-start-3 row-start-1 text-right font-mono text-[.7rem] text-muted-foreground/80">
        {status}
      </span>
      <span className="col-start-2 col-end-4 row-start-2 block truncate text-xs text-muted-foreground">
        {kind === "side" ? "Side conversation in this thread" : "Parallel agent in this workspace"}
      </span>
      <span className="col-start-2 col-end-4 row-start-3 flex items-center gap-1 truncate font-mono text-[.7rem] text-muted-foreground/70">
        <Icon aria-hidden className="size-3" />
        {kind === "side" ? "Open sidechat" : "Open multi-agent"}
      </span>
    </button>
  );
}
...
export function AgentsPanel({
  ...
  spawnedThreads?: ReadonlyArray<SpawnedThreadLike>;
  environmentId?: EnvironmentId | null;
  threadId?: ThreadId | null;
  onOpenThread?: (threadId: ThreadId) => void;
}) {
  if (!model.hasAgents && spawnedThreads.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-2 p-6 text-center">
        <Bot aria-hidden className="size-6 text-muted-foreground/60" />
        <p className="text-sm font-medium">No agents yet</p>
        <p className="max-w-56 text-xs text-muted-foreground">
          Sidechats, additional agents, and provider subagents show up here with live status.
        </p>
      </div>
    );
  }
        ...
        <div className="flex flex-col gap-2 p-2">
          {spawnedThreads.length > 0 ? (
            <section>
              ...
              {spawnedThreads.map((thread) => (
                <SpawnedThreadRow key={thread.id} thread={thread} onOpen={onOpenThread} />
              ))}
            </section>
          ) : null}
