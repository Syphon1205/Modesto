...
import { isWorkspaceImagePreviewPath } from "@modesto/shared/filePreview";
...
const FILE_EXPLORER_STORAGE_KEY = "modesto.fileExplorerOpen";
const RENDER_MARKDOWN_STORAGE_KEY = "modesto.renderMarkdown";
const IDE_MODE_STORAGE_KEY = "modesto.fileIdeMode";
...
/**
 * The IDE surface: VS Code's editor over the same file, saving through the
 * same coordinator as the review surface so debounce, pending state, and
 * conflict confirmation cannot drift between the two.
 */
function IdeFileSurface({
  environmentId,
  cwd,
  relativePath,
  contents,
  ...
  EditableFileSurfaceProps,
  "environmentId" | "cwd" | "relativePath" | "contents" | "resolvedTheme" | "onPendingChange"
>) {
  ...
  return (
    <MonacoFileEditor
      ...
      onChange={(next: string) => coordinator.change(next)}
    />
  );
}
...
export default function FilePreviewPanel({
  ...
  // Which editing surface a developer prefers is a standing preference, not a
  // property of one file, so it persists the same way word wrap does.
  const [ideMode, setIdeMode] = useLocalStorage(IDE_MODE_STORAGE_KEY, false, Schema.Boolean);
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-background">
      {relativePath ? (
        ...
        >
          ...
          <Tooltip>
            <TooltipTrigger
              render={
                <Toggle
                  className="shrink-0"
                  pressed={ideMode}
                  onPressedChange={setIdeMode}
                  aria-label={ideMode ? "Switch to review view" : "Switch to editor view"}
                  variant="ghost"
                  size="sm"
                >
            ...
            <TooltipPopup>
              {ideMode
                ? "Review view - inline comments and annotations"
                : "Editor view - multi-cursor, find and replace"}
            </TooltipPopup>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        {/* Explorer first, so it sits on the leading edge the way every IDE
            puts it. It used to trail the editor, which is the single most
            recognizable difference from an editor people already know. */}
        {explorerOpen || relativePath === null ? (
          <aside
            className={cn(
              "flex min-h-0 shrink-0 bg-background",
              relativePath
                ? "w-[min(22rem,46%)] min-w-64 border-e border-border/60"
                : "min-w-0 flex-1",
            )}
          >
            <FileBrowserPanel
