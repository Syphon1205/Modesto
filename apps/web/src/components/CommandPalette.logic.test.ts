...
import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@modesto/contracts";
...
describe("canUseNativeFolderPicker", () => {
  it("lets the browser open Finder against the local environment", () => {
    expect(
      canUseNativeFolderPicker({
        environmentId: "local",
        primaryEnvironmentId: "local",
        hasDesktopBridge: false,
        isDesktopLocalWithPicker: false,
      }),
    ).toBe(true);
  });

  it("keeps WSL picking on the desktop picker once the pool id is known", () => {
    expect(
      canUseNativeFolderPicker({
        environmentId: "wsl",
        primaryEnvironmentId: "local",
        hasDesktopBridge: true,
        isDesktopLocalWithPicker: true,
      }),
    ).toBe(true);
    expect(
      canUseNativeFolderPicker({
...
describe("reduceCommandPaletteUiState", () => {
  ...
  it("routes open intents to command mode", () => {
    ...
    expect(reduceCommandPaletteUiState(filesOpen, { _tag: "OpenCloneProject" })).toEqual({
      open: true,
      mode: "command",
      openIntent: { kind: "clone-project" },
    });
    expect(
      ...
      open: true,
      mode: "command",
      openIntent: { kind: "clone-project", repository: "octocat/hello-world" },
    });
    expect(reduceCommandPaletteUiState(filesOpen, { _tag: "OpenRemoteProject" })).toEqual({
      open: true,
      mode: "command",
      openIntent: { kind: "remote-project" },
    });
    expect(
      reduceCommandPaletteUiState(filesOpen, {
...
describe("buildThreadActionItems", () => {
  ...
  it("keeps message excerpts searchable without replacing thread metadata", () => {
    const [item] = buildThreadActionItems({
      ...
      projectTitleById: new Map([[PROJECT_ID, "Modesto"]]),
      sortOrder: "updated_at",
      icon: null,
      getContentMatch: () => ({
        source: "assistant",
    ...
    expect(item?.description).toBe("Modesto · #feat/search");
  });
  ...
  it("prefers renderDescription when provided", () => {
    ...
    expect(item?.description).toBe("Modesto:feat/search:wt");
  });
