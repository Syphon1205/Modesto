...
describe("draft hero submission transition", () => {
  ...
  it("lets an existing chat send after its project is removed", () => {
    expect(
      canSendWithoutActiveProject({
        isServerThread: true,
  ...
  it("does not block typing in a chat draft without a project", () => {
    expect(
      isProjectSelectionRequired({
  ...
  it("lets a chat draft create a thread without a live project", () => {
    const draftProjectId = ProjectId.make("project-1");
    expect(
      resolveDraftThreadCreateProjectId({
        conversationMode: "chat",
...
describe("deriveLockedProvider", () => {
  const providers = [
    { instanceId: ProviderInstanceId.make("codex") },
    { instanceId: ProviderInstanceId.make("grok"), requiresNewThreadForModelChange: true },
  ];
  const startedThreadWithSession = (providerName: string, providerInstanceId: string) =>
    ({
      session: {
        providerName,
  ...
  it("does not lock an unstarted thread", () => {
    expect(
      deriveLockedProvider({
        thread: null,
  ...
  it("does not lock a started thread whose current provider allows switching (the common case - a handoff is allowed)", () => {
    expect(
      deriveLockedProvider({
        thread: startedThreadWithSession("codex", "codex"),
        selectedProvider: null,
        threadProvider: null,
        providers,
      }),
  ...
  it("locks a started thread whose current provider requires staying (e.g. Grok)", () => {
    expect(
      deriveLockedProvider({
        thread: startedThreadWithSession("grok", "grok"),
        selectedProvider: null,
        threadProvider: null,
        providers,
      }),
...
describe("reconcileMountedTerminalThreadIds", () => {
  ...
  it("keeps the active drawer mounted after close so its exit motion can finish", () => {
    expect(
      reconcileMountedTerminalThreadIds({
