...
import type { ProjectId, ThreadId } from "@modesto/contracts";
import { buildPromptThreadTitleFallback } from "@modesto/shared/chatThreads";
import {
...
// This tree's session.providerName / modelSelection.instanceId are plain
// strings, not the closed ProviderKind union the original board used - kept
// as a type alias so a future tightened contract only needs one edit here.
export type KanbanProviderId = string;
...
export interface KanbanComposerDraftSnapshot {
  ...
  provider: KanbanProviderId | null;
}
...
export function deriveKanbanColumn(thread: SidebarThreadSummary): KanbanColumnKey {
  ...
  if (hasActionablePendingRequests || Boolean(thread.backgroundLiveness)) {
    return "inProgress";
  }
...
function resolveComposerDraft(
  ...
  threadId: ThreadId,
): { prompt: string; hasAttachments: boolean; provider: KanbanProviderId | null } {
...
function buildThreadCard(
  ...
  return {
    ...
    envMode: resolveThreadEnvironmentMode({ worktreePath: thread.worktreePath }),
...
export function resolveOptimisticDispatchOutcome(
  ...
  if (deriveKanbanColumn(thread) === "inProgress" && thread.session?.status !== "starting") {
    return "settled";
  }
  ...
  if (sessionStatus === "error" || sessionStatus === "stopped") {
