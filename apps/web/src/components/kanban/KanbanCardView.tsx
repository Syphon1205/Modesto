...
import { deriveThreadActivitySummary, formatElapsed } from "../../session-logic";
import { useStore } from "../../store";
import { createThreadSelector } from "../../storeSelectors";
import { WORK_LOG_TONE_ICON } from "../WorkspaceTaskCard";
...
/** Live "what is it doing right now" strip — only worth subscribing to the full
 * thread (activities aren't on the lightweight SidebarThreadSummary) for cards
 * that are actually in progress; draft/done cards have nothing to show. */
function KanbanCardActivity({ card }: { card: KanbanCard }) {
  const isLive = card.column === "inProgress" && !card.isTerminal && card.thread !== null;
  const thread = useStore(
    useMemo(() => createThreadSelector(isLive ? card.threadId : null), [isLive, card.threadId]),
  );
  if (!thread) {
    return null;
  }
  const { activityLabel, activityTone, taskProgressPercent } = deriveThreadActivitySummary(
    thread.activities,
    thread.latestTurn?.turnId ?? undefined,
  );
  const ToneIcon = activityTone ? (WORK_LOG_TONE_ICON[activityTone] ?? null) : null;
  if (!activityLabel && taskProgressPercent === null) {
    return null;
  }
  return (
    <span className="flex min-w-0 flex-col gap-1 pt-0.5">
      {activityLabel ? (
        <span className="flex min-w-0 items-center gap-1.5 text-[11px] text-muted-foreground/80">
          {ToneIcon ? <ToneIcon className="size-3 shrink-0" aria-hidden /> : null}
          ...
          <span className="h-1 flex-1 overflow-hidden rounded-full bg-foreground/[0.08]">
            <span
              className={cn(
                ...
                taskProgressPercent >= 100 ? "bg-emerald-400/80" : "bg-blue-400/80",
              )}
