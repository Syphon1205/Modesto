...
vi.mock("./ui/toast", () => ({
  toastManager: { add: testState.addMainToast },
  updatesToastManager: { add: testState.addToast },
}));
...
describe("showDesktopUpdateDownloadedToast", () => {
  beforeEach(() => {
    ...
    testState.addMainToast.mockReset();
  });

  it("posts to the updates stack, so app-update messages stay in one corner", () => {
    showDesktopUpdateDownloadedToast({ openExternal: async () => true }, downloadedState());
    expect(testState.addToast).toHaveBeenCalledTimes(1);
    expect(testState.addMainToast).not.toHaveBeenCalled();
  });
  ...
  it("opens the downloaded version's release notes", async () => {
    ...
    await vi.waitFor(() => {
      expect(openExternal).toHaveBeenCalledWith(
        "https://github.com/Syphon1205/Modesto/releases/tag/v0.0.30",
      );
    });
    expect(testState.addToast).toHaveBeenCalledTimes(1);
  });
  ...
  it.each([
    ...
    await vi.waitFor(() => {
      expect(testState.addMainToast).toHaveBeenLastCalledWith({
        type: "error",
