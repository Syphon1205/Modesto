...
import { isGenericChatThreadTitle } from "@modesto/shared/chatThreads";
import { getDefaultModel } from "@modesto/shared/model";
import { pluralize } from "@modesto/shared/text";
import { localServerAddressLabel, localServerMatchesRun } from "@modesto/shared/localServers";
...
import {
  ...
  shouldShowDesktopUpdateAvailableBadge,
...
import { coworkThreadKind, useCoworkThreadStore } from "../coworkThreadStore";
import { workspaceIdForPath } from "../workspaces";
...
type DebugFeatureFlagsWindow = Window & {
  modestoShowFeatureFlags?: () => void;
  modestoHideFeatureFlags?: () => void;
};
...
function GitHubAccountSidebarRow({
  isOnSettings,
  onOpenProfile,
  onOpenProviders,
  onOpenSettings,
  ...
  const authQuery = useQuery(
    gitGithubAuthQueryOptions({ refetchInterval: awaitingSignIn ? 2_000 : false }),
  );
  ...
  useEffect(() => {
    if (authQuery.data?.authenticated) setAwaitingSignIn(false);
  ...
  const account = authQuery.data;
  const signingIn = signInMutation.isPending || awaitingSignIn;
    ...
    <>
      <div className="flex items-center gap-1">
        {account?.authenticated ? (
          <button
            type="button"
            className="flex min-w-0 flex-1 items-center gap-2.5 rounded-lg px-1 py-1.5 text-left transition-colors hover:bg-[var(--sidebar-accent)]"
          ...
          <button
            type="button"
            disabled={authQuery.isPending || signingIn}
            className="flex h-9 min-w-0 flex-1 items-center gap-2 rounded-lg px-1 text-xs font-medium text-foreground transition-colors hover:bg-[var(--sidebar-accent)] disabled:opacity-60"
            onClick={() => {
          ...
          >
            {authQuery.isPending || signingIn ? (
              <Loader2Icon className="size-3.5 animate-spin" />
            ) : (
              <GitHubIcon className="size-3.5" />
            )}
            {signingIn
              ? signInMutation.data?.userCode
                ? `Code ${signInMutation.data.userCode}`
                : "Waiting for GitHub…"
          ...
          <TooltipTrigger
            render={
              <button
                ...
                className={cn(
                  "flex size-9 shrink-0 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-[var(--sidebar-accent)] hover:text-foreground",
                  isOnSettings && "text-foreground",
                )}
      ...
      <GitHubDeviceCodeDialog
        open={awaitingSignIn}
        userCode={signInMutation.data?.userCode ?? null}
        onOpenChange={(open) => {
...
export default function Sidebar() {
  ...
  const activeModestoWorkspace = workspaceIdForPath(pathname);
  const isOnResearch = activeModestoWorkspace === "research";
  const isOnTeams = activeModestoWorkspace === "teams";
  ...
  // The legacy Studio and terminal-only Workspace surfaces remain readable for
  // migration compatibility, but are no longer user-facing sidebar modes.
  const chatsSectionVisible = appSettings.showChatsSection;
  const studioSectionVisible = false;
  const workspaceSectionVisible = false;
  const { handleNewThread } = useHandleNewThread();
  const { handleNewChat } = useHandleNewChat();
  const { handleNewStudioChat } = useHandleNewStudioChat();
  const {
    handoffBusy: sidebarHandoffBusy,
    handoffDialogOpen: sidebarHandoffDialogOpen,
    setHandoffDialogOpen: setSidebarHandoffDialogOpen,
    handoffDraft: sidebarHandoffDraft,
    setHandoffDraft: setSidebarHandoffDraft,
    openHandoffDialog,
    confirmThreadHandoff,
  } = useThreadHandoff();
  const routeParamThreadId = useParams({
    strict: false,
  ...
  const routeThreadId =
    routeParamThreadId ??
    (isOnResearch && typeof settingsSectionSearch.threadId === "string"
      ? ThreadId.makeUnsafe(settingsSectionSearch.threadId)
      : null);
  ...
  useEffect(() => {
    ...
    debugWindow.modestoShowFeatureFlags = showFeatureFlags;
    debugWindow.modestoHideFeatureFlags = hideFeatureFlags;
    window.addEventListener("storage", updateVisibility);
    updateVisibility();

    return () => {
      window.removeEventListener("storage", updateVisibility);
      if (debugWindow.modestoShowFeatureFlags === showFeatureFlags) {
        delete debugWindow.modestoShowFeatureFlags;
      }
      if (debugWindow.modestoHideFeatureFlags === hideFeatureFlags) {
        delete debugWindow.modestoHideFeatureFlags;
      }
    };
  }, []);
  ...
  const coworkThreadIds = useCoworkThreadStore((state) => state.coworkThreadIds);
  const threadKindById = useCoworkThreadStore((state) => state.threadKindById);
      ...
      const title =
        provider === "claudeAgent"
          ...
          : provider === "cursor"
            ...
            : provider === "gemini"
              ? `Imported Gemini session${suffix ? ` ${suffix}` : ""}`
              : provider === "grok"
                ? `Imported Grok session${suffix ? ` ${suffix}` : ""}`
                : provider === "droid"
                  ...
                  : provider === "kilo"
                    ...
                    : provider === "opencode"
                      ...
                      : provider === "pi"
                        ? `Imported Pi session${suffix ? ` ${suffix}` : ""}`
  ...
  const handoffThread = useCallback(
    async (thread: Thread, targetProvider: ProviderKind) => {
      try {
        await openHandoffDialog(thread, targetProvider);
      } catch (error) {
        toastManager.add({
          type: "error",
  ...
  const confirmSidebarHandoff = useCallback(async () => {
    if (!sidebarHandoffDraft) {
      return;
    }
    try {
      await confirmThreadHandoff(sidebarHandoffDraft);
    } catch (error) {
      toastManager.add({
        type: "error",
        title: "Could not create handoff thread",
  ...
  }, [confirmThreadHandoff, sidebarHandoffDraft]);
    ...
    return buildProjectThreadTree({
      threads: sortThreadsForSidebar(
        chatProjects
          ...
          .filter((thread) => {
            const kind = coworkThreadKind(threadKindById, thread.id);
            if (activeModestoWorkspace === "research") return kind === "research";
            if (activeModestoWorkspace === "teams") return kind === "teams";
            return !coworkThreadIds.includes(thread.id);
          }),
        appSettings.sidebarThreadSortOrder,
      ),
  ...
  // port/process scan while no Modesto-owned run needs near-real-time status.
  ...
  const projectRunServerByProjectId = useMemo(() => {
    ...
    // 1. Authoritative: Modesto-tracked runs matched by pid/ppid.
    ...
    //    servers started outside Modesto still light up the running indicator.
  ...
  function renderProjectItem(
    ...
    // A project reads as "running" when Modesto tracks a run for it or when a
    // local server (possibly started outside Modesto) is attributed by cwd.
          ...
          >
            ...
            <SidebarSectionToolbar placement="overlay" revealOnHover>
              ...
              <SidebarIconButton
                ...
                tooltip={
                  newThreadShortcutLabel ? `New agent (${newThreadShortcutLabel})` : "New agent"
                }
                tooltipSide="top"
  ...
  const desktopUpdateButtonInteractivityClasses = desktopUpdateButtonDisabled
    ...
    : "hover:bg-sidebar-accent hover:text-sidebar-foreground active:scale-95";
  const desktopUpdateDownloadPercent = getDesktopUpdateDownloadPercent(desktopUpdateState);
  const showDesktopUpdateAvailableBadge = shouldShowDesktopUpdateAvailableBadge(desktopUpdateState);
  const desktopUpdateCircleButtonClasses = cn(
    "relative flex size-9 shrink-0 items-center justify-center rounded-full text-sidebar-foreground/75 transition-[transform,background-color,color,opacity] duration-200",
  ...
  const searchPaletteActions = useMemo<SidebarSearchAction[]>(
    () => [
      ...
      },
      {
        id: "new-thread",
        label: "New agent",
        description: "Start a fresh agent in the current project.",
        keywords: ["agent", "thread", "new", "project"],
  ...
  const handleDesktopUpdateButtonClick = useCallback(() => {
        ...
        .then((nextState) => {
          ...
          if (nextState.status === "available") {
            toastManager.add({
              ...
              description: `Version ${nextState.availableVersion ?? ""} is ready to download.`.trim(),
            });
            return;
          }
          ...
          if (nextState.status === "downloading") {
            toastManager.add({
              ...
              description: "Modesto is downloading the update in the background.",
            });
            return;
          }
          ...
          if (nextState.status === "up-to-date") {
            toastManager.add({
              ...
              description: `Modesto ${nextState.currentVersion} is already the newest version.`,
            });
            return;
          }
    ...
    if (desktopUpdateButtonAction === "download") {
      toastManager.add({
        type: "info",
        title: "Downloading update",
        description: `Fetching Modesto ${desktopUpdateState.availableVersion ?? "update"}…`,
      });
      void bridge
        .downloadUpdate()
        .then((result) => {
          ...
          if (result.completed) {
            toastManager.add({
              type: "success",
              title: "Update ready",
              description: "Click the download icon again to restart and install.",
            });
          }
  ...
  const wordmark = (
    <div className="flex w-full min-w-0 items-center gap-1.5">
      ...
      <div className="flex min-w-0 flex-1 items-center gap-2 px-1.5">
        <ModestoLogo className="size-5 text-foreground" />
        <span className="truncate text-[13px] font-semibold tracking-[-0.02em]">Modesto</span>
      </div>
      {headerControls}
    </div>
  );
    ...
    <>
      ...
      <SidebarContent className="gap-0 font-system-ui">
          ...
          <>
            ...
            >
              ...
              <SidebarGroup className="px-3 pt-1 pb-2">
                <WorkspaceSwitcher className="mb-2.5 w-full px-2 py-1.5" />
              ...
              {isOnWorkspace ? (
                ...
                <SidebarGroup className="px-3 py-1.5">
                    ...
                    <div className="mb-2.5 px-1">
                      {!showManualPathInput ? (
                        <div className="flex gap-1.5">
                          ...
                          >
                            <SidebarGlyph icon={CursorTextIcon} variant="chrome" />
                            Type path
                          </button>
                        </div>
                      ) : (
                  ...
                  {isManualProjectSorting ? (
                    ...
                    <SidebarMenu ref={attachProjectListAutoAnimateRef} className="gap-1.5">
                      {standardProjects.map((project) => (
        ...
        {!isOnSettings && !isOnStudio && chatsSectionVisible ? (
          ...
          <SidebarGroup className="sidebar-surface-enter px-3 pt-1 pb-2">
            <div className="group/collapsible">
            ...
            <div className="flex flex-col gap-1">
              ...
              <GitHubAccountSidebarRow
                ...
                onOpenProviders={() =>
                  void navigate({
                    to: "/settings",
                    search: { section: "providers", target: "provider-installs" },
                  })
                }
                      ...
                      <TooltipTrigger
                        render={
                          <button
                            ...
                            disabled={desktopUpdateButtonDisabled}
                            className={desktopUpdateCircleButtonClasses}
                            onClick={handleDesktopUpdateButtonClick}
                          >
                            <DownloadIcon
                              className={cn(
                                "size-4.5",
                                desktopUpdateState?.status === "downloading" && "animate-pulse",
                              )}
                            />
                            {desktopUpdateDownloadPercent !== null ? (
                              <span className="absolute -right-1 -bottom-1 flex min-w-5 items-center justify-center rounded-full border-2 border-[var(--sidebar-background)] bg-white px-1 text-[8px] font-bold tabular-nums text-[#0066cc]">
                                {desktopUpdateDownloadPercent}
                              </span>
                            ) : desktopUpdateState?.status === "downloaded" ? (
                              <span className="absolute top-0 right-0 size-2.5 rounded-full border-2 border-[var(--sidebar-background)] bg-emerald-400" />
                            ) : showDesktopUpdateAvailableBadge ? (
                              <span className="absolute top-0 right-0 size-2.5 rounded-full border-2 border-[var(--sidebar-background)] bg-[#0a84ff]" />
                            ) : null}
                          </button>
                        }
                      />
      ...
      <ThreadProviderHandoffDialog
        open={sidebarHandoffDialogOpen}
        draft={sidebarHandoffDraft}
        busy={sidebarHandoffBusy}
        onOpenChange={setSidebarHandoffDialogOpen}
        onDraftChange={setSidebarHandoffDraft}
        onConfirm={confirmSidebarHandoff}
      />
      ...
      {searchPaletteOpen && !isOnTeams && !isOnResearch ? (
...
function SidebarSearchPaletteController(props: {
  ...
  const importProviderCapabilityQueries = useQueries({
    ...
    ).map((provider) => providerComposerCapabilitiesQueryOptions(provider)),
  });
