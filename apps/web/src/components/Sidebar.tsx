...
import type { ScopedThreadRef, ThreadId } from "@modesto/contracts";
import type { TimestampFormat } from "@modesto/contracts/settings";
import {
  AlarmClockIcon,
  AlarmClockOffIcon,
  BotIcon,
...
import { getLocalFileManagerName, isMacPlatform } from "~/lib/utils";
...
import {
  ...
  groupChildThreadsByParentId,
  selectNestedCodeThreads,
  selectTopLevelCodeThreads,
  hasUnseenCompletion,
...
import { CreateProjectDialog, type CreateProjectType } from "./CreateProjectDialog";
...
const SETTLED_SHELF_EXPANDED_KEY = "modesto:sidebar-v2:settled-expanded";
const SNOOZED_SHELF_EXPANDED_KEY = "modesto:sidebar-v2:snoozed-expanded";
...
const SidebarDraftRow = memo(function SidebarDraftRow(props: {
  ...
  const isChat = composer.conversationMode === "chat";
        ...
        <div className="relative z-10 px-[var(--sidebar-row-content-inset)] py-[var(--sidebar-content-inset)]">
          <div className="flex h-5 min-w-0 items-center gap-1.5">
            {isChat ? (
              <MessageSquareIcon
                aria-hidden
                className="size-3 shrink-0 text-sky-600 dark:text-sky-300/80"
              />
            ) : (
              <SquarePenIcon
                aria-hidden
            ...
            <span className="min-w-0 flex-1 truncate text-xs font-medium text-secondary-label">
              {isChat ? "Chat draft" : props.projectTitle}
            </span>
  ...
  if (frozenActive.routeDraftId !== props.routeDraftId) {
    ...
    if (props.routeDraftId !== null) {
      ...
      row =
        ...
        composer &&
        composer.conversationMode === props.conversationMode &&
                    ...
                    isWokeStatus ? (
                      ...
                      >
                        {topStatus.icon === "working" ? (
                          <ThreadRunningSpinner className="size-4" />
                        ) : topStatus.icon === "done" ? (
...
// Compact rows for the Chats list and the nested project previews. Full
// cards live further down; these stay title-only so the new sections stay
// scannable, but they still have to rename, right-click, and delete.
const SidebarCompactThreadRow = memo(function SidebarCompactThreadRow(props: {
  ...
  onStartThread?: (threadRef: ScopedThreadRef) => void;
  nestedContent?: ReactNode;
}) {
  const {
    deleteLabel,
  ...
  const handleActivate = useCallback(() => onActivate(threadRef), [onActivate, threadRef]);
  const handleContextMenu = useCallback(
    (event: ReactMouseEvent) => {
      event.preventDefault();
      onContextMenu(threadRef, { x: event.clientX, y: event.clientY });
  ...
  const title = isRenaming ? (
    <input
      autoFocus
      value={renamingTitle}
      aria-label={variant === "chat" ? "Chat title" : "Thread title"}
      onChange={(event) => onRenameTitleChange(event.target.value)}
      onFocus={(event) => event.currentTarget.select()}
      onKeyDown={handleRenameKeyDown}
      onBlur={handleRenameBlur}
  ...
  const deleteButtonClassName =
    "rounded-sm p-1 text-sidebar-muted-foreground hover:text-sidebar-foreground focus-visible:opacity-100";
  const deleteControl = (
    <button
      type="button"
  ...
  const activity = (
    <span className="shrink-0 group-hover/menu-item:hidden group-hover/preview-thread:hidden group-focus-within/menu-item:hidden group-focus-within/preview-thread:hidden">
  ...
  return (
    <li className="group/preview-thread relative">
      ...
      >
        {title}
        {multiAgent ? (
          <span className="shrink-0 rounded-sm border border-sidebar-border/60 px-1 font-mono text-[9px] text-sidebar-muted-foreground">
            Multi-Agent
          </span>
        ) : null}
        {activity}
      </button>
      <div className="absolute right-1 top-1/2 hidden -translate-y-1/2 group-hover/preview-thread:flex group-focus-within/preview-thread:flex">
        {deleteControl}
      </div>
      {nestedContent}
    </li>
  ...
  return (
    <SidebarMenuItem>
      ...
      >
        ...
        {shortcutLabel ? (
          <span className="ml-auto opacity-0 transition-opacity group-hover/menu-item:opacity-100">
            <KbdGroup>
...
export default function Sidebar() {
  ...
  const openCreateProjectDialog = useCallback(() => setCreateProjectDialogOpen(true), []);
  const { environments } = useEnvironments();
  const primaryEnvironmentId = usePrimaryEnvironmentId();
  const pickFolder = useAtomCommand(filesystemEnvironment.pickFolder, {
    reportFailure: false,
  });
  const remoteProjectAvailable = environments.some(
    (environment) =>
      environment.environmentId !== primaryEnvironmentId &&
      environment.connection.phase === "connected",
  );
  const continueCreateProject = useCallback((type: CreateProjectType) => {
    setCreateProjectDialogOpen(false);
    // Let the modal release focus before the follow-on picker opens.
    window.setTimeout(() => {
      openCommandPalette({
        open:
          type === "local" ? "add-project" : type === "cloud" ? "clone-project" : "remote-project",
      });
    }, 0);
  }, []);
  const pickLocalProjectFolder = useCallback(async (): Promise<string | null> => {
    const fileManagerName = getLocalFileManagerName(
      typeof navigator === "undefined" ? "" : navigator.platform,
    );
    try {
      if (typeof window !== "undefined" && window.desktopBridge !== undefined) {
        return (await readLocalApi()?.dialogs.pickFolder()) ?? null;
      }
        ...
        if (!isAtomCommandInterrupted(result)) {
          ...
          toastManager.add(
            stackedThreadToast({
              ...
              title: `Could not open ${fileManagerName}`,
              description:
                error instanceof Error ? error.message : "Try choosing the folder again.",
            }),
          );
        }
        return null;
      ...
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: `Could not open ${fileManagerName}`,
          description: error instanceof Error ? error.message : "Try choosing the folder again.",
        }),
      );
      return null;
    }
  }, [pickFolder, primaryEnvironmentId]);
  const createLocalProject = useCallback(
    (input: { readonly path: string; readonly title: string }) => {
      setCreateProjectDialogOpen(false);
      window.setTimeout(() => {
        openCommandPalette({ localProject: input });
      }, 0);
    },
    [],
  );
  ...
  const projectGroups = useMemo(
    () =>
      sortLogicalProjectsForSidebar(unsortedProjectGroups, codeThreads, sidebarProjectSortOrder),
    [codeThreads, sidebarProjectSortOrder, unsortedProjectGroups],
  );
  ...
  // Codex-style default: every project stays visible with its own short
  // thread preview underneath it, instead of one project's full list
  // hidden behind a click. Opening a project (scoping it) swaps into the
  // richer pin/snooze/settle list below - the two views never render at
  // once, so nothing shows twice.
  const showFlatProjectPreviewMode = projectGroups.length > 0 && projectScopeKey === null;
  const projectKeyByMemberRef = useMemo(
    () =>
      new Map(
        projectGroups.flatMap((group) =>
  ...
  const PROJECT_PREVIEW_ROW_LIMIT = 5;
  const [expandedPreviewProjectKeys, setExpandedPreviewProjectKeys] = useState<ReadonlySet<string>>(
    new Set(),
  );
  // Open/closed per project row - a plain disclosure toggle over that
  // project's own preview list, nothing more. Defaults to open (every
  // project's threads visible at once, matching the reference) and never
  // swaps in a separate rich card view: that "other box" behavior (the old
  // projectScopeKey-driven scoped list) is exactly what got rejected.
  const [collapsedProjectPreviewKeys, setCollapsedProjectPreviewKeys] = useState<
    ReadonlySet<string>
  >(new Set());
  ...
  const visibleDraftSessionCount = useComposerDraftStore((store) => {
    ...
    for (const [draftKey, session] of Object.entries(store.draftThreadsByThreadKey)) {
      ...
      if (store.draftsByThreadKey[draftKey]?.conversationMode === "chat") {
        continue;
      }
      if (
  ...
  const deleteProjectCommand = useAtomCommand(projectEnvironment.delete, {
    reportFailure: false,
  });
  // Removing a project entry only clears it from the sidebar - the folder on
  // disk is untouched. force:true deletes that project's code threads. Chats
  // are a separate surface and stay.
    ...
    async (projectGroup: SidebarProjectSnapshot) => {
      ...
      const confirmed = await api.dialogs.confirm(
        [
          ...
          "This removes the project and its code threads from Modesto. Chats stay.",
      ...
      const results = await Promise.all(
        projectGroup.memberProjectRefs.map((ref) =>
      ...
      const failure = results.find(
        (result): result is Extract<(typeof results)[number], { _tag: "Failure" }> =>
      ...
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: `Failed to remove "${projectGroup.displayName}"`,
  ...
  const {
    ...
    const visible = codeThreads.filter(
      (thread) =>
        thread.archivedAt === null &&
        (scopedProjectKeys === null ||
  ...
  // Pinned-then-active order, same priority the main list uses - only
  // computed while the flat tree is showing (scoping a project already
  // narrows pinnedThreads/activeThreads to it, so this grouping would be
  // redundant with the main list in that state).
  const previewThreadsByProjectKey = useMemo(() => {
    if (!showFlatProjectPreviewMode) return null;
  ...
  // The inline search row only renders once opened, so it doesn't sit as a
  // second, permanently-visible "Search" affordance right below the nav
  // row's own Search button - one entry point, revealed on demand.
  ...
  const orderedThreads = useMemo(
    ...
    [chatThreads, pinnedThreads, activeThreads, visibleSnoozedThreads, renderedSettledThreads],
  );
        ...
        const clicked = await settlePromise(() =>
          api.contextMenu.show(
            buildThreadActionMenuItems({
              ...
              snoozePresets,
              conversationMode: thread.conversationMode,
            }),
            position,
          ),
        );
          ...
          case "delete": {
            ...
            if (confirmThreadDelete) {
              const confirmed = await settlePromise(() =>
                api.dialogs.confirm(
                  [
                    `Delete ${noun} "${thread.title}"?`,
                    `This permanently clears conversation history for this ${noun}.`,
                  ].join("\n"),
                  { variant: "destructive" },
                ),
              );
  ...
  const attemptDeleteFromRow = useCallback(
    (threadRef: ScopedThreadRef) => {
      void (async () => {
  ...
  // New Chat opens the empty landing in chat mode. Chat vs Work can still be
  // picked there before the first send; after that the mode is locked. Chats
  // do not require a project — without one we open an unscoped chat draft.
    ...
    void startNewThreadFromContext(
      {
        ...
        unscopedChatProjectRef: newThreadContext.unscopedChatProjectRef,
        handleNewThread: newThreadContext.handleNewThread,
      },
      { conversationMode: "chat" },
    ).then((didStart) => {
      if (!didStart) void router.navigate({ to: "/" });
    });
  ...
  const handleStartThreadInChat = useCallback(
    (threadRef: ScopedThreadRef) => {
      ...
      if (!thread) return;
      if (isMobile) setOpenMobile(false);
      void startNestedThread(thread);
    },
    [isMobile, setOpenMobile, startNestedThread],
  );

  // The primary action and chat.new both open the same direct landing.
  ...
  const renderNestedChildThreads = (parentThread: EnvironmentThreadShell) => {
    ...
    return (
      <ul
        role="list"
        aria-label={`Threads in ${parentThread.title}`}
            ...
            <SidebarGroup className="relative z-[1] gap-1 p-[var(--sidebar-content-inset)] pt-0">
              {projectGroups.length > 0 ? (
                <>
                  ...
                  {/* Codex-style: every project stays visible with its own plain
                    thread-title list open beneath it by default. The chevron only
                    toggles that project's own list open/closed - it never swaps in
                    a separate scoped view. */}
                  <SidebarMenu className="gap-0.5">
                    {projectGroups.map((project) => {
                      const scopeKey = project.projectKey;
                      const isOpen = !collapsedProjectPreviewKeys.has(scopeKey);
                      return (
                        <SidebarMenuItem key={scopeKey} className="group/project-row">
                          {/* `relative` scoped to just the header row (not the
                              whole `SidebarMenuItem`, which also contains the
                              disclosure preview-thread list below when open) -
                              otherwise `top-1/2` on the action buttons centers
                              within the item's full expanded height instead of
                              this row, drifting the icons down onto whichever
                              preview thread happens to sit at the midpoint. */}
                          <div className="relative">
                            <SidebarMenuButton
                              className="ps-[calc(var(--sidebar-row-content-inset)-1px)] pe-14"
                              onClick={() =>
                                setCollapsedProjectPreviewKeys((previous) => {
                              ...
                              onContextMenu={(event) => {
                                // Right-click opens the same menu as the "..."
                                // trigger - it must never fire the destructive
                                // action directly, one misclick away from
                                // "Remove project" is exactly the kind of thing
                                // that caused real data loss earlier.
                                event.preventDefault();
                                setOpenProjectMenuKey(scopeKey);
                              }}
                            >
                              <ProjectFavicon
                              ...
                              <ChevronDownIcon
                                className={disclosureChevronClassName(
                                  isOpen,
                                  "size-3.5 shrink-0 group-hover/project-row:opacity-0",
                                )}
                              />
                            </SidebarMenuButton>
                            <div className="absolute right-1.5 top-1/2 flex -translate-y-1/2 items-center gap-0.5">
                              <button
                                type="button"
                                aria-label={`New code task in ${project.displayName}`}
                                title={`New code task in ${project.displayName}`}
                                className="rounded-sm p-1 text-sidebar-muted-foreground opacity-0 hover:text-sidebar-foreground focus-visible:opacity-100 group-hover/project-row:opacity-100"
                                onPointerDown={(event) => event.stopPropagation()}
                                onClick={(event) => {
                                  event.stopPropagation();
                                  if (isMobile) setOpenMobile(false);
                              ...
                              <Menu
                                open={openProjectMenuKey === scopeKey}
                                onOpenChange={(nextOpen) =>
                                  setOpenProjectMenuKey(nextOpen ? scopeKey : null)
                                }
                              >
                                <MenuTrigger
                                  render={
                                    <button
                                      ...
                                      title={`More options for ${project.displayName}`}
                                      className="rounded-sm p-1 text-sidebar-muted-foreground opacity-0 hover:text-sidebar-foreground focus-visible:opacity-100 data-[popup-open]:opacity-100 group-hover/project-row:opacity-100"
                                      onPointerDown={(event) => event.stopPropagation()}
                                      onClick={(event) => event.stopPropagation()}
                                    />
                                  }
                          ...
                          {(() => {
                            const previewThreads = previewThreadsByProjectKey?.get(scopeKey) ?? [];
                            if (previewThreads.length === 0) return null;
                            const expanded = expandedPreviewProjectKeys.has(scopeKey);
                            const visibleThreads = expanded
                              ? previewThreads
                              : previewThreads.slice(0, PROJECT_PREVIEW_ROW_LIMIT);
                            const hiddenCount = previewThreads.length - visibleThreads.length;
                            return (
                              <div className={disclosureShellClassName(isOpen, "mb-0.5")}>
                                <div className={DISCLOSURE_INNER_CLASS}>
                                  <div className={disclosureContentClassName(isOpen)}>
                                    <ul
                                      role="list"
                                      aria-label={`Recent threads in ${project.displayName}`}
                                      className="flex flex-col gap-px ms-[calc(var(--sidebar-row-content-inset)+0.6rem)] border-s border-sidebar-border/50 ps-2"
                                    >
                                      ...
                                      {hiddenCount > 0 ? (
                                        <li>
                                          <button
                                            type="button"
                                            onClick={() =>
                                              setExpandedPreviewProjectKeys((previous) => {
                                                const next = new Set(previous);
                                                next.add(scopeKey);
                                                return next;
                                              })
                                            }
                                            className="block w-full rounded-sm px-2 py-1 text-left text-[12.5px] text-sidebar-muted-foreground/60 hover:text-sidebar-foreground"
                                          >
                                            Show more
                                          </button>
                                        </li>
                ...
                {(() => {
                  ...
                  const items: ReactNode[] = [
                    <SidebarDraftBlock
                      ...
                      onNavigateToDraft={navigateToDraft}
                    />,
                    !showFlatProjectPreviewMode && pinnedThreads.length > 0 ? (
                  ...
                  // Flat tree mode already shows every project's threads as
                  // capped previews above - the pinned/active/snoozed/settled
                  // sections below only add value once a project is scoped,
                  // otherwise they'd just repeat the same threads a second
                  // time (the exact bug the nested per-project list caused
                  // before this was reworked).
                  if (showFlatProjectPreviewMode) {
                    return items;
                  }
