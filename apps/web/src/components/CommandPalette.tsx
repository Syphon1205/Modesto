...
} from "@modesto/client-runtime/operations/projects";
import { connectionStatusText } from "@modesto/client-runtime/connection";
import { threadSearchMatchKey } from "@modesto/client-runtime/state/thread-search";
import {
  canPreloadBrowsePath,
  createBrowseNavigationCoordinator,
  filterFilesystemBrowseEntries,
  getFilesystemBrowsePath,
} from "@modesto/client-runtime/state/filesystem";
import {
...
export function CommandPalette({ children }: { children: ReactNode }) {
  ...
  const openRemoteProject = useCallback(() => dispatch({ _tag: "OpenRemoteProject" }), []);
  const createLocalProject = useCallback(
    (path: string, title?: string) =>
      dispatch({
        _tag: "CreateLocalProject",
        path,
        ...(title ? { title } : {}),
      }),
    [],
  ...
  useEffect(
    () =>
      onOpenCommandPalette((detail) => {
        if (detail.localProject) {
          createLocalProject(detail.localProject.path, detail.localProject.title);
        } else if (detail.open === "new-thread-in") {
          openNewThreadIn();
        } else if (detail.open === "clone-project") {
          dispatch({
            _tag: "OpenCloneProject",
            ...(detail.repository ? { repository: detail.repository } : {}),
          });
        } else if (detail.open === "remote-project") {
          openRemoteProject();
    ...
    [createLocalProject, openAddProject, openNewThreadIn, openRemoteProject, setOpen],
  );
...
function OpenCommandPaletteDialog(props: {
  ...
  const isPickingProjectFolderRef = useRef(false);
  const [isPickingProjectFolder, setIsPickingProjectFolder] = useState(false);
  const [shouldAutoOpenProjectPicker, setShouldAutoOpenProjectPicker] = useState(false);
  ...
  const canPickFolderForEnvironment = useCallback(
    (environmentId: EnvironmentId): boolean => {
      ...
      const isDesktopLocalWithPicker =
  ...
  const startAddProjectBrowse = useCallback(
    async (environmentId: EnvironmentId): Promise<void> => {
      ...
      // The local environment already has a native folder picker (Finder on
      // macOS): desktop via IPC, the browser via the server. Make it the
      // primary local flow and skip the filesystem prefetch that powered the
      // old in-app browser. Remote environments retain that browser as a
      // fallback.
      if (canPickFolderForEnvironment(environmentId)) {
        ...
        setShouldAutoOpenProjectPicker(true);
        return;
      }
  ...
  const buildAddProjectSourceGroups = useCallback(
    (
      ...
      return [
        {
          value: `${options.includeLocalFolder === false ? "repository-sources" : "sources"}:${environmentId}`,
          label: options.includeLocalFolder === false ? "Repositories" : "Sources",
          items: sourceItems,
        },
      ];
    },
  ...
  const remoteProjectEnvironmentGroups = useMemo<CommandPaletteView["groups"]>(
    () => [
      {
        value: "remote-environments",
        label: "Connected agents",
        items: addProjectEnvironmentOptions
          .filter((option) => !option.isPrimary)
          .map(
            (option): CommandPaletteActionItem => ({
              kind: "action",
              value: `action:add-remote-project:environment:${option.environmentId}`,
              searchTerms: [option.label, option.environmentId, "remote", "agent", "machine"],
              title: option.label,
              description: option.isConnected ? option.environmentId : option.status,
              disabled: !option.isConnected,
    ...
    [addProjectEnvironmentOptions, startAddProjectBrowse],
  );
  ...
  useLayoutEffect(() => {
    if (openIntent?.kind !== "clone-project") {
      return;
    }
    const repository = openIntent.repository?.trim();
    clearOpenIntent();
    if (primaryEnvironmentId === null) {
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: "Local environment unavailable",
          description: "Connect the local environment before choosing a repository.",
        }),
      );
      setOpen(false);
      return;
    }
    setAddProjectEnvironmentId(primaryEnvironmentId);
    if (repository) {
      startAddProjectClone(primaryEnvironmentId, "github", repository);
      return;
    }
    setAddProjectCloneFlow(null);
    pushPaletteView({
      ...
      groups: buildAddProjectSourceGroups(
        primaryEnvironmentId,
        buildAddProjectRemoteSourceReadiness(
          browseEnvironmentId === primaryEnvironmentId ? sourceControlDiscovery.data : null,
        ),
  ...
  useLayoutEffect(() => {
    if (openIntent?.kind !== "remote-project") {
      return;
    }
    clearOpenIntent();
    const remoteItems = remoteProjectEnvironmentGroups[0]?.items ?? [];
    if (remoteItems.length === 0) {
      toastManager.add(
        stackedThreadToast({
          type: "info",
          title: "No connected agents",
          description: "Connect a remote environment before opening one of its folders.",
        }),
      );
      setOpen(false);
      return;
    ...
    pushPaletteView({
      ...
      groups: remoteProjectEnvironmentGroups,
    });
  }, [clearOpenIntent, openIntent, pushPaletteView, remoteProjectEnvironmentGroups, setOpen]);
  ...
  const repositorySourceSelectionViewValue =
    addProjectEnvironmentId === null ? null : `repository-sources:${addProjectEnvironmentId}`;
  const activeGroups =
    addProjectEnvironmentId !== null &&
    currentView !== null &&
    (currentView.groups[0]?.value === sourceSelectionViewValue ||
      currentView.groups[0]?.value === repositorySourceSelectionViewValue)
      ? buildAddProjectSourceGroups(
          addProjectEnvironmentId,
          buildAddProjectRemoteSourceReadiness(sourceControlDiscovery.data),
          { includeLocalFolder: currentView.groups[0]?.value === sourceSelectionViewValue },
        )
  ...
  useEffect(() => {
    if (openIntent?.kind !== "create-local-project") {
      return;
    }
    clearOpenIntent();
    if (primaryEnvironmentId === null) {
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: "Local environment unavailable",
          description: "Connect the local environment before creating this project.",
        }),
      );
      setOpen(false);
      return;
  ...
  const canOpenProjectFromFileManager =
    ...
    canUseNativeFolderPicker({
      ...
      isDesktopLocalWithPicker: browseEnvironmentIsDesktopLocal && browseDesktopInstanceId !== null,
    });
  ...
  const handleOpenProjectFromFileManager = useCallback(
    async (closeOnCancel = false) => {
      if (!canOpenProjectFromFileManager || isPickingProjectFolderRef.current) {
        return;
      }
      const api = readLocalApi();

      // State alone cannot lock this synchronously: two rapid clicks (or a
      // development effect replay) can share the same stale render. The ref
      // ensures only one native modal can be opened at a time.
      isPickingProjectFolderRef.current = true;
      setIsPickingProjectFolder(true);
      let pickedPath: string | null = null;
      let desktopWslState: DesktopWslState | null = null;
      try {
        if (window.desktopBridge !== undefined && api) {
          desktopWslState =
        ...
        } else if (browseEnvironmentId !== null) {
          const result = await pickFolder({
            environmentId: browseEnvironmentId,
            input: fileManagerInitialPath ? { initialPath: fileManagerInitialPath } : {},
          });
          if (result._tag === "Failure") {
            ...
            isPickingProjectFolderRef.current = false;
            setIsPickingProjectFolder(false);
            if (closeOnCancel) setOpen(false);
            return;
          }
          pickedPath = result.value;
        } else {
          isPickingProjectFolderRef.current = false;
          setIsPickingProjectFolder(false);
          if (closeOnCancel) setOpen(false);
        ...
        // A manually opened picker returns to the browser; a direct Add Project
        // action closes cleanly instead of revealing the old path-search UI.
        isPickingProjectFolderRef.current = false;
        setIsPickingProjectFolder(false);
        if (closeOnCancel) setOpen(false);
        return;
  ...
  useEffect(() => {
    if (!shouldAutoOpenProjectPicker || !isBrowsing) {
      return;
    }
    setShouldAutoOpenProjectPicker(false);
    void handleOpenProjectFromFileManager(true);
  }, [handleOpenProjectFromFileManager, isBrowsing, shouldAutoOpenProjectPicker]);
