...
} from "@modesto/client-runtime/connection";
import { wasBootstrapThreadDeleted } from "@modesto/client-runtime/errors";
import {
  changeRequestAutoSettles,
  effectiveSettled,
  effectiveSnoozed,
  threadWokeAt,
} from "@modesto/client-runtime/state/thread-settled";
import {
  codexFeedbackMessage,
  parseCodexFeedbackCommand,
  submitCodexFeedback,
  type CodexFeedbackSubmission,
} from "@modesto/client-runtime/state/threads";
import {
  parseScopedThreadKey,
  scopedThreadKey,
  scopeProjectRef,
  scopeThreadRef,
} from "@modesto/client-runtime/environment";
import {
  applyClaudePromptEffortPrefix,
  createModelSelection,
  resolvePromptInjectedEffort,
} from "@modesto/shared/model";
import { deriveRateLimitsSnapshot } from "@modesto/shared/rateLimits";
import { CHAT_LIST_ANCHOR_OFFSET } from "@modesto/shared/chatList";
import { projectScriptCwd, projectScriptRuntimeEnv } from "@modesto/shared/projectScripts";
import { truncate } from "@modesto/shared/String";
import {
  getTerminalLabel,
  nextTerminalId,
  resolveTerminalSessionLabel,
} from "@modesto/shared/terminalLabels";
...
import {
  browserTabIdsExcludingMusic,
  musicPreviewTabIdForThread,
  useMusicPlayerStore,
...
import { openMentionedWebApps } from "../connections/openMentionedWebApps";
import {
  openNativeApp,
  openNativeAppsFromChat,
  rewriteNativeAppSendPrompt,
} from "../connections/openNativeApps";
import {
  detectCodingTaskIntent,
...
import {
  compactChatSearch,
  openFloatingChat,
  useCompactChatChrome,
} from "../lib/floatingChatChrome";
...
} from "@modesto/client-runtime/state/subagentRuntime";
...
import { useSpeakAssistantReplies } from "../hooks/useSpeakAssistantReplies";
import { useThinkingChime } from "../hooks/useThinkingChime";
...
import { accountRateLimitsCacheKey, resolveVisibleRateLimits } from "../state/accountRateLimits";
import {
...
import { appendAppshotContextsToPrompt, formatAppshotContextLabel } from "../lib/appshotContext";
import { attachAppshotToComposer } from "../lib/attachAppshot";
...
import {
  parseCanvasComposerCommand,
  resolveCanvasSendPrompt,
  studioKindForCanvasMode,
} from "../studio/canvasCommand";
import { parseWebAppComposerCommand, resolveWebAppSendPrompt } from "../connections/webAppSlash";
...
import { useStudioDocumentStore } from "../studio/documentStore";
import { isStudioDocumentPath } from "../studio/studioKinds";
import { useStudioFileSync } from "../studio/useStudioFileSync";
import { useStudioTranscriptSync } from "../studio/useStudioTranscriptSync";
import { useVisualArtifactAutoOpen } from "../studio/useVisualArtifactAutoOpen";
import { isVisualGenerationAsk } from "../studio/visualArtifacts";
import { buildInlineVisualPrompt } from "../studio/inlineVisualPrompt";
import { CanvasPanel } from "./studio/CanvasPanel";
import { useSlideDeckFileSync } from "../slides/useSlideDeckFileSync";
import { useSlideDeckSync } from "../slides/useSlideDeckSync";
import { useSlidesStore } from "../slides/slidesStore";
import { ChatTabStrip } from "./chat/ChatTabStrip";
import { ChatTabsTip } from "./chat/ChatTabsTip";
...
import { UsageClock } from "./chat/UsageClock";
import { environmentDiffStats, environmentProjectLabel } from "./RightPanelEnvironments.logic";
...
import { disclosurePanelClassName } from "../lib/disclosureMotion";
...
const DevicePanel = lazy(() => import("./DevicePanel"));
...
type ChatViewProps =
  | {
      ...
      /** Forces compact chrome for equal multi-agent split panes. */
      embedded?: boolean;
...
const PersistentThreadTerminalDrawer = memo(function PersistentThreadTerminalDrawer({
  ...
  return (
    <div
      className={active ? disclosurePanelClassName(visible, "bottom") : "hidden"}
      style={{ height: visible ? `${terminalUiState.terminalHeight}px` : "0px" }}
      aria-hidden={!visible}
...
function ChatViewContent(props: ChatViewProps) {
  ...
  const startNestedThread = useStartNestedThread();
  ...
  useEffect(() => {
    const subscribe = window.desktopBridge?.onAppshotReceived;
    if (typeof subscribe !== "function") return;
    return subscribe((payload) => {
      void attachAppshotToComposer(composerDraftTarget, payload);
    });
  }, [composerDraftTarget]);
  ...
  // ── Chat tabs ────────────────────────────────────────────────────────
  // The strip mirrors navigation rather than owning it: whatever thread or
  // draft this view is showing becomes (or focuses) a tab, so tabs stay in
  ...
  const draftConversationMode = useComposerDraftStore(
    (store) => store.getComposerDraft(composerDraftTarget)?.conversationMode ?? "code",
  );
  const conversationModeSelectable = useComposerDraftStore(
    (store) => store.getComposerDraft(composerDraftTarget)?.conversationModeSelectable === true,
  );
  const conversationMode: ConversationMode =
    routeKind === "server"
      ? (activeServerThread?.conversationMode ?? "code")
      : draftConversationMode;
  const isRegularChat = conversationMode === "chat";
  const urlCompactChrome = useCompactChatChrome();
  const compactChrome = props.embedded === true || urlCompactChrome;
  const floatingChatTarget: ThreadRouteTarget | null =
  ...
  const composerDraftModelSelection = useComposerDraftStore((store) => {
    ...
    return draft.modelSelectionByProvider[provider] ?? null;
  });
  ...
  const setComposerDraftAppshotContexts = useComposerDraftStore(
    (store) => store.setAppshotContexts,
  );
  ...
  const setComposerDraftConversationMode = useComposerDraftStore(
    (store) => store.setConversationMode,
  );
  ...
  const attachedCreateQueueRef = useRef(createSerialAsyncQueue());
  ...
  // Register whatever this view is showing as a tab. A draft that has been
  // promoted to a real thread deliberately registers as a *thread* tab (the
  // route swaps under it), so a promoted draft does not leave a dead draft tab
  // behind next to its own thread tab.
  const chatTabForRoute = useMemo<ChatTab | null>(() => {
    ...
    return routeKind === "draft" && draftId ? { type: "draft", draftId } : null;
  }, [activeThread, draftId, isServerThread, routeKind]);

  const chatTabForRouteKey = chatTabForRoute ? chatTabKey(chatTabForRoute) : null;
  // Tabs are opt-in. With the strip off this effect must not run at all:
  // mirroring navigation into the store while nothing renders it would
  // silently accumulate (and persist) tabs, so turning the feature on later
  ...
  const visibleRightPanelSurfaces = useMemo(
    ...
    [isRegularChat, rightPanelState.surfaces],
  );
  const activeRightPanelSurface =
    visibleRightPanelSurfaces.find((surface) => surface.id === storedActiveRightPanelSurface?.id) ??
    visibleRightPanelSurfaces[0] ??
    null;
  ...
  const musicPreviewTabId = useMusicPlayerStore((state) =>
    musicPreviewTabIdForThread(state.tabIdByThreadKey, activeThreadRef),
  );
  const panelTerminalIds = useMemo(
    () =>
      new Set(
  ...
  useEffect(() => {
    ...
    useRightPanelStore
      ...
      .reconcileBrowserSurfaces(
        activeThreadRef,
        browserTabIdsExcludingMusic(Object.keys(activePreviewState.sessions), musicPreviewTabId),
      );
  }, [activePreviewState.sessions, activeThreadRef, musicPreviewTabId]);
  ...
  useEffect(() => {
    ...
    const sameTabOpenInPanel =
      ...
      (activeRightPanelSurface?.kind === "music" &&
        musicPreviewTabId === activePreviewMiniPlayer.tabId);
  ...
  }, [activeThread, isRegularChat, isServerThread, startNestedThread]);
  ...
  const spawnedThreads = useMemo(
    () =>
      activeThread
        ? selectSpawnedThreadsForParent(
            threadShells.filter((thread) => thread.environmentId === activeThread.environmentId),
            activeThread.id,
          )
        : [],
    [activeThread, threadShells],
  );
  const spawnedLiveCount = useMemo(
    () => spawnedThreads.filter(spawnedThreadIsLive).length,
    [spawnedThreads],
  );
  ...
  useSpeakAssistantReplies({
    enabled: settings.voice.speakReplies ?? false,
    threadId: activeThreadId,
    messages: displayServerMessages,
  });
  useThinkingChime({
    enabled: settings.thinkingSoundEnabled,
    threadId: activeThreadId,
  ...
  const slideCheckpointFiles = useMemo(
    () => turnDiffSummaries.flatMap((summary) => summary.files),
    [turnDiffSummaries],
  );
  useSlideDeckSync({
    threadRef: activeThreadRef,
    messages: activeThread?.messages ?? null,
    checkpointFiles: slideCheckpointFiles,
    turnSettled: latestTurnSettled,
    environmentId: activeProject?.environmentId,
    cwd: activeWorkspaceRoot,
  });
  useSlideDeckFileSync({
    threadRef: activeThreadRef,
    environmentId: activeProject?.environmentId,
    cwd: activeWorkspaceRoot,
  });
  useStudioTranscriptSync({
    kind: "docs",
    threadRef: activeThreadRef,
    messages: activeThread?.messages ?? null,
    checkpointFiles: slideCheckpointFiles,
  ...
  const handleConversationModeChange = useCallback(
    (mode: ConversationMode) => {
      ...
      setComposerDraftConversationMode(composerDraftTarget, mode);
      scheduleComposerFocus();
    },
    [
      conversationMode,
      composerDraftTarget,
      scheduleComposerFocus,
      setComposerDraftConversationMode,
    ],
  );
  ...
  const addMusicSurface = useCallback(() => {
    if (!activeThreadRef) return;
    useRightPanelStore.getState().open(activeThreadRef, "music");
  }, [activeThreadRef]);
  const addCanvasSurface = useCallback(
    (
      mode: "slides" | "docs" | "spreadsheets" | "dashboard" = "dashboard",
      options?: { readonly start?: boolean },
    ) => {
      if (!activeThreadRef) return;
      if (options?.start) {
        useCanvasStore.getState().start(activeThreadRef, mode);
      } else {
        useCanvasStore.getState().setMode(activeThreadRef, mode);
      }
  ...
  const openNativeAppFromComposer = useCallback(
    (app: NativeApp) => {
      if (conversationMode !== "chat" || window.desktopBridge?.openApplication === undefined) {
        return;
      }
      void (async () => {
        await openNativeApp(app);
        if (!compactChrome && floatingChatTarget) {
          await openFloatingChat(floatingChatTarget);
        }
      })();
    },
    [compactChrome, conversationMode, floatingChatTarget],
  );
  const popOutFloatingChat = useCallback(() => {
    if (!floatingChatTarget) return;
    void openFloatingChat(floatingChatTarget);
  }, [floatingChatTarget]);
  const openSlideDeckSurface = useCallback(
    (relativePath: string) => {
      if (!activeThreadRef) return;
      useCanvasStore.getState().start(activeThreadRef, "slides");
      useRightPanelStore.getState().open(activeThreadRef, "canvas");
      const cwd = activeWorkspaceRoot;
      const environmentId = activeProject?.environmentId;
      if (!cwd || !environmentId || !/\.(md|html|htm)$/i.test(relativePath)) return;
      void (async () => {
        ...
        useSlidesStore.getState().setDeck(activeThreadRef, data.contents, relativePath);
      })();
    },
    [activeProject?.environmentId, activeThreadRef, activeWorkspaceRoot],
  );
  const openFileSurface = useCallback(
    (relativePath: string) => {
      if (!activeThreadRef || !activeProject) return;
      if (
        isSlideDeckPath(relativePath) &&
        /\.(md|html|htm)$/i.test(relativePath.replaceAll("\\", "/"))
      ) {
        openSlideDeckSurface(relativePath);
        return;
      }
      if (
        isStudioDocumentPath("docs", relativePath) ||
        isStudioDocumentPath("sheets", relativePath) ||
        isStudioDocumentPath("dashboard", relativePath)
      ) {
        const kind = isStudioDocumentPath("dashboard", relativePath)
          ? "dashboard"
          : isStudioDocumentPath("docs", relativePath)
            ? "docs"
            : "sheets";
        addCanvasSurface(
          kind === "sheets" ? "spreadsheets" : kind === "docs" ? "docs" : "dashboard",
          { start: true },
        );
        const cwd = activeWorkspaceRoot;
        const environmentId = activeProject.environmentId;
        if (cwd) {
          void (async () => {
            ...
            useStudioDocumentStore
              .getState()
              .setDocument(kind, activeThreadRef, data.contents, relativePath);
          })();
        }
        return;
      }
    ...
    [activeProject, activeThreadRef, addCanvasSurface, openSlideDeckSurface],
  );
  useVisualArtifactAutoOpen({
    threadRef: activeThreadRef,
    messages: activeThread?.messages ?? null,
    openImage: openFileSurface,
  });
  ...
  const backgroundLivenessBannerItem = useMemo<ComposerBannerStackItem | null>(() => {
    ...
    const liveCount = agentPanelModel.liveCount + spawnedLiveCount;
    return {
  ...
  const onSend = async (
    ...
    const {
      ...
      appshotContexts: composerAppshotContexts,
    ...
    const {
      ...
      elementContextCount:
        ...
        composerAppshotContexts.length +
    ...
    const canParseAttachedThreadCommand =
      composerImages.length === 0 &&
      sendableComposerTerminalContexts.length === 0 &&
      composerElementContexts.length === 0 &&
      composerAppshotContexts.length === 0 &&
      composerPreviewAnnotations.length === 0 &&
      composerReviewComments.length === 0;
    const sideCommand = canParseAttachedThreadCommand ? parseSideComposerCommand(trimmed) : null;
    if (sideCommand) {
      if (!activeProject) {
        toastManager.add(
          stackedThreadToast({
            ...
            description: "Sidechat needs a project so it stays attached to this conversation.",
          }),
        );
        return;
      }
      ...
      clearComposerDraftContent(composerDraftTarget);
      composerRef.current?.resetCursorState();
      await onCreateAttachedThread({ kind: "side", task: sideCommand.task });
      return;
    }
    const multiagentCommand = canParseAttachedThreadCommand
      ? parseMultiagentComposerCommand(trimmed)
      : null;
    if (multiagentCommand) {
      if (!multiagentCommand.ok) {
        toastManager.add(
          stackedThreadToast({
            type: "warning",
            title: "Add a task after /multiagent",
            description: "Example: /multiagent investigate the flaky auth test",
          }),
        );
        return;
      }
      if (!activeProject) {
        toastManager.add(
          stackedThreadToast({
            type: "warning",
            title: "Choose a project first",
            description: "Additional agents need a project so they share this workspace.",
          }),
        );
        return;
      }
      ...
      clearComposerDraftContent(composerDraftTarget);
      composerRef.current?.resetCursorState();
      await onCreateAttachedThread({ kind: "multiagent", task: multiagentCommand.task });
      return;
    }
    ...
    const canvasCommand =
      ...
      composerPreviewAnnotations.length === 0 &&
      composerReviewComments.length === 0
        ? parseCanvasComposerCommand(trimmed)
        : null;
    if (canvasCommand) {
      addCanvasSurface(canvasCommand.mode, { start: true });
      if (activeThreadRef) {
        if (canvasCommand.mode === "slides") {
          useSlidesStore.getState().beginBuild(activeThreadRef);
        } else {
          const studioKind = studioKindForCanvasMode(canvasCommand.mode);
          if (studioKind) useStudioDocumentStore.getState().beginBuild(studioKind, activeThreadRef);
        }
      }
      promptForSend = resolveCanvasSendPrompt(canvasCommand, {
      ...
      composerPreviewAnnotations.length === 0 &&
      composerReviewComments.length === 0 &&
      isVisualGenerationAsk(trimmed)
    ) {
      promptForSend = buildInlineVisualPrompt(trimmed);
      promptRef.current = promptForSend;
    } else {
      const canRewriteAppCommand =
        composerImages.length === 0 &&
        sendableComposerTerminalContexts.length === 0 &&
        composerElementContexts.length === 0 &&
        composerAppshotContexts.length === 0 &&
        composerPreviewAnnotations.length === 0 &&
        composerReviewComments.length === 0;
      const nativeAppCommand =
        isRegularChat && canRewriteAppCommand ? parseNativeAppComposerCommand(trimmed) : null;
      const nativeAppsToOpen =
        isRegularChat && canRewriteAppCommand && !nativeAppCommand
          ? detectNativeAppsToOpen(trimmed)
          : [];
      if (nativeAppCommand) {
        promptForSend = rewriteNativeAppSendPrompt(trimmed);
        promptRef.current = promptForSend;
      } else if (nativeAppsToOpen[0]) {
        promptForSend = resolveNativeAppSendPrompt(nativeAppsToOpen[0], trimmed);
        promptRef.current = promptForSend;
      } else {
        const webAppCommand = canRewriteAppCommand ? parseWebAppComposerCommand(trimmed) : null;
        if (webAppCommand) {
          promptForSend = resolveWebAppSendPrompt(webAppCommand.app, webAppCommand.task);
          promptRef.current = promptForSend;
        }
      }
    }
    ...
    if (!activeProject && !canSendWithoutActiveProject({ isServerThread, conversationMode })) {
      toastManager.add(
        stackedThreadToast({
          type: "warning",
          title: "Choose a project first",
    ...
    const draftCreateProjectId = resolveDraftThreadCreateProjectId({
      conversationMode,
      activeProjectId: activeProject?.id ?? null,
      draftProjectId: activeThread.projectId,
    });
    ...
    if (isPreviewSupportedInRuntime() && activeThreadRef) {
      void openMentionedWebApps({
        prompt: promptForSend,
        threadRef: activeThreadRef,
        openPreview,
      });
    }
    if (isRegularChat && window.desktopBridge?.openApplication !== undefined) {
      void openNativeAppsFromChat({
        prompt: trimmed,
        threadTarget: floatingChatTarget,
        popOutChat: !compactChrome,
      });
    }
    ...
    const composerAppshotContextsSnapshot = [...composerAppshotContexts];
    ...
    const messageTextWithContexts = appendAppshotContextsToPrompt(
      ...
      composerAppshotContextsSnapshot,
    );
    ...
    let firstComposerAttachmentKind: "image" | "file" | null = null;
    if (composerImagesSnapshot.length > 0) {
      ...
      if (firstComposerAttachment) {
        ...
        firstComposerAttachmentKind = firstComposerAttachment.type;
      }
    }
    ...
    if (!titleSeed) {
      if (firstComposerAttachmentName) {
        titleSeed =
          firstComposerAttachmentKind === "file"
            ? `Attachment: ${firstComposerAttachmentName}`
            : `Image: ${firstComposerAttachmentName}`;
      } else if (composerTerminalContextsSnapshot.length > 0) {
        titleSeed = formatTerminalContextLabel(composerTerminalContextsSnapshot[0]!);
      } else if (composerElementContextsSnapshot.length > 0) {
        titleSeed = formatElementContextLabel(composerElementContextsSnapshot[0]!);
      } else if (composerAppshotContextsSnapshot.length > 0) {
        titleSeed = formatAppshotContextLabel(composerAppshotContextsSnapshot[0]!);
      } else {
    ...
    if (failure === null && turnAttachmentsResult._tag === "Success") {
      const bootstrap =
        isLocalDraftThread || baseBranchForWorktree
          ? {
              ...(isLocalDraftThread && draftCreateProjectId
                ? {
                    createThread: {
                      projectId: draftCreateProjectId,
                      title,
    ...
    if (failure !== null) {
      if (
        ...
        (useComposerDraftStore.getState().getComposerDraft(composerDraftTarget)?.appshotContexts
          .length ?? 0) === 0 &&
        ...
        setComposerDraftAppshotContexts(composerDraftTarget, composerAppshotContextsSnapshot);
  ...
  const onCreateAttachedThread = useCallback(
    async (input: {
      kind: "side" | "multiagent" | "code-task";
      task: string | null;
      open?: boolean;
    }) => {
      if (!activeThread || !activeProject || activeEnvironmentUnavailable) {
        return;
      }
      if (!isServerThread) {
        toastManager.add(
          stackedThreadToast({
            type: "warning",
            title: "Save this thread first",
            description:
              "Multi-agent and sidechats need a saved parent thread so they nest under it.",
          }),
        );
        return;
      }
      ...
      const parentThreadId = activeThread.id;
      const parentEnvironmentId = activeThread.environmentId;
      const parentProjectId = activeProject.id;
      const parentBranch = activeThreadBranch;
      const parentWorktreePath = activeThread.worktreePath;
      ...
      if (
        outgoingPrompt !== null &&
        composerRef.current?.validateProviderInput(outgoingPrompt) === false
      ) {
        return;
      }
      const copy = attachedThreadCopy(input.kind);
      const nextThreadTitle = task !== null ? truncate(task) : copy.emptyTitle;
      const nextThreadModelSelection: ModelSelection = ctxSelectedModelSelection;
      const attachedConversationMode = attachedThreadConversationMode(input.kind);
      const failureTitle = copy.failureTitle;
      const successTitle = copy.successTitle;
      const openAfterCreate = input.open === true;

      await attachedCreateQueueRef.current.enqueue(async () => {
        ...
        try {
          const createResult = await createThread({
            ...
            input: {
              ...
              conversationMode: attachedConversationMode,
              parentThreadId,
              branch: parentBranch,
          ...
          if (failure === null && outgoingPrompt !== null) {
            const startResult = await startThreadTurn({
              environmentId,
              input: {
                threadId: nextThreadId,
          ...
          if (failure === null && outgoingPrompt !== null) {
            const startedResult = await settlePromise(() =>
              waitForStartedServerThread(scopeThreadRef(parentEnvironmentId, nextThreadId)),
            );
        ...
        if (failure !== null) {
          ...
          if (cleanupResult._tag === "Failure" && !isAtomCommandInterrupted(cleanupResult)) {
            console.warn(
              "Failed to clean up spawned thread after start failure.",
        ...
        if (input.kind === "multiagent" && props.embedded !== true) {
          ...
          let multiagentPanes: Array<
            Pick<
              EnvironmentThreadShell,
              "id" | "title" | "parentThreadId" | "conversationMode" | "createdAt" | "environmentId"
            >
          > = selectMultiagentSplitPanes(environmentShells, parentThreadId);
          if (!multiagentPanes.some((thread) => thread.id === nextThreadId)) {
            multiagentPanes = [
              ...multiagentPanes,
              {
                id: nextThreadId,
          ...
          const paneRefs = toAgentSplitPaneRefs(multiagentPanes, parentEnvironmentId);
          const splitStore = useAgentSplitStore.getState();
          const openSession = selectAgentSplitForParent(
            splitStore,
            parentEnvironmentId,
            parentThreadId,
          );
          if (openSession !== null) {
            splitStore.openSplit({
              parentEnvironmentId,
              parentThreadId,
              panes: paneRefs,
            });
          ...
          if (
            shouldOfferAgentSplitView(paneRefs.length) &&
            !selectAgentSplitOfferDismissed(splitStore, parentEnvironmentId, parentThreadId)
          ) {
            toastManager.add(
              stackedThreadToast({
                type: "info",
                title: "Watch agents side by side?",
                description: "Open a split view of your Multi-Agent threads.",
                actionProps: {
                  children: "Open split",
                  onClick: () => {
  ...
  const onStartSidechat = useCallback(() => {
    if (!activeProject) {
      toastManager.add(
        stackedThreadToast({
          type: "warning",
    ...
    void onCreateAttachedThread({ kind: "side", task: null });
  }, [activeProject, onCreateAttachedThread]);
    ...
    if (failure === null) {
      const navigateResult = await settlePromise(() =>
        navigate({
          ...
          search: compactChatSearch(compactChrome),
        }),
      );
  ...
  const onProviderModelSelect = useCallback(
    (instanceId: ProviderInstanceId, model: string) => {
      ...
      const currentProviderEntry = activeThread.session?.providerInstanceId
      ...
      if (lockedProvider !== null && activeThread.session?.providerInstanceId) {
        if (
          currentProviderEntry?.continuation?.groupKey &&
          entry?.continuation?.groupKey &&
          currentProviderEntry.continuation.groupKey !== entry.continuation.groupKey
        ) {
          scheduleComposerFocus();
          return;
        }
      ...
      // A started thread whose session driver differs from the one just
      // picked is a handoff, not a plain model swap: the new provider starts
      // a fresh session with no native memory of this thread, so the server
      // replays the prior conversation as context instead. Worth saying out
      // loud once, since it's not a lossless switch (e.g. no resumable
      // native edit state carries over).
      if (
        activeThread.session !== null &&
        currentProviderEntry !== undefined &&
        resolvedDriverKind !== null &&
        currentProviderEntry.driver !== resolvedDriverKind
      ) {
        toastManager.add({
          type: "info",
          title: `Switching to ${entry?.displayName ?? resolvedDriverKind}`,
          description: `${entry?.displayName ?? resolvedDriverKind} starts a new session with the conversation so far included as compacted context. It won't have ${currentProviderEntry.displayName ?? currentProviderEntry.driver}'s native session state.`,
        });
      } else if (
        activeThread.session !== null &&
        (activeThread.modelSelection.instanceId !== instanceId ||
          activeThread.modelSelection.model !== resolvedModel)
      ) {
        toastManager.add({
          type: "info",
          title: "Context reset for the new model",
          description:
            "Usage starts over in the new model's window. The conversation so far is included as compacted context.",
        });
      }
  ...
  const usageModelSelection =
    ...
    null;
  const headerUsageLimits = useMemo(
    () =>
      resolveVisibleRateLimits({
        cacheKey: accountRateLimitsCacheKey(
          activeThread?.environmentId ?? environmentId,
          usageModelSelection?.instanceId,
        ),
        threadSnapshot: deriveRateLimitsSnapshot(activeThread?.activities ?? []),
      }),
    [
  ...
  const headerUsageModelDisplayName = useMemo(() => {
    if (!usageModelSelection) return null;
    const provider = providerStatuses.find(
      (status) => status.instanceId === usageModelSelection.instanceId,
    );
    const model = provider?.models.find((entry) => entry.slug === usageModelSelection.model);
    return model?.name ?? usageModelSelection.model;
  }, [providerStatuses, usageModelSelection]);
  ...
  const environmentDiff = environmentDiffStats(gitStatusQuery.data?.workingTree ?? null);
  ...
  const rightPanelContent = activeThreadRef ? (
    activeRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's Modesto server to browse pull requests."
      />
      ...
      <ArtifactsPanel
        threadRef={activeThreadRef}
        // Opening an artifact reuses the file surface, so an artifact and a
        // file opened from the tree land in the same viewer rather than two
    ...
    ) : activeRightPanelSurface?.kind === "slides" ||
      activeRightPanelSurface?.kind === "docs" ||
      activeRightPanelSurface?.kind === "sheets" ||
      activeRightPanelSurface?.kind === "canvas" ? (
      <CanvasPanel
        threadRef={activeThreadRef}
        environmentId={activeProject?.environmentId ?? activeThreadRef?.environmentId ?? null}
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      ...
      >
        ...
        {chatTabsEnabled && !compactChrome ? (
          ...
          <ChatTabsTip
            ...
            onDismiss={() => updateClientSettings({ chatTabsTipDismissed: true })}
            shortcutLabel={shortcutLabelForCommand(keybindings, "tab.new") ?? undefined}
          />
        )}
                    ...
                    <div className="absolute inset-x-0 bottom-full z-0">
                      ...
                      >
                        <DraftHeroHeadline
                          ...
                          conversationMode={conversationMode}
                          allowConversationModeChange={showConversationModeToggle}
                      ...
                      <div className="chat-composer-glass-host relative z-10 w-full rounded-[22px]">
                        <div ref={attachDraftHeroComposerAnchorRef} className="relative z-10">
                          <ChatComposer
                            ...
                            projectSelectionRequired={isProjectSelectionRequired({
                              isLocalDraftThread,
                              hasActiveProject: activeProject !== null,
                              conversationMode,
                            })}
                            ...
                            onSend={onSend}
                            onOpenCanvas={(mode = "dashboard") =>
                              addCanvasSurface(mode, { start: mode !== "dashboard" })
                            }
                            onOpenWebApp={openWebApp}
                            onOpenNativeApp={openNativeAppFromComposer}
        ...
        {mountedTerminalThreadRefs.map(({ key: mountedThreadKey, threadRef: mountedThreadRef }) => (
          <PersistentThreadTerminalDrawer
            ...
            visible={
              !isRegularChat && mountedThreadKey === activeThreadKey && terminalUiState.terminalOpen
            }
      ...
      {!compactChrome && !shouldUseRightPanelSheet && activeThreadRef ? (
        <RightPanelTabs
          mode="inline"
          open={rightPanelOpen}
          maximized={rightPanelMaximized}
          ...
          conversationMode={conversationMode}
          contextAvailable={activeThreadRef !== null}
          artifactsAvailable={activeThreadRef !== null}
          musicAvailable
          canvasAvailable
          deviceAvailable={activeThreadRef !== null}
          browserAvailable={isPreviewSupportedInRuntime()}
          terminalAvailable={!isRegularChat && activeProject !== null}
