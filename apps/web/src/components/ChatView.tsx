...
import { getModelCapabilities, normalizeModelSlug } from "@modesto/shared/model";
import { resolveTailUserMessageEditTarget } from "@modesto/shared/conversationEdit";
import { threadExportBlockedReason } from "@modesto/shared/threadExport";
import { buildTemporaryWorktreeBranchName } from "@modesto/shared/git";
import {
  buildPromptThreadTitleFallback,
  GENERIC_CHAT_THREAD_TITLE,
} from "@modesto/shared/chatThreads";
import {
  resolveThreadWorkspaceState,
  resolveThreadBranchSourceCwd,
  resolveThreadWorkspaceCwd as resolveSharedThreadWorkspaceCwd,
} from "@modesto/shared/threadEnvironment";
import {
  deriveAssociatedWorktreeMetadata,
  workspaceRootsEqual,
} from "@modesto/shared/threadWorkspace";
import {
  useCallback,
  useEffect,
  useLayoutEffect,
...
import { ThreadProviderHandoffDialog } from "./ThreadProviderHandoffDialog";
import { ThreadHandoffReturnDialog } from "./ThreadHandoffReturnDialog";
import { HandoffReturnBanner } from "./chat/HandoffReturnBanner";
import { HandoffSeamBanner } from "./chat/HandoffSeamBanner";
import { summarizePatchTotals } from "../lib/diffRendering";
import { HANDOFF_DIFF_ACK_PROMPT } from "../lib/threadHandoff";
import {
  formatShortcutLabel,
...
import {
  ...
  withCodexCustomEndpointOverride,
...
import { isCoworkThreadId, isResearchThreadId, useCoworkThreadStore } from "../coworkThreadStore";
...
const TERMINAL_DRAWER_DISCLOSURE_MS = 220;

/**
 * Keeps an open terminal mounted through its closing motion, then releases the
 * expensive xterm surface. The shell itself remains mounted so a newly opened
 * terminal can grow upward from the bottom edge instead of appearing in place.
 */
function TerminalDrawerDisclosure(props: { open: boolean; children: ReactNode }) {
  const [retainContent, setRetainContent] = useState(props.open);
  ...
  useEffect(() => {
    ...
    }, TERMINAL_DRAWER_DISCLOSURE_MS);
    return () => window.clearTimeout(timeout);
  }, [props.open]);

  return (
    <DisclosureRegion open={props.open} className="shrink-0">
      {props.open || retainContent ? props.children : null}
    </DisclosureRegion>
  );
}
...
// back-and-forth (user request, Modesto's clarifying questions) inline like Codex.
...
export default function ChatView({
  ...
  const {
    handoffBusy: providerHandoffBusy,
    handoffDialogOpen,
  ...
  // Threads started from the Teams/Research task launchers get a simplified
  // composer — same model picker, no Full access/Approval required control.
  const isCoworkThread = useCoworkThreadStore((state) =>
    isCoworkThreadId(state.coworkThreadIds, activeThreadId),
  );
  const isResearchThread = useCoworkThreadStore((state) =>
    isResearchThreadId(state.threadKindById, activeThreadId),
  );
  ...
  // While set, Modesto is mid-conversation gathering an automation's missing details.
  ...
  // A thread keeps one transcript even when its next turn moves to another
  // provider. The server replaces the inactive native session and supplies
  // the prior transcript to the new session's first turn, so model selection
  // must remain provider-selectable after a conversation has started.
  ...
  const modelOptionsByProvider = useMemo(() => {
    ...
    result.codex = applyCustomModelEndpoints(
      result.codex,
  ...
  // Picked once per landing mount (not on every render) so the greeting stays put while
  // typing, but rolls a fresh time-aware variant each time a new empty draft is opened.
  ...
  const handoffActionLabel = activeThread
    ? "Open this thread in another provider"
    : "Open a new thread in another provider";
  ...
  const hasReachedSplitLimit =
    ...
    MAX_TERMINALS_PER_GROUP;
  // The standalone terminal workspace has been retired from the chat shell.
  // Keep its persisted state readable for older sessions, but always present
  // terminals as the bottom drawer.
  const terminalWorkspaceOpen = false;
  const terminalWorkspaceTerminalTabActive = false;
  ...
  const environmentEnabled = !isEditorRail && workspaceMode !== "research";
  ...
  const terminalDrawerProps = useMemo(
    () => ({
      threadId,
      onTogglePanel: hasRightDockPanes && !rightDockOpen ? toggleRightDock : undefined,
  ...
  const combinedProviderHandoffBusy = handoffBusy || providerHandoffBusy;
  ...
  const toggleInteractionMode = useCallback(() => {
    handleInteractionModeChange(interactionModeRef.current === "plan" ? "default" : "plan");
    ...
    (style: ThreadMarkerStyle, color: ThreadMarkerColor) => {
      ...
      if (!range) {
        toastManager.add({
          ...
          description: "Try including a few more words so Modesto can find the exact place.",
        });
        return;
      }
      dismissTranscriptSelectionAction();
  ...
  const onProviderModelSelect = useCallback(
    (provider: ProviderKind, model: ModelSlug) => {
      ...
      const customEndpointId =
        provider === "codex"
          ? modelOptionsByProvider[provider]?.find((option) => option.slug === resolvedModel)
              ?.customEndpointId
          : undefined;
      const nextModelSelection = (
        customEndpointId
          ? { provider, model: resolvedModel, options: { customEndpointId } }
  ...
  const onConfirmProviderHandoff = useCallback(async () => {
    if (!handoffDraft) {
      return;
    }
    try {
      await confirmThreadHandoff(handoffDraft);
    } catch (error) {
      toastManager.add({
        type: "error",
        title: "Could not create handoff thread",
  ...
  }, [confirmThreadHandoff, handoffDraft]);

  const onAcknowledgeHandoffDiff = useCallback(async () => {
    const api = readNativeApi();
    if (!api || !activeThread?.handoff) {
      return;
    }
    try {
      await api.orchestration.dispatchCommand({
      ...
      toastManager.add({
        type: "error",
        title: "Could not acknowledge diff",
        description: error instanceof Error ? error.message : "An error occurred.",
      });
    }
  }, [activeThread, syncServerShellSnapshot]);

  const resolveActiveHandoffCwd = useCallback((): string | null => {
    if (!activeThread || !activeProject) {
      return null;
    }
    return resolveSharedThreadWorkspaceCwd({
      projectCwd: activeProject.cwd,
      envMode: activeThread.envMode ?? (activeThread.worktreePath ? "worktree" : "local"),
      worktreePath: activeThread.worktreePath ?? null,
    });
  ...
  const onInspectHandoffCheckpoint = useCallback(async () => {
    const api = readNativeApi();
    const cwd = resolveActiveHandoffCwd();
    if (!api || !activeThread?.handoff || !cwd) {
      return;
    }
    try {
      const result = await api.orchestration.getHandoffCheckpointDiff({
        threadId: activeThread.id,
        cwd,
      });
      const stats = summarizePatchTotals(result.diff);
      if (stats && stats.fileCount > 0) {
        toastManager.add({
          type: "success",
          title: "Seam checkpoint",
          description: `${stats.fileCount} file${stats.fileCount === 1 ? "" : "s"} · +${stats.additions}/-${stats.deletions}`,
        });
      } else if (result.checkpointStatus === "missing") {
        toastManager.add({
          type: "error",
          title: "Seam checkpoint missing",
          description: "The durable handoff checkpoint could not be read from git.",
        });
        return;
      } else {
        toastManager.add({
          type: "success",
          title: "Seam checkpoint",
          description: "Working tree matched HEAD when this seam was captured.",
        });
      }
      onOpenDiff();
    } catch (error) {
      toastManager.add({
        type: "error",
        title: "Could not inspect seam checkpoint",
        description: error instanceof Error ? error.message : "An error occurred.",
      });
    }
  }, [activeThread, onOpenDiff, resolveActiveHandoffCwd]);

  const onRollbackHandoffCheckpoint = useCallback(async () => {
    const api = readNativeApi();
    const cwd = resolveActiveHandoffCwd();
    if (!api || !activeThread?.handoff || !cwd) {
      return;
    ...
    const confirmed = await api.dialogs.confirm(
      "Restore the working tree to the durable handoff seam checkpoint? Uncommitted changes after the handoff will be discarded.",
    );
    if (!confirmed) {
      return;
    }
    try {
      const result = await api.orchestration.restoreHandoffCheckpoint({
        threadId: activeThread.id,
        cwd,
      });
      if (!result.restored) {
        toastManager.add({
          type: "error",
          title: "Could not roll back to seam",
          description: "The handoff checkpoint was missing or could not be restored.",
        });
        return;
      }
      await api.orchestration.dispatchCommand({
      ...
      toastManager.add({
        type: "success",
        title: "Rolled back to seam",
        description: "Working tree restored to the durable handoff checkpoint.",
      });
    } catch (error) {
      toastManager.add({
        type: "error",
  ...
  }, [activeThread, resolveActiveHandoffCwd, syncServerShellSnapshot]);

  const onInsertHandoffDiffAckPrompt = useCallback(() => {
    if (!activeThread) {
      return;
    }
    setPrompt(
      prompt.trim().length > 0
        ? `${prompt.trim()}\n\n${HANDOFF_DIFF_ACK_PROMPT}`
        : HANDOFF_DIFF_ACK_PROMPT,
    );
    scheduleComposerFocus();
  }, [activeThread, prompt, scheduleComposerFocus, setPrompt]);

  const onOpenHandoffReturnDialog = useCallback(async () => {
    if (!activeThread) {
      return;
    }
    try {
      await openReturnDialog(activeThread);
    } catch (error) {
      toastManager.add({
        type: "error",
        title: "Could not open handoff return",
        description: error instanceof Error ? error.message : "An error occurred.",
      });
    }
  }, [activeThread, openReturnDialog]);

  const onConfirmHandoffReturn = useCallback(async () => {
    if (!returnDraft) {
      return;
    }
    try {
      await confirmThreadHandoffReturn(returnDraft);
    } catch (error) {
      toastManager.add({
        type: "error",
        title: "Could not return handoff thread",
        description: error instanceof Error ? error.message : "An error occurred.",
      });
    }
  }, [confirmThreadHandoffReturn, returnDraft]);
    ...
    async (input: {
      ...
      try {
          ...
          void (async () => {
            try {
              ...
              toastManager.add({
                ...
                description:
                  "The automation was created, but Modesto could not add the activity note.",
              });
            }
          })();
        }
        ...
        toastManager.add({
          ...
          description:
            error instanceof Error ? error.message : "Modesto could not save the automation.",
        });
        return false;
      } finally {
      ...
      try {
        ...
        if (result === "unavailable") {
          toastManager.add({
            ...
            description: "Modesto could not promote this draft before saving the automation.",
          });
          return null;
        }
        ...
        toastManager.add({
          ...
          description:
            error instanceof Error
              ? error.message
              : "Modesto could not promote this draft before saving the automation.",
        });
        return null;
      }
    },
  ...
  const onSend = async (
    ...
    const providerOptionsForDispatchForSend = withCodexCustomEndpointOverride(
    ...
    await (async () => {
      ...
      await api.orchestration.dispatchCommand({
        ...
        runtimeMode: nextRuntimeModeForSend,
        interactionMode: interactionModeForSend,
        ...(isResearchThread ? { workspaceMode: "research" as const } : {}),
        ...(sourceProposedPlanForSend ? { sourceProposedPlan: sourceProposedPlanForSend } : {}),
        createdAt: messageCreatedAt,
      });
  ...
  const runtimeUsageControlsProps = {
    // RuntimeUsageControls renders nothing unless both of these are set, so
    // cowork-style threads simply never get the picker instead of always
    // defaulting to (and only ever running in) approval-required mode.
    runtimeMode: isCoworkThread ? undefined : runtimeMode,
    onRuntimeModeChange: isCoworkThread ? undefined : handleRuntimeModeChange,
  ...
  const emptyLandingControls = showEmptyLandingControls ? (
    <div
      className={cn(
        // Cursor-style picker row floating above the composer: plain muted
        // controls (project · branch · temporary) with no tray surface.
        "relative z-0 flex min-h-7 min-w-0 flex-nowrap items-center gap-x-1.5 px-1 pb-2",
        COMPOSER_COLUMN_FRAME_CLASS_NAME,
      )}
    >
      {isEmptyChatLanding ? (
  ...
  const showComposerChromeBubbles = !isComposerApprovalState;
  const workingAgentCount = 1 + (activeBackgroundTasks?.activeCount ?? 0);
  const onOpenComposerTasks = useCallback(() => {
    setPlanSidebarOpen(true);
  }, []);

  // Composer layout keeps the chrome bubbles and footer actions in one render path so
            ...
            <div ref={measureComposerStackedChrome}>
              {showComposerChromeBubbles ? (
                <ComposerChromeBubbles
                  ...
                  onOpenChanges={
                    activeTurnLiveDiffState.turnId ? onReviewComposerLiveChanges : onOpenDiff
                  }
                  commitControl={
                    isGitRepo && showGitActions && !isEditorRail && workspaceMode !== "research" ? (
                      <GitActionsControl
                        gitCwd={threadWorkspaceCwd}
                  ...
                  activeTaskList={activeTaskList && !planSidebarOpen ? activeTaskList : null}
                  onOpenTasks={onOpenComposerTasks}
                />
              ) : null}
            ...
            >
              <div
                className={cn(
                  ...
                  composerMenuOpen && !isComposerApprovalState && "overflow-visible",
                  interactionMode === "plan" && "composer-surface-plan-mode",
                )}
                          ...
                          : showPlanFollowUpPrompt && activeProposedPlan
                            ...
                            : hasLiveTurn
                              ...
                              : phase === "disconnected"
                                ...
                                : interactionMode === "plan"
                                  ? "Describe what to plan — explore only, no edits. @tag context or / for commands"
                  ...
                  >
                    <div
                      ...
                      className={cn(
                        "flex items-center",
                        isVoiceRecording || isVoiceTranscribing
                          ? "min-w-0 shrink-0 gap-2"
                          : isComposerFooterCompact
                            ? "min-w-0 flex-1 gap-2 overflow-hidden"
                            : "min-w-0 flex-1 gap-2 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:min-w-max sm:overflow-visible",
                      )}
                    >
                    ...
                    >
                      ...
                      composerFooterControlsPlan.showContextMeter ? (
                        <ContextWindowMeter
                          ...
                          onOpenReport={onOpenContextPanel}
    ...
    >
      ...
      >
        <ChatHeader
          ...
          activeProvider={activeThread.session?.provider ?? activeThread.modelSelection.provider}
          activeProjectName={
            isEditorRail || workspaceMode === "research" ? undefined : activeProjectDisplayName
          }
          ...
          hideHandoffControls={
            terminalWorkspaceTerminalTabActive || isEditorRail || workspaceMode === "research"
          }
          isGitRepo={isGitRepo}
          openInTarget={threadWorkspaceCwd}
          activeProjectScripts={
            isEditorRail || workspaceMode === "research" ? undefined : activeProjectScripts
          }
          preferredScriptId={
          ...
          gitCwd={threadWorkspaceCwd}
          diffTotals={repoDiffTotals}
          showGitActions={showGitActions && !isEditorRail && workspaceMode !== "research"}
          showDiffToggle={!isEditorRail && workspaceMode !== "research"}
          diffOpen={resolvedDiffOpen}
          hasRightDockPanes={hasRightDockPanes}
          rightDockOpen={rightDockOpen}
          onToggleRightDock={toggleRightDock}
          diffDisabledReason={diffDisabledReason}
          terminalOpen={terminalState.terminalOpen && !terminalWorkspaceOpen}
      ...
      <HandoffSeamBanner
        ...
        busy={combinedProviderHandoffBusy}
        onAcknowledgeDiff={() => {
          void onAcknowledgeHandoffDiff();
        }}
        onInsertAckPrompt={onInsertHandoffDiffAckPrompt}
        onOpenReturnDialog={() => {
          void onOpenHandoffReturnDialog();
        }}
        onInspectCheckpoint={() => {
          void onInspectHandoffCheckpoint();
        }}
        onRollbackCheckpoint={() => {
          void onRollbackHandoffCheckpoint();
        }}
      />
      <HandoffReturnBanner thread={activeThread} />
      <RateLimitBanner
      ...
      <TerminalDrawerDisclosure open={terminalState.terminalOpen && !terminalWorkspaceOpen}>
        {activeThread ? (
      ...
      />
      <ThreadProviderHandoffDialog
        open={handoffDialogOpen}
        draft={handoffDraft}
        busy={combinedProviderHandoffBusy}
        onOpenChange={setHandoffDialogOpen}
        onDraftChange={setHandoffDraft}
        onConfirm={onConfirmProviderHandoff}
      />
      <ThreadHandoffReturnDialog
        open={returnDialogOpen}
        draft={returnDraft}
        busy={combinedProviderHandoffBusy}
        onOpenChange={setReturnDialogOpen}
        onDraftChange={setReturnDraft}
        onConfirm={onConfirmHandoffReturn}
      />
