...
import { ReviewFindingsPanel } from "./review/ReviewFindingsPanel";
...
export default function DiffPanel({
  ...
  // A sidebar review should open as a complete review workspace: patch on the
  // left, searchable changed-file tree on the right. The inline/editor variants
  // keep their previous lightweight behavior because they already have their
  // own navigation surface.
  const shouldOpenFileTreeInitially = mode === "sidebar" && !hideHeader;
  const [fileTreeOpen, setFileTreeOpen] = useState(shouldOpenFileTreeInitially);
  ...
  const [fileTreeMounted, setFileTreeMounted] = useState(shouldOpenFileTreeInitially);
  ...
  const activeThreadShell = useStore((state) =>
    activeThreadId ? state.threadShellById?.[activeThreadId] : undefined,
  );
  ...
  const dispatchReviewFollowUp = useCallback(
    (finding: ReviewFinding, intent: "fix" | "explain") => {
      if (!activeThreadId || !activeThreadShell) return;
      ...
      const prompt =
        intent === "fix"
          ? [
              `Fix this Modesto Review finding in ${location}.`,
              `Title: ${finding.title}`,
              finding.explanation,
              finding.suggestedFix ? `Suggested fix:\n${finding.suggestedFix}` : "",
              "Inspect the current code and diff first. Make the smallest correct change, preserving unrelated work.",
            ]
              .filter(Boolean)
              .join("\n\n")
          : [
              `Explain this Modesto Review finding in ${location}.`,
              `Title: ${finding.title}`,
              finding.explanation,
              finding.suggestedFix ? `Suggested fix:\n${finding.suggestedFix}` : "",
              "Explain whether it is valid in this codebase, the concrete risk, and the safest resolution. Do not edit files.",
            ]
              .filter(Boolean)
              .join("\n\n");
    ...
    [activeThreadId, activeThreadShell],
  );
  ...
  // A single selected turn backed only by a provider-reported live diff (no
  // git checkpoint) can still be reviewed without a git repo — the server
  // serves the provider's own stored diff text for this exact single-turn
  // range. Everything else (multi-turn ranges, full-thread diff) stays
  // git-only.
  const selectedTurnHasProviderDiff =
    selectedTurn?.checkpointRef?.startsWith("provider-diff:") === true;
  const canQueryTurnDiffWithoutGitRepo =
    Boolean(selectedTurn) && selectedTurnHasProviderDiff && selectedCheckpointRange !== null;
  ...
  const activeCheckpointDiffQuery = useQuery(
    checkpointDiffQueryOptions({
      ...
      enabled:
        diffQueriesEnabled &&
        (isGitRepo || canQueryTurnDiffWithoutGitRepo) &&
  ...
  const showDiffToolbar = Boolean(
    activeThreadContext && (isGitRepo || canQueryTurnDiffWithoutGitRepo) && !diffEnvironmentPending,
  );
  ...
  return (
    <DiffPanelShell mode={mode} header={shellHeader}>
      ...
      ) : gitRepoStatus === false && !canQueryTurnDiffWithoutGitRepo ? (
        <PanelStateMessage density="compact" fill="flex">
          ...
          >
            {!hideReviewPanel ? (
              <ReviewFindingsPanel
                ...
                selectedAgent={
                  activeThreadShell
                    ? PROVIDER_DISPLAY_NAMES[activeThreadShell.modelSelection.provider]
                    : "selected agent"
                }
                enabled={diffQueriesEnabled}
                diffFiles={renderableFiles}
                onFix={(finding) => dispatchReviewFollowUp(finding, "fix")}
                onExplain={(finding) => dispatchReviewFollowUp(finding, "explain")}
              />
            ) : null}
