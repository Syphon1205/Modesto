...
import type { ProfileStats, ProfileTokenStats } from "@modesto/contracts";
import { Dialog, DialogPopup, DialogTitle } from "~/components/ui/dialog";
import { CopyIcon, DownloadIcon, LinkedInIcon, RedditIcon, XSocialIcon } from "~/lib/icons";
...
export function ShareDialog({
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const blob = await renderNodeToPngBlob(node, CARD_EXPORT_SIZE);
      if (blob) {
        downloadBlob(blob, `modesto-stats-${stats.timezone.today}.png`);
