...
import { filterFilesystemBrowseEntries } from "@modesto/client-runtime/state/filesystem";
import type { SidebarThreadSortOrder } from "@modesto/contracts/settings";
...
/**
 * Finder (and the other host file managers) can open against the local
 * environment from either the desktop shell or the browser talking to a
 * local server. Secondary desktop-local filesystems (WSL today) still go
 * through the desktop picker so the path is routed to the right backend.
 */
export function canUseNativeFolderPicker(input: {
  ...
  readonly isDesktopLocalWithPicker: boolean;
}): boolean {
  ...
  return input.hasDesktopBridge && input.isDesktopLocalWithPicker;
}
...
export type CommandPaletteOpenIntent =
  | { readonly kind: "add-project" | "remote-project" | "new-thread-in" }
  | { readonly kind: "clone-project"; readonly repository?: string }
  | { readonly kind: "create-local-project"; readonly path: string; readonly title?: string };
...
export type CommandPaletteUiAction =
  ...
  | { readonly _tag: "OpenCloneProject"; readonly repository?: string }
  | { readonly _tag: "OpenRemoteProject" }
  | { readonly _tag: "CreateLocalProject"; readonly path: string; readonly title?: string }
...
export function reduceCommandPaletteUiState(
  ...
  switch (action._tag) {
    ...
    case "OpenCloneProject":
      return {
        open: true,
        mode: "command",
        openIntent: {
          kind: "clone-project",
          ...(action.repository ? { repository: action.repository } : {}),
        },
      };
    case "OpenRemoteProject":
      return { open: true, mode: "command", openIntent: { kind: "remote-project" } };
    case "CreateLocalProject":
      return {
        open: true,
        mode: "command",
        openIntent: {
          kind: "create-local-project",
          path: action.path,
