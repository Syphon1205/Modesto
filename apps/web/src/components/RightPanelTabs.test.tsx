import type { DesktopPreviewFavicon, PreviewSessionSnapshot } from "@modesto/contracts";
import { renderToStaticMarkup } from "react-dom/server";
import { describe, expect, it } from "vite-plus/test";

import {
  RightPanelTabs,
  surfaceActionVisibleInConversation,
  surfaceShortcutActionForKey,
  surfaceShortcutTargetsTypingContext,
  tabMuteMenuItem,
} from "./RightPanelTabs";

describe("conversation surface visibility", () => {
  it("keeps work surfaces in Chat and reserves repository surfaces for Code", () => {
    expect(surfaceActionVisibleInConversation("chat", {})).toBe(true);
    expect(surfaceActionVisibleInConversation("chat", { codeOnly: true })).toBe(false);
    expect(surfaceActionVisibleInConversation("code", { codeOnly: true })).toBe(true);
  });

  it("keeps Music in Chat", () => {
    expect(surfaceActionVisibleInConversation("chat", { codeOnly: undefined })).toBe(true);
  });
});
...
describe("surface shortcuts", () => {
  const actions = [
    ...
    { shortcut: "S", available: true, label: "Slides" },
  ...
  it("matches available surface shortcuts case-insensitively", () => {
    ...
    expect(surfaceShortcutActionForKey(actions, shortcutEvent("B"))).toBe(actions[0]);
    expect(surfaceShortcutActionForKey(actions, shortcutEvent("s"))).toBe(actions[1]);
  });
