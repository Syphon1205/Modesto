...
import type { ThreadMarker } from "@modesto/contracts";
...
const BARE_FILE_REFERENCE_PATTERN =
  /(^|[^\w@.$/\\-])((?:(?:\.{1,2}|[\w@.$-]+)[/\\])*[\w@.$-]+\.[A-Za-z0-9]{1,12}(?::\d+(?::\d+)?)?)/gu;

function isNegatedBareFileReference(text: string, start: number, length: number): boolean {
  const before = text.slice(Math.max(0, start - 48), start);
  const after = text.slice(start + length, start + length + 48);
  return (
    /(?:^|\b)(?:no|missing)\s+$/iu.test(before) ||
    /(?:^|\b)(?:without|lacking)\s+$/iu.test(before) ||
    /\bnot\s+(?:found|present):?\s*$/iu.test(before) ||
    /^\s+(?:(?:does|do|is)\s+not|doesn['’]t|isn['’]t)\s+(?:exist|present)\b/iu.test(after) ||
    /^\s+is\s+(?:also\s+)?(?:absent|missing)\b/iu.test(after) ||
    /^\s+(?:was|were)\s+(?:never\s+)?(?:created|added|present)\b/iu.test(after)
  );
}
...
// Assistant prose often calls out the important files without wrapping them in
// backticks or Markdown links. Recognize only known file types, leaving version
// numbers and ordinary dotted prose untouched, and give those references the
// same blue, language-aware, openable treatment as explicit file links.
function renderBareFileReferences(
  children: ReactNode,
  cwd: string | undefined,
  theme: "light" | "dark",
): ReactNode {
  return Children.map(children, (child) => {
    ...
    BARE_FILE_REFERENCE_PATTERN.lastIndex = 0;
    while ((match = BARE_FILE_REFERENCE_PATTERN.exec(child)) !== null) {
      const prefix = match[1] ?? "";
      const candidate = match[2] ?? "";
      const candidateStart = match.index + prefix.length;
      const iconPath = candidate.replace(MARKDOWN_LINK_POSITION_SUFFIX_PATTERN, "");
      if (
        !pathLooksLikeKnownFile(iconPath) ||
        isNegatedBareFileReference(child, candidateStart, candidate.length)
      ) {
        continue;
      }

      if (candidateStart > cursor) rendered.push(child.slice(cursor, candidateStart));
      const targetPath = resolveMarkdownFileLinkTarget(candidate, cwd) ?? candidate;
      rendered.push(
        <OpenableFileChip
          key={`${candidateStart}:${candidate}`}
    ...
    if (cursor < child.length) rendered.push(child.slice(cursor));
    return rendered;
  });
}
    ...
    () => ({
      ...
      img({ node: _node, src, alt = "", ...props }) {
        ...
        return (
          <img
            ...
            alt={alt}
            loading="lazy"
            className="chat-remote-evidence-image"
          />
        );
      },
      p({ node: _node, children, ...props }) {
        return <p {...props}>{renderBareFileReferences(children, cwd, resolvedTheme)}</p>;
      },
      li({ node, children, ...props }) {
        ...
        if (!isTaskItem || sourceLine === null) {
          return <li {...props}>{renderBareFileReferences(children, cwd, resolvedTheme)}</li>;
        }
        return (
          <li {...props}>
            <TaskItemSourceLineContext.Provider value={sourceLine}>
              {renderBareFileReferences(children, cwd, resolvedTheme)}
