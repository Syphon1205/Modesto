...
export function parsePullRequestQuery(raw: string): {
  ...
  for (const [token] of raw.matchAll(QUERY_TOKEN)) {
    ...
    switch (value.length === 0 ? "" : (qualifier?.[2]?.toLowerCase() ?? "")) {
      ...
      case "assignee":
        if (negated) break;
        assignee = value.slice(0, MAX_QUALIFIER_LENGTH).trim();
        continue;
      case "mentions":
        if (negated) break;
        mentions = value.slice(0, MAX_QUALIFIER_LENGTH).trim();
        continue;
      case "involves":
        if (negated) break;
        involves = value.slice(0, MAX_QUALIFIER_LENGTH).trim();
        continue;
      case "draft":
...
export const pullRequestEnvironmentSetKey = (environmentIds: ReadonlyArray<string>): string =>
  [...environmentIds].toSorted((left, right) => left.localeCompare(right)).join(",");
