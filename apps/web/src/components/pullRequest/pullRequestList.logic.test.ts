import type { EnvironmentId, ProjectId, PullRequestListEntry } from "@modesto/contracts";
...
function entry(
  ...
  return {
    ...
    url: `https://github.com/pingdotgg/modesto/pull/${overrides.number}`,
...
describe("reading qualifiers out of a typed query", () => {
  ...
  it("reads assignee, mentions, and involves the way GitHub's notification filters do", () => {
    expect(parsePullRequestQuery("assignee:me mentions:octocat involves:me").filters).toEqual({
      assignee: "me",
      mentions: "octocat",
      involves: "me",
    });
...
describe("the list snapshot across a reload", () => {
  ...
  const data = {
    ...
    truncated: true,
    nextCursors: { "pingdotgg/modesto": "cursor-1" },
  } as never;
...
describe("merging the environments' own listings", () => {
  ...
  it("keeps each environment's continuation to itself", () => {
    const merged = mergePullRequestLists([
      [ENV_1, answer({ nextCursors: { "github.com pingdotgg/modesto": "cursor-1" } })],
      [ENV_2, answer()],
    ]);
    expect(merged?.nextCursors).toEqual({
      [ENV_1]: { "github.com pingdotgg/modesto": "cursor-1" },
    });
  });
...
describe("colon-namespaced labels typed as a search", () => {
  ...
  it("leaves a pasted link alone rather than naming a label after its scheme", () => {
    const parsed = parsePullRequestQuery("https://github.com/pingdotgg/modesto/pull/1");
    expect(parsed.filters.labels).toBeUndefined();
    expect(parsed.text).toBe("https://github.com/pingdotgg/modesto/pull/1");
  });
