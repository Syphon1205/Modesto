import type { GitBranch } from "@modesto/contracts";
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.modesto/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a branch already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.modesto/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("keeps legacy .modesto worktree paths working for migrated threads", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.modesto/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("preserves associated worktree metadata during local branch-only syncs", () => {
    expect(
      resolveAssociatedWorktreeMetadataAfterWorkspacePatch({
        branch: "main",
        worktreePath: null,
        existingAssociatedWorktreePath: "/repo/.worktrees/modesto-pi",
        existingAssociatedWorktreeBranch: "modesto/pi",
        existingAssociatedWorktreeRef: "modesto/pi",
      }),
    ).toEqual({
      associatedWorktreePath: "/repo/.worktrees/modesto-pi",
      associatedWorktreeBranch: "modesto/pi",
      associatedWorktreeRef: "modesto/pi",
    });
  });
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected branch", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        activeWorktreePath: "/repo/.modesto/worktrees/feature-a",
        branch: {
          isDefault: false,
          worktreePath: "/repo/.modesto/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.modesto/worktrees/feature-b",
      nextWorktreePath: "/repo/.modesto/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default branches", () => {
    expect(
      ...
      checkoutCwd: "/repo/.modesto/worktrees/feature-a",
      nextWorktreePath: "/repo/.modesto/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
