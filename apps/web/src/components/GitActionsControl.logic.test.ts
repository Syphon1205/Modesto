import type { GitStatusResult } from "@modesto/contracts";
...
describe("resolveDefaultCreateBranchName", () => {
  it("uses Modesto as the default namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "fix toast copy");
    assert.equal(branch, "modesto/fix-toast-copy");
  });

  it("normalizes an existing legacy modesto namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "modesto/refine-toolbar-actions");
    assert.equal(branch, "modesto/refine-toolbar-actions");
  });

  it("preserves nested namespaces under Modesto", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "feature/refine-toolbar-actions");
    assert.equal(branch, "modesto/feature/refine-toolbar-actions");
  });

  it("increments suffix when the Modesto branch already exists", () => {
    const branch = resolveDefaultCreateBranchName(
      ["main", "modesto/fix-toast-copy", "modesto/fix-toast-copy-2"],
      "fix toast copy",
    );
    assert.equal(branch, "modesto/fix-toast-copy-3");
  });

  it("falls back to modesto/update when no preferred name is provided", () => {
    const branch = resolveDefaultCreateBranchName(["main"]);
    assert.equal(branch, "modesto/update");
  });
});
...
describe("resolveLiveThreadBranchUpdate", () => {
  it("does not regress a semantic thread branch back to a temporary worktree branch", () => {
    const update = resolveLiveThreadBranchUpdate({
      ...
      gitStatus: status({ branch: "modesto/deadbeef" }),
    });
...
describe("shouldOfferCreateBranchPrompt", () => {
  const temporaryBranch = "modesto/deadbeef";
