...
import { type ServerProviderSkill } from "@modesto/contracts";
import { serializeComposerFileLink } from "@modesto/shared/composerTrigger";
import { nativeAppForMentionPath, nativeAppMentionLabel } from "@modesto/shared/nativeApps";
import {
  serializeComposerWebAppMention,
  webAppForMentionPath,
  webAppMentionLabel,
} from "~/connections/webApps";
import {
...
function ComposerMentionDecorator(props: { path: string }) {
  ...
  const app = webAppForMentionPath(props.path);
  const nativeApp = nativeAppForMentionPath(props.path);
  const chip = (
    <span
    ...
    >
      <FileTagChipContent
        path={props.path}
        label={app?.name ?? nativeApp?.name ?? basenameOfPath(props.path)}
        theme={theme}
      />
    </span>
  );
  ...
  return (
    <Tooltip>
      ...
      <TooltipPopup side="top" className="max-w-120 whitespace-normal leading-tight wrap-anywhere">
        {app
          ? `${webAppMentionLabel(app)} · ${app.name}`
          : nativeApp
            ? `${nativeAppMentionLabel(nativeApp)} · ${nativeApp.name}`
            : props.path}
      </TooltipPopup>
    </Tooltip>
  );
...
class ComposerMentionNode extends DecoratorNode<React.ReactElement> {
  ...
  override getTextContent(): string {
    return serializeComposerWebAppMention(this.__path) ?? serializeComposerFileLink(this.__path);
  }
...
export function ComposerPromptEditor({
  ...
  const initialConfig = useMemo<InitialConfigType>(
    () => ({
      namespace: "modesto-composer-editor",
      editable: true,
