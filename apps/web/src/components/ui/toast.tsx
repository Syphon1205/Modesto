...
import { type ScopedThreadRef, type ThreadId } from "@modesto/contracts";
import {
  CheckIcon,
  ChevronDownIcon,
  ChevronUpIcon,
...
/**
 * Update announcements - a new app build, a newer provider CLI, a model a
 * provider has just started offering.
 *
 * These are deliberately a separate stack from `toastManager`. Everything in
 * the main stack is a consequence of something the user just did and is read
 * immediately; an update is news that arrives on its own schedule, is worth
 * keeping on screen, and must not push a transient confirmation out of view.
 * Separating the managers is also what lets the two stacks sit in different
 * corners without fighting over the same viewport.
 */
const updatesToastManager = Toast.createToastManager<ThreadToastData>();
type ToastId = ReturnType<typeof toastManager.add>;
type ThreadToastManager = typeof toastManager;
const threadToastVisibleTimeoutRemainingMs = new Map<ToastId, number>();

// "loading" is handled separately (FlickerLoadingIcon needs a `grids` prop the
// other icon components don't take), so it's intentionally absent here.
const TOAST_ICONS = {
      ...
      <div className={cn("flex min-w-0 gap-2", !stackedActionLayout && "flex-1")}>
        {leadingIcon ? (
          ...
          <div
            ...
            data-slot="toast-icon"
          >
            <FlickerLoadingIcon grids={SEARCHING_GRIDS} variant="5x5" title="Loading" />
          </div>
        ) : Icon ? (
          <div
...
/**
 * Update announcements, pinned bottom-right.
 *
 * Bottom-right is where an OS puts news you have not asked for - it is out of
 * the way of the work, and it is where a user already looks for "something is
 * available". Keeping it distinct from the top-right stack means an update
 * that sits there for a week never competes with a copy confirmation.
 */
function UpdatesToastProvider({
  children,
  position = "bottom-right",
  ...props
}: ToastProviderProps) {
  return (
    <Toast.Provider toastManager={updatesToastManager} {...props}>
      {children}
      <Toasts manager={updatesToastManager} position={position} />
    </Toast.Provider>
  );
}

function Toasts({ position, manager }: { position: ToastPosition; manager: ThreadToastManager }) {
