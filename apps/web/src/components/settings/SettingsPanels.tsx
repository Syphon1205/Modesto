...
import { openWelcomeSetup } from "../welcome/welcomeSetupStore";
...
export function useSettingsRestore(onRestored?: () => void) {
  ...
  const changedSettingLabels = useMemo(
    () => [
      ...
      ...(settings.ambientPresenceEnabled !== DEFAULT_UNIFIED_SETTINGS.ambientPresenceEnabled
        ? ["Ambient presence"]
        : []),
      ...(settings.voice.speakReplies !== DEFAULT_UNIFIED_SETTINGS.voice.speakReplies
        ? ["Speak replies"]
        : []),
  ...
  const restoreDefaults = useCallback(async () => {
    ...
    updateSettings({
      ...
      voice: {
        speakReplies: DEFAULT_UNIFIED_SETTINGS.voice.speakReplies ?? false,
        ...(DEFAULT_UNIFIED_SETTINGS.voice.modelId === undefined
          ? {}
          : { modelId: DEFAULT_UNIFIED_SETTINGS.voice.modelId }),
        ...(DEFAULT_UNIFIED_SETTINGS.voice.binaryPath === undefined
          ? {}
          : { binaryPath: DEFAULT_UNIFIED_SETTINGS.voice.binaryPath }),
      },
    });
    onRestored?.();
  }, [
...
export function GeneralSettingsPanel() {
  ...
  // `ensureLocalApi().shell.openExternal` routes through the desktop bridge in
  // Electron and falls back to `window.open` in the browser, so the About
  // links work on both surfaces.
  const openAboutLink = useCallback((url: string) => {
    void ensureLocalApi()
  ...
  return (
    <SettingsPageContainer>
      ...
      <SettingsSection title="About">
        ...
        <SettingsRow
          title="Updates"
          description="Release notes for every published Modesto build."
          control={
            <Button
              size="xs"
              variant="outline"
              onClick={() => openAboutLink(DESKTOP_RELEASES_PAGE_URL)}
            >
              View updates
            </Button>
          }
        ...
        />
        <SettingsRow
          {...searchableSetting("welcome-setup")}
          description="Replay the first-run tour: what Modesto is, appearance, and which agents this machine can run."
          control={
            <Button size="xs" variant="outline" onClick={() => openWelcomeSetup()}>
              Run setup
            </Button>
          }
        />
