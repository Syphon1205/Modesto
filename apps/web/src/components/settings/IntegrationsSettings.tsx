/**
 * Integrations settings - preferences for surfaces Modesto embeds rather than
...
import {
  ...
  PREVIEW_ZOOM_LEVELS,
  type DesktopMicrophoneAccessStatus,
  type DesktopAppshotPermissionStatus,
  type PreviewAppearancePreference,
  type PreviewViewportSetting,
  type VoiceModelId,
} from "@modesto/contracts";
import { PREVIEW_VIEWPORT_PRESETS } from "@modesto/shared/previewViewport";
...
import { VOICE_MODEL_OPTIONS } from "~/voiceModels";
...
function VoiceModelSetting() {
  const settings = usePrimarySettings();
  const updateSettings = useUpdatePrimarySettings();
  const modelId = settings.voice.modelId;

  return (
    <SettingsRow
      {...searchableSetting("voice-model")}
      description="Optional advanced local speech-to-text (Whisper) used when a client transcribes through the server. Composer dictation already uses the system speech recognizer; pick an English model for smaller downloads, or a multilingual model to dictate in other languages."
      resetAction={
        modelId !== DEFAULT_VOICE_MODEL_ID ? (
          <SettingResetButton
            ...
            onClick={() =>
              updateSettings({
                voice: { modelId: DEFAULT_VOICE_MODEL_ID },
              })
            }
          />
        ) : null
      ...
      control={
        <Select
          value={modelId}
          onValueChange={(value) => {
            updateSettings({ voice: { modelId: value as VoiceModelId } });
          }}
        >
          <SelectTrigger className="w-full sm:w-56" aria-label="Local speech model">
            <SelectValue>
              {VOICE_MODEL_OPTIONS.find((option) => option.id === modelId)?.label ?? modelId}
            </SelectValue>
          </SelectTrigger>
          <SelectPopup align="end" alignItemWithTrigger={false}>
...
function SpeakRepliesSetting() {
  const settings = usePrimarySettings();
  const updateSettings = useUpdatePrimarySettings();
  const speakReplies = settings.voice.speakReplies;

  return (
    <SettingsRow
      {...searchableSetting("speak-replies")}
      description="Read assistant replies aloud with the best natural voice installed on this device. Independent of ambient presence; dictation on the composer mic is unchanged."
      resetAction={
        speakReplies !== DEFAULT_UNIFIED_SETTINGS.voice.speakReplies ? (
          <SettingResetButton
            label="speak replies"
            onClick={() =>
              updateSettings({
                voice: { speakReplies: DEFAULT_UNIFIED_SETTINGS.voice.speakReplies ?? false },
              })
            }
          />
        ) : null
      ...
      control={
        <Switch
          ...
          onCheckedChange={(checked) =>
            updateSettings({ voice: { speakReplies: Boolean(checked) } })
          }
...
function ThinkingSoundSetting() {
  const settings = usePrimarySettings();
  const updateSettings = useUpdatePrimarySettings();
  const enabled = settings.thinkingSoundEnabled;

  return (
    <SettingsRow
      {...searchableSetting("thinking-sound")}
      description="Play a short chime when the thread you're looking at starts thinking. No spoken words, no loop — a brief tone once per turn. Separate from speaking replies aloud."
      resetAction={
        enabled !== DEFAULT_UNIFIED_SETTINGS.thinkingSoundEnabled ? (
          <SettingResetButton
            label="thinking sound"
            onClick={() =>
              updateSettings({
                thinkingSoundEnabled: DEFAULT_UNIFIED_SETTINGS.thinkingSoundEnabled,
              })
            }
          />
        ) : null
      ...
      control={
        <Switch
          checked={enabled}
          onCheckedChange={(checked) => updateSettings({ thinkingSoundEnabled: Boolean(checked) })}
...
function AppshotsEnabledSetting() {
  const settings = usePrimarySettings();
  const updateSettings = useUpdatePrimarySettings();
  const enabled = settings.appshotsEnabled;

  return (
    <SettingsRow
      {...searchableSetting("appshots")}
      description="Press both ⌥ keys to capture the frontmost window into the active chat (screenshot plus available accessibility text)."
      resetAction={
        enabled !== DEFAULT_UNIFIED_SETTINGS.appshotsEnabled ? (
          <SettingResetButton
            label="appshots"
            onClick={() =>
              updateSettings({
                appshotsEnabled: DEFAULT_UNIFIED_SETTINGS.appshotsEnabled,
              })
            }
          />
        ) : null
      ...
      control={
        <Switch
          checked={enabled}
          onCheckedChange={(checked) => updateSettings({ appshotsEnabled: Boolean(checked) })}
          aria-label="Appshots"
        />
      }
    />
...
const APSHOT_PERMISSION_LABELS: Readonly<
  Record<DesktopAppshotPermissionStatus["screenRecording"], string>
> = {
  granted: "Granted",
  denied: "Denied",
...
function AppshotPermissionsSetting({ disabled }: { readonly disabled: boolean }) {
  const [status, setStatus] = useState<DesktopAppshotPermissionStatus | null>(null);
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    if (disabled) return;
    const bridge = window.desktopBridge?.appshots;
    if (!bridge?.getPermissionStatus) return;
    let cancelled = false;
    void bridge.getPermissionStatus().then((next) => {
  ...
  if (disabled) {
    return (
      <SettingsRow
        {...searchableSetting("appshot-permissions")}
        description="Screen Recording and Accessibility permissions for capturing other apps."
        control={<span className="text-xs text-muted-foreground">Desktop only</span>}
      />
    );
  }

  const screenLabel = status ? APSHOT_PERMISSION_LABELS[status.screenRecording] : "Checking…";
  const accessibilityLabel = status ? APSHOT_PERMISSION_LABELS[status.accessibility] : "Checking…";
  const needsAttention =
    status !== null && (status.screenRecording !== "granted" || status.accessibility !== "granted");

  return (
    <SettingsRow
      {...searchableSetting("appshot-permissions")}
      description={`Screen Recording: ${screenLabel}. Accessibility: ${accessibilityLabel}. macOS needs both for Appshots.`}
      control={
        <Button
          type="button"
          variant="outline"
          size="sm"
          disabled={busy || !needsAttention}
          onClick={() => {
            const open = window.desktopBridge?.appshots?.openPrivacySettings;
            if (!open) return;
            setBusy(true);
            void open().finally(() => setBusy(false));
          }}
        >
          {needsAttention ? "Open System Settings" : "Ready"}
        </Button>
      }
    />
  );
...
const MICROPHONE_STATUS_LABELS: Readonly<Record<DesktopMicrophoneAccessStatus, string>> = {
  granted: "Granted",
  denied: "Denied",
  restricted: "Restricted",
  "not-determined": "Not asked yet",
...
function MicrophoneAccessSetting({ disabled }: { readonly disabled: boolean }) {
  const [status, setStatus] = useState<DesktopMicrophoneAccessStatus | null>(null);
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    if (disabled) return;
    const bridge = window.desktopBridge;
    const getStatus = bridge?.getMicrophoneStatus;
    if (!getStatus) {
      setStatus("unknown");
      return;
    }
  ...
  const statusLabel = status === null ? "Checking…" : MICROPHONE_STATUS_LABELS[status];
  const canRequest = !disabled && status !== "granted" && status !== "restricted";

  return (
    <SettingsRow
      {...searchableSetting("microphone-access")}
      description="OS microphone access for composer dictation. If access was denied, grant it again in System Settings, then use Request access here."
      status={disabled ? undefined : statusLabel}
      control={
        <Button
          type="button"
          variant="outline"
          size="sm"
          disabled={disabled || busy || !canRequest || !window.desktopBridge?.requestMicrophone}
          onClick={() => {
            const request = window.desktopBridge?.requestMicrophone;
            if (!request) return;
            setBusy(true);
            void request()
        ...
        >
          {busy ? "Requesting…" : status === "granted" ? "Granted" : "Request access"}
        </Button>
      }
    />
  );
...
export function IntegrationsSettingsPanel() {
  ...
  const microphoneDisabled = !isElectron;
  const appshotsDisabled = !isElectron;
  ...
  const microphoneSetting = <MicrophoneAccessSetting disabled={microphoneDisabled} />;
  const appshotsSettings = (
    <>
      <AppshotsEnabledSetting />
      <AppshotPermissionsSetting disabled={appshotsDisabled} />
    </>
  );
  ...
  return (
    <SettingsPageContainer>
      ...
      <SettingsSection id="voice" title="Voice">
        <SpeakRepliesSetting />
        <ThinkingSoundSetting />
        {microphoneDisabled ? (
          <DesktopOnlyBrowserDefaults>{microphoneSetting}</DesktopOnlyBrowserDefaults>
        ) : (
          microphoneSetting
        )}
        <VoiceModelSetting />
      </SettingsSection>
      <SettingsSection id="appshots" title="Appshots">
        {appshotsDisabled ? (
          <DesktopOnlyBrowserDefaults>{appshotsSettings}</DesktopOnlyBrowserDefaults>
        ) : (
          appshotsSettings
        )}
      </SettingsSection>
