...
// catalog (~/.modesto/skills plus each provider's skills folder), shows which provider
...
import type { ProviderKind, ServerSettings } from "@modesto/contracts";
...
export function SkillsSettingsPanel() {
  ...
  const modestoSkillsDir = catalogQuery.data?.modestoSkillsDir;

  return (
    <div className="space-y-8">
      <SettingsSection title="Portable skills">
        <SettingsRow
          title="Modesto skills folder"
          description="Skills placed here are available on every provider. When a provider already ships its own copy of a skill, that copy is used; otherwise Modesto's copy is the fallback."
          status={
            modestoSkillsDir ? (
      ...
      {catalogQuery.isError ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Modesto could not scan the skill folders. Retry after checking that the server is running."
          />
        </SettingsSection>
      ) : null}
      ...
      {!catalogQuery.isLoading && !catalogQuery.isError && totalSkills === 0 ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Add a skill folder containing a SKILL.md to the Modesto skills folder above, or install skills for any supported provider."
          />
        </SettingsSection>
      ) : null}
