...
describe("assistant visual fences", () => {
  const text = "```html\n<button>Explore</button>\n```";
  it("opts assistant replies into inline previews while leaving other messages as code", () => {
    expect(
      renderToStaticMarkup(<ChatMarkdown cwd={undefined} text={text} renderVisuals />),
    ).toContain("<iframe");
    expect(renderToStaticMarkup(<ChatMarkdown cwd={undefined} text={text} />)).not.toContain(
      "<iframe",
    );
  });
  it("does not execute visual fences during streaming", () => {
    const markup = renderToStaticMarkup(
      <ChatMarkdown cwd={undefined} text={text} renderVisuals isStreaming />,
    );
