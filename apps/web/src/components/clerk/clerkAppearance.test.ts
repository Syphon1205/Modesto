...
import { clerkAppearance, clerkSignInPageAppearance } from "./clerkAppearance";
...
describe("clerkAppearance", () => {
  ...
  it("keeps the dedicated sign-in surface on Modesto's live semantic tokens", () => {
    expect(clerkSignInPageAppearance.variables).toMatchObject({
    ...
    expect(clerkSignInPageAppearance.elements.formFieldInput).toContain("border-input");
    expect(clerkSignInPageAppearance.elements.formButtonPrimary).toContain("bg-primary");
  });
