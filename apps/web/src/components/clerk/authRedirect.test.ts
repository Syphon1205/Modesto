  ...
  it("removes a Clerk virtual pathname and callback params while preserving the desktop route", () => {
    expect(
      resolveClerkSignInProps(
        "modesto://app/CLERK-ROUTER/VIRTUAL/sign-up?__clerk_status=complete#/settings/connections",
        true,
      ),
    ).toEqual({
      forceRedirectUrl: "modesto://app/#/settings/connections",
      signUpForceRedirectUrl: "modesto://app/#/settings/connections",
    });
  });

  it("preserves a clean development desktop route", () => {
    expect(resolveClerkSignInProps("modesto-dev://app/#/settings/general", true)).toEqual({
      forceRedirectUrl: "modesto-dev://app/#/settings/general",
      signUpForceRedirectUrl: "modesto-dev://app/#/settings/general",
    });
  });
});
