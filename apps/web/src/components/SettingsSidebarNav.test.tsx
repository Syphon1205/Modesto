...
describe("rankSettingsSearchEntries", () => {
  ...
  it("routes typography searches to the dedicated fonts section", () => {
    const results = rankSettingsSearchEntries("font", SETTINGS_SEARCH_ENTRIES.length);
    expect(results.some((entry) => entry.section === "fonts")).toBe(true);
    expect(results.some((entry) => entry.id === "fonts:dark-ui-font")).toBe(true);
  });
...
describe("SettingsSidebarNav", () => {
  ...
  it("renders a leading icon for each settings section", () => {
    const markup = renderToStaticMarkup(
      <SettingsSidebarNav activeSection="general" onBack={vi.fn()} onSelectSection={vi.fn()} />,
    );

    expect(markup).toContain("settings-gear-2");
    expect(markup).toContain("circle-person");
    expect(markup).toContain("cursor-click");
    expect(markup).toContain("lightning-bolt");
  });
});
