...
import { isRepoScopedConversation } from "./threadActionMenu.logic";
...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const memberThreadCountByPhysicalKey = useMemo(() => {
    ...
    for (const thread of projectThreads) {
      if (!isRepoScopedConversation(thread.conversationMode)) {
        continue;
      }
                ...
                void (async () => {
                  ...
                  const latestProjectThreads = Array.from(
                    ...
                    (thread) =>
                      ...
                      isRepoScopedConversation(thread.conversationMode),
                  );
                  const confirmed = await api.dialogs.confirm(
                    latestProjectThreads.length > 0
                      ? [
                          `Remove project "${member.title}" and delete its ${latestProjectThreads.length} code thread${
                          ...
                          ...(member.environmentLabel
                            ...
                            : []),
                          "This permanently clears conversation history for those code threads.",
                          "Chats stay.",
  ...
  const handleThreadContextMenu = useCallback(
    async (threadRef: ScopedThreadRef, position: { x: number; y: number }) => {
      ...
      const clicked = await api.contextMenu.show(
        [
          ...(thread.branch && isRepoScopedConversation(thread.conversationMode)
