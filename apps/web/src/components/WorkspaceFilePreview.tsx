...
} from "@modesto/shared/localPreviewFiles";
import {
  isLocalAbsolutePath,
  isWorkspaceRelativePathSafe,
  joinWorkspaceRelativePath,
} from "@modesto/shared/path";
import { isScratchWorkspacePath } from "@modesto/shared/threadWorkspace";
...
import { formatWorkspaceFileReadError } from "~/lib/workspaceFileErrors";
...
export interface WorkspaceFilePreviewProps {
  ...
  onCodeSelectionChange?: ((reference: ChatFileReference | null) => void) | undefined;
  reviewReveal?: { readonly line: number; readonly token: number } | null;
}
...
export function WorkspaceFilePreview(props: WorkspaceFilePreviewProps) {
  ...
  const handleSourceMouseUp = useCallback(
    (event: ReactMouseEvent<HTMLDivElement>) => {
      previewSelectionAction.onContainerMouseUp(event);
      if (!props.onCodeSelectionChange) return;
      requestAnimationFrame(() => {
        const container = contentsRef.current;
        const selection = container ? readPreviewSelection(container) : null;
        props.onCodeSelectionChange?.(
          filePath && selection ? { path: filePath, ...selection } : null,
        );
      });
    },
    [filePath, previewSelectionAction, props.onCodeSelectionChange, readPreviewSelection],
  );

  useEffect(() => {
    const reveal = props.reviewReveal;
    if (!reveal || showMarkdownPreview || reveal.line < 1) return;
    let highlightTimer: ReturnType<typeof setTimeout> | null = null;
    const locateLine = () => {
      const container = contentsRef.current;
      const line = container?.querySelectorAll<HTMLElement>(".line")[reveal.line - 1];
      if (!line) return false;
      line.scrollIntoView({ block: "center", behavior: "smooth" });
      line.classList.add("editor-file-viewer__review-line-highlight");
      highlightTimer = setTimeout(() => {
        line.classList.remove("editor-file-viewer__review-line-highlight");
      }, 1_600);
      return true;
    };
    const firstTimer = setTimeout(() => {
      if (!locateLine()) setTimeout(locateLine, 180);
    }, 0);
    return () => {
      clearTimeout(firstTimer);
      if (highlightTimer) clearTimeout(highlightTimer);
    };
  }, [fileContents, props.reviewReveal, showMarkdownPreview]);
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-1 flex-col bg-[var(--color-background-surface)]">
      ...
      {fileIsImage ? (
        ...
        <PanelStateMessage density="compact" fill="flex" className="items-start justify-start p-3">
          <p className="text-left text-[11px] text-destructive/85">
            {formatWorkspaceFileReadError(fileQuery.error, {
              filePath,
              workspaceRoot: props.workspaceRoot,
            })}
          </p>
