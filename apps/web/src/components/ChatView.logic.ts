...
export const LAST_INVOKED_SCRIPT_BY_PROJECT_KEY = "modesto:last-invoked-script-by-project";
...
/** Chat/Work is only choosable on the empty New Chat landing. */
...
export function isProjectSelectionRequired(input: {
  isLocalDraftThread: boolean;
  hasActiveProject: boolean;
  conversationMode: ConversationMode;
}): boolean {
  return input.isLocalDraftThread && !input.hasActiveProject && input.conversationMode !== "chat";
}

/** Chats can send without a live project. Work still needs one. */
export function canSendWithoutActiveProject(input: {
  isServerThread: boolean;
  conversationMode: ConversationMode;
}): boolean {
  return input.conversationMode === "chat";
}

/** Chat drafts may create a server thread against an orphaned or sentinel project id. */
export function resolveDraftThreadCreateProjectId(input: {
  conversationMode: ConversationMode;
  activeProjectId: ProjectId | null;
  draftProjectId: ProjectId | null;
}): ProjectId | null {
  if (input.activeProjectId) return input.activeProjectId;
  if (input.conversationMode === "chat") return input.draftProjectId;
  return null;
}
...
export function reconcileRetainedMountedThreadIds(input: {
  ...
  const hiddenThreadIds = input.currentThreadIds.filter(
    (threadId) =>
      ...
      (threadId !== input.activeThreadId && openThreadIdSet.has(threadId)),
  );
...
// A started thread only locks the picker to its current provider when that
// provider itself can't switch mid-conversation (`requiresNewThreadForModelChange`
// - today just Grok). Every other provider supports a mid-thread handoff: the
// server starts a fresh session for the new provider and replays the prior
// conversation as context, so the picker stays open instead of forcing a new
// thread. See getStartedThreadModelChangeBlockReason below, which the server
// mirrors exactly (ProviderCommandReactor.ts's ensureSessionForThread).
export function deriveLockedProvider(input: {
  ...
  const currentProviderRequiresNewThread =
    ...
    input.providers.find((snapshot) => snapshot.instanceId === sessionInstanceId)
      ?.requiresNewThreadForModelChange === true;
  if (!currentProviderRequiresNewThread) {
    return null;
  }
