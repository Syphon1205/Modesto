...
import { normalizeModelSlug } from "@modesto/shared/model";
import { buildModestoBranchName } from "@modesto/shared/git";
import { isGenericChatThreadTitle } from "@modesto/shared/chatThreads";
import { isGenericTerminalThreadTitle } from "@modesto/shared/terminalThreads";
import {
...
export const DISMISSED_PROVIDER_HEALTH_BANNERS_KEY = "modesto:dismissed-provider-health-banners";
...
export function resolveActiveTurnLiveDiffState(input: {
  ...
  workLogEntries?: ReadonlyArray<
    Pick<
      WorkLogEntry,
      "activityKind" | "changedFiles" | "itemType" | "requestKind" | "toolDetails" | "turnId"
    >
  >;
}): {
  ...
  const workLogFileStats = new Map<string, { additions: number; deletions: number }>();
  ...
  if (input.latestTurnId) {
    for (const entry of input.workLogEntries ?? []) {
      ...
      for (const filePath of entry.changedFiles ?? []) {
        ...
        const patchStats = fileDiffStatsByPath(entry.toolDetails?.diff);
        const stat = resolveFileDiffStatByChangedPath(
          patchStats,
          filePath,
          entry.changedFiles?.length ?? 0,
        );
        if (stat) workLogFileStats.set(filePath, stat);
      }
    }
  }

  if (hasFileEditWork && input.latestTurnId) {
    const liveFiles = Array.from(workLogFilePaths, (path) => ({
      path,
      additions: workLogFileStats.get(path)?.additions ?? 0,
      deletions: workLogFileStats.get(path)?.deletions ?? 0,
    }));
    return {
      turnId: null,
      fileCount: workLogFilePaths.size > 0 ? workLogFilePaths.size : null,
      additions: liveFiles.reduce((total, file) => total + file.additions, 0),
      deletions: liveFiles.reduce((total, file) => total + file.deletions, 0),
      hasChanges: true,
      files: liveFiles,
    };
  }
...
export function describeVoiceRecordingStartError(error: unknown): string {
  ...
  if (errorName === "NotAllowedError" || errorName === "PermissionDeniedError") {
    return "Microphone access was denied. Enable it in macOS Privacy & Security > Microphone for Modesto, then try again.";
  }
...
export function buildSuggestedWorktreeName(input: {
  ...
  return buildModestoBranchName(input.associatedWorktreeBranch ?? input.title);
}
