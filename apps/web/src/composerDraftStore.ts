import {
  ...
  ConversationMode as ConversationModeSchema,
...
import {
  type AppshotContextDraft,
  type AppshotContextSelection,
  newAppshotContextId,
  normalizeAppshotContextSelection,
} from "./lib/appshotContext";
import { create } from "zustand";
import { createJSONStorage, persist } from "zustand/middleware";
import { useShallow } from "zustand/react/shallow";
import { createDebouncedStorage, createMemoryStorage } from "./lib/storage";
import { getDefaultServerModel } from "./providerModels";
import { UnifiedSettings } from "@modesto/contracts/settings";
import { ReviewCommentContextSchema, type ReviewCommentContext } from "./reviewCommentContext";
const isRuntimeMode = Schema.is(RuntimeMode);
const isProviderDriverKind = Schema.is(ProviderDriverKind);
const isReviewCommentContext = Schema.is(ReviewCommentContextSchema);

export const COMPOSER_DRAFT_STORAGE_KEY = "modesto:composer-drafts:v1";
const COMPOSER_DRAFT_STORAGE_VERSION = 8;
const DraftThreadEnvModeSchema = Schema.Literals(["local", "worktree"]);
export type DraftThreadEnvMode = typeof DraftThreadEnvModeSchema.Type;
export type ConversationMode = import("@modesto/contracts").ConversationMode;
...
const PersistedAppshotContextDraft = Schema.Struct({
  id: Schema.String,
  threadId: ThreadId,
...
type PersistedAppshotContextDraft = typeof PersistedAppshotContextDraft.Type;
...
const PersistedComposerThreadDraftState = Schema.Struct({
  ...
  appshotContexts: Schema.optionalKey(Schema.Array(PersistedAppshotContextDraft)),
  ...
  conversationMode: Schema.optionalKey(ConversationModeSchema),
  conversationModeSelectable: Schema.optionalKey(Schema.Boolean),
});
...
export interface ComposerThreadDraftState {
  ...
  /** Chat keeps the conversation lightweight; Code enables project tooling. */
  conversationMode: ConversationMode;
  /**
   * New Chat landings can still pick Chat vs Work. Nested threads and
   * "New code task" drafts stay locked to the mode they were created with.
   */
...
export function composerDraftHasUserContent(
  ...
  return (
    ...
    draft.appshotContexts.length > 0 ||
...
interface ComposerDraftStoreState {
  ...
  addAppshotContext: (threadRef: ComposerThreadTarget, selection: AppshotContextSelection) => void;
  setAppshotContexts: (
    threadRef: ComposerThreadTarget,
    contexts: ReadonlyArray<AppshotContextDraft>,
  ) => void;
  removeAppshotContext: (threadRef: ComposerThreadTarget, contextId: string) => void;
  clearAppshotContexts: (threadRef: ComposerThreadTarget) => void;
...
const EMPTY_APPSHOT_CONTEXTS: AppshotContextDraft[] = [];
...
Object.freeze(EMPTY_APPSHOT_CONTEXTS);
...
const EMPTY_THREAD_DRAFT = Object.freeze<ComposerThreadDraftState>({
  ...
  appshotContexts: EMPTY_APPSHOT_CONTEXTS,
...
function shouldRemoveDraft(draft: ComposerThreadDraftState): boolean {
  return (
    ...
    draft.conversationMode === "code" &&
    !draft.conversationModeSelectable
  );
}
...
function normalizePersistedAppshotContextDraft(
  value: unknown,
): PersistedAppshotContextDraft | null {
  if (!value || typeof value !== "object") return null;
  const candidate = value as Record<string, unknown>;
  const id = candidate.id;
  const threadId = candidate.threadId;
  ...
  return {
    ...
    threadId: threadId as ThreadId,
    appName,
    windowTitle: typeof candidate.windowTitle === "string" ? candidate.windowTitle : "",
    accessibilityText:
      typeof candidate.accessibilityText === "string" ? candidate.accessibilityText : "",
    capturedAt,
  };
}
...
function normalizePersistedDraftsByThreadId(
  ...
  for (const [threadKeyOrId, draftValue] of Object.entries(
    ...
    const elementContexts = Array.isArray(draftCandidate.elementContexts)
      ...
      : [];
    const appshotContexts = Array.isArray(draftCandidate.appshotContexts)
      ? draftCandidate.appshotContexts.flatMap((entry) => {
          const normalized = normalizePersistedAppshotContextDraft(entry);
          return normalized ? [normalized] : [];
        })
      : [];
    ...
    nextDraftsByThreadKey[normalizedThreadKey] = {
      ...
      ...(appshotContexts.length > 0 ? { appshotContexts } : {}),
...
function partializeComposerDraftStoreState(
  ...
  for (const [threadKey, draft] of Object.entries(state.draftsByThreadKey)) {
    ...
    const persistedDraft: DeepMutable<PersistedComposerThreadDraftState> = {
      ...
      ...(draft.appshotContexts.length > 0
        ? {
            appshotContexts: draft.appshotContexts.map((context) => ({
              id: context.id,
              threadId: context.threadId,
      ...
      ...(draft.conversationMode === "chat" ? { conversationMode: "chat" as const } : {}),
      ...(draft.conversationModeSelectable ? { conversationModeSelectable: true as const } : {}),
    };
...
export function hydrateImagesFromPersisted(
  ...
  return attachments.flatMap((attachment) => {
    ...
    const type =
      attachment.type === "file" || !isProviderSendTurnSupportedImageMimeType(attachment.mimeType)
        ? ("file" as const)
        : ("image" as const);
    return [
      {
...
function toHydratedThreadDraft(
  ...
  return {
    ...
    appshotContexts:
      persistedDraft.appshotContexts?.map((context) => ({
        ...context,
      })) ?? [],
    previewAnnotations:
    ...
    conversationMode: persistedDraft.conversationMode ?? "code",
    conversationModeSelectable: persistedDraft.conversationModeSelectable === true,
  };
}
  ...
  persist(
    (setBase, get) => {
      ...
      return {
        ...
        setConversationMode: (threadRef, conversationMode, options) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) {
            return;
          }
          set((state) => {
            const existing = state.draftsByThreadKey[threadKey];
            const base = existing ?? createEmptyThreadDraft();
            const conversationModeSelectable =
              options?.selectable ?? base.conversationModeSelectable;
            if (
              base.conversationMode === conversationMode &&
              base.conversationModeSelectable === conversationModeSelectable
            ) {
              return state;
            }
            const nextDraft: ComposerThreadDraftState = {
        ...
        addAppshotContext: (threadRef, selection) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef);
          const threadId = resolveComposerThreadId(get(), threadRef);
          if (!threadKey || !threadId) return;
          const normalized = normalizeAppshotContextSelection(selection);
          if (!normalized) return;
          set((state) => {
            const existing = state.draftsByThreadKey[threadKey] ?? createEmptyThreadDraft();
            const draft: AppshotContextDraft = {
              ...normalized,
              id: newAppshotContextId(),
              threadId,
            };
            return {
              draftsByThreadKey: {
                ...
                [threadKey]: {
                  ...existing,
                  appshotContexts: [...existing.appshotContexts, draft],
                },
              },
            };
          });
        },
        setAppshotContexts: (threadRef, contexts) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef);
          if (!threadKey) return;
          set((state) => {
            const existing = state.draftsByThreadKey[threadKey] ?? createEmptyThreadDraft();
        ...
        removeAppshotContext: (threadRef, contextId) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0 || contextId.length === 0) return;
          set((state) => {
            const current = state.draftsByThreadKey[threadKey];
            if (!current) return state;
            const filtered = current.appshotContexts.filter((entry) => entry.id !== contextId);
            if (filtered.length === current.appshotContexts.length) return state;
            const nextDraft: ComposerThreadDraftState = {
              ...current,
              appshotContexts: filtered,
            };
            const nextDraftsByThreadKey = { ...state.draftsByThreadKey };
            if (shouldRemoveDraft(nextDraft)) {
              delete nextDraftsByThreadKey[threadKey];
        ...
        clearAppshotContexts: (threadRef) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) return;
          set((state) => {
            const current = state.draftsByThreadKey[threadKey];
            if (!current || current.appshotContexts.length === 0) return state;
            const nextDraft: ComposerThreadDraftState = {
              ...current,
              appshotContexts: [],
            };
