...
// Purpose: Imports Modesto browser state before renderer stores hydrate after a desktop origin move.
...
export function importModestoStorageSnapshot(
  snapshot: ModestoStorageSnapshot | null,
...
export function bootstrapModestoStorageOriginMigration(): void {
  const bridge = globalThis.window?.desktopBridge?.storageMigration;
  if (!bridge) return;

  try {
    const snapshot = bridge.readSnapshot();
    if (snapshot && importModestoStorageSnapshot(snapshot)) {
...
bootstrapModestoStorageOriginMigration();
