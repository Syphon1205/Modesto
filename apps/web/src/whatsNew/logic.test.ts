...
describe("resolveWhatsNewState", () => {
  ...
  it("shows the 0.1.1 entry when upgrading from 0.1.0", () => {
    const milpitas = entry("0.1.1", {
      features: [
        {
          id: "milpitas",
          title: "Milpitas",
        ...
        },
        {
          ...
          description: "Import provider conversations",
        },
      ],
    });
    const state = resolveWhatsNewState({
      entries: [milpitas, entry("0.1.0")],
    ...
    expect(state.currentEntry.version).toBe("0.1.1");
    expect(state.currentEntry.features.map((feature) => feature.id)).toEqual([
      "milpitas",
      "connected-conversations",
    ]);
    expect(state.nextLastSeenVersion).toBe("0.1.1");
  });

  it("shows the WSL entry for the 0.1.2 upgrade", () => {
    const wslRelease = entry("0.1.2", {
      features: [
        {
          id: "wsl2-support",
          title: "Built for WSL2",
          description: "Develop and test from a Linux workspace on Windows.",
        },
      ],
    });
    const state = resolveWhatsNewState({
      entries: [wslRelease, entry("0.1.1")],
    ...
    expect(state.currentEntry.features.map((feature) => feature.id)).toEqual(["wsl2-support"]);
    expect(state.nextLastSeenVersion).toBe("0.1.2");
  });
});
