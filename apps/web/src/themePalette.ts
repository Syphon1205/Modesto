...
import {
  ...
  type ThemeVariants,
} from "@modesto/shared/themePalettes";

export { EMBER_THEME, GROVE_THEME, IRIS_THEME, OCEAN_THEME, MODESTO_THEME, THEME_COLOR_ROLES };
export type { ThemeAppearance, ThemeColorRole, ThemeColors, ThemeDefinition, ThemeVariants };

export const MODESTO_THEME_ID = "modesto" as const;
export const MODESTO_THEME_LABEL = "Signal";
...
export const CUSTOM_THEMES_STORAGE_KEY = "modesto:themes:v1";
export const THEME_FOLLOW_SYSTEM_STORAGE_KEY = "modesto:theme-follow-system";
export const THEME_APPEARANCE_MODE_STORAGE_KEY = "modesto:theme-appearance-mode";
export const THEME_HALVES_STORAGE_KEY = "modesto:theme-halves:v1";
...
// Earlier builds shipped these palettes under a t3- prefix, and shipped a
// T3-branded default that Modesto's own palette has replaced. Stored
// preferences keep working through this table rather than silently resetting
// someone's theme on upgrade.
const LEGACY_THEME_ID_ALIASES: Readonly<Record<string, string>> = {
  [LEGACY_T3_CHAT_DARK_THEME_ID]: MODESTO_THEME_ID,
  // The retired T3-branded default. Anyone sitting on it lands on Modesto's
  // own palette instead of having their theme silently reset to nothing.
  "t3-chat": MODESTO_THEME_ID,
...
/**
 * The palette Modesto wears with no theme installed, captured from the app's
...
/**
 * The standard Modesto look as a theme palette, for seeding a new theme when
 * no theme is installed. Distinct from {@link getDefaultThemeColors}, which
 * carries the Signal (Astro brand) palette used to fill roles omitted by theme
 * files.
 */
...
/**
 * The status colors Modesto shows without a theme, read from the app's own
 ...
 *
 * Resolved through the shared helper rather than reaching for a named variant:
 * the flagship is dark-first, so assuming `variants.dark` exists - as this did
 * while the flagship was light-first - crashes the moment the orientation
 * changes. The helper returns `colors` when the appearance already matches.
 */
export function getDefaultThemeColors(appearance: ThemeAppearance): ThemeColors {
  const colors = getThemeColorsForAppearance(MODESTO_THEME, appearance);
  if (!colors) {
    throw new Error(`The Modesto palette is missing its ${appearance} colors.`);
  }
  return colors;
}
