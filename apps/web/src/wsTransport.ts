// FILE: wsTransport.ts
// Purpose: Browser-side Effect RPC transport over the Modesto WebSocket endpoint.
// Layer: Web transport
  ...
  private startChannelStream(channel: WsPushChannel): void {
    void this.getClient()
      .then((client) => {
        ...
        } else if (channel === WS_CHANNELS.openClawEvent) {
          this.startStream(
            "openclaw.events",
            client[WS_METHODS.subscribeOpenClawEvents]({}),
            (event: OpenClawStreamEvent) => this.emit(WS_CHANNELS.openClawEvent, event),
            restartChannel,
          );
        } else if (channel === WS_CHANNELS.reviewEvent) {
          this.startStream(
            "review.events",
            client[WS_METHODS.subscribeReviewEvents]({}),
            (event: ReviewStreamEvent) => this.emit(WS_CHANNELS.reviewEvent, event),
            restartChannel,
          );
