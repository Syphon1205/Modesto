...
export const SETTINGS_PAGE_BACKGROUND_CLASS_NAME =
  "app-settings-surface bg-[radial-gradient(circle_at_50%_-12%,color-mix(in_srgb,var(--color-foreground)_5%,transparent),transparent_34rem)]";

/** Section label above a settings card — same tone as sidebar "Threads"/"Pinned". */
export const SETTINGS_SECTION_LABEL_CLASS_NAME = `px-1.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] ${SIDEBAR_SECTION_LABEL_CLASS_NAME}`;

/** Vertical rhythm between stacked settings groups in the content panel. */
export const SETTINGS_PANEL_SECTION_CLASS_NAME = "flex flex-col gap-2 not-first:mt-6";
...
export const SETTINGS_CARD_CLASS_NAME = [
  "overflow-hidden bg-card/30 shadow-[0_1px_0_color-mix(in_srgb,var(--color-foreground)_3%,transparent)] backdrop-blur-sm",
