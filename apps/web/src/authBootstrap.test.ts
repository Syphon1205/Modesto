...
/** A loopback server that offers same-machine bootstrap: no token to type. */
const LOOPBACK_TRUSTED_AUTH = {
  policy: "loopback-browser",
  bootstrapMethods: ["loopback-bootstrap", "one-time-token"],
  sessionMethods: ["browser-session-cookie"],
  sessionCookieName: "t3_session",
} as const;
...
function installTestBrowser(url: string) {
  ...
  vi.stubGlobal("document", { title: "Modesto" });
...
async function installAuthApi(input: {
  ...
  readonly loopbackSession?: () => Effect.Effect<
  ...
  const testApi = await installEnvironmentHttpTest({
    ...
    ...(input.browserSession
      ...
      : {}),
    ...(input.loopbackSession ? { loopbackSession: input.loopbackSession } : {}),
...
describe("resolveInitialServerAuthGateState", () => {
  ...
  it("opens a loopback environment without asking for a pairing token", async () => {
    const testApi = await installAuthApi({
      session: sequence(
        unauthenticatedSession(LOOPBACK_TRUSTED_AUTH),
        authenticatedSession(LOOPBACK_TRUSTED_AUTH),
      ),
      loopbackSession: () => Effect.succeed(browserSession(["orchestration:read", "access:write"])),
    });
    ...
    expect(testApi.calls.loopbackSession).toBe(1);
    expect(testApi.calls.browserSession).toEqual([]);
  });

  it("falls back to the pairing surface when the server refuses the loopback claim", async () => {
    const testApi = await installAuthApi({
      session: () => unauthenticatedSession(LOOPBACK_TRUSTED_AUTH),
      loopbackSession: () =>
        Effect.fail(
  ...
  it("does not attempt a loopback claim when the server does not offer one", async () => {
    const testApi = await installAuthApi({
    ...
    expect(testApi.calls.loopbackSession).toBe(0);
  });

  it("shows an unreachable gate when the server cannot be reached", async () => {
