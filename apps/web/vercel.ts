...
const HOSTED_WEB_CHANNEL_COOKIE = "modesto_web_channel";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @modesto/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --ignore-scripts --filter '@modesto/scripts...' --filter '@modesto/web...'",
  routes: [
    {
      src: "/__modesto/channel",
