  ...
  describe("createDevRunnerEnv", () => {
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        // Browser dev modes (dev/dev:server/dev:web) never set VITE_WS_URL -
        // it pins the client to a direct off-proxy connection, breaking
        // single-origin dev and remote/tailnet browsers. Only dev:desktop
        // sets it, since Electron's renderer has no dev proxy to go through.
    ...
    it.effect("uses an IPv4-only loopback path when running inside WSL", () =>
      Effect.gen(function* () {
        // VITE_WS_URL is only ever set for dev:desktop (see above) - use
        // that mode here so this test still exercises the loopback-host
        // value it's actually named for.
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
