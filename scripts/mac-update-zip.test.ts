...
`,
  ...
  it("builds Electron framework symlink paths for the top-level app bundle", () => {
    assert.deepStrictEqual(buildMacUpdateZipSymlinkEntries("Modesto.app"), [
      "Modesto.app/Contents/Frameworks/Electron Framework.framework/Electron Framework",
      "Modesto.app/Contents/Frameworks/Electron Framework.framework/Helpers",
      "Modesto.app/Contents/Frameworks/Electron Framework.framework/Libraries",
      "Modesto.app/Contents/Frameworks/Electron Framework.framework/Resources",
      "Modesto.app/Contents/Frameworks/Electron Framework.framework/Versions/Current",
    ]);
  });
  ...
  it("resolves exactly one top-level .app from update zip entries", () => {
    assert.equal(
      resolveSingleTopLevelMacAppBundle([
        "__MACOSX/Modesto.app/Contents/Info.plist",
        "Modesto.app/Contents/Info.plist",
        "Modesto.app/Contents/MacOS/Modesto",
      ]),
      "Modesto.app",
    );
  ...
  it("resolves exactly one macOS update zip artifact", () => {
    assert.equal(
      resolveSingleMacUpdateZipFileName([
        "Modesto-0.1.5-arm64.dmg",
        "Modesto-0.1.5-arm64.zip",
        "latest-mac.yml",
      ]),
      "Modesto-0.1.5-arm64.zip",
    );
    ...
    assert.throws(
      () => resolveSingleMacUpdateZipFileName(["Modesto-0.1.5-arm64.zip", "Modesto-0.1.5-x64.zip"]),
  ...
  it("requires at least one macOS update manifest", () => {
    ...
    assert.throws(
      () => resolveMacUpdateManifestFileNames(["Modesto-0.1.5-arm64.dmg"]),
...
files:
  - url: Modesto-0.1.4-arm64.zip
    sha512: oldzip
    size: 100
  - url: Modesto-0.1.4-arm64.dmg
    sha512: olddmg
    size: 200
path: 'Modesto-0.1.4-arm64.zip'
sha512: oldzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;

    const updated = updateMacUpdateManifestZipEntry(manifest, "Modesto-0.1.4-arm64.zip", {
      sha512: "newzip",
      size: 12345,
    });
...
releaseDate: '2026-06-07T12:00:00.000Z'
`,
          "Modesto-0.1.4-arm64.zip",
          {
            sha512: "newzip",
            size: 12345,
          },
        ),
      /Could not update Modesto-0.1.4-arm64.zip entry/,
    );
  });
...
files:
  - url: Modesto-0.1.5-arm64.zip
    sha512: newzip
    size: 12345
path: Modesto-0.1.5-arm64.zip
sha512: newzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;
    ...
    assert.deepStrictEqual(
      validateMacUpdateManifestZipMetadata(manifest, "Modesto-0.1.5-arm64.zip", metadata),
      {
    ...
    assert.deepStrictEqual(
      assertMacUpdateManifestZipMetadata(manifest, "Modesto-0.1.5-arm64.zip", metadata),
      {
        manifestHasZipPath: true,
        manifestHasZipSha: true,
        manifestHasZipSize: true,
