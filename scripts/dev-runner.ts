...
import { layer as NetServiceLayer, NetService } from "@modesto/shared/Net";
...
export const DEFAULT_MODESTO_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(homedir(), ".modesto"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@modesto/contracts",
    "--filter=@modesto/web",
    "--filter=@modesto/cli",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=@modesto/cli"],
  "dev:web": ["run", "dev", "--filter=@modesto/web"],
  "dev:desktop": ["run", "dev", "--filter=@modesto/desktop", "--filter=@modesto/web", "--parallel"],
...
export function isWslEnvironment(
  ...
  kernelRelease: string = osRelease(),
): boolean {
  if (platform !== "linux") {
    return false;
  }
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("MODESTO_PORT_OFFSET"),
  devInstance: optionalStringConfig("MODESTO_DEV_INSTANCE"),
});
const HomeConfig = optionalStringConfig("MODESTO_HOME");
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid MODESTO_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `MODESTO_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric MODESTO_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed MODESTO_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_MODESTO_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly modestoHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(modestoHome);
    const isWsl = isWslEnvironment(platform, baseEnv, kernelRelease);
    // The backend (apps/server/src/bin.ts) binds explicitly to 127.0.0.1,
    // not a dual-stack/IPv6 address - always point the desktop renderer's
    // WS URL at the literal bind address instead of "[::1]", which connects
    // to nothing on hosts where the IPv6 loopback has no listener.
    const loopbackHost = "127.0.0.1";

    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      MODESTO_PORT: String(serverPort),
      PORT: String(webPort),
      ELECTRON_RENDERER_PORT: String(webPort),
      // Only the desktop renderer wants a direct, baked-in backend URL -
      // browser dev is single-origin and Vite proxies /api, /ws, and
      // /.well-known, so setting this unconditionally pins the browser
      // client to a direct off-proxy connection it (and any remote browser,
      // e.g. over a tailnet) generally cannot reach, breaking every RPC call
      // silently. See the mode === "dev" branch below, which deletes the
      // desktop-only WS URL for browser modes the same way.
      ...(mode === "dev:desktop" ? { VITE_WS_URL: `ws://${loopbackHost}:${serverPort}` } : {}),
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://localhost:${webPort}`,
      MODESTO_HOME: resolvedBaseDir,
    };
    ...
    if (host !== undefined) {
      output.MODESTO_HOST = host;
    } else if (isWsl) {
      // WSL's Windows-side localhost forwarding is most predictable over IPv4.
      // Keep this loopback-only: exposing the dev server to the LAN still
      // requires the contributor to explicitly pass --host.
      output.MODESTO_HOST = "127.0.0.1";
    }

    if (authToken !== undefined) {
      output.MODESTO_AUTH_TOKEN = authToken;
    } else {
      delete output.MODESTO_AUTH_TOKEN;
    }

    if (noBrowser !== undefined) {
      output.MODESTO_NO_BROWSER = noBrowser ? "1" : "0";
    } else {
      delete output.MODESTO_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.MODESTO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.MODESTO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.MODESTO_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.MODESTO_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.MODESTO_MODE = "web";
      delete output.MODESTO_DESKTOP_WS_URL;
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read MODESTO_PORT_OFFSET/MODESTO_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("MODESTO_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv(
        "MODESTO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
      ),
      logWebSocketEvents: readOptionalBooleanEnv("MODESTO_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.MODESTO_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.MODESTO_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  modestoHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all Modesto data (MODESTO_HOME)."),
    Flag.withFallbackConfig(HomeConfig),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to MODESTO_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("MODESTO_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to MODESTO_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("MODESTO_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to MODESTO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("MODESTO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to MODESTO_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("MODESTO_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to MODESTO_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("MODESTO_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to MODESTO_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("MODESTO_PORT")),
  ),
