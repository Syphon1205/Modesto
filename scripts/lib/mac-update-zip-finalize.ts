...
function verifyMacAppSignature(appBundlePath: string, requireSignature: boolean): void {
  ...
  if (requireSignature) {
    // A valid signature alone is insufficient for downloaded apps. Verify the
    // notarization ticket and the same Gatekeeper policy users encounter after
    // downloading Modesto before allowing a release artifact to proceed.
    runTextCommand("xcrun", ["stapler", "validate", appBundlePath]);
    runTextCommand("spctl", ["--assess", "--type", "execute", "--verbose=4", appBundlePath]);
  }
}
...
export async function finalizeMacUpdateZip(
  ...
  const extractedZipRoot = mkdtempSync(join(tmpdir(), "modesto-mac-update-zip-"));
  try {
