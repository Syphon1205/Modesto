...
// dedicated `modesto` channel aliases.
...
export function resolveReleaseUpdatePolicy(
  ...
  return {
    ...
    channel: requested.isPrerelease
      ? normalizedConfig.developmentChannel
...
export function prepareReleaseUpdateManifests(
  ...
  const requiredSources =
    normalizedConfig.lane === "bridge"
      ? sourceNames
      : (["latest-mac.yml", "latest.yml"] as const);
  const missingRequired = requiredSources.filter((name) => !presentSources.includes(name));
  if (missingRequired.length > 0) {
    const prefix =
      normalizedConfig.lane === "bridge" ? "Compatibility release" : "Latest release";
