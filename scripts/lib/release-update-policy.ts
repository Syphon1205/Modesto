// FILE: release-update-policy.ts
// Purpose: Publishes the current public line through GitHub's Latest updater feed
// (plus dedicated `modesto` channel aliases) so older GitHub installs hop forward.
// Hidden development identifiers (nightly/dev) stay off Latest.

import { constants, copyFileSync, existsSync, readFileSync } from "node:fs";
import { resolve } from "node:path";

export type ReleaseLane = "bridge" | "clean";

export interface ReleaseUpdatePolicyConfig {
  readonly lane: ReleaseLane;
  readonly bridgeVersion: string;
  readonly channel: string;
  readonly developmentChannel: string;
  readonly legacyChannels?: readonly string[];
}

export interface ResolvedReleaseUpdatePolicy {
  readonly version: string;
  readonly tag: string;
  readonly isPrerelease: boolean;
  readonly makeLatest: boolean;
  readonly mirrorToStableChannel: boolean;
  readonly lane: ReleaseLane;
  readonly bridgeTag: string;
  readonly channel: string;
}

const VERSION_PATTERN =
  /^(\d+)\.(\d+)\.(\d+)(?:\.([1-9]\d*)|-([0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*))?$/;
const CHANNEL_PATTERN = /^[a-z0-9-]+$/;
// Packaged 0.3/0.4 apps check GitHub Latest with allowPrerelease=false, so a
// GitHub prerelease flag would hide this line from them. Public staged
// identifiers stay on Latest; only hidden development channels do not.
const PUBLIC_STAGED_PRERELEASE_HEADS = new Set(["alpha", "beta", "rc"]);

function parseVersion(value: string): {
  core: readonly number[];
  isPrerelease: boolean;
  prereleaseHead: string | null;
} {
  const match = VERSION_PATTERN.exec(value);
  if (!match) throw new Error(`Invalid release version: ${value}`);
  const prerelease = match[5];
  return {
    core: [Number(match[1]), Number(match[2]), Number(match[3]), Number(match[4] ?? 0)],
    isPrerelease: prerelease !== undefined,
    prereleaseHead:
      prerelease === undefined ? null : (prerelease.split(".")[0]?.toLowerCase() ?? null),
  };
}

function isHiddenDevelopmentPrerelease(parsed: ReturnType<typeof parseVersion>): boolean {
  if (!parsed.isPrerelease || parsed.prereleaseHead === null) return false;
  return !PUBLIC_STAGED_PRERELEASE_HEADS.has(parsed.prereleaseHead);
}

/** Nightly/dev identifiers only. Public alphas stay on the production bundle and Latest feed. */
export function isHiddenDevelopmentReleaseVersion(rawVersion: string): boolean {
  const version = rawVersion.startsWith("v") ? rawVersion.slice(1) : rawVersion;
  return isHiddenDevelopmentPrerelease(parseVersion(version));
}

function compareCoreVersions(left: readonly number[], right: readonly number[]): number {
  for (let index = 0; index < 4; index += 1) {
    const difference = (left[index] ?? 0) - (right[index] ?? 0);
    if (difference !== 0) return difference;
  }
  return 0;
}

export function validateReleaseUpdatePolicyConfig(config: unknown): ReleaseUpdatePolicyConfig {
  if (typeof config !== "object" || config === null) {
    throw new Error("Release update policy must be an object.");
  }
  const candidate = config as Partial<ReleaseUpdatePolicyConfig>;
  if (candidate.lane !== "bridge" && candidate.lane !== "clean") {
    throw new Error(`Invalid release lane: ${String(candidate.lane)}`);
  }
  if (typeof candidate.bridgeVersion !== "string") {
    throw new Error("Compatibility release version must be a string.");
  }
  parseVersion(candidate.bridgeVersion);
  if (
    typeof candidate.channel !== "string" ||
    !CHANNEL_PATTERN.test(candidate.channel) ||
    candidate.channel === "latest"
  ) {
    throw new Error(`Invalid dedicated update channel: ${String(candidate.channel)}`);
  }
  if (
    typeof candidate.developmentChannel !== "string" ||
    !CHANNEL_PATTERN.test(candidate.developmentChannel) ||
    candidate.developmentChannel === "latest" ||
    candidate.developmentChannel === candidate.channel
  ) {
    throw new Error(`Invalid development update channel: ${String(candidate.developmentChannel)}`);
  }
  if (
    candidate.legacyChannels !== undefined &&
    (!Array.isArray(candidate.legacyChannels) ||
      candidate.legacyChannels.some(
        (channel) =>
          typeof channel !== "string" ||
          !CHANNEL_PATTERN.test(channel) ||
          channel === "latest" ||
          channel === candidate.channel,
      ))
  ) {
    throw new Error("Invalid legacy update channels.");
  }
  return candidate as ReleaseUpdatePolicyConfig;
}

export function readReleaseUpdatePolicyConfig(rootDirectory: string): ReleaseUpdatePolicyConfig {
  const path = resolve(rootDirectory, "scripts/release-update-policy.json");
  return validateReleaseUpdatePolicyConfig(JSON.parse(readFileSync(path, "utf8")) as unknown);
}

export function resolveReleaseUpdatePolicy(
  rawVersion: string,
  config: ReleaseUpdatePolicyConfig,
): ResolvedReleaseUpdatePolicy {
  const normalizedConfig = validateReleaseUpdatePolicyConfig(config);
  const version = rawVersion.startsWith("v") ? rawVersion.slice(1) : rawVersion;
  const requested = parseVersion(version);
  const bridge = parseVersion(normalizedConfig.bridgeVersion);

  if (normalizedConfig.lane === "bridge" && version !== normalizedConfig.bridgeVersion) {
    throw new Error(
      `The compatibility lane may publish only v${normalizedConfig.bridgeVersion}, not v${version}.`,
    );
  }
  if (normalizedConfig.lane === "clean" && compareCoreVersions(requested.core, bridge.core) <= 0) {
    throw new Error(
      `Modesto releases must be newer than the compatibility release v${normalizedConfig.bridgeVersion}.`,
    );
  }

  const githubPrerelease = isHiddenDevelopmentPrerelease(requested);
  return {
    version,
    tag: `v${version}`,
    isPrerelease: githubPrerelease,
    makeLatest: normalizedConfig.lane === "clean" && !githubPrerelease,
    mirrorToStableChannel: false,
    lane: normalizedConfig.lane,
    bridgeTag: `v${normalizedConfig.bridgeVersion}`,
    channel: githubPrerelease ? normalizedConfig.developmentChannel : normalizedConfig.channel,
  };
}

export function channelManifestNames(channel: string): readonly string[] {
  if (!CHANNEL_PATTERN.test(channel) || channel === "latest") {
    throw new Error(`Invalid dedicated update channel: ${channel}`);
  }
  return [`${channel}-mac.yml`, `${channel}.yml`, `${channel}-linux.yml`];
}

function copyPresentChannelManifests(
  assetDirectory: string,
  sourceNames: readonly string[],
  destinationNames: readonly string[],
): readonly string[] {
  const created: string[] = [];
  for (const [index, sourceName] of sourceNames.entries()) {
    const sourcePath = resolve(assetDirectory, sourceName);
    if (!existsSync(sourcePath)) continue;
    const destinationName = destinationNames[index];
    if (!destinationName) throw new Error(`Missing channel manifest mapping for ${sourceName}.`);
    const destinationPath = resolve(assetDirectory, destinationName);
    if (existsSync(destinationPath)) {
      throw new Error(`Refusing to overwrite existing update manifest: ${destinationName}`);
    }
    copyFileSync(sourcePath, destinationPath, constants.COPYFILE_EXCL);
    created.push(destinationName);
  }
  return created;
}

export function prepareReleaseUpdateManifests(
  assetDirectory: string,
  config: ReleaseUpdatePolicyConfig,
  channel?: string,
): readonly string[] {
  const normalizedConfig = validateReleaseUpdatePolicyConfig(config);
  const targetChannel = channel ?? normalizedConfig.channel;
  const sourceNames = ["latest-mac.yml", "latest.yml", "latest-linux.yml"] as const;
  const destinationNames = channelManifestNames(targetChannel);
  const presentSources = sourceNames.filter((name) => existsSync(resolve(assetDirectory, name)));
  // Desktop installs request the dedicated `modesto` channel. Require the Mac and
  // Windows feeds we actually ship; Linux stays optional until AppImage ships.
  const requiredSources =
    normalizedConfig.lane === "bridge" ? sourceNames : (["latest-mac.yml", "latest.yml"] as const);
  const missingRequired = requiredSources.filter((name) => !presentSources.includes(name));
  if (missingRequired.length > 0) {
    const prefix = normalizedConfig.lane === "bridge" ? "Compatibility release" : "Latest release";
    throw new Error(`${prefix} is missing update manifests: ${missingRequired.join(", ")}`);
  }

  const createdDestinations = [
    ...copyPresentChannelManifests(assetDirectory, sourceNames, destinationNames),
  ];
  for (const legacyChannel of normalizedConfig.legacyChannels ?? []) {
    createdDestinations.push(
      ...copyPresentChannelManifests(
        assetDirectory,
        sourceNames,
        channelManifestNames(legacyChannel),
      ),
    );
  }
  return [...presentSources, ...createdDestinations];
}
