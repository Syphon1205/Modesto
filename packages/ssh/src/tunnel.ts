...
} from "@modesto/shared/httpReadiness";
import * as NetService from "@modesto/shared/Net";
import { extractJsonObject, fromLenientJson } from "@modesto/shared/schemaJson";
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env MODESTO_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
...
>()("@modesto/ssh/tunnel/SshEnvironmentManager") {
