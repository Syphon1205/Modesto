import type { AutomationRevision } from "./types.ts";
...
export function automationOccurrenceIdentity(input: AutomationOccurrenceIdentityInput): {
  ...
  if (input.scheduledFor === null && !input.nonce)
    throw new Error("A manual occurrence needs a nonce");
  const occurrence =
    input.scheduledFor === null ? `manual:${input.nonce}` : String(input.scheduledFor);
  const stable = [input.automationId, occurrence].map(encodeURIComponent).join(":");
  return {
