import type { AutomationRepository } from "./ports.ts";

export * from "./engine-testing.ts";
...
export async function verifyAutomationRepositoryConformance(
  ...
  if (fetched?.revision.id !== created.revision.id)
    throw new Error("created revision was not durable");
  ...
  if (updated.revision.version !== created.revision.version + 1)
    throw new Error("revision version did not increase");
