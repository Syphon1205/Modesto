...
export const automationEngineAttachmentValueSchema: z.ZodType<AutomationEngineAttachmentValue> =
  z.lazy(() =>
    z.union([
      z.string(),
      z.number().finite(),
...
export const automationEngineAdmissionRequestSchema = z
  .object({
  ...
  .superRefine((request, context) => {
    ...
    if (
      ...
      context.addIssue({
        code: "custom",
        message: "Run does not belong to the Automation revision",
      });
    }
  });
...
export type AutomationEngineExecutionState = z.infer<typeof automationEngineExecutionStateSchema>;
...
export const automationEngineResultSchema = z
  .object({
    executionId: engineIdSchema,
    runId: engineIdSchema,
  ...
  .superRefine((result, context) => {
    if (result.status === "succeeded" && result.error !== null) {
      context.addIssue({
        code: "custom",
        message: "A succeeded engine result cannot contain an error",
      });
    }
    if (result.status !== "succeeded" && result.error === null) {
      context.addIssue({
        code: "custom",
        message: "A failed or cancelled engine result needs an error",
      });
    }
  });
...
export type AutomationEnginePendingResult = z.infer<typeof automationEnginePendingResultSchema>;

export const automationEngineReadResultSchema = z.union([
  automationEnginePendingResultSchema,
  automationEngineResultSchema,
]);
export type AutomationEngineReadResult = z.infer<typeof automationEngineReadResultSchema>;
...
export interface AutomationEngineAdapter {
  ...
  admit(request: AutomationEngineAdmissionRequest): Promise<AutomationEngineAdmissionReceipt>;
  observe(
    receipt: AutomationEngineAdmissionReceipt,
    options?: AutomationEngineObserveOptions,
  ): AsyncIterable<AutomationEngineEvent>;
  read(receipt: AutomationEngineAdmissionReceipt): Promise<AutomationEngineReadResult | null>;
  cancel(receipt: AutomationEngineAdmissionReceipt): Promise<AutomationEngineCancellationResult>;
}
