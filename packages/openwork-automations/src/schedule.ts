import { automationScheduleSchema, type AutomationSchedule } from "./types.ts";
...
export function recoverableAutomationOccurrence(
  ...
  const occurrences = automationOccurrences(schedule, {
    after: input.after,
    count: 5,
  }).occurrences.filter((occurrence) => occurrence <= input.now);
