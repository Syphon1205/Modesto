...
export function createFakeAutomationEngineAdapter(input?: {
  ...
  const adapter: AutomationEngineAdapter = {
    ...
    observe(rawReceipt, options) {
      ...
      const events = execution?.events.filter((event) => event.sequence > afterSequence) ?? [];
    ...
    read(rawReceipt) {
      ...
      return Promise.resolve(
        automationEnginePendingResultSchema.parse({
          status: "pending",
    ...
    cancel(rawReceipt) {
      ...
      if (!execution) {
        return Promise.resolve(
          automationEngineCancellationResultSchema.parse({
...
export async function verifyFakeAutomationEngineAdapterConformance(
  ...
  const cancelReceipt = await harness.adapter.admit(
