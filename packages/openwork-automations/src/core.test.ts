...
import { automationScheduleSchema } from "./types.ts";
import { automationOccurrenceIdentity, automationRevisionDigest } from "./contracts.ts";
import { nextAutomationOccurrence, recoverableAutomationOccurrence } from "./schedule.ts";
import {
  assertAutomationTransition,
  canTransitionAutomation,
  isTerminalAutomationRunStatus,
} from "./state.ts";
import { selectDueAutomations } from "./tick.ts";
...
describe("portable Automations core", () => {
  test("calculates once, daily, and weekly occurrences", () => {
    ...
    expect(
      new Date(
        nextAutomationOccurrence(
          { kind: "daily", timezone: "UTC", hour: 9, minute: 30 },
          Date.UTC(2026, 0, 1),
        ) ?? 0,
      ).toISOString(),
    ...
    expect(
      new Date(
        nextAutomationOccurrence(
          { kind: "weekly", timezone: "UTC", daysOfWeek: [1], hour: 9, minute: 30 },
          Date.UTC(2026, 0, 1),
        ) ?? 0,
      ).toISOString(),
  ...
  test("deduplicates scheduled and recovery claims for one occurrence", () => {
    const runs = new Map<
      string,
      { id: string; revisionId: string; trigger: "scheduled" | "recovery" }
    >();
  ...
  test("selects due active Automations deterministically", () => {
    const revision = {
      ...
      schedule: { kind: "daily" as const, timezone: "UTC", hour: 9, minute: 0 },
      model: { providerId: "provider", modelId: "model" },
      // Added on vendoring: upstream's automations package and its types
      // package are out of sync here - the revision schema requires
      // executionTarget and this fixture predates it.
    ...
    expect(
      selectDueAutomations(
        [
          {
            automation: { ...automation, id: "automation_b", nextDueAt: 20 },
            revision: { ...revision, id: "revision_b", automationId: "automation_b" },
            latestRun: null,
          },
          { automation, revision, latestRun: null },
        ],
        { now: 20, limit: 1 },
      ).map((item) => item.automation.id),
    ).toEqual(["automation_a"]);
  });
});
