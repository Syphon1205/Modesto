...
import {
  ...
  buildModestoBranchName,
  buildTemporaryWorktreeBranchName,
  isTemporaryWorktreeBranch,
  resolveUniqueModestoBranchName,
...
describe("buildModestoBranchName", () => {
  it("uses modesto as the branch namespace", () => {
    expect(buildModestoBranchName("fix toast copy")).toBe("modesto/fix-toast-copy");
  });

  it("keeps non-Modesto namespaces inside the Modesto branch", () => {
    expect(buildModestoBranchName("feature/refine-toolbar-actions")).toBe(
      "modesto/feature/refine-toolbar-actions",
    );
  });

  it("normalizes legacy prefixes before rebuilding the branch", () => {
    for (const namespace of PRE_CUTOVER_NAMESPACE_FIXTURES) {
      expect(buildModestoBranchName(`${namespace}/refine toolbar actions`)).toBe(
        "modesto/refine-toolbar-actions",
      );
    }
  });

  it("falls back to modesto/update when no preferred name is provided", () => {
    expect(buildModestoBranchName()).toBe("modesto/update");
  });
});

describe("resolveUniqueModestoBranchName", () => {
