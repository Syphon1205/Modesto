...
const MODEL_SLUG_SET_BY_PROVIDER: Record<ProviderKind, ReadonlySet<ModelSlug>> = {
  ...
  poolside: new Set(MODEL_OPTIONS_BY_PROVIDER.poolside.map((option) => option.slug)),
...
export function getGeminiThinkingModelAlias(
  ...
  if (kind === "level" && nextOptions.thinkingLevel) {
    return `modesto-gemini-${base}-thinking-level-${nextOptions.thinkingLevel.toLowerCase()}`;
  }
  if (kind === "budget" && nextOptions.thinkingBudget !== undefined) {
    ...
    return `modesto-gemini-${base}-thinking-budget-${budget}`;
  }
  return null;
}
