...
/**
 * Parse a context-window setting into a token count.
 *
 * Accepts raw numbers, compact labels (`200k`, `1m`), and Copilot's
 * `long_context` flag. `default` and unknown values stay unknown rather than
 * inventing a window size the provider never confirmed.
 */
export function parseContextWindowTokenCount(
  ...
  if (trimmed === "longcontext" || trimmed === "long") {
    return 1_000_000;
  }
  const match = /^(\d+(?:\.\d+)?)(k|m|b)?$/.exec(trimmed);
...
/** Token count from the selected model's `contextWindow` option, if any. */
export function resolveModelSelectionContextWindowTokens(
  modelSelection: ModelSelection | null | undefined,
): number | null {
  return parseContextWindowTokenCount(
