...
describe("serverSettings helpers", () => {
  ...
  it("merges a voice model patch without dropping the rest of voice settings", () => {
    const next = applyServerSettingsPatch(DEFAULT_SERVER_SETTINGS, {
      voice: { modelId: "small" },
    });
    expect(next.voice.modelId).toBe("small");
    expect(next.voice.speakReplies).toBe(DEFAULT_SERVER_SETTINGS.voice.speakReplies);
  });

  it("merges a speakReplies patch without dropping the rest of voice settings", () => {
    const withModel = applyServerSettingsPatch(DEFAULT_SERVER_SETTINGS, {
      voice: { modelId: "small" },
    });
    const next = applyServerSettingsPatch(withModel, {
      voice: { speakReplies: true },
    });
    expect(next.voice.speakReplies).toBe(true);
    expect(next.voice.modelId).toBe("small");
  });
