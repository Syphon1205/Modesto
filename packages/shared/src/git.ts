 ...
 */
export const WORKTREE_BRANCH_PREFIX = "modesto";
...
const MODESTO_BRANCH_FALLBACK = "update";
...
export function buildModestoBranchName(preferredBranch?: string | null): string {
  const preferred = preferredBranch?.trim() ?? "";
  ...
  return `${WORKTREE_BRANCH_PREFIX}/${sanitizeBranchFragment(
    normalizedExisting || MODESTO_BRANCH_FALLBACK,
  )}`;
}

export function resolveUniqueModestoBranchName(
  existingBranchNames: readonly string[],
  preferredBranch?: string | null,
): string {
  const resolvedBase = buildModestoBranchName(preferredBranch);
  const existingNames = new Set(existingBranchNames.map((branch) => branch.toLowerCase()));
