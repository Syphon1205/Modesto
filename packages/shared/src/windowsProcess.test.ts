...
describe("windowsProcess", () => {
  ...
  it("uses process.cwd for current-directory filtering when cwd is omitted", () => {
    vi.spyOn(process, "cwd").mockReturnValue("C:\\projects\\modesto");
    const spawnSync = vi.fn(() => ({
      stdout: [
        "C:\\projects\\modesto\\codex.cmd",
        "C:\\Users\\test\\AppData\\Roaming\\npm\\codex.cmd",
    ...
    expect(spawnSync).toHaveBeenCalledWith(
      "C:\\Windows\\System32\\where.exe",
      ["codex"],
      expect.objectContaining({ cwd: "C:\\projects\\modesto" }),
    );
  });
  ...
  it.runIf(process.platform === "win32")(
    ...
    () => {
      const root = mkdtempSync(Path.join(tmpdir(), "modesto-windows-process-"));
