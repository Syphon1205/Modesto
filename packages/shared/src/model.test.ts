...
import { ProviderDriverKind, ProviderInstanceId, type ModelCapabilities } from "@modesto/contracts";
...
import {
  ...
  normalizeModelSlug,
  parseContextWindowTokenCount,
  resolveModelSelectionContextWindowTokens,
} from "./model.ts";
...
describe("context window token counts", () => {
  it("parses compact labels and long-context flags", () => {
    expect(parseContextWindowTokenCount(200_000)).toBe(200_000);
    expect(parseContextWindowTokenCount("200k")).toBe(200_000);
    expect(parseContextWindowTokenCount("1M")).toBe(1_000_000);
    expect(parseContextWindowTokenCount("long_context")).toBe(1_000_000);
    expect(parseContextWindowTokenCount("default")).toBeNull();
    expect(parseContextWindowTokenCount("")).toBeNull();
  });

  it("reads the selected model's contextWindow option", () => {
    expect(
      resolveModelSelectionContextWindowTokens(
