...
export const ServerProviderStatus = Schema.Struct({
  ...
  // Whether this provider can be authenticated from Settings without leaving the app,
  // and (for apiKeySupported) whether a Modesto-managed key is currently stored. The raw
  // key itself never appears in this struct or any other client-visible payload.
  apiKeySupported: Schema.optional(Schema.Boolean),
  apiKeyConfigured: Schema.optional(Schema.Boolean),
  signInSupported: Schema.optional(Schema.Boolean),
...
// Custom OpenAI-compatible endpoints, enriched with a derived `apiKeyConfigured`
// flag (never the raw key) the same way ServerProviderStatus enriches settings with
// live-checked auth state.
...
export const ServerProviderUpdateInput = Schema.Struct({
  provider: ProviderKind,
  action: Schema.optional(Schema.Literals(["update", "install"])),
});
...
// In-app credential linking: paste an API key (stored server-side, never echoed back)
// or trigger the provider CLI's own login flow. See ServerProviderStatus.apiKeySupported /
// .signInSupported for which providers accept which input.
export const ServerSetProviderApiKeyInput = Schema.Struct({
  provider: ProviderKind,
  apiKey: TrimmedNonEmptyString.check(Schema.isMaxLength(4_096)),
});
export type ServerSetProviderApiKeyInput = typeof ServerSetProviderApiKeyInput.Type;

export const ServerClearProviderApiKeyInput = Schema.Struct({
  provider: ProviderKind,
});
export type ServerClearProviderApiKeyInput = typeof ServerClearProviderApiKeyInput.Type;

export class ServerProviderCredentialError extends Schema.TaggedErrorClass<ServerProviderCredentialError>()(
  ...
  override get message(): string {
    return `Provider credential update failed for ${this.provider}: ${this.reason}`;
  }
}

export const ServerSetProviderApiKeyResult = ServerProviderStatusesUpdatedPayload;
export type ServerSetProviderApiKeyResult = typeof ServerSetProviderApiKeyResult.Type;

export const ServerClearProviderApiKeyResult = ServerProviderStatusesUpdatedPayload;
export type ServerClearProviderApiKeyResult = typeof ServerClearProviderApiKeyResult.Type;

export const ServerProviderSignInInput = Schema.Struct({
  provider: ProviderKind,
});
export type ServerProviderSignInInput = typeof ServerProviderSignInInput.Type;

export const ServerProviderSignInResult = Schema.Struct({
  started: Schema.Boolean,
});
export type ServerProviderSignInResult = typeof ServerProviderSignInResult.Type;

// Upserts by id: omit `id` (or pass one that doesn't exist yet) to create, pass an
// existing id to update. `apiKey` is optional on update so callers can edit the
// definition without re-pasting a key that's already saved.
