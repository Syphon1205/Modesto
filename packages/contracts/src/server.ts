...
// Custom OpenAI-compatible endpoints, enriched with a derived
// `apiKeyConfigured` flag (never the raw key) the same way provider status
// enriches settings with live-checked auth state.
export const CustomModelEndpointStatus = Schema.Struct({
...
export type CustomModelEndpointStatus = typeof CustomModelEndpointStatus.Type;

const CustomModelEndpointStatuses = Schema.Array(CustomModelEndpointStatus);
...
export const ServerConfig = Schema.Struct({
  ...
  customModelEndpoints: CustomModelEndpointStatuses,
...
// Upserts by id: omit `id` (or pass one that doesn't exist yet) to create,
// pass an existing id to update. `apiKey` is optional on update so callers
// can edit the definition without re-pasting a key that's already saved.
export const ServerSetCustomModelEndpointInput = Schema.Struct({
  id: Schema.optional(CustomModelEndpointId),
  ...
  // Speech-to-text model served by this endpoint; enables the composer mic
  // through it. Absent means this endpoint does not transcribe.
...
export type ServerDeleteCustomModelEndpointInput = typeof ServerDeleteCustomModelEndpointInput.Type;

export class ServerCustomModelEndpointError extends Schema.TaggedErrorClass<ServerCustomModelEndpointError>()(
  ...
  override get message(): string {
    return `Custom model endpoint error: ${this.reason}`;
  }
}

export const ServerCustomModelEndpointsUpdatedPayload = Schema.Struct({
  customModelEndpoints: CustomModelEndpointStatuses,
});
export type ServerCustomModelEndpointsUpdatedPayload =
  typeof ServerCustomModelEndpointsUpdatedPayload.Type;

export const ServerSetCustomModelEndpointResult = ServerCustomModelEndpointsUpdatedPayload;
export type ServerSetCustomModelEndpointResult = typeof ServerSetCustomModelEndpointResult.Type;

export const ServerDeleteCustomModelEndpointResult = ServerCustomModelEndpointsUpdatedPayload;
export type ServerDeleteCustomModelEndpointResult =
  typeof ServerDeleteCustomModelEndpointResult.Type;

/**
 * Cap on a single voice clip, expressed in base64 characters.
 *
 * The clip travels inside a JSON message, so the limit has to sit on the
 * encoded length rather than the decoded byte count - roughly 10MB of audio.
 * The server re-checks the decoded size; this bound exists so an oversized
 * payload is rejected at the protocol edge instead of being buffered first.
 */
...
/**
 * Whether the whisper.cpp executable could be found.
 *
 * `missing` is a normal, recoverable state rather than an error: upstream
 * publishes no macOS CLI build, so a mac user who has not run
 * `brew install whisper-cpp` lands here and needs a hint, not a stack trace.
 */
export const VoiceEngineState = Schema.Literals(["ready", "missing"]);
export type VoiceEngineState = typeof VoiceEngineState.Type;

/**
 * Where the model file stands. `downloading` is observable because the
 * smallest model is 78MB and the default is 148MB - large enough that
 * silence would read as a hang.
 */
export const VoiceModelState = Schema.Literals([
  "ready",
  "missing",
  "downloading",
  "verifying",
...
export const ServerVoiceStatus = Schema.Struct({
  engine: Schema.Struct({
    state: VoiceEngineState,
    /** Absolute path to the resolved executable, or null when missing. */
    path: Schema.NullOr(Schema.String),
    /** Platform-specific instruction for getting the engine, when missing. */
    hint: Schema.NullOr(Schema.String),
  }),
  model: Schema.Struct({
    id: VoiceModelId,
    state: VoiceModelState,
    /** Bytes on disk so far; equals `totalBytes` once ready. */
    downloadedBytes: NonNegativeInt,
    totalBytes: NonNegativeInt,
    /** Populated only in the `failed` state. */
    error: Schema.NullOr(Schema.String),
  }),
  /** True only when a clip recorded right now would transcribe. */
  ready: Schema.Boolean,
});
export type ServerVoiceStatus = typeof ServerVoiceStatus.Type;

export const ServerInstallVoiceModelInput = Schema.Struct({
  modelId: VoiceModelId,
});
export type ServerInstallVoiceModelInput = typeof ServerInstallVoiceModelInput.Type;

/**
 * Returns the status the install started from rather than waiting for the
 * download: a 148MB fetch outlives any sensible request timeout, so the
 * command starts a background install and the client follows `voiceStatus`.
 */
export const ServerInstallVoiceModelResult = ServerVoiceStatus;
export type ServerInstallVoiceModelResult = typeof ServerInstallVoiceModelResult.Type;
