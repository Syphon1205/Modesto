...
export const PullRequestListFilters = Schema.Struct({
  ...
  /** One login, as GitHub `assignee:` names it. `me` resolves to the signed-in viewer. */
  assignee: Schema.optional(PullRequestQualifierValue),
  /** One login, as GitHub `mentions:` names it. */
  mentions: Schema.optional(PullRequestQualifierValue),
  /** One login, as GitHub `involves:` names it (author, assignee, mentioned, or commenter). */
  involves: Schema.optional(PullRequestQualifierValue),
});
...
> = {
  ...
  bitbucket: {
    missing:
      "Bitbucket needs API credentials on the server. Set MODESTO_BITBUCKET_EMAIL and MODESTO_BITBUCKET_API_TOKEN, or MODESTO_BITBUCKET_ACCESS_TOKEN.",
    unauthenticated:
      "Bitbucket rejected the configured credentials. Check MODESTO_BITBUCKET_EMAIL and MODESTO_BITBUCKET_API_TOKEN.",
  },
};
