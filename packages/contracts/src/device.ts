...
export class DeviceError extends Schema.TaggedErrorClass<DeviceError>()("DeviceError", {
...
/**
 * Who owns the boot. Modesto only auto-shuts down devices it booted itself;
 * anything the user started (pane picker, Simulator.app) outlives the session.
 */
export const DeviceBootSource = Schema.Literals(["modesto", "user"]);
...
export const DeviceDescriptor = Schema.Struct({
  ...
  /** Human-readable runtime label, e.g. `iOS 26.2` or `Android 17`. */
  ...
  /**
   * Whether the runtime accepts an injected gravity vector (`device.setGravity`).
   *
   * True for Android emulators, which expose virtual sensors over the emulator
   * console. False for iOS simulators: CoreMotion is unavailable in the
   * simulator and `simctl` offers no sensor surface, so there is nothing to
   * inject into. The pane reads this to decide whether turning the device in
   * free look also turns it for the guest, or is only a change of viewpoint —
   * a distinction it has to state rather than let the user infer from an app
   * that quietly does not react.
   *
   * Optional so an older server that never reports it is read as "no", rather
   * than failing to decode.
   */
  supportsMotion: Schema.optional(Schema.Boolean),
});
...
export const ThreadDeviceState = Schema.Struct({
  ...
  /** Devices the pane may show: booted devices plus anything Modesto is booting. */
...
/** Devices Modesto itself booted are capped globally; viewing already-booted devices is not. */
export const DEVICE_MODESTO_BOOT_LIMIT = 3;
...
/**
 * Boot is refusable rather than fatal: past DEVICE_MODESTO_BOOT_LIMIT the caller
...
export const DeviceBootResult = Schema.Union([
  ...
  Schema.Struct({
    ...
    modestoBooted: Schema.Array(DeviceDescriptor).check(Schema.isMaxLength(64)),
  }),
]);
...
/**
 * The gravity vector the guest's accelerometer should report, in m/s², in
 * Android's sensor frame: X right, Y up, Z out of the screen. Face-up on a
 * table is (0, 0, 9.81); upright portrait is (0, 9.81, 0).
 *
 * Bounded at 2g per axis. The pane only ever sends a rotated 1g vector, and a
 * larger magnitude reaching a guest would read as a drop or an impact — worth
 * rejecting at the contract rather than passing to the emulator console.
 */
const DEVICE_GRAVITY_LIMIT_MS2 = 19.62;

const GravityComponent = Schema.Finite.check(
  Schema.isBetween({ minimum: -DEVICE_GRAVITY_LIMIT_MS2, maximum: DEVICE_GRAVITY_LIMIT_MS2 }),
);

export const DeviceSetGravityInput = Schema.Struct({
  udid: DeviceUdid,
  x: GravityComponent,
  y: GravityComponent,
  z: GravityComponent,
});
export type DeviceSetGravityInput = typeof DeviceSetGravityInput.Type;
...
export const DeviceInstallAppInput = Schema.Struct({
  udid: DeviceUdid,
  /** Absolute path to a built `.app` bundle or `.apk`; Modesto never runs the build itself. */
 ...
 * the bitstream. Runtime encode/decode lives in `@modesto/shared/deviceFrame`;
