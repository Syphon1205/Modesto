 ...
 * - `loopback-bootstrap`: a same-machine browser handoff, offered only by a
 *   server bound to loopback. Anything that can reach such a server is already
 *   running on the machine, so a second browser window does not need to be
 *   paired by hand. The server still refuses cross-site callers, which is what
 *   keeps a web page the user happens to be visiting from claiming a session.
