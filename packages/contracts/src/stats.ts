...
// the shareable activity card. All metrics are backed by Modesto's local DB
// projections. Commit activity is loaded separately from local Git repositories
// and, when connected, the user's GitHub account.
...
export const StatsGetProfileCommitActivityInput = Schema.Struct({});
export type StatsGetProfileCommitActivityInput = typeof StatsGetProfileCommitActivityInput.Type;
...
export const ProfileActivity = Schema.Struct({
  ...
  // totalPromptsSent), i.e. days the user actually used Modesto.
...
// Token totals come from Modesto's projected context-window updates. `available`
...
export const ProfileCommit = Schema.Struct({
  source: Schema.Literals(["workspace", "github"]),
...
export type ProfileCommit = typeof ProfileCommit.Type;

export const ProfileCommitActivity = Schema.Struct({
  workspace: Schema.Array(ProfileCommit),
  github: Schema.Array(ProfileCommit),
  githubInstalled: Schema.Boolean,
  githubAuthenticated: Schema.Boolean,
  githubFetchFailed: Schema.Boolean,
  githubLogin: Schema.NullOr(TrimmedNonEmptyString),
});
export type ProfileCommitActivity = typeof ProfileCommitActivity.Type;

export const StatsGetProfileCommitActivityResult = ProfileCommitActivity;
export type StatsGetProfileCommitActivityResult = typeof StatsGetProfileCommitActivityResult.Type;
