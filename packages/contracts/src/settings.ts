...
// Custom OpenAI-compatible endpoints (vLLM, LM Studio, OpenRouter, Portkey, LiteLLM,
// etc). Only Codex consumes these today (see ProviderKind); `provider` exists on the
// summary type (server.ts) but not here, since this list has exactly one target.
...
// A stable slug: used as the config.toml `[model_providers.<id>]` section name and
// as the ServerSecretStore key, so it's restricted to safe identifier characters.
...
export const CustomModelEndpointConfig = Schema.Struct({
  ...
  wireApi: CustomModelEndpointWireApi.pipe(Schema.withDecodingDefault(() => "chat" as const)),
  models: CustomModelEndpointModels,
});
export type CustomModelEndpointConfig = typeof CustomModelEndpointConfig.Type;

const CustomModelEndpoints = Schema.Array(CustomModelEndpointConfig).pipe(
  Schema.withDecodingDefault(() => []),
);
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  // Whole-list replace, same shape as `customModels` on each provider — callers
  // read the current list, apply an upsert/delete, and patch the full result back.
  customModelEndpoints: Schema.optionalKey(CustomModelEndpoints),
