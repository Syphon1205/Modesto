 ...
 * relying on Modesto's own orchestration projections, so usage stays complete
 * even for turns that were never driven through Modesto. This mirrors the
...
/**
 * Providers the usage page knows about. Local CLI/DB scanners cover most
 * kinds; `cursor` is filled from the Cursor cloud usage API (machine login or
 * Enterprise Admin key). Kinds without a local or cloud ledger (`qwen`,
 * `kimi`, `grok`, `meta`, `devin`, `droid`, `customAcp`) report as
 * {@link UsageSourceStatus} `"unsupported"` — surfaced honestly in the UI
 * rather than omitted or faked.
 */
