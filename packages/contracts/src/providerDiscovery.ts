...
// Unified cross-provider skills catalog (Modesto portable skills). Descriptors use
// `scope` to carry the origin label ("modesto", "codex", "claude", "cursor", ...).
...
export const ProviderSkillsCatalogResult = Schema.Struct({
  skills: Schema.Array(ProviderSkillDescriptor),
  modestoSkillsDir: Schema.optional(TrimmedNonEmptyString),
});
...
// Lightweight skill metadata included with marketplace plugin summaries. The
// owning plugin remains the installable unit; these records let clients offer
// skill-first discovery without pretending an uninstalled skill has a local
// filesystem path yet.
export const ProviderPluginSkillSummary = Schema.Struct({
  name: TrimmedNonEmptyString,
  description: Schema.optional(TrimmedNonEmptyString),
  interface: Schema.optional(ProviderSkillInterface),
});
export type ProviderPluginSkillSummary = typeof ProviderPluginSkillSummary.Type;
...
export const ProviderPluginDescriptor = Schema.Struct({
  ...
  bundledSkills: Schema.optional(Schema.Array(ProviderPluginSkillSummary)),
});
...
export type ProviderSetPluginInstalledInput = typeof ProviderSetPluginInstalledInput.Type;
