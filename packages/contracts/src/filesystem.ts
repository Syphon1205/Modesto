...
export const FilesystemPickFolderResult = Schema.NullOr(Schema.String);
export type FilesystemPickFolderResult = typeof FilesystemPickFolderResult.Type;

export const FilesystemPickFolderFailure = Schema.Literals([
  "unsupported_platform",
  "dialog_failed",
]);
export type FilesystemPickFolderFailure = typeof FilesystemPickFolderFailure.Type;

export class FilesystemPickFolderError extends Schema.TaggedErrorClass<FilesystemPickFolderError>()(
  "FilesystemPickFolderError",
  {
    initialPath: Schema.optional(Schema.NullOr(Schema.String)),
    failure: Schema.optional(FilesystemPickFolderFailure),
  ...
  constructor(props: {
    readonly initialPath?: string | null | undefined;
    readonly failure: FilesystemPickFolderFailure;
    ...
    super({
      ...props,
      message:
        decodedFilesystemPickFolderErrorMessage(props) ??
        (props.failure === "unsupported_platform"
          ? "This environment cannot open a native folder picker."
          : "Could not open the native folder picker."),
    } as any);
  }
}

function decodedFilesystemPickFolderErrorMessage(props: object): string | undefined {
  if (!("message" in props)) return undefined;
  return typeof props.message === "string" ? props.message : undefined;
}
