...
>()("@modesto/contracts/environmentHttp/EnvironmentAuthenticatedPrincipal") {}
...
export class EnvironmentAuthHttpApi extends HttpApiGroup.make("auth")
  ...
  .add(
    /**
     * Claims a session for a browser that is already on the machine, with no
     * credential to type. Served only by a loopback-bound environment, and only
     * to a same-origin caller - see `ServerAuthBootstrapMethod`.
     */
    HttpApiEndpoint.post("loopbackSession", "/api/auth/loopback-session", {
      success: AuthBrowserSessionResult,
      error: EnvironmentSessionCreationErrors,
    }),
  )
