...
// GitHub account sign-in (the app's own identity, via `gh` CLI — distinct
// from the source-control PR integration above).
...
export const GitHubSignInResult = Schema.Struct({
  ...
  /** Ready-to-open device verification URL (includes `user_code` when known). */
...
export class GitHubAuthCliError extends Schema.TaggedErrorClass<GitHubAuthCliError>()(
  "GitHubAuthCliError",
  {
    operation: Schema.Literals(["status", "signIn", "signOut"]),
    detail: Schema.String,
  },
) {
  override get message(): string {
    return `GitHub CLI auth ${this.operation} failed: ${this.detail}`;
  }
}
