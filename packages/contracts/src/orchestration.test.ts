...
import {
  ...
  formatProviderSendTurnAttachmentPathLine,
  inferProviderSendTurnFileExtension,
  inferProviderSendTurnFileMimeType,
  isProviderSendTurnSupportedFileAttachment,
  isProviderSendTurnSupportedImageMimeType,
  providerSendTurnComposerAcceptAttribute,
...
it("isProviderSendTurnSupportedFileAttachment accepts documents and rejects executables", () => {
  assert.strictEqual(
    isProviderSendTurnSupportedFileAttachment({
...
it("treats Figma, Framer, Adobe, and SVG as design files, not vision images", () => {
  assert.strictEqual(providerSendTurnFileKind("hero.fig", "application/vnd.figma"), "design");
  assert.strictEqual(providerSendTurnFileKind("site.framer", ""), "design");
  assert.strictEqual(providerSendTurnFileKind("poster.psd", "image/vnd.adobe.photoshop"), "design");
  assert.strictEqual(providerSendTurnFileKind("logo.ai", "application/postscript"), "design");
  assert.strictEqual(providerSendTurnFileKind("mark.svg", "image/svg+xml"), "design");
  assert.strictEqual(isProviderSendTurnSupportedImageMimeType("image/svg+xml"), false);
  assert.strictEqual(isProviderSendTurnSupportedImageMimeType("image/vnd.adobe.photoshop"), false);
});

it("keeps native design extensions when the browser lies about the mime type", () => {
  assert.strictEqual(
    inferProviderSendTurnFileExtension({
...
it("mentions the on-disk path and the live design tool for attached sources", () => {
  assert.strictEqual(
    formatProviderSendTurnAttachmentPathLine({
      type: "file",
      name: "hero.fig",
      mimeType: "application/vnd.figma",
      path: "/tmp/attachments/hero.fig",
    }),
    '[Attached design file "hero.fig" is saved at: /tmp/attachments/hero.fig. Use the Figma MCP connection for the live file.]',
  );
  assert.strictEqual(
    formatProviderSendTurnAttachmentPathLine({
      type: "file",
      name: "poster.psd",
      mimeType: "image/vnd.adobe.photoshop",
      path: "/tmp/attachments/poster.psd",
    }),
    '[Attached design file "poster.psd" is saved at: /tmp/attachments/poster.psd. Open it in Photoshop (@photoshop) to edit the native file.]',
  );
});

it("lists design extensions on the composer paperclip accept attribute", () => {
  const accept = providerSendTurnComposerAcceptAttribute();
  assert.strictEqual(accept.includes(".fig"), true);
  assert.strictEqual(accept.includes(".framer"), true);
  assert.strictEqual(accept.includes(".psd"), true);
  assert.strictEqual(accept.includes(".ai"), true);
  assert.strictEqual(accept.includes(".svg"), true);
  assert.strictEqual(accept.includes("image/png"), true);
});
