...
it.effect("accepts provider plugin installation requests", () =>
  Effect.gen(function* () {
    const parsed = yield* decode(WebSocketRequest, {
      id: "req-plugin-install-1",
      body: {
        _tag: WS_METHODS.providerSetPluginInstalled,
        provider: "codex",
    ...
    assert.strictEqual(parsed.body._tag, WS_METHODS.providerSetPluginInstalled);
    if (parsed.body._tag === WS_METHODS.providerSetPluginInstalled) {
      assert.strictEqual(parsed.body.marketplaceName, "openai-curated");
      assert.strictEqual(parsed.body.pluginName, "figma");
      assert.strictEqual(parsed.body.installed, true);
    }
  }),
);
