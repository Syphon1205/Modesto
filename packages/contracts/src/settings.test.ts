...
describe("provider enabled defaults", () => {
  it("enables the stable bindings by default", () => {
    const decoded = decodeServerSettings({});
    expect(decoded.providers.codex.enabled).toBe(true);
    expect(decoded.providers.claudeAgent.enabled).toBe(true);
    expect(decoded.providers.cursor.enabled).toBe(true);
    expect(decoded.providers.grok.enabled).toBe(true);
    expect(decoded.providers.gemini.enabled).toBe(true);
    expect(decoded.providers.meta.enabled).toBe(true);
  ...
  it("derives per-driver defaults from the settings schemas", () => {
    ...
    expect(defaultEnabledForDriver(ProviderDriverKind.make("meta"))).toBe(true);
...
describe("ServerSettings voice", () => {
  it("defaults to the English base model with TTS off", () => {
    const voice = decodeServerSettings({}).voice;
    expect(voice.modelId).toBe("base.en");
    expect(voice.speakReplies).toBe(false);
  });

  it("accepts a multilingual model patch", () => {
    expect(decodeServerSettingsPatch({ voice: { modelId: "base" } }).voice?.modelId).toBe("base");
  });

  it("accepts speakReplies patches and keeps legacy configs silent", () => {
    expect(decodeServerSettings({ voice: { modelId: "small" } }).voice.speakReplies).toBe(false);
    expect(decodeServerSettingsPatch({ voice: { speakReplies: true } }).voice?.speakReplies).toBe(
      true,
    );
  });
});
...
describe("ambient presence settings", () => {
  it("defaults off and lives on the client, not under voice", () => {
    // It used to be `voice.enabled`, which made a plain floating status
    // bubble look like a speech feature. It has nothing to do with voice.
    const settings = decodeClientSettings({});
    expect(settings.ambientPresenceEnabled).toBe(false);
    expect(decodeServerSettings({}).voice).not.toHaveProperty("enabled");
  });

  it("round-trips through the client patch schema", () => {
    const patch = decodeClientSettingsPatch({ ambientPresenceEnabled: true });
    expect(patch).toEqual({ ambientPresenceEnabled: true });
  });
});

describe("thinking sound settings", () => {
  it("defaults off and lives on the client, alongside ambient presence", () => {
    expect(decodeClientSettings({}).thinkingSoundEnabled).toBe(false);
  });

  it("round-trips through the client patch schema", () => {
    const patch = decodeClientSettingsPatch({ thinkingSoundEnabled: true });
    expect(patch).toEqual({ thinkingSoundEnabled: true });
  });
});

describe("chat tabs settings", () => {
  it("defaults tabs off and the tip unshown", () => {
    // Tabs are opt-in: the strip mirrors navigation, so defaulting it on is
    // what made tabs appear without anyone asking for them.
    const settings = decodeClientSettings({});
    expect(settings.chatTabsEnabled).toBe(false);
    expect(settings.chatTabsTipDismissed).toBe(false);
  });

  it("round-trips an enabled, dismissed state through the patch schema", () => {
    const patch = decodeClientSettingsPatch({
      chatTabsEnabled: true,
      chatTabsTipDismissed: true,
    });
    expect(patch).toEqual({ chatTabsEnabled: true, chatTabsTipDismissed: true });
  });
});
