...
export const PROVIDER_SEND_TURN_FILE_MIME_TYPE_BY_EXTENSION: Record<string, string> = {
  ".csv": "text/csv",
  ...
  ".yml": "text/yaml",
  ".zip": "application/zip",
  // Design sources stay files, not vision images: most providers reject SVG
  // as an image part, and Figma/Adobe binaries are opaque to the model.
  ".fig": "application/vnd.figma",
  ".framer": "application/vnd.framer",
  ".framerx": "application/vnd.framer.x",
  ...
  ".aepx": "application/vnd.adobe.aftereffects.project+xml",
  ...
  ".sesx": "application/vnd.adobe.audition.session",
  ".ait": "application/vnd.adobe.illustrator.template",
  ".indt": "application/x-indesign-template",
  ".psdt": "image/vnd.adobe.photoshop.template",
  ".mogrt": "application/vnd.adobe.premiere.mogrt",
};

export type ProviderSendTurnFileKind = "pdf" | "spreadsheet" | "design" | "document";

const PROVIDER_SEND_TURN_DESIGN_FILE_EXTENSIONS = new Set([
  ".fig",
  ".framer",
  ".framerx",
  ".psd",
...
/** How the composer and timeline should present a non-image attachment. */
export function providerSendTurnFileKind(
  fileName: string,
  mimeType: string,
): ProviderSendTurnFileKind {
  const extension = providerSendTurnFileExtension(fileName);
  ...
  if (
    PROVIDER_SEND_TURN_DESIGN_FILE_EXTENSIONS.has(extension) ||
    lowerMime === "image/svg+xml" ||
    lowerMime.includes("figma") ||
    lowerMime.includes("framer") ||
    lowerMime.includes("photoshop") ||
    lowerMime.includes("illustrator") ||
    lowerMime.includes("indesign") ||
    lowerMime.includes("aftereffects") ||
    lowerMime.includes("premiere") ||
    lowerMime.includes("affinity") ||
    lowerMime.includes("sketch")
  ) {
    return "design";
  }
  return "document";
...
function providerSendTurnDesignToolHint(fileName: string): string | null {
  const extension = providerSendTurnFileExtension(fileName);
  if (extension === ".fig") {
    return "Use the Figma MCP connection for the live file";
  }
  if (extension === ".framer" || extension === ".framerx") {
    return "Use the Framer MCP plugin for the live project";
  }
  if (extension === ".psd" || extension === ".psb") {
    return "Open it in Photoshop (@photoshop) to edit the native file";
  }
  if (extension === ".ai" || extension === ".eps") {
    return "Open it in Illustrator (@illustrator) to edit the native file";
  }
  if (extension === ".indd" || extension === ".idml") {
    return "Open it in InDesign (@indesign) to edit the native file";
  }
  if (extension === ".xd") {
    return "Open it in Adobe XD (@xd) to edit the native file";
  }
  if (extension === ".aep" || extension === ".aepx") {
    return "Open it in After Effects (@after-effects) to edit the native file";
  }
  if (extension === ".prproj" || extension === ".mogrt") {
    return "Open it in Premiere (@premiere-pro) to edit the native file";
  }
  if (extension === ".fla") {
    return "Open it in Animate (@animate) to edit the native file";
  }
  if (extension === ".sesx") {
    return "Open it in Audition (@audition) to edit the native file";
  }
  return null;
}

/** Paperclip `accept` tokens — keep the picker in lockstep with persist/lookup. */
export function providerSendTurnComposerAcceptAttribute(): string {
  return Array.from(
    new Set([
      ...PROVIDER_SEND_TURN_SUPPORTED_IMAGE_MIME_TYPES,
      ...Object.values(PROVIDER_SEND_TURN_FILE_MIME_TYPE_BY_EXTENSION),
      ...Object.keys(PROVIDER_SEND_TURN_FILE_MIME_TYPE_BY_EXTENSION),
      "text/*",
    ]),
  ).join(",");
}

/** Prompt footnote so tools can find the bytes even when the adapter inlines pixels. */
export function formatProviderSendTurnAttachmentPathLine(input: {
  ...
  if (input.type === "image") {
    return `[Attached image "${input.name}" is saved at: ${input.path}]`;
  }
  if (providerSendTurnFileKind(input.name, input.mimeType) === "design") {
    const hint = providerSendTurnDesignToolHint(input.name);
    return hint
      ? `[Attached design file "${input.name}" is saved at: ${input.path}. ${hint}.]`
      : `[Attached design file "${input.name}" is saved at: ${input.path}]`;
  }
  return `[Attached file "${input.name}" is saved at: ${input.path}]`;
}

const PROVIDER_SEND_TURN_FILE_EXTENSION_BY_MIME_TYPE: Record<string, string> = Object.fromEntries(
  Object.entries(PROVIDER_SEND_TURN_FILE_MIME_TYPE_BY_EXTENSION).map(([extension, mimeType]) => [
    mimeType,
    extension,
  ]),
);

const PROVIDER_SEND_TURN_BLOCKED_FILE_EXTENSIONS = new Set([
  ".apk",
  ".app",
  ".bat",
  ".cmd",
...
function providerSendTurnFileExtension(fileName: string): string {
  const match = /\.([a-z0-9]{1,8})$/i.exec(fileName.trim());
  return match ? `.${match[1]!.toLowerCase()}` : "";
}

/** Infer a document mime type from a browser File, falling back to the extension. */
export function inferProviderSendTurnFileMimeType(input: {
  readonly mimeType?: string;
  ...
  const extension = providerSendTurnFileExtension(input.fileName);
  return (
    PROVIDER_SEND_TURN_FILE_MIME_TYPE_BY_EXTENSION[extension] ??
    (mimeType.length > 0 ? mimeType : "application/octet-stream")
  );
}

export function inferProviderSendTurnFileExtension(input: {
  readonly mimeType: string;
  readonly fileName?: string;
}): string {
  const fromName = providerSendTurnFileExtension(input.fileName ?? "");
  if (
    fromName.length > 0 &&
    Object.hasOwn(PROVIDER_SEND_TURN_FILE_MIME_TYPE_BY_EXTENSION, fromName)
  ) {
    return fromName;
  }
  const mimeType = input.mimeType.trim().toLowerCase();
  const fromMime = PROVIDER_SEND_TURN_FILE_EXTENSION_BY_MIME_TYPE[mimeType];
  if (fromMime) {
    return fromMime;
  }
  return fromName.length > 0 ? fromName : ".bin";
}

/**
 * Whether a non-image file can be attached to a provider turn. Raster images
 * stay on the image pipeline; this covers PDFs, Office docs, text, design
 * sources (Figma, Framer, Adobe, SVG), and other ordinary documents.
 * Executables and disk images are rejected.
 */
export function isProviderSendTurnSupportedFileAttachment(input: {
  readonly mimeType: string;
  readonly fileName: string;
}): boolean {
  const extension = providerSendTurnFileExtension(input.fileName);
  if (extension.length > 0 && PROVIDER_SEND_TURN_BLOCKED_FILE_EXTENSIONS.has(extension)) {
    return false;
  }
  const mimeType = input.mimeType.trim().toLowerCase();
  ...
  if (
    mimeType.length > 0 &&
    Object.hasOwn(PROVIDER_SEND_TURN_FILE_EXTENSION_BY_MIME_TYPE, mimeType)
  ) {
    return true;
  }
  if (
    extension.length > 0 &&
    Object.hasOwn(PROVIDER_SEND_TURN_FILE_MIME_TYPE_BY_EXTENSION, extension)
  ) {
    return true;
  }
  ...
  return (
    mimeType.length > 0 && mimeType !== "application/octet-stream" && !mimeType.startsWith("image/")
  );
}
...
/** Product-level conversation lane. Older threads omit this and are code tasks. */
export const ConversationMode = Schema.Literals(["chat", "code"]);
export type ConversationMode = typeof ConversationMode.Type;
...
export const OrchestrationThread = Schema.Struct({
  ...
  conversationMode: Schema.optionalKey(ConversationMode),
