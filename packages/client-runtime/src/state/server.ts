...
export function createServerEnvironmentAtoms<R, E>(
  ...
  const customModelEndpointsValueAtom = Atom.family((environmentId: EnvironmentId) =>
    Atom.make((get) => get(configValueAtom(environmentId))?.customModelEndpoints ?? null).pipe(
      Atom.withLabel(`environment-data:server:custom-model-endpoints:${environmentId}`),
    ),
  );
  ...
  return {
    ...
    settingsValueAtom,
    providersValueAtom,
    customModelEndpointsValueAtom,
    // Polled while a speech model downloads, so it must never be served from
    // cache; a stale byte count is the one thing this query cannot report.
    voiceStatus: createEnvironmentRpcQueryAtomFamily(runtime, {
      label: "environment-data:server:voice-status",
      tag: WS_METHODS.serverVoiceStatus,
      staleTimeMs: 0,
    }),
    ...
    setCustomModelEndpoint: createEnvironmentRpcCommand(runtime, {
      label: "environment-data:server:set-custom-model-endpoint",
      tag: WS_METHODS.serverSetCustomModelEndpoint,
      scheduler: configScheduler,
      concurrency: configConcurrency,
    }),
    ...
    // No scheduler/concurrency limit: a transcription is user-initiated, one
    // at a time by construction (you can only hold one recording), and making
    // it queue behind config traffic would add latency to a spoken message.
    ...
    installVoiceModel: createEnvironmentRpcCommand(runtime, {
      label: "environment-data:server:install-voice-model",
      tag: WS_METHODS.serverInstallVoiceModel,
    }),
