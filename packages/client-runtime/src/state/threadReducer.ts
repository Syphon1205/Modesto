...
import { isRateLimitsActivity } from "@modesto/shared/rateLimits";
...
export function applyThreadDetailEvent(
  ...
  switch (event.type) {
    ...
    case "thread.created":
      return {
        ...
        thread: {
          ...
          conversationMode: event.payload.conversationMode ?? "code",
      ...
      return {
        ...
        thread: {
          ...
          ...(event.payload.modelSelection !== undefined
            ? { modelSelection: event.payload.modelSelection }
            : {}),
          ...(event.payload.conversationMode !== undefined
            ? { conversationMode: event.payload.conversationMode }
            : {}),
    ...
    case "thread.activity-appended": {
      ...
      //
      // Account rate limits are account-scoped (not per turn). Claude may
      // push one window at a time; the footer merges windows client-side, so
      // we still keep a short history of rate-limit rows but drop earlier
      // duplicates of the same activity id and cap retained rate-limit rows.
      const supersedesContextWindow = isResolvableContextWindowActivity(activity);
      const isRateLimits = isRateLimitsActivity(activity);
      const activities = pipe(
        thread.activities,
        Arr.filter(
          (entry) =>
        ...
        Arr.append(activity),
        (rows) => {
          if (!isRateLimits) return rows;
          // Keep the newest handful so Claude's staggered five_hour / weekly
          // events can still merge, without unbounded growth.
          const rateLimitIndexes: number[] = [];
          for (let index = 0; index < rows.length; index += 1) {
            if (isRateLimitsActivity(rows[index]!)) rateLimitIndexes.push(index);
          }
          if (rateLimitIndexes.length <= 12) return rows;
          const drop = new Set(rateLimitIndexes.slice(0, rateLimitIndexes.length - 12));
