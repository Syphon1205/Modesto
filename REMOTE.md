...
Use this when you want to open Modesto from another device (phone, tablet, another laptop).

## CLI ↔ Env option map

The Modesto CLI accepts the following configuration options, available either as CLI flags or environment variables. The established `MODESTO_*` environment names remain compatibility aliases during migration.

| CLI flag                | Env var               | Notes                              |
| ----------------------- | --------------------- | ---------------------------------- |
| `--mode <web\|desktop>` | `MODESTO_MODE`        | Runtime mode.                      |
| `--port <number>`       | `MODESTO_PORT`        | HTTP/WebSocket port.               |
| `--host <address>`      | `MODESTO_HOST`        | Bind interface/address.            |
| `--home-dir <path>`     | `MODESTO_HOME`        | Base directory.                    |
| `--dev-url <url>`       | `VITE_DEV_SERVER_URL` | Dev web URL redirect/proxy target. |
| `--no-browser`          | `MODESTO_NO_BROWSER`  | Disable auto-open browser.         |
| `--auth-token <token>`  | `MODESTO_AUTH_TOKEN`  | WebSocket auth token.              |
